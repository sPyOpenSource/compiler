...
public final class Util {
    ...
    public static String getMethodRawName(MethodSource method) {
        return getMethodRawName(method.getDeclaringClassSource().getClassName(), method.getMethodName(), method.getMethodType());
    }
...
//                return s;
                ...
                ClassData c = ClassManger.getClassFile(className);
                List<FieldData> fieldsContainSuperClass = ClassManger.getFieldTable(className);
                StringJoiner joiner = new StringJoiner("; ", "{", ";}");
                joiner.add(getInstPropType() + " prop");
                joiner.add(getVMTableTypePtr() + " vm_table");
                for (FieldData f : fieldsContainSuperClass) {
                    if ((f.getModifiers() & Modifier.STATIC) == 0) {
                        joiner.add(getJavaSignatureCtype(f.getType()) + " " + getFieldVarName(f));
                    }
                }
    ...
    public static String getFieldVarName(FieldData field) {
        if (field == null) {
            int debug = 1;
            System.out.println("field is null");
        }
        return regulString(field.getName()) + "_" + ClassManger.getFieldIndexInFieldTable(field);
    }
