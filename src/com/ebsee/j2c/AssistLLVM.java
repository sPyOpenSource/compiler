...
public class AssistLLVM {
    ...
    static Set<MethodSource> nativemethod = new LinkedHashSet<>();//native
    //llfile
...
//        conv("test.TestParent", classesPath, csrcPath);
    ...
    static void conv(String className, String cPath) throws IOException {
        ...
        File outfile = new File(cPath, outFileName.replace("/", "."));
        ...
        ClassReader cr = new ClassReader(ClassManger.helper.getClassFile(fn).getClassFileStream());
        cr.accept(cv, 0);
        ps.flush();
    ...
    static private void genTypeDef(PrintStream ps) {
        ...
        if (microDefFields != null) {
            for (String[] pair : microDefFields) {
                FieldData field = ClassManger.findField(pair[0].replace('.', '/'), pair[1], pair[2]);
                if (field == null) {
    ...
    static private void genJNI(String outpath) {
        try {
            ...
            MethodSource[] sorted = nativemethod.toArray(new MethodSource[nativemethod.size()]);
            Arrays.sort(sorted, (t0, t1) -> {
                return (t0.getDeclaringClassSource().getClassName() + t0.getMethodName()).compareTo(t1.getDeclaringClassSource().getClassName() + t1.getMethodName());
            });
            for (MethodSource m : sorted) {
                JSignature sig = new JSignature(m);
                String rawName = Util.getMethodRawName(m.getDeclaringClassSource().getClassName(), m.getMethodName(), sig.getJavaSignature());
    ...
    static private int addMethodInfo(MethodSource m) {
        String className = m.getDeclaringClassSource().getClassName();
        JSignature sig = new JSignature(m);
        ...
        methodinfo += ", ";
        methodinfo += addGlobalStringDefine(m.getMethodType());
        methodinfo += ", ";
        methodinfo += addGlobalStringDefine(m.getSignature());
        methodinfo += ", ";
        methodinfo += addGlobalStringDefine(m.getDeclaringClassSource().getClassName());//
        methodinfo += ", ";
        methodinfo += addGlobalStringDefine("0");//bytecode
        methodinfo += ", ";
        methodinfo += m.getModifiers();
        methodinfo += ", ";
        methodinfo += ca == null ? 0 : ca.getNumStackSlots();
        methodinfo += ", ";
        methodinfo += ca == null ? 0 : ca.getNumLocalVariables();
        methodinfo += ", ";
        ...
        methodinfo += "},";
        String s = m.getDeclaringClassSource().getClassName() + "." + m.getMethodName() + m.getMethodType();
    ...
    static public int getMethodIndex(MethodSource method) {
        return getMethodIndex(method.getDeclaringClassSource().getClassName(), method.getMethodName(), method.getMethodType());
    }
    ...
    static public int getMethodIndex(String className, String methodName, String signature) {
        ...
        Integer i = methodraw2index.get(s);
        if (i == null) {
            MethodSource m = ClassManger.findMethod(className, methodName, signature);
            i = addMethodInfo(m);
        }
        return i;
    }

    static private int addFieldInfo(FieldData f) {
        String className = f.getDeclaringClass().getClassName();
        String fieldinfo = "    {";
        fieldinfo += addGlobalStringDefine(f.getName());
        fieldinfo += ", ";
        fieldinfo += addGlobalStringDefine(f.getType());
        fieldinfo += ", ";
        fieldinfo += addGlobalStringDefine(f.getSignature());
        fieldinfo += ", ";
        fieldinfo += addGlobalStringDefine(f.getDeclaringClass().getClassName());//
        fieldinfo += ", ";
        fieldinfo += f.getModifiers();
        fieldinfo += ", ";
        ...
        fieldinfo += ", ";
        fieldinfo += "},";
        String s = className + "." + f.getName() + " " + f.getType();
    ...
    static public int getFieldIndex(String className, String fieldName, String signature) {
        ...
        Integer i = fieldraw2index.get(s);
        if (i == null) {
            FieldData f = ClassManger.findField(fieldName, signature, className);
            i = addFieldInfo(f);
        }
        return i;
    }

    static private String getClassInfo(ClassData cf) {
        String className = cf.getClassName();

        List<Integer> methods = new ArrayList<>();
        for (MethodSource m : cf.getMethods()) {
...
//        }
        ...
        classinfo += ", ";
        classinfo += addGlobalStringDefine(cf.getSourceFileAttribute());//source
        classinfo += ", ";
...
//        }
        ...
        classinfo += declare1;
        classinfo += ", ";
        MethodSource finalizeM = ClassManger.findFinalizeMethod(className);
        classinfo += finalizeM == null ? "NULL" : Util.getMethodRawName(finalizeM.getDeclaringClassSource().getClassName(), finalizeM.getMethodName(), finalizeM.getMethodType());
        classinfo += ", ";
    ...
    static public void updateClassInfo(String className) {
        ...
        ClassData f = ClassManger.getClassFile(className);
