...
public class MV extends MethodVisitor {
    ...
    public void visitInsn(int opcode) {
        switch (opcode) {
            ...
            case Opcodes.IDIV: // 108
                add("if (!stack[sp - 1].i) {");
                add("    rstack[sp++].obj = " + AssistLLVM.FUNC_CONSTRUCT_AND_THROW_EXCEPTION + "(runtime, " + AssistLLVM.getClassIndex(CLASS_JAVA_LANG_ARITHMETIC_EXCEPTION) + ", ");// + curLabel.getOffset() + ", " + curLineNo + ");");
            ...
            case Opcodes.ATHROW: {
                ...
                add("{");
                add("    __frame->bytecodeIndex = ");// + curLabel.getOffset() + ";");
    ...
    public void visitTypeInsn(int opcode, String className) {
        ...
        switch (opcode) {
            ...
            case Opcodes.CHECKCAST: {// 192
                ...
                add("    if(!cast_r){");
                add("        rstack[sp++].obj = " + AssistLLVM.FUNC_CONSTRUCT_AND_THROW_EXCEPTION + "(runtime, " + AssistLLVM.getClassIndex(CLASS_JAVA_LANG_CLASS_CAST_EXCEPTION) + ", ");// + curLabel.getOffset() + ", " + curLineNo + ");");
                add("        goto " + EXCEPTION_HANDLER + ";");
                add("    }");
                add("}");
                break;
    ...
    public void visitLineNumber(int i, Label label) {
        comment(" line no " + i + " , " + label + " , bytecode index = ");// + label.getOffset());
        curLabel = label;
        curLineNo = i;
        if (!firstLabAccessed) {
            if (!canSkipCodeTrack()) {
                add("__frame->bytecodeIndex = ");// + label.getOffset() + "; //first");
                add("__frame->lineNo = " + curLineNo + ";");
            }
    ...
    void genTryCatchLabel(Label label) {
        for (ExceptionItem item : exceptionTables) {
            if (label.toString().equals(item.startLabel.toString())) {
                add("__frame->bytecodeIndex = ");// + label.getOffset() + "; //try catch begin");
            } else if (label.toString().equals(item.endLabel.toString())) {
                add("__frame->bytecodeIndex = ");// + label.getOffset() + "; //try catch end");
            }
        }
    }
...
//        }
        comment("try catch :" + start + " " + end + " " + handler + " (");// + start.getOffset() + "," + end.getOffset() + ")->" + handler.getOffset());
