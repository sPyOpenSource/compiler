...
public abstract class VmX86Architecture extends BaseVmArchitecture {
    ...
    protected final void initializeProcessors(ResourceManager rm) {
        ...
        final HashMap<Integer, MPProcessorEntry> physCpus = new HashMap<>();
        mpConfigTable.entries().stream().filter((e) -> (e.getEntryType() == 0)).map((e) -> (MPProcessorEntry) e).filter((cpuEntry) -> (cpuEntry.isEnabled() && !cpuEntry.isBootstrap())).forEachOrdered((cpuEntry) -> {
        ...
        physCpus.values().stream().map((cpuEntry) -> cpuEntry.getApicID()).map((apicId) -> (VmX86Processor) createProcessor(
                apicId, VmUtils.getVm().getSharedStatics(), bootCpu
                        .getIsolatedStatics(), bootCpu.getScheduler())).map((newCpu) -> {
            // New CPU
            initX86Processor(newCpu);
            return newCpu;
        }).forEachOrdered((newCpu) -> {
            try {
                newCpu.startup(rm);
            } catch (ResourceNotFreeException ex) {
     ...
     * @return
     */
    private String mmapTypeToString(int type) {
        switch (type) {
