...
public final class VmSystemClassLoader extends VmAbstractClassLoader {
    ...
    public VmSystemClassLoader(URL[] classesURL, BaseVmArchitecture arch,
        ...
        this.resourceLoaders = new BootableArrayList<>();
    ...
    public Collection<VmType> getLoadedClasses() {
        if (classInfos != null) {
            final ArrayList<VmType> list = new ArrayList<>();
            classInfos.values().stream().filter((ci) -> (ci.isLoaded())).forEachOrdered((ci) -> {
                try {
    ...
    public URL findResource(String name) {
        ...
        if (classesURL != null) {
            if (verbose) {
                System.out.println("Loading resource " + name + " from " + Arrays.toString(classesURL));
            }
            try {
    ...
    public void setVerbose(boolean verbose) {
        VmSystemClassLoader.verbose = verbose;
    }
    ...
    protected void initialize() {
        if (classInfos == null) {
            final TreeMap<String, ClassInfo> classInfos = new TreeMap<>();
            final VmType[] list = bootClasses;
            final int count = list.length;
            for (int i = 0; i < count; i++) {
