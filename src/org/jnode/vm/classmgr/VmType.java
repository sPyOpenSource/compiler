...
public abstract class VmType<T> extends VmAnnotatedElement implements
     ...
     * @return VmClass
     */
    private VmArrayClass<T[]> createArrayClass(boolean link,
        ...
        final VmArrayClass<T[]> arrayClass = new VmArrayClass<>(name, this
     ...
     */
    @SuppressWarnings("unchecked")
    private Class<T> asClass(boolean isBuildEnv) {
        if (javaClassHolder == null) {
            javaClassHolder = new VmIsolateLocal<>();
        }
...
//                javaClassHolder.set(new Class(this));
     ...
     */
    @Inline
    private void addIsolatedState(int value) {
        if (VmMagic.isRunningJNode()) {
     ...
     * @return The method
     */
    private VmMethod getMethod(String name, String signature,
    ...
    private synchronized void doPrepare() {
        ...
        final HashSet<VmInterfaceClass<?>> all = new HashSet<>();
     ...
     * @return The number of compiled methods
     */
    private int doCompileRuntime(int optLevel, boolean enableTestCompilers) {
        final VmMethod[] mt = this.methodTable;
        int compileCount = 0;
        if (mt != null) {
            final int count = mt.length;
     ...
     * @return The number of compiled methods
     */
    private int doCompileRuntime(String methodName, int optLevel, boolean enableTestCompilers) {
        final VmMethod[] mt = this.methodTable;
        int compileCount = 0;
        if (mt != null) {
            final int count = mt.length;
    ...
    private int doDisassemble(String methodName, int optLevel,
                                    boolean enableTestCompilers, Writer writer) {
        final VmMethod[] mt = this.methodTable;
        int disasmCount = 0;
        if (mt != null) {
     ...
     */
    @NoInline
    private void linkAndInitialize() {
        link();
