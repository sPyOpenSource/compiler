...
class PluginListEditor {
    ...
    public PluginListEditor(PluginRepository pluginRepository, String fileName) {
        ...
        file.add(openDir);
        openDir.addActionListener((ActionEvent e) -> {
            try {
    ...
    private void deselectPlugin(Set<Plugin> plugins) {
        Set<Plugin> usedby = new HashSet<>();
        plugins.forEach((p) -> {
            p.allUsed().stream().filter((u) -> (repository.isSelected(u.getId()))).forEachOrdered((u) -> {
                usedby.add(u);
            });
        });
    ...
    private void selectPlugin(Set<Plugin> plugins) {
        Set<Plugin> requires = new HashSet<>();
        plugins.forEach((p) -> {
            p.allRequired().stream().filter((u) -> (!repository.isSelected(u.getId()))).forEachOrdered((u) -> {
                requires.add(u);
            });
        });
