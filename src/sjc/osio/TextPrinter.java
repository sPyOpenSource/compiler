...
public abstract class TextPrinter {
  ...
  public void printHexFix(int i, int l) {
    ...
    for (c=0; c<l; c++) {
      d=(i>>>((l-c-1)<<2))&0xF;
      if (d<10) print((char)(d+48));
      else print((char)(d+55));
    }
  }
  
  public void printSequence(byte[] data, int srcStart, int srcLength,
      boolean suppressSpecialChars, boolean stopAtLineFeed) {
    byte t;
    while (srcLength-->0) {
      if (data.length>srcStart) {
        ...
        else if (suppressSpecialChars && t<(byte)32) t=(byte)' ';
        print((char)t);
      }
      else return;
    }
