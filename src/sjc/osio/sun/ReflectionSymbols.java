...
public class ReflectionSymbols extends SymbolInformer {
  ...
  private class Converter {
     ...
     */
    private final HashMap<String, VrblOffset> varToPos;
    ...
    private Converter(Class<?> clss, Unit sjcClass, Unit javaLangObject) {
      ...
      while (unit!=null && unit!=javaLangObject) {
        vars=unit.vars;
        while (vars!=null) {
          if (unit.name.equals(CLSS_MARKER) && vars.name.equals(VAR_MARKER)) hasMarker=true;          
          if ((vars.modifier&Modifier.M_STAT)==0) varToPos.put(vars.name, new VrblOffset(vars.relOff, vars.location==AccVar.L_INSTIDS));
          vars=vars.nextVrbl;            
        }
    ...
    private int arrDim(String javaArray) {
      ...
      for (int i=0; i<len; i++) if (javaArray.charAt(i)=='[') result++;
      return result;
    }
    ...
    private int getArrayType(String javaArray) {
      switch (javaArray.charAt(javaArray.length()-1)) {
        case 'B': return StdTypes.T_BYTE;
        case 'S': return StdTypes.T_SHRT;
        case 'I': return StdTypes.T_INT;
        case 'C': return StdTypes.T_CHAR;
        case 'J': return StdTypes.T_LONG;
        case 'Z': return StdTypes.T_BOOL;
      }
      if (javaArray.indexOf("[L")!=-1) return StdTypes.T_PTR;
      return -1;
    }
    ...
    private boolean isExcludeable(Object instance) {
      ...
      if (fullBlown) return false;
      String className=instance.getClass().getCanonicalName();
      for (int i=0; i<CLSS_EXCLUDE.length; i++) if (className.equals(CLSS_EXCLUDE[i])) return true;
      if (skipSJC) {
        if (instance instanceof Pack && isInSJCPack((Pack)instance)) return true;
        if (instance instanceof Unit && isInSJCPack(((Unit)instance).pack.packDest)) return true;
      }
      return false;
    }
    ...
    private boolean isInSJCPack(Pack p) {
      if (p==null || p.name.equals("sjc")) return false;
      while (p.outer!=null && p.outer.name!=null) p=p.outer;
    ...
    private Object createSJCArray(Object arrayRef) {
      ...
      switch (arrayType=getArrayType(arrayTypeName)) {
        case StdTypes.T_BYTE:
          ...
          for (int i=0; i<arrayLength; i++) {
            if (ctx.dynaMem) ctx.mem.putByte(indirOutputLocation, sarray.instIndirScalarTableSize+i, Array.getByte(arrayRef, i));
            else ctx.mem.putByte(outputLocation, sarray.instScalarTableSize+i, Array.getByte(arrayRef, i));
          }
          break;
        case StdTypes.T_SHRT: case StdTypes.T_CHAR:
          ...
          //set data
          for (int i=0; i<arrayLength; i++) {
            if (ctx.dynaMem) ctx.mem.putShort(indirOutputLocation, sarray.instIndirScalarTableSize+(i<<1), Array.getShort(arrayRef, i));
            else ctx.mem.putShort(outputLocation, sarray.instScalarTableSize+(i<<1), Array.getShort(arrayRef, i));
          }
          break;
        case StdTypes.T_INT:
          ...
          //set data
          for (int i=0; i<arrayLength; i++) {
            if (ctx.dynaMem) ctx.mem.putInt(indirOutputLocation, sarray.instIndirScalarTableSize+(i<<2), Array.getInt(arrayRef, i));
            else ctx.mem.putInt(outputLocation, sarray.instScalarTableSize+(i<<2), Array.getInt(arrayRef, i));
          }
          break;
        case StdTypes.T_LONG:
          ...
          //set data
          for (int i=0; i<arrayLength; i++) {
            if (ctx.dynaMem) ctx.mem.putLong(indirOutputLocation, sarray.instIndirScalarTableSize+(i<<3), Array.getLong(arrayRef, i));
            else ctx.mem.putLong(outputLocation, sarray.instScalarTableSize+(i<<3), Array.getLong(arrayRef, i));
          }
          break;
        case StdTypes.T_BOOL:
          ...
          //set data
          for (int i=0; i<arrayLength; i++) {
            if (ctx.dynaMem) ctx.mem.putByte(indirOutputLocation, sarray.instIndirScalarTableSize+i, Array.getBoolean(arrayRef, i) ? (byte)1 : (byte)0);
            else ctx.mem.putByte(outputLocation, sarray.instScalarTableSize+i, Array.getBoolean(arrayRef, i) ? (byte)1 : (byte)0);
          }
          break;
        case StdTypes.T_PTR:
          ...
          outputLocation=ctx.mem.allocateArray(arrayLength, 1, -1, 0, 
              sjcType.outputLocation);
          for (int i=0; i<arrayLength; i++) {
            ...
            if (arrElement!=null) {
              ...
              //check for strings
              else if (arrElement instanceof String string) {
                Object strAddr=ctx.allocateString(string);
                ...
                if (!toDo.contains(arrElement)) toDo.add(arrElement);
      ...
      if (outputLocation!=null) createdInstances.put(arrayRef, outputLocation);
      else {
    ...
    public boolean createSJCInstance(Object instance) {
      ...
      if (createdInstances.get(instance)!=null) return true;
      if (instance.getClass()!=sunClass) {
        ctx.out.println("instance type and transformer type do not match");
        return false;
      }
      //allocate empty object
      location=ctx.mem.allocate(sjcClass.instScalarTableSize,
          sjcClass.instIndirScalarTableSize, 
          sjcClass.instRelocTableEntries, sjcClass.outputLocation);
            ...
            if ((variable=varToPos.get(fieldName))!=null) {
              ...
              try {
                ...
                if (fields[i].getType().isPrimitive()) {
                  ...
                  targetAddr=(ctx.indirScalars && variable.isIndirect) ?
                      indirAddr : location;
                  ...
                  else ctx.out.println("! unknown primitive type");
                }
                //reference handling
                  ...
                  //otherwise add this instance to the toDo-list
                    ...
                    if (!toDo.contains(value)) toDo.add(value);
              ...
              if (!allowParents) { //if parents are allowed, silently skip non-existing fields
                ctx.out.print("couldn't find variable with name ");
                ctx.out.print(fieldName); ctx.out.print(" in unit ");
...
//    }
    while (!toDo.isEmpty()) {
      ...
      if ((converter=transformers.get(className))==null) {
        ...
        converter=new Converter(currentInstance.getClass(), targetUnit, 
            javaLangObject);
    ...
    //tell statistics
    ctx.out.print(instanceCount); ctx.out.println(" instances traversed...");
    ctx.out.print("created "); ctx.out.print(transformers.size()); ctx.out.println(" transformer...");
    ...
    if ((targetVar=targetUnit.searchVariable(VAR_ROOT, ctx))==null
        || (targetVar.modifier&(Modifier.M_FIN|Modifier.M_STAT|Modifier.M_STRUCT))!=Modifier.M_STAT
        || targetVar.type.baseType!=TypeRef.T_QID || targetVar.type.arrDim!=0) {
  ...
  public boolean setParameter(String parm, TextPrinter v) {
    if (parm.equals("full")) fullBlown=true;
    else if (parm.equals("skipSJC")) skipSJC=true;
    else if (parm.equals("allowParents")) allowParents=true;
    else {
  ...
  private Unit findUnitByName(String fullQualName) {
    ...
    while (tokenizer.hasMoreTokens()) {
      ...
      if (u!=null) { //already found outer unit, search within
        Unit inner=u.innerUnits;
        while (inner!=null) {
          if (inner.name.equals(name)) break;
          inner=inner.nextUnit;
        }
        if (inner==null) return null; //did not find inner unit
        u=inner;
      }
      else if ((u=pack.searchUnit(name))==null && (pack=pack.searchSubPackage(name))==null) return null; //did not find unit or pack within pack
    }
    return u;
  }
}
