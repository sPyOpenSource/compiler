...
public class RTESymbols extends SymbolInformer {
  ...
  public boolean generateSymbols(UnitList newUnits, Context ictx) {
    ...
    if ((packUnit=ctx.rte.searchUnit("SPackage"))==null
        || (packUnit.modifier&(Modifier.M_ABSTR|Modifier.M_INDIR|Modifier.M_STRUCT))!=0) {
      ctx.out.println("unit rte.Package required for RTESymbols (non-abstract, non-indir, non-struct)");
      return false;
    }
    if ((rootVar=packUnit.searchVariable("root", ctx))==null
        || (rootVar.modifier&(Modifier.M_STAT|Modifier.M_STRUCT))!=Modifier.M_STAT
        || rootVar.type.baseType!=TypeRef.T_QID || rootVar.type.qid.unitDest!=packUnit
        || rootVar.type.arrDim!=0) {
      ctx.out.println("variable static SPackage root needed in rte.SPackage");
      return false;
    }
    if ((packNamePos=checkVrbl(packUnit, "name"))==AccVar.INV_RELOFF
        || (!noPackOuter && (packOuterPos=checkVrbl(packUnit, "outer"))==AccVar.INV_RELOFF)
        || (packSubPacksPos=checkVrbl(packUnit, "subPacks"))==AccVar.INV_RELOFF
        || (packNextPackPos=checkVrbl(packUnit, "nextPack"))==AccVar.INV_RELOFF
        || (packUnitsPos=checkVrbl(packUnit, "units"))==AccVar.INV_RELOFF
        || (unitNamePos=checkVrbl(ctx.rteSClassDesc, "name"))==AccVar.INV_RELOFF
        || (!noClassMod && (unitModifierPos=checkVrbl(ctx.rteSClassDesc, "modifier"))==AccVar.INV_RELOFF)
        || (unitNextUnitPos=checkVrbl(ctx.rteSClassDesc, "nextUnit"))==AccVar.INV_RELOFF
        || (!noClassPack && (unitPackPos=checkVrbl(ctx.rteSClassDesc, "pack"))==AccVar.INV_RELOFF)
        || (unitMthdsPos=checkVrbl(ctx.rteSClassDesc, "mthds"))==AccVar.INV_RELOFF
        || (mthdNameParPos=checkVrbl(ctx.rteSMthdBlock, "namePar"))==AccVar.INV_RELOFF
        || (mthdOwnerPos=checkVrbl(ctx.rteSMthdBlock, "owner"))==AccVar.INV_RELOFF
        || (mthdNextMthdPos=checkVrbl(ctx.rteSMthdBlock, "nextMthd"))==AccVar.INV_RELOFF
        || (mthdModifierPos=checkVrbl(ctx.rteSMthdBlock, "modifier"))==AccVar.INV_RELOFF) {
      return false;
    }
    if (noSLHI) {
    ...
    if ((rootObj=genPackage(ctx.root, null))==null) return false;
  ...
  private Object genPackage(Pack pack, Object outerObj) {
    ...
    while (pack!=null) {
      if ((packObj=ctx.mem.allocate(packUnit.instScalarTableSize, packUnit.instIndirScalarTableSize,
          packUnit.instRelocTableEntries, packUnit.outputLocation))==null) {
        ctx.out.println("error allocating package");
        ctx.err=true;
        return null;
      }
      if (pack.name!=null) {
        if ((nameObj=ctx.allocateString(pack.name))==null) return null;
        ctx.arch.putRef(packObj, packNamePos, nameObj, 0);
      }
      if (!noPackOuter) ctx.arch.putRef(packObj, packOuterPos, outerObj, 0);
      if (firstPackObj==null) lastPackObj=firstPackObj=packObj; //first run
      else ctx.arch.putRef(lastPackObj, packNextPackPos, lastPackObj=packObj, 0); //all other runs
  ...
  private Object genUnit(Unit unit, Object packObj) {
    ...
    while (unit!=null) {
      if (unit.outputLocation!=null && (unit.modifier&Modifier.M_INDIR)==0
          && (ctx.dynaMem || (unit.modifier&Modifier.MA_ACCSSD)!=0)) { //skip dummy-units of compiler and not-generated descriptors
        ctx.symGenSize+=addedUnitSize;
        if (firstUnitObj==null) lastUnitObj=firstUnitObj=unit.outputLocation; //first run
        else ctx.arch.putRef(lastUnitObj, unitNextUnitPos, lastUnitObj=unit.outputLocation, 0); //all other runs
        ctx.arch.putRef(unit.outputLocation, unitNamePos, ctx.allocateString(unit.name), 0);
        if (!noClassPack) ctx.arch.putRef(unit.outputLocation, unitPackPos, packObj, 0);
        ctx.arch.putRef(unit.outputLocation, unitMthdsPos, genMthds(unit.mthds), 0);
        if (!noClassMod) ctx.mem.putInt(unit.outputLocation, unitModifierPos, unit.modifier);
      }
      unit=unit.nextUnit;
    }
    return firstUnitObj;
  ...
  private Object genMthds(Mthd mthd) {
    ...
    while (mthd!=null) {
      if (mthd.outputLocation!=null) { //skip not generated methods
        ctx.symGenSize+=addedMthdSize;
        if (firstMthdObj==null) lastMthdObj=firstMthdObj=mthd.outputLocation; //first run
        else ctx.arch.putRef(lastMthdObj, mthdNextMthdPos, lastMthdObj=mthd.outputLocation, 0); //all other runs
        myBuffer.used=0;
        mthd.printNamePar(myBuffer);
        ctx.arch.putRef(mthd.outputLocation, mthdNameParPos,
            ctx.allocateString(new String(myBuffer.data, 0, myBuffer.used)), 0);
        ctx.arch.putRef(mthd.outputLocation, mthdOwnerPos, mthd.owner.outputLocation, 0);
        if (mthd.param!=null &&
            mthd.param.type.compareType(ctx.stringType, true, ctx)!=TypeRef.C_NP
            && mthd.param.nextParam!=null && mthd.param.nextParam.type.isIntType()
            && mthd.param.nextParam.nextParam==null) command=0x80000000;
        else command=0;
