...
public class MthdSymbols extends SymbolInformer {
  ...
  public boolean generateSymbols(UnitList newUnits, Context ictx) {
    ...
    if ((mthdNameParPos=checkVrbl(ictx.rteSMthdBlock, "namePar"))==AccVar.INV_RELOFF) return false;
    if (chainMethodBlocks) {
      if ((mthdNextMthdPos=checkVrbl(ictx.rteSMthdBlock, "nextMthd"))==AccVar.INV_RELOFF
          || (mthdFirstMthdPos=checkVrbl(ictx.rteSMthdBlock, "firstMthd"))==AccVar.INV_RELOFF) return false;
    }
    ...
    while (newUnits!=null) {
      ...
      while (mthds!=null) {
        if (mthds.outputLocation!=null) {
          ...
          ictx.arch.putRef(mthds.outputLocation, mthdNameParPos,
              ictx.allocateString(new String(myBuffer.data, 0, myBuffer.used)), 0);
          if (chainMethodBlocks) {
            if (lastMthd==null) { //enter first method in static field
              ctx.arch.putRef(ictx.rteSMthdBlock.outputLocation, mthdFirstMthdPos, mthds.outputLocation, 0);
            }
            else { //enter method in last method
