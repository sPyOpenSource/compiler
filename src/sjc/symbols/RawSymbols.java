...
public class RawSymbols extends SymbolInformer {
  ...
  public boolean generateSymbols(UnitList newUnits, Context ictx) {
    ...
    if ((myUnit=ctx.root.searchUnit(list))==null
        || (myUnit.modifier&(Modifier.M_ABSTR|Modifier.M_INDIR|Modifier.M_STRUCT))!=0) {
      ctx.out.print("unit ");
      ctx.out.print(clssBootstrap);
      ctx.out.println(" required for RawSymbols (non-abstract, non-indir, non-struct)");
      return false;
    }
    if ((myVar=myUnit.searchVariable("info", ctx))==null
        || (myVar.modifier&(Modifier.M_STAT|Modifier.M_STRUCT))!=Modifier.M_STAT
        || myVar.type.baseType!=TypeRef.T_BYTE || myVar.type.arrDim!=1) {
    ...
    else addrOff=ctx.rteSArray.instScalarTableSize;
    //copy information
    while (off<myBuffer.used) ctx.mem.putByte(addr, addrOff++, (byte)myBuffer.data[off++]);
    //everything ok
    return true;
  }
  ...
  private void genUnitSymbols(Unit unit) {
    while (unit!=null) {
      ...
      if (unit.extsID!=null && unit.extsID.unitDest!=ctx.langRoot) unit.extsID.printFullQID(myBuffer);
      myBuffer.print('!');
