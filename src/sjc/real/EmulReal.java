...
public class EmulReal extends Real {
  ...
  public int buildFloat(long mant, int base10exp) {
    ...
    if (mant == 0l) return FLT_POS_ZERO;
    //get sign bit
    if (mant < 0l) {
      neg = true;
      mant = -mant;
    ...
    if (base10exp != 0) {
      //check for overflow
      if (base10exp < -38) return neg ? FLT_NEG_ZERO : FLT_POS_ZERO;
      if (base10exp > 39) return neg ? FLT_NEG_INF : FLT_POS_INF;
    ...
    if ((tmp = bitsUsed(mant) - 31) > 0) return buildFloat(neg, (int)(mant >>> tmp), base2exp + tmp);
  ...
  public int buildFloat(boolean negative, int mant, int base2exp) {
    ...
    if (mant == 0) return negative ? FLT_POS_ZERO : FLT_NEG_ZERO;
    //set res as mant using bit 24
    if ((tmp = bitsUsed(mant) - (FLT_EXP_SHIFT + 1)) == 0) res = mant; //nothing to adopt
    else { //adjust base2exp and shift
      base2exp += tmp;
      if (tmp > 0) { //too many bits used, reduce precision with rounding
        res = shiftRounded32(mant, tmp);
        if (bitsUsed(res) > FLT_EXP_SHIFT + 1) { //overflow after rounding
          res = res >>> 1;
          base2exp++;
        }
      }
      else res = mant << -tmp; //too few bits used, fill up with zeros
    }
    //check exponent and build value
    if (base2exp > 127) res = FLT_POS_INF; //infinity
    else if (base2exp < -173) res = FLT_POS_ZERO; //result is 0
    else if (base2exp < -FLT_EXP_BIAS + 1) res = shiftRounded32(res, -FLT_EXP_BIAS + 1 - base2exp); //subnormal value
    else res = (res & FLT_MANT_MASK) | ((base2exp + FLT_EXP_BIAS) << FLT_EXP_SHIFT); //normal value
    //return value
  ...
  public int binOpFloat(int op1, int op2, int op) {
    switch (op) {
      case A_MINUS: return addFloat(op1, negateFloat(op2));
      case A_PLUS:  return addFloat(op1, op2);
      case A_MUL:   return mulFloat(op1, op2);
      case A_DIV:   return divFloat(op1, op2);
    }
    return 0;
  }
  ...
  public int addFloat(int op1, int op2) {
    ...
    if (isNaNFloat(op1) || isNaNFloat(op2)) return FLT_NAN;
    neg1 = isNegativeFloat(op1);
    neg2 = isNegativeFloat(op2);
    if ((inf1 = isInfiniteFloat(op1)) | (inf2 = isInfiniteFloat(op2))) { //at least one op is infinite
      if (inf1 && inf2) { //both infinite
        if (neg1 == neg2) return op1; //both have same sign
        return FLT_NAN; //different signs -> result is NaN
      }
      if (inf1) return op1; //infinite+finite
      return op2; //finite+infinite
    }
    if ((zero1 = isZeroFloat(op1)) | (zero2 = isZeroFloat(op2))) { //at least one op is zero
      if (zero1 && zero2) { //both zero
        if (neg1 == neg2) return op1; //keep sign if same zero-values
        return FLT_POS_ZERO; //positive zero as result of addition of different zero-values
      }
      if (zero1) return op2; //zero+nonzero
    ...
    if ((expDiff = exp1 - exp2) < 0) { //exp1<exp2
      mant1 = shiftRounded32(mant1, -expDiff);
      exp1 = exp2;
    }
    else if (expDiff > 0) mant2 = shiftRounded32(mant2, expDiff); //exp1>exp2
    
    //get resulting sign in neg1, if signs different negate smaller mantissa
    if (neg1 ^ neg2) { //signs different
      if (mant1 > mant2) mant2 = -mant2; //negate mant2, keep neg1
      else { //negate mant1, invert neg1
        mant1 = -mant1;
        neg1 = !neg1;
      }
    }
    ...
    mant1 += mant2;
    if ((tmp = buildFloat(neg1, mant1, exp1)) == FLT_NEG_ZERO) return FLT_POS_ZERO; //make zero positive
    return tmp;
  }
  ...
  public int mulFloat(int op1, int op2) {
    ...
    if (isInfiniteFloat(op1) || isInfiniteFloat(op2)) {
      if (isZeroFloat(op1) || isZeroFloat(op2)) return FLT_NAN;
    ...
    if ((bits = bitsUsed(mant) - 31) > 0) return buildFloat(neg, (int)(mant >>> bits), exp + bits);
  ...
  public int divFloat(int op1, int op2) {
    ...
    neg = isNegativeFloat(op1) ^ isNegativeFloat(op2);
    if ((inf1 = isInfiniteFloat(op1)) | (inf2 = isInfiniteFloat(op2))) { //at least one op is infinite
      if (inf1 && inf2) return FLT_NAN; //both infinite
      if (inf1) return neg ? FLT_NEG_INF : FLT_POS_INF; //first operand infinite
      return neg ? FLT_NEG_ZERO : FLT_POS_ZERO; //second operand infinite
    }
    if ((zero1=isZeroFloat(op1)) | (zero2 = isZeroFloat(op2))) { //at least one op is zero
      if (zero1 && zero2) return FLT_NAN; //both zero
      if (zero1) return neg ? FLT_NEG_ZERO : FLT_POS_ZERO; //first operand zero
    ...
    mant = (long)getMantissaFloat(op1);
  ...
  public int compareConstFloat(int le, int ri, int op) {
    switch (op) {
      case C_LW:
        if (isNaNFloat(le) || isNaNFloat(ri)) return 0;
        if (le == FLT_NEG_ZERO) le = FLT_POS_ZERO;
        if (ri == FLT_NEG_ZERO) ri = FLT_POS_ZERO;
  ...
  public long getLongOfFloat(int number) {
    ...
    if (isNaNFloat(number)) return 0l;
    negative = isNegativeFloat(number);
    exp = getExponentFloat(number);
    mant = (long)getMantissaFloat(number);
    if (exp > 0) {
  ...
  public long buildDouble(long mant, int base10exp) {
    ...
    if (mant == 0l) return DBL_POS_ZERO;
    //get sign bit
    if (mant < 0l) {
      neg = true;
      mant = -mant;
    ...
    if (base10exp != 0) {
      //check for overflow
      if (base10exp < -345) return neg ? DBL_NEG_ZERO : DBL_POS_ZERO;
      if (base10exp > 345) return neg ? DBL_NEG_INF : DBL_POS_INF;
      //left align for better precision
      if ((tmp = 59 - bitsUsed(mant)) > 0) {
        mant = mant << tmp;
        base2exp -= tmp;
  ...
  public long buildDouble(boolean negative, long mant, int base2exp) {
    ...
    if (mant == 0l) return negative ? DBL_NEG_ZERO : DBL_POS_ZERO;
    //set res as mant using bit 52
    if ((tmp = bitsUsed(mant) - (DBL_EXP_SHIFT + 1)) == 0) res = mant; //nothing to adopt
    else { //adjust base2exp and shift
      base2exp += tmp;
      if (tmp > 0) { //too many bits used, reduce precision with rounding
        res = shiftRounded64(mant, tmp);
        if (bitsUsed(res) > DBL_EXP_SHIFT + 1) { //overflow after rounding
          res = res >>> 1;
          base2exp++;
        }
      }
    ...
    //check exponent and build value
    if (base2exp > 960) res = DBL_POS_INF; //infinity
    else if (base2exp < -1085) res = DBL_POS_ZERO; //result is 0
    else if (base2exp < -DBL_EXP_BIAS + 1) res = shiftRounded64(res, -DBL_EXP_BIAS + 1 - base2exp); //subnormal value
    else res = (res & DBL_MANT_MASK) | (((long)(base2exp + DBL_EXP_BIAS)) << DBL_EXP_SHIFT); //normal value
    //return value
  ...
  public long binOpDouble(long op1, long op2, int op) {
    switch (op) {
      case A_MINUS: return addDouble(op1, negateDouble(op2));
      case A_PLUS:  return addDouble(op1, op2);
      case A_MUL:   return mulDouble(op1, op2);
      case A_DIV:   return divDouble(op1, op2);
    }
    return 0l;
  }
  ...
  public long addDouble(long op1, long op2) {
    ...
    //check special cases NaN and infinite and zero, extract components of operands
    if (isNaNDouble(op1) || isNaNDouble(op2)) return DBL_NAN;
    neg1 = isNegativeDouble(op1);
    neg2 = isNegativeDouble(op2);
    if ((inf1 = isInfiniteDouble(op1)) | (inf2 = isInfiniteDouble(op2))) { //at least one op is infinite
      if (inf1 && inf2) { //both infinite
        if (neg1 == neg2) return op1; //both have same sign
    ...
    if ((zero1 = isZeroDouble(op1)) | (zero2 = isZeroDouble(op2))) { //at least one op is zero
      if (zero1 && zero2) { //both zero
        if (neg1 == neg2) return op1; //keep sign if same zero-values
    ...
    else if (expDiff > 0) mant2 = shiftRounded64(mant2, expDiff); //exp1>exp2
    ...
    if ((tmp = buildDouble(neg1, mant1, exp1)) == DBL_NEG_ZERO) return DBL_POS_ZERO; //make zero positive
    return tmp;
  }
  ...
  public long mulDouble(long op1, long op2) {
    ...
    if (isInfiniteDouble(op1) || isInfiniteDouble(op2)) {
      if (isZeroDouble(op1) || isZeroDouble(op2)) return DBL_NAN;
    ...
    if (tmp3 == 0l) return buildDouble(neg, tmp1, exp); //upper bits stored in tmp3 all zero
    return buildDouble(neg, ((tmp1 & 0x03FFFFFFFFFFFFFFl) >>> (bits = bitsUsed(tmp3))) | (tmp3 << (58 - bits)),
        exp + bits); //shift tmp1 after masking carry to the right in a way that tmp3 has enough space
  }
  ...
  public long divDouble(long op1, long op2) {
    ...
    neg = isNegativeDouble(op1) ^ isNegativeDouble(op2);
    if ((inf1 = isInfiniteDouble(op1)) | (inf2 = isInfiniteDouble(op2))) { //at least one op is infinite
      if (inf1 && inf2) return DBL_NAN; //both infinite
      if (inf1) return neg ? DBL_NEG_INF : DBL_POS_INF; //first operand infinite
      return neg ? DBL_NEG_ZERO : DBL_POS_ZERO; //second operand infinite
    }
    if ((zero1 = isZeroDouble(op1)) | (zero2 = isZeroDouble(op2))) { //at least one op is zero
      if (zero1 && zero2) return DBL_NAN; //both zero
      if (zero1) return neg ? DBL_NEG_ZERO : DBL_POS_ZERO; //first operand zero
  ...
  public int compareConstDouble(long le, long ri, int op) {
    switch (op) {
      case C_LW:
        if (isNaNDouble(le) || isNaNDouble(ri)) return 0;
        if (le == DBL_NEG_ZERO) le = DBL_POS_ZERO;
        if (ri == DBL_NEG_ZERO) ri = DBL_POS_ZERO;
  ...
  public long getLongOfDouble(long number) {
    ...
    if (isNaNDouble(number)) return 0l;
  ...
  public int buildFloatFromDouble(long number) {
    int exp, shift;
    long mant;
    if (isNaNDouble(number)) return FLT_NAN;
    exp = getExponentDouble(number);
    mant = getMantissaDouble(number);
  ...
  public long buildDoubleFromFloat(int number) {
    if (isNaNFloat(number)) return DBL_NAN;
    return buildDouble(isNegativeFloat(number), (long)getMantissaFloat(number), getExponentFloat(number));
  }
  
  // ### --- ### little helper ### --- ###
  
  private int compareHelperFloat(int le, int ri) {
    if (le < 0) return (ri < 0) ? ri - le : -1;
    return (ri < 0) ? 1 : le - ri;
  }
  
  private int compareHelperDouble(long le, long ri) {
    //conversion to int possible because only sign and ==0 matters
    if (le < 0l) return (ri < 0l) ? (int)((ri - le) >>> 32) : -1;
  ...
  private static int bitsUsed(long number) {
    ...
    if ((int)(number >>> 32) == 0) return bitsUsed((int)number);
