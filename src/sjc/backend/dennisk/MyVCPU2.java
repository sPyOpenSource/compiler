...
public class MyVCPU2 extends Architecture {
  ...
  private int freeRegSearch(int mask, int type) {
    int reg, regd;
    if ((type==StdTypes.T_BYTE) || (type==StdTypes.T_BOOL)
        || (type==StdTypes.T_SHRT) || (type==StdTypes.T_CHAR)) { //need one register
      return bitSearch(mask, 1);
    }
    //need two or four registers
    reg=doubleBitSearch(mask);
    if ((type==StdTypes.T_LONG) || (type==StdTypes.T_DBL)) { //need four registers
      if ((regd=doubleBitSearch(mask&~reg))==0) return 0;
      reg|=regd;
    }
    return reg;
  }
  ...
  protected int storeReg(int regs) {
    ...
    for (int i=0; i<=RegMaxGP; i++) if ((regs&(1<<i))!=0) {
      int start=i;
  ...
  protected void restoreReg(int regs) {
    usedRegs|=regs;
    writtenRegs|=regs;
    for (int i=RegMaxGP; i>=0; i--) if ((regs&(1<<i))!=0) {
      int end=i;
  ...
  private int getRegCount(int type) {
    switch (type) {
      ...
      case StdTypes.T_SHRT: case StdTypes.T_CHAR: return 1;
      case StdTypes.T_INT:  case StdTypes.T_FLT:
      case StdTypes.T_PTR:                        return 2;
      case StdTypes.T_LONG: case StdTypes.T_DBL:
      case StdTypes.T_DPTR:                       return 4;
    }
  ...
  public void codeProlog() {
    ...
    if (curMthd.varSize>0) {
      ...
      if (curMthd.varSize<=8) for (i=0; i<curMthd.varSize; i+=2) ins(I_PUSHrr, R_TMP_LO, R_TMP_LO);
      else {
  ...
  public void codeEpilog(Mthd outline) {
    ...
    if (stackRed!=0) {
      if (stackRed<=0xFFFF) ins(I_ADDDri, R_STACK_32, stackRed);
      else {
  ...
  public void genLoadConstDoubleOrLongVal(int dst, long val, boolean asDouble) {
    int reg, i;
    for (i=1; i<=4; i++) {
      if ((reg=getReg(dst, i, true))==-1) return;
  ...
  public void genLoadVarAddr(int dst, int src, Object loc, int off) {
    ...
    if ((dst1=getReg(dst, 1, true))==-1
        || (dst2=getReg(dst, 2, true))==-1
        || !checkConsecReg(dst1, dst2)) return;
    if (loc!=null) {
      if (src!=0) {
    ...
    else if (src==0) ins(I_LDPra, dst1, off);
    else {
      if (off==0 && src==dst) return;
      if (src==regBase) {
        off+=curVarOffParam;
        ...
        int src1, src2;
        if ((src1=getReg(src, 1, false))==-1
            || (src2=getReg(src, 2, false))==-1
            || !checkConsecReg(src1, src2)) return;
        ins(I_MOVDrr, dst1, src1);
      }
      if (off!=0) {
        if (off>0 && off<=65535) ins(I_ADDDri, dst1, off);
        else if (off<0 && off>=-65535) ins(I_SUBDri, dst1, -off);
        else {
  ...
  public void genLoadVarVal(int dst, int src, Object loc, int off, int type) {
    ...
    int i_rr, i_rro, i_ra;
    int src1=0, src2=0;
    if ((dst1=getReg(dst, 1, true))==-1) return;
    switch (type) {
    ...
    if (src!=0) {
      ...
      else if ((src1=getReg(src, 1, false))==-1
          || (src2=getReg(src, 2, false))==-1
          || !checkConsecReg(src1, src2)) return;
      if (off==0) ins(i_rr, dst1, src1);
      else if (off>=0 && off<=0xFFFF) ins(i_rro, dst1, src1, off);
      else {
      ...
      int pos=mem.getAddrAsInt(loc, off);
      if (pos>=0 && pos<0x100000) ins(i_ra, dst1, pos);
      else {
  ...
  public void genConvertVal(int dst, int src, int toType, int fromType) {
    ...
    Instruction jd, end;
    if (toType==fromType) {
      if (dst!=src) fatalError(ERR_INVREGUSE);
      return;
    }
    switch (toType) {
      case StdTypes.T_BYTE:
      ...
      case StdTypes.T_CHAR:
        if ((src1=getReg(src, 1, false))==-1
            || (dst1=getReg(dst, 1, true))==-1) return;
        if (src1!=dst1) {
  ...
  public void genDup(int dst, int src, int type) {
    ...
    if (dst==src) return;
    if ((dstReg=getReg(dst, 1, true))==-1
        || (srcReg=getReg(src, 1, false))==-1) return;
    ins(I_MOVrr, dstReg, srcReg);
    if (cnt>1) {
      if ((dstReg=getReg(dst, 2, true))==-1
          || (srcReg=getReg(src, 2, false))==-1) return;
      ins(I_MOVrr, dstReg, srcReg);
      if (cnt==4) {
        if ((dstReg=getReg(dst, 3, true))==-1
            || (srcReg=getReg(src, 3, false))==-1) return;
        ins(I_MOVrr, dstReg, srcReg);
        if ((dstReg=getReg(dst, 4, true))==-1
            || (srcReg=getReg(src, 4, false))==-1) return;
        ins(I_MOVrr, dstReg, srcReg);
      }
    }
  }
  ...
  public void genPush(int src, int type) {
    ...
    if ((src1Reg=getReg(src, 1, false))==-1) return;
    if (cnt==4) {
      if ((src1Reg=getReg(src, 3, false))==-1
          || (src2Reg=getReg(src, 4, false))==-1
          || !checkConsecReg(src1Reg, src2Reg)) return;
      insPush(src2Reg, src1Reg);
    }
    if (cnt==1) insPush(src1Reg, src1Reg);
    else {
      if ((src2Reg=getReg(src, 2, false))==-1) return;
      if (!checkConsecReg(src1Reg, src2Reg)) return;
      insPush(src2Reg, src1Reg);
    }
  }
  ...
  public void genPop(int dst, int type) {
    ...
    if ((dst1Reg=getReg(dst, 1, true))==-1) return;
    if (cnt==1) insPop(dst1Reg, dst1Reg);
    else {
      if ((dst2Reg=getReg(dst, 2, true))==-1) return;
      if (!checkConsecReg(dst1Reg, dst2Reg)) return;
      insPop(dst2Reg, dst1Reg);
      if (cnt==4) {
        if ((dst1Reg=getReg(dst, 3, true))==-1
            || (dst2Reg=getReg(dst, 4, true))==-1
            || !checkConsecReg(dst1Reg, dst2Reg)) return;
        insPop(dst2Reg, dst1Reg);
      }
    }
  }
  ...
  public void genAssign(int dst, int src, int type) {
    ...
    if ((dst1=getReg(dst, 1, true))==-1
        || (dst2=getReg(dst, 2, true))==-1
        || !checkConsecReg(dst1, dst2)
        || (src1=getReg(src, 1, true))==-1) return;
    switch (type) {
      case StdTypes.T_BYTE: case StdTypes.T_BOOL:
      ...
      case StdTypes.T_SHRT: case StdTypes.T_CHAR:
        ...
        break;
      case StdTypes.T_INT: case StdTypes.T_FLT: case StdTypes.T_PTR:
        if ((src2=getReg(src, 2, true))==-1
            || !checkConsecReg(src1, src2)) return;
        ins(I_STDrr, src1, dst1);
        break;
      case StdTypes.T_LONG: case StdTypes.T_DBL: case StdTypes.T_DPTR:
        if ((src2=getReg(src, 2, true))==-1
            || !checkConsecReg(src1, src2)) return;
        ins(I_STDrr, src1, dst1);
        if ((src1=getReg(src, 3, true))==-1
            || (src2=getReg(src, 4, true))==-1
            || !checkConsecReg(src1, src2)) return;
  ...
  public void genBinOp(int dst, int src1, int src2, int op, int type) {
    ...
    switch (type) {
      case StdTypes.T_BOOL:
        if ((r1=getReg(src1, 1, false))==-1 || (r2=getReg(src2, 1, false))==-1) return;
        switch (opType) {
          case Ops.S_ARI:
            switch (opPar) {
              case Ops.A_AND:
      ...
      case StdTypes.T_INT:
        if ((r1=getReg(src1, 1, false))==-1 || (r2=getReg(src2, 1, false))==-1
          || !checkConsecReg(r1, getReg(src1, 2, false)) || !checkConsecReg(r2, getReg(src2, 2, false))) return;
        switch (opType) {
          case Ops.S_ARI:
            switch (opPar) {
              case Ops.A_AND:
  ...
  public void genUnaOp(int dst, int src, int op, int type) {
    ...
    if ((reg=getReg(dst, 1, false))==-1) return;
    switch (type) {
      case StdTypes.T_BOOL:
        if (opPar==Ops.L_NOT) {
          ins(I_XORri, reg, 1);
      ...
      case StdTypes.T_SHRT: case StdTypes.T_CHAR:
        ...
        return;
      case StdTypes.T_INT:
        if (!checkConsecReg(reg, getReg(dst, 2, false))) return;
        switch (opPar) {
  ...
  public void genIncMem(int dst, int type) {
    ...
    if ((reg=getReg(dst, 1, false))==-1
        || !checkConsecReg(reg, getReg(dst, 2, false))) return;
    switch (type) {
      case StdTypes.T_BYTE:
  ...
  public void genLoadInstContext(int src) {
    ...
    if ((srcR=getReg(src, 1, false))==-1
        || !checkConsecReg(srcR, getReg(src, 2, false))) return;
  ...
  public void genCall(int off, int clssReg, int parSize) {
    int cR;
    if ((cR=getReg(clssReg, 0, false))==-1
        || !checkConsecReg(cR, getReg(clssReg, 2, false))) return;
    if (off!=0) ins(I_LDDrro, R_TMP_32, cR, off);
    else ins(I_LDDrr, R_TMP_32, cR);
  ...
  public int genComp(int src1, int src2, int type, int cond) {
    int src1R, src2R;
    if ((src1R=getReg(src1, 1, false))==-1
      || (src2R=getReg(src2, 1, false))==-1) return 0;
    switch (getRegCount(type)) {
      case 1: //byte, boolean, short, char
        if (cond==Ops.C_BO) ins(I_CMPUrr, src1R, src2R);
        else ins(I_CMPSrr, src1R, src2R);
        break;
      case 2: //int, ptr
        if (!checkConsecReg(src1R, getReg(src1, 2, false))
          || !checkConsecReg(src2R, getReg(src2, 2, false))) return 0;
        if (cond==Ops.C_BO) ins(I_CMPUDrr, src1R, src2R);
        else ins(I_CMPSDrr, src1R, src2R);
        break;
      default:
  ...
  public int genCompValToConstVal(int src, int val, int type, int cond) {
    int srcR;
    if ((srcR=getReg(src, 1, false))==-1) return 0;
    switch (getRegCount(type)) {
      case 1:
        if (cond==Ops.C_BO) ins(I_CMPUri, srcR, val&0xFFFF);
        else ins(I_CMPSri, srcR, val&0xFFFF);
        break;
      case 2:
        if (!checkConsecReg(srcR, getReg(src, 2, false))) return 0;
        ins(I_LDri, R_TMP_LO, val&0xFFFF);
        ins(I_LDri, R_TMP_HI, val>>>16);
        if (cond==Ops.C_BO) ins(I_CMPUDrr, srcR, R_TMP_32);
        else ins(I_CMPSDrr, srcR, R_TMP_32);
        break;
      default:
  ...
  public int genCompPtrToNull(int src, int cond) {
    int reg;
    if ((reg=getReg(src, 1, false))==-1
        || !checkConsecReg(reg, getReg(src, 2, false))) return 0;
    ins(I_TSTDr, reg);
    return cond;
  }
  ...
  public void genMoveToPrimary(int src, int type) {
    ...
    for (i=0; i<count; i++) {
      if ((srcR=getReg(src, i+1, false))==-1) return;
      if (srcR!=i) ins(I_MOVrr, i, srcR);
    }
  }
  ...
  public void genMoveFromPrimary(int dst, int type) {
    ...
    for (i=count-1; i>=0; i--) {
      if ((dstR=getReg(dst, i+1, true))==-1) return;
      if (dstR!=i) ins(I_MOVrr, dstR, i);
    }
  }
  ...
  public void genCheckBounds(int addrReg, int offReg, int checkToOffset, Instruction onSuccess) {
    //code from Benedikt
    int addr = -1, off = -1;
    if ((addr = getReg(addrReg, 1, true)) == -1 || (off = getReg(offReg, 1, false)) == -1) return;
  ...
  public void genLoadDerefAddr(int destReg, int objReg, int indReg, int baseOffset, int entrySize) {
    int dstR, objR, indR;
    if ((dstR=getReg(destReg, 1, true))==-1
        || !checkConsecReg(dstR, getReg(destReg, 2, true))
        || (objR=getReg(objReg, 1, false))==-1
        || !checkConsecReg(objR, getReg(objReg, 2, false))
        || (indR=getReg(indReg, 1, false))==-1
        || !checkConsecReg(indR, getReg(indReg, 2, false))) return;
    if (dstR!=objR) ins(I_MOVDrr, dstR, objR);
    if (entrySize<0) {
    ...
    else ins(I_MOVDrr, R_TMP_32, indR);
    switch (entrySize) {
    ...
    if (baseOffset!=0) {
      if (baseOffset>0 && baseOffset<=0xFFFF) ins(I_ADDDri, R_TMP_32, baseOffset);
        ...
        if (baseOffset<=0xFFFF) ins(I_SUBDri, R_TMP_32, baseOffset);
        else {
  ...
  private void ins(int op, int par1, int par2) {
    ...
    i.type=op;
    switch (op&PAR_P1M) {
      case PAR_IMM1: case PAR_RMA1:
        i.iPar1=par1;
        break;
      case PAR_REG1: case PAR_ADR1:
        i.reg0=par1;
        break;
      default:
    ...
    switch (op&PAR_P2M) {
      case PAR_IMM2: case PAR_RMA2: case PAR_AAD2:
        i.iPar2=par2;
        break;
      case PAR_REG2: case PAR_ADR2:
        i.reg1=par2;
        break;
      default:
  ...
  private void insPush(int from, int to) {
    ...
    if (from<=to) curVarOffParam+=2*(to-from+1);
    else if (from==to+1) curVarOffParam+=4;
    else fatalError("invalid from/to in insPush");
  }
  ...
  private void insPop(int from, int to) {
    ...
    else fatalError("invalid from/to in insPop");
  }
  
  protected void printCode(TextPrinter v, Instruction first, String comment, boolean asmMode) {
    Instruction now;
    int insCnt=0;
    if (asmMode) ctx.printUniqueMethodName(v, mthdContainer);
    else {
  ...
  private int print(TextPrinter v, Instruction i, boolean asmMode) {
    ...
    if (i.type==I_MAGC) {
      ...
      for (int c=0; c<i.size; c++) {
        if (c>0) v.print(',');
        v.print("0x");
        v.printHexFix((int)i.code[c], 2);
      }
      return 1;
    }
    switch (i.type) {
      case I_RET:     v.print("RET"); break;
      case I_CLRC:    v.print("CLRC"); break;
      case I_SETC:    v.print("SETC"); break;
      case I_JUMPabs: v.print("JUMP"); break;
      case I_CALLabs: case I_CALLr: v.print("CALL"); break;
      case I_LDPra:   v.print("LDP"); break;
      case I_JPZra:   v.print("JPZ"); break;
      case I_JPNZra:  v.print("JPNZ"); break;
      case I_JPZabs:  v.print("JPZ"); break;
      case I_JPNZabs: v.print("JPNZ"); break;
      case I_JPGRabs: v.print("JPGR"); break;
      case I_JPLEabs: v.print("JPLE"); break;
      case I_JPEGabs: v.print("JPEG"); break;
      case I_JPELabs: v.print("JPEL"); break;
      case I_SEXTri:  v.print("SEXT"); break;
      case I_TSTri:   v.print("TST"); break;
      case I_NOTr:    v.print("NOT"); break;
      case I_TSTDr:   v.print("TSTD"); break;
      case I_ASRDr:   v.print("ASRD"); break;
      case I_SFTLri:  v.print("SFTL"); break;
      case I_SFTRri:  v.print("SFTR"); break;
      case I_ROTLri:  v.print("ROTL"); break;
      case I_ROTRri:  v.print("ROTR"); break;
      case I_INCri:   v.print("INC"); break;
      case I_DECri:   v.print("DEC"); break;
      case I_INCDri:  v.print("INCD"); break;
      case I_DECDri:  v.print("DECD"); break;
      case I_MOVrr:   v.print("MOV"); break;
      case I_PUSHrr:  v.print("PUSH"); break;
      case I_POPrr:   v.print("POP"); break;
      case I_MOVDrr:  v.print("MOVD"); break;
      case I_MULDrr:  v.print("MULD"); break;
      case I_DIVDrr:  v.print("DIVD"); break;
      case I_MODDrr:  v.print("MODD"); break;
      case I_ANDrr:   v.print("AND"); break;
      case I_ORrr:    v.print("OR"); break;
      case I_XORrr:   v.print("XOR"); break;
      case I_ADCrr:   v.print("ADC"); break;
      case I_SBCrr:   v.print("SBC"); break;
      case I_CMPUrr:  v.print("CMPU"); break;
      case I_CMPSrr:  v.print("CMPS"); break;
      case I_CMPUDrr: v.print("CMPUD"); break;
      case I_CMPSDrr: v.print("CMPSD"); break;
      case I_ADDDrr:  v.print("ADDD"); break;
      case I_ANDDrr:  v.print("ANDD"); break;
      case I_ORDrr:   v.print("ORD"); break;
      case I_XORDrr:  v.print("XORD"); break;
      case I_SBCDrr:  v.print("SBCD"); break;
      case I_LDBra: case I_LDBrr: case I_LDBrro: v.print("LDB"); break;
      case I_LDWra: case I_LDWrr: case I_LDWrro: v.print("LDW"); break;
      case I_LDDra: case I_LDDrr: case I_LDDrro: v.print("LDD"); break;
      case I_STBrr:   v.print("STB"); break;
      case I_STWrr:   v.print("STW"); break;
      case I_STDrr: case I_STDror: v.print("STD"); break;
      case I_ADDDri:  v.print("ADDD"); break;
      case I_SUBDri:  v.print("SUBD"); break;
      case I_ANDri:   v.print("AND"); break;
      case I_ORri:    v.print("OR"); break;
      case I_XORri:   v.print("XOR"); break;
      case I_SBCri:   v.print("SBC"); break;
      case I_CMPUri:  v.print("CMPU"); break;
      case I_CMPSri:  v.print("CMPS"); break;
      case I_ADCri:   v.print("ADC"); break;
      case I_LDri:    v.print("LD"); break;
      case I_ADDri:   v.print("ADD"); break;
      case I_SUBri:   v.print("SUB"); break;
      default:
    ...
    if ((i.type&PAR_P1M)!=0) {
      ...
      switch (i.type&PAR_P1M) {
        case PAR_REG1: v.print('r'); v.print(i.reg0); break;
        case PAR_IMM1: v.print("#0x"); v.printHexFix(i.iPar1, 4); break;
        case PAR_ADR1: v.print("(r"); v.print(i.reg0); v.print(')'); break;
        case PAR_AOF1: v.print("(r"); v.print(i.reg0); v.print('+'); v.print(i.iPar3); v.print(')'); break;
        case PAR_AAD1: v.print('('); printAddressLabel(v, i.iPar1); v.print(')'); break;
        case PAR_RMA1: v.print('#'); printAddressLabel(v, i.iPar1); break;
        case PAR_JDT1: printJumpDest(v, i.jDest); break;
        case PAR_CDT1: printCallDest(v, i.refMthd); break;
      }
      if ((i.type&PAR_P2M)!=0) {
        v.print(',');
        switch (i.type&PAR_P2M) {
          case PAR_REG2: v.print('r'); v.print(i.reg1); break;
          case PAR_IMM2: v.print("#0x"); v.printHexFix(i.iPar2, 4); break;
          case PAR_ADR2: v.print("(r"); v.print(i.reg1); v.print(')'); break;
          case PAR_AOF2: v.print("(r"); v.print(i.reg1); v.print('+'); v.print(i.iPar3); v.print(')'); break;
          case PAR_AAD2: v.print('('); printAddressLabel(v, i.iPar2); v.print(')'); break;
          case PAR_RMA2: v.print('#'); printAddressLabel(v, i.iPar2); break;
          case PAR_JDT2: printJumpDest(v, i.jDest); break;
          case PAR_CDT2: printCallDest(v, i.refMthd); break;
        }
      }
    }
    return 1;
