...
public class MyCPU extends Architecture {
  ...
  public int genCompPtrToNull(int reg, int cond) {
    ...
    if ((cr1=getRegZPAddr(1, reg, StdTypes.T_PTR, false))==-1
        || (cr2=getRegZPAddr(2, reg, StdTypes.T_PTR, false))==-1) return 0;
    ins(I_LDAzp, cr1);
    ins(I_ORzp, cr2);
    return cond;
  }
  ...
  public void genLoadDerefAddr(int destReg, int objReg, int indReg, int baseOffset, int entrySize) {
    ...
    if ((dstR1=getRegZPAddr(1, destReg, StdTypes.T_SHRT, true))==-1
        || (dstR2=getRegZPAddr(2, destReg, StdTypes.T_SHRT, true))==-1
        || (objR1=getRegZPAddr(1, objReg, StdTypes.T_SHRT, false))==-1
        || (objR2=getRegZPAddr(2, objReg, StdTypes.T_SHRT, false))==-1
        || (indR1=getRegZPAddr(1, indReg, StdTypes.T_SHRT, false))==-1
        || (indR2=getRegZPAddr(2, indReg, StdTypes.T_SHRT, false))==-1) return;
    if (entrySize<0) {
      ins(I_CLA);
      ins(I_SEC);
      ins(I_SBCzp, indR1);
  ...
  public void genStoreVarVal(int objReg, Object loc, int off, int valReg, int type) {
    ...
    if (objReg==regBase) {
      ...
      if (objReg==0) {
        ...
        ins(I_ADCimm, pos&0xFF);
        ins(I_TAX);
        if ((objR=getRegZPAddr(2, objReg, StdTypes.T_PTR, false))==-1) return;
        ins(I_LDAzp, objR);
        ins(I_ADCimm, pos>>>8);
        ins(I_TAY);
      }
      ...
      case StdTypes.T_INT: case StdTypes.T_LONG:
        switch (opType) {
          ...
          case Ops.S_BSH:
            ...
            redo=getUnlinkedInstruction();
            appendInstruction(redo);
            if ((dstR=getRegZPAddr(opPar==Ops.B_SHL ? 1 : count, dst, StdTypes.T_BYTE, true))==-1
                || (srcR1=getRegZPAddr(opPar==Ops.B_SHL ? 1 : count, src1, StdTypes.T_BYTE, false))==-1) return;
            switch (opPar) {
              case Ops.B_SHL:
                if (srcR1!=dstR) {
                  ins(I_LDAzp, srcR1);
  ...
  public void genBinOpConstDoubleOrLongRi(int dst, int src1, long val, int op, boolean asDouble) {
    ...
    switch (opType) {
      case Ops.S_ARI:
        for (i=0; i<8; i++) {
          if ((dstR=getRegZPAddr(i+1, dst, StdTypes.T_LONG, true))==-1
              || (srcR1=getRegZPAddr(i+1, src1, StdTypes.T_LONG, false))==-1) return;
      ...
      case Ops.S_BSH:
        ...
        redo=getUnlinkedInstruction();
        appendInstruction(redo);
        if ((dstR=getRegZPAddr(opPar==Ops.B_SHL ? 1 : 8, dst, StdTypes.T_LONG, true))==-1
            || (srcR1=getRegZPAddr(opPar==Ops.B_SHL ? 1 : 8, src1, StdTypes.T_LONG, false))==-1) return;
        switch (opPar) {
          case Ops.B_SHL:
            if (srcR1!=dstR) {
              ins(I_LDAzp, srcR1);
        ...
        count=8;
        while (--count>0) { //higher bytes - respect carry
          if ((dstR=getRegZPAddr(i, dst, StdTypes.T_LONG, true))==-1
              || (srcR1=getRegZPAddr(i, src1, StdTypes.T_LONG, false))==-1) return;
          if (opPar==Ops.B_SHL) {
            if (dstR!=srcR1) {
              ins(I_LDAzp, srcR1);
              ins(I_ROL);
