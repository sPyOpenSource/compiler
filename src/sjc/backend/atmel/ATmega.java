...
//  protected final static int RegZ=0xC0000000; //general purpose temp register (main use: ptr)
  ...
  public int prepareFreeReg(int avoidReg1, int avoidReg2, int reUseReg, int type) {
    ...
    if (getByteCount(type)>1) {
      ...
      for (i=0; i<32; i+=2) if ((mask&(3<<i))!=(3<<i)) mask&=~(3<<i);
  ...
  protected int getByteCount(int type) {
    switch (type) {
      case StdTypes.T_BOOL: case StdTypes.T_BYTE:                        return 1;
      case StdTypes.T_CHAR: case StdTypes.T_PTR:  case StdTypes.T_SHRT:  return 2;
      case StdTypes.T_INT:  case StdTypes.T_DPTR: case StdTypes.T_FLT:   return 4;
      case StdTypes.T_LONG: case StdTypes.T_DBL:                         return 8;
    }
  ...
  public void codeProlog() {
    ...
    if ((curMthd.marker&Marks.K_INTR)!=0) {
      ...
      for (i=0; i<=31; i++) ins(I_PUSHreg, i, 0, 0);
  ...
  public void codeEpilog(Mthd outline) {
    ...
    if (curMthd.varSize!=0) {
      if (curMthd.varSize>3) {
        if (curMthd.varSize<64) ins(I_ADDIWregimm, R_YL, 0, curMthd.varSize); //add var-count to Z-register
        else {
      ...
      else for (i=0; i<curMthd.varSize; i++) ins(I_POPreg, R_HLP, 0, 0);
    }
    ...
    if ((curMthd.marker&Marks.K_INTR)!=0) {
      ...
      for (i=31; i>=0; i--) ins(I_POPreg, i, 0, 0);
      ins(I_RETI, 0, 0, 0);
      curMthd=null;
    }
  ...
  public void genAssign(int dst, int src, int type) {
    ...
    regCount=getByteCount(type);
    if ((reg1=getReg(1, dst, StdTypes.T_PTR, false))==-1
        || (reg2=getReg(2, dst, StdTypes.T_PTR, false))==-1) return;
    insMOVpair(R_ZH, R_ZL, reg2, reg1);
    if (regCount==1) {
      if ((reg1=getReg(1, src, type, false))==-1) return;
      ...
      for (i=regCount; i>0; i--) {
        if ((reg1=getReg(i, src, type, false))==-1) return;
    ...
    switch (type) {
      ...
      case StdTypes.T_BYTE:
        if (opPar==Ops.A_MUL) { //special case: mul for byte is supported
          if ((dstR=getReg(1, dst, StdTypes.T_BYTE, false))==-1
              || (srcR=getReg(1, src2, StdTypes.T_BYTE, false))==-1) return;
          if (dstR!=0) stored=storeReg(3);
          else stored=storeReg(2);
          ins(I_MULregreg, dstR, srcR, 0); //requires in-registers >=16, output in r1:r0
          if (dstR!=0) ins(I_MOVregreg, dstR, 0, 0);
          restoreReg(stored);
          return;
        }
        //has to do the following, too!
      ...
      case StdTypes.T_LONG:
        switch (opType) {
          case Ops.S_ARI:
            for (i=1; i<=count; i++) {
              if ((dstR=getReg(i, dst, type, false))==-1
                  || (srcR=getReg(i, src2, type, false))==-1) return;
              switch (opPar) {
                case Ops.A_AND:
                ...
                case Ops.A_MINUS: //no minus with carry->invert and add
                  if (i==1) ins(I_SUBregreg, dstR, srcR, 0);
                  else ins(I_SBCregreg, dstR, srcR, 0);
                  break;
                case Ops.A_PLUS:
                  if (i==1) ins(I_ADDregreg, dstR, srcR, 0);
                  else ins(I_ADCregreg, dstR, srcR, 0);
                  break;
                default:
          ...
          case Ops.S_BSH:
            ...
            if ((srcR=getReg(1, src2, StdTypes.T_INT, false))==-1) return;
            ...
            if ((dstR=getReg(opPar==Ops.B_SHL ? 1 : count, dst, type, false))==-1) return; //first run - omit carry
            switch (opPar) {
              case Ops.B_SHL:
            ...
            while (--count>0) { //higher bytes - respect carry
              if ((dstR=getReg(i, dst, type, false))==-1) return;
              if (opPar==Ops.B_SHL) {
  ...
  public void genCall(int off, int clssReg, int parSize) {
    ...
    if ((i=getReg(1, clssReg, StdTypes.T_PTR, false))==-1
        || (j=getReg(2, clssReg, StdTypes.T_PTR, false))==-1) return;
    insMOVpair(R_ZH, R_ZL, j, i);
    if (off>=0 && off<64) ins(I_ADDIWregimm, R_ZL, 0, off);
    else {
    ...
    if (ctx.codeStart!=0) {
      if (ctx.codeStart<64) ins(I_ADDIWregimm, R_ZL, 0, ctx.codeStart);
      else {
  ...
  public void genCallIndexed(int intfReg, int off, int parSize) {
    ...
    if ((j=getReg(2, intfReg, StdTypes.T_DPTR, false))==-1) return;
    insMOVpair(R_ZH, R_ZL, j, i);
    if (off>0 && off<64) ins(I_ADDIWregimm, R_ZL, 0, off);
    else if (off!=0) {
  ...
  public void genCheckBounds(int addrReg, int offReg, int checkToOffset, Instruction onSuccess) {
    ...
    if ((i=getReg(1, addrReg, StdTypes.T_PTR, false))==-1
        || (j=getReg(2, addrReg, StdTypes.T_PTR, false))==-1) return;
    insMOVpair(R_ZH, R_ZL, j, i);
    if (checkToOffset>0 && checkToOffset<64) ins(I_ADDIWregimm, R_ZL, 0, checkToOffset);
  ...
  public int genCompPtrToNull(int reg, int cond) {
    ...
    if ((tmp=getReg(1, reg, StdTypes.T_PTR, false))==-1
      || ((reg=getReg(2, reg, StdTypes.T_PTR, false)))==-1) return 0;
  ...
  public int genComp(int src1, int src2, int type, int cond) {
    ...
    switch (cond) { //replace unsupported jumps by flipping registers and changing condition
    ...
    for (i=1; i<=max; i++) {
      if ((r1=getReg(i, src1, type, false))==-1 ||
          (r2=getReg(i, src2, type, false))==-1)
        return 0;
  ...
  public int genCompValToConstVal(int src, int val, int type, int cond) {
    ...
    max=getByteCount(type);
    for (i=1; i<=max; i++) {
      if ((r1=getReg(i, src, type, false))==-1) return 0;
      if ((v=val>>>((i-1)<<3))!=0) ins(I_LDIregimm, r2=R_HLP, 0, v&0xFF);
      else r2=R_ZERO;
      ins(withCarry ? I_CPCregreg : I_CPregreg, r1, r2, 0);
      withCarry=true;
    }
  ...
  public int genCompValToConstDoubleOrLongVal(int src, long val, boolean asDouble, int cond) {
    ...
    int i, v, r1, r2;
    for (i=1; i<=8; i++) {
      if ((r1=getReg(i, src, StdTypes.T_LONG, false))==-1) return 0;
      if ((v=(int)(val>>>((i-1)<<3)))!=0) ins(I_LDIregimm, r2=R_HLP, 0, v&0xFF);
      else r2=R_ZERO;
      ins(withCarry ? I_CPCregreg : I_CPregreg, r1, r2, 0);
      withCarry=true;
    }
  ...
  public void genConvertVal(int dst, int src, int toType, int fromType) {
    ...
    for (; i<=temp; i++) { //take care of lower bytes
      if ((regDst=getReg(i, dst, toType, true))==-1 || 
        (regSrc=getReg(i, src, fromType, false))==-1) return;
      if (regDst!=regSrc) ins(I_MOVregreg, regDst, regSrc, 0);
    }
    if (countTo>countFrom) { //conversion was an expansion
      if (fromType!=StdTypes.T_CHAR) { //not char -> sign extension
        ...
        for (; i<=countTo; i++) {
          if ((regDst=getReg(i, dst, toType, true))==-1) return;
  ...
  public void genDup(int dst, int src, int type) {
    ...
    for (i=1; i<=regCount; i++) {
      if ((dstReg=getReg(i, dst, type, true))==-1 || (srcReg=getReg(i, src, type, false))==-1) return;
      if (dstReg!=srcReg) ins(I_MOVregreg, dstReg, srcReg, 0);
    }
  }
  ...
  public void genDecMem(int dst, int type) {
    ...
    if ((i=getReg(1, dst, StdTypes.T_PTR, false))==-1
        || (j=getReg(2, dst, StdTypes.T_PTR, false))==-1) return;
    insMOVpair(R_ZH, R_ZL, j, i);
  ...
  public void genLoadConstUnitContext(int dst, Object unitLoc) {
    ...
    if ((reg=getReg(1, dst, StdTypes.T_PTR, false))==-1) return;
    if (reg<16) {
      ins(I_LDIregimm, R_HLP, 0, unitAddr&0xFF);
      ins(I_MOVregreg, reg, R_HLP, 0);
    }
    else ins(I_LDIregimm, reg, 0, unitAddr&0xFF);
    if ((reg=getReg(2, dst, StdTypes.T_PTR, false))==-1) return;
    if (reg<16) {
      ins(I_LDIregimm, R_HLP, 0, (unitAddr>>>8)&0xFF);
      ins(I_MOVregreg, reg, R_HLP, 0);
    }
    else ins(I_LDIregimm, reg, 0, (unitAddr>>>8)&0xFF);
  }

  public void genLoadConstDoubleOrLongVal(int dst, long val, boolean asDouble) {
    int reg, i, tempVal;
    for (i=0; i<8; i++) {
      if ((reg=getReg(i+1, dst, StdTypes.T_LONG, true))==-1) return;
      tempVal=(int)(val>>>(i<<3))&0xFF; //determine value to load
      if (tempVal==0) ins(I_EORregreg, reg, reg, 0);
      else if (reg<16) { //ldi only for regs 16..31
        ins(I_LDIregimm, R_HLP, 0, tempVal);
        ins(I_MOVregreg, reg, R_HLP, 0);
      }
      else ins(I_LDIregimm, reg, 0, tempVal);
    }
  }
  ...
  public void genLoadConstVal(int dst, int val, int type) {
    ...
    for (i=0; i<count; i++) {
      if ((reg=getReg(i+1, dst, type, true))==-1) return;
  ...
  public void genLoadDerefAddr(int destReg, int objReg, int indReg, int baseOffset, int entrySize) {
    int i, j;
    if ((i=getReg(1, indReg, StdTypes.T_SHRT, false))==-1
        || (j=getReg(2, indReg, StdTypes.T_SHRT, false))==-1) return;
    if (entrySize<0) {
    ...
    if (baseOffset>0 && baseOffset<64) ins(I_ADDIWregimm, R_ZL, 0, baseOffset);
    ...
    if ((i=getReg(1, destReg, StdTypes.T_PTR, false))==-1
        || (j=getReg(2, destReg, StdTypes.T_PTR, false))==-1) return;
    if (objReg!=destReg) insMOVpair(j, i,
        getReg(2, objReg, StdTypes.T_PTR, false), getReg(1, objReg, StdTypes.T_PTR, false));
  ...
  public void genLoadInstContext(int src) {
    ...
    if ((i=getReg(1, src, StdTypes.T_PTR, false))==-1
        || (j=getReg(2, src, StdTypes.T_PTR, false))==-1) return;
  ...
  public void genLoadVarAddr(int dst, int src, Object loc, int off) {
    ...
    if ((dstReg1=getReg(1, dst, StdTypes.T_PTR, true))==-1) return;
    if ((dstReg2=getReg(2, dst, StdTypes.T_PTR, true))==-1) return;
    if (dstReg1<16) tempReg1=R_ZL; //dst maybe has to be loaded with immediate
    else tempReg1=dstReg1;
    if (dstReg2<16) tempReg2=R_ZH;
    else tempReg2=dstReg2;
    pos=mem.getAddrAsInt(loc, off);
    if (src==regBase) {
      if (pos>=0) pos+=curVarOffParam; //step over old stack pointer, return address for method parameters and saved R_BSE
      else pos++;
    ...
    if (src!=0) {
      ...
      if (tempReg1!=srcReg1 && tempReg2!=srcReg2)
        ...
        if (tempReg1!=srcReg1) ins(I_MOVregreg, tempReg1, srcReg1, 0);
        if (tempReg2!=srcReg2) ins(I_MOVregreg, tempReg2, srcReg2, 0);
      }
      if (pos!=0) {
    ...
    if (dstReg2!=tempReg2 && dstReg1!=tempReg1)
      ...
      if (dstReg2!=tempReg2) ins(I_MOVregreg, dstReg2, tempReg2, 0);
      if (dstReg1!=tempReg1) ins(I_MOVregreg, dstReg1, tempReg1, 0);
    }
  }
  ...
  public void genLoadVarVal(int dst, int src, Object loc, int off, int type) {
    ...
    else if (!nextValInFlash && src==0 && count<=2) { //special handling for known constant addresses if byte count is below or equal to 2
      if ((dstReg=getReg(1, dst, type, true))==-1) return;
      ins(I_LDSregimm, dstReg, 0, pos);
      if (count==2) {
        if ((dstReg=getReg(2, dst, type, true))==-1) return;
        ins(I_LDSregimm, dstReg, 0, pos+1);
      }
      return;
    }
    if (src!=0) {
      if (src==RegY && pos>=0 && (pos+count)<64) { //use displacement for stack access if possible
        if (nextValInFlash) {
          fatalError("src==RegY with nextVarValFromFlash");
          return;
        }
        while (i<count) {
          if ((dstReg=getReg(i+1, dst, type, true))==-1) return;
    ...
    while (i<count) {
      if ((dstReg=getReg(++i, dst, type, true))==-1) return;
  ...
  public void genMoveFromPrimary(int dst, int type) {
    ...
    while (i>0) {
      if ((tempReg=getReg(i, dst, type, false))==-1) return;
      writtenRegs|=(1<<tempReg);
      primReg=R_PRIM_START+--i;
      if (primReg==tempReg) continue;
  ...
  public void genMoveIntfMapFromPrimary(int dst) {
    int i, j;
    if ((i=getReg(3, dst, StdTypes.T_DPTR, false))==-1
        || (j=getReg(4, dst, StdTypes.T_DPTR, false))==-1) return;
    if (i!=R_PRIM_START && j!=R_PRIM_START+1) 
      insMOVpair(j, i, R_PRIM_START+1, R_PRIM_START);
    else {
      if (i!=R_PRIM_START) ins(I_MOVregreg, i, R_PRIM_START, 0);
      if (j!=R_PRIM_START+1) ins(I_MOVregreg, j, R_PRIM_START+1, 0);
    }
  }
  ...
  public void genMoveToPrimary(int src, int type) {
    ...
    for (count=getByteCount(type); i<=count; i++) {
      if ((tempReg=getReg(i, src, type, false))==-1) return;
      primReg=R_PRIM_START+i-1;
      if (tempReg!=primReg) ins(I_MOVregreg, primReg, tempReg, 0);
    }
  }
  ...
  public void genPop(int dst, int type) {
    ...
    while (i<m) ins(I_POPreg, getReg(++i, dst, type, true), 0, 0);
  }

  public void genPush(int src, int type) {
    int i;
    stackLevel+=i=getByteCount(type);
    while (i>0) ins(I_PUSHreg, getReg(i--, src, type, false), 0, 0);
  }
  ...
  public void genPushConstDoubleOrLongVal(long val, boolean asDouble) {
    ...
    stackLevel+=8;
    for (; i>0; i--) {
      if ((byteVal=(int)(val>>>((i-1)<<3))&0xFF)==0) ins(I_PUSHreg, R_ZERO, 0, 0);
      else {
  ...
  public void genPushConstVal(int val, int type) {
    ...
    for (stackLevel+=i=count; i>0; i--) {
      if ((byteVal=(val>>>((i-1)<<3))&0xFF)==0) ins(I_PUSHreg, R_ZERO, 0, 0);
      else {
        ins(I_LDIregimm, R_HLP, 0, byteVal);
        ins(I_PUSHreg, R_HLP, 0, 0);
      }
  ...
  public void genStoreVarConstVal(int objReg, Object loc, int off, int val, int type) {
    int cnt, tempVal, pos;
    if ((cnt=getByteCount(type))==-1) return;
    pos=mem.getAddrAsInt(loc, off);
    ...
    while (--cnt>=0) {
      ...
      if (tempVal==0) ins(I_STY_DISPregimm, R_ZERO, 0, pos+cnt);
      else {
        ins(I_LDIregimm, R_HLP, 0, tempVal);
  ...
  public void genStoreVarVal(int objReg, Object loc, int off, int valReg, int type) {
    ...
    pos+=curMthd.varSize; //BSE points not to first parameter but last local variable
    while (cnt>0) {
      if ((reg=getReg(cnt--, valReg, type, false))==-1) return;
  ...
  public void genUnaOp(int dst, int src, int op, int type) {
    ...
    if (type==StdTypes.T_BOOL) {
      if (opPar==Ops.L_NOT) { //I_EOR doesn't work with imm => get register
        if ((reg=getReg(1, dst, StdTypes.T_BYTE, false))==-1) return;
      ...
      count=getByteCount(type);
      for (; i<=count; i++) {
        if ((reg=getReg(i, dst, type, false))==-1) return;
        ins(I_COMreg, reg, 0, 0); //one's complement
      }
      if (minus) { //add 1 to complete value in case of two's complement
        for (i=1; i<=count; i++) {
          if ((reg=getReg(i, dst, type, false))==-1) return;
  ...
  public void genReadIO(int dst, int src, int type, int memLoc) {
    ...
    switch (memLoc) {
      case 0: //default == I/O
        ...
        while (cnt<=count) {
          if ((tempReg=getReg(cnt++, dst, type, true))==-1) return;
  ...
  public void genWriteIO(int dst, int src, int type, int memLoc) {
    ...
    switch (memLoc) {
      case 0: //default == I/O
        ...
        while (cnt>=1) {
          if ((tempReg=getReg(cnt--, src, type, false))==-1) return;
  ...
  public void inlineVarOffset(int inlineMode, int objReg, Object loc, int offset, int baseValue) {
    ...
    while (byteCount-->0) {
      ...
      i.putByte(pos&0xFF);
      pos=pos>>>8;
      if (i.size+1==maxInstrCodeSize) i=null;
    }
  }
  ...
  public void genThrowFrameBuild(int globalAddrReg, Instruction dest, int throwBlockOffset) {
    ...
    if (globalAddrReg!=((1<<16)|(1<<17)) || usedRegs!=((1<<16)|(1<<17))) { //globalAddrReg is the only allocated register
    ...
    if (throwBlockOffset<64) ins(I_ADDIWregimm, R_ZL, 0, throwBlockOffset);
    else {
  ...
  public void genThrowFrameUpdate(Instruction oldDest, Instruction newDest, int throwBlockOffset) {
    if (usedRegs!=0) { //no allocated register
      fatalError(ERR_INVGLOBADDRREG);
      return;
    }
    throwBlockOffset+=curMthd.varSize+1; //BSE points not to first parameter but last local variable
  ...
  private void insMOVpair(int dh, int dl, int sh, int sl) {
    if (dh==sh && dl==sl) return;
    if (sl+1==sh && (sl&1)==0 && dl+1==dh && (dl&1)==0)
      ins(I_MOVWregreg, dl, sl, 0);
    else {
      if (dl!=sl) ins(I_MOVregreg, dl, sl, 0);
      if (dh!=sh) ins(I_MOVregreg, dh, sh, 0);
    }
  }
  ...
  private void insCleanStackAfterCall(int parSize) {
    ...
    if (parSize!=0) { //clean parameters from stack
      ...
      if (parSize>2 && stackLevel==0) {
        ...
        if (parSize<64) ins(I_ADDIWregimm, R_ZL, 0, parSize);
        else {
      ...
      else for (i=0; i<parSize; i++) ins(I_POPreg, R_HLP, 0, 0);
    }
  }
  ...
  protected void insJump(Instruction dest, int cond) {
    ...
    me.size=2;
    me.type=I_JUMP;
    switch (cond) { //special cases cond==0 and ==Ops.C_BO: no jump helper
      case 0:
      case Ops.C_BO:
        me.iPar1=cond;
        me.jDest=dest;
    ...
    switch (cond) {
      ...
      case Ops.C_LW: case Ops.C_EQ: case Ops.C_NE: case Ops.C_GE:
  ...
  protected boolean fixJump(Instruction jump) {
    ...
    if ((relative=getJumpRel(jump, jump.jDest))==0) { //do not jump to next instruction
      jump.type=I_NONE;
      jump.size=0;
      return true;
    }
    ...
    while (check!=null) {
      switch (check.type) {
        ...
        case I_PUSHreg:
          ...
          break;
        case I_POPreg:
          if (check.reg0!=R_HLP) curStackUsage--; //helper is only popped for call, which is handled there, and variables at end of the method (not relevant)
          break;
        case I_CALLpatched:
        case I_ICALL:
          hasCall=true;
      ...
      if (curStackUsage>maxStackUsage) maxStackUsage=curStackUsage;
      check=check.next;
    }
    me.size=0;
    me.next.size=0;
  ...
  protected void code(Instruction i) {
    ...
    switch (i.type) {
      ...
      case I_ADDIWregimm:
        ...
        switch (reg0) {
          case 24: i.putShort(0x9600|(imm&0xF)|((imm&0x30)<<2)); return;
          case 26: i.putShort(0x9610|(imm&0xF)|((imm&0x30)<<2)); return;
          case 28: i.putShort(0x9620|(imm&0xF)|((imm&0x30)<<2)); return;
          case 30: i.putShort(0x9630|(imm&0xF)|((imm&0x30)<<2)); return;
        }
  ...
  protected int print(Instruction i) { //returns number of real instructions
    ...
    switch (i.type) {
      case I_LDIregimm:
        ctx.out.print("LDI r"); ctx.out.print(i.reg0); ctx.out.print(",0x"); ctx.out.printHexFix(i.iPar1, 2);
        break;
      case I_MOVregreg:
        ctx.out.print("MOV r"); ctx.out.print(i.reg0); ctx.out.print(",r"); ctx.out.print(i.reg1);
        break;
      case I_MOVWregreg:
        ctx.out.print("MOVW r"); ctx.out.print(i.reg0); ctx.out.print(",r"); ctx.out.print(i.reg1);
        break;
      case I_PUSHreg:
        ctx.out.print("PUSH r"); ctx.out.print(i.reg0);
        break;
      case I_POPreg:
        ctx.out.print("POP r"); ctx.out.print(i.reg0);
        break;
      case I_ANDregreg:
        ctx.out.print("AND r"); ctx.out.print(i.reg0); ctx.out.print(",r"); ctx.out.print(i.reg1);
        break;
      case I_ANDIregimm:
        ctx.out.print("ANDI r"); ctx.out.print(i.reg0); ctx.out.print(",0x"); ctx.out.printHexFix(i.iPar1, 2);
        break;
      case I_ORregreg:
        ctx.out.print("OR r"); ctx.out.print(i.reg0); ctx.out.print(",r"); ctx.out.print(i.reg1);
        break;
      case I_ORIregimm:
        ctx.out.print("ORI r"); ctx.out.print(i.reg0); ctx.out.print(",0x"); ctx.out.printHexFix(i.iPar1, 2);
        break;
      case I_ADCregreg:
        ctx.out.print("ADC r"); ctx.out.print(i.reg0); ctx.out.print(",r"); ctx.out.print(i.reg1);
        break;
      case I_SBCregreg:
        ctx.out.print("SBC r"); ctx.out.print(i.reg0); ctx.out.print(",r"); ctx.out.print(i.reg1);
        break;
      case I_ADDregreg:
        ctx.out.print("ADD r"); ctx.out.print(i.reg0); ctx.out.print(",r"); ctx.out.print(i.reg1);
        break;
      case I_SUBregreg:
        ctx.out.print("SUB r"); ctx.out.print(i.reg0); ctx.out.print(",r"); ctx.out.print(i.reg1);
        break;
      case I_EORregreg:
        ctx.out.print("EOR r"); ctx.out.print(i.reg0); ctx.out.print(",r"); ctx.out.print(i.reg1);
        break;
      case I_SUBIregimm:
        ctx.out.print("SUBI r"); ctx.out.print(i.reg0); ctx.out.print(",0x"); ctx.out.printHexFix(i.iPar1, 2);
        break;
      case I_SBCIregimm:
        ctx.out.print("SBCI r"); ctx.out.print(i.reg0); ctx.out.print(",0x"); ctx.out.printHexFix(i.iPar1, 2);
        break;
      case I_COMreg:
        ctx.out.print("COM r"); ctx.out.print(i.reg0);
        break;
      case I_LSRreg:
        ctx.out.print("LSR r"); ctx.out.print(i.reg0);
        break;
      case I_RORreg:
        ctx.out.print("ROR r"); ctx.out.print(i.reg0);
        break;
      case I_ROLreg:
        ctx.out.print("ROL r"); ctx.out.print(i.reg0);
        break;
      case I_LSLreg:
        ctx.out.print("LSL r"); ctx.out.print(i.reg0);
        break;
      case I_NEGreg:
        ctx.out.print("NEG r"); ctx.out.print(i.reg0);
        break;
      case I_ASRreg:
        ctx.out.print("ASR r"); ctx.out.print(i.reg0);
        break;
      case I_INCreg:
        ctx.out.print("INC r"); ctx.out.print(i.reg0);
        break;
      case I_DECreg:
        ctx.out.print("DEC r"); ctx.out.print(i.reg0);
        break;
      case I_LDZreg:
        ctx.out.print("LD r"); ctx.out.print(i.reg0); ctx.out.print(",Z");
        break;
      case I_STZ_INCreg:
        ctx.out.print("ST Z+,r"); ctx.out.print(i.reg0);
        break;
      case I_STZ_DECreg:
        ctx.out.print("ST -Z,r"); ctx.out.print(i.reg0);
        break;
      ...
      case I_LDZ_DECreg:
        ...
        break;
      case I_LPM_INCreg:
        ctx.out.print("LPM r"); ctx.out.print(i.reg0); ctx.out.print(",Z+");
        break;
      case I_INregimm:
        ctx.out.print("IN r"); ctx.out.print(i.reg0); ctx.out.print(",0x"); ctx.out.printHexFix(i.iPar1, 2);
        break;
      case I_OUTimmreg:
        ctx.out.print("OUT 0x"); ctx.out.printHexFix(i.iPar1, 2); ctx.out.print(",r"); ctx.out.print(i.reg0); 
        break;
      case I_CPIregimm:
        ctx.out.print("CPI r"); ctx.out.print(i.reg0); ctx.out.print(",0x"); ctx.out.printHexFix(i.iPar1, 2);
        break;
      case I_CPregreg:
        ctx.out.print("CP r"); ctx.out.print(i.reg0); ctx.out.print(",r"); ctx.out.print(i.reg1);
        break;
      case I_CPCregreg:
        ctx.out.print("CPC r"); ctx.out.print(i.reg0); ctx.out.print(",r"); ctx.out.print(i.reg1);
        break;
      case I_ICALL:
        ctx.out.print("ICALL");
        break;
      case I_ADDIWregimm:
        ctx.out.print("ADDIW r");ctx.out.print(i.reg0); ctx.out.print(",0x"); ctx.out.printHexFix(i.iPar1, 2);
        break;
      case I_MULregreg:
        ctx.out.print("MUL r"); ctx.out.print(i.reg0); ctx.out.print(",r"); ctx.out.print(i.reg1);
        break;
      case I_STX_DECreg:
        ctx.out.print("ST -X,r"); ctx.out.print(i.reg0);
        break;
      case I_LDX_INCreg:
        ctx.out.print("LD r"); ctx.out.print(i.reg0); ctx.out.print(",X+");
        break;
      case I_RET:
        ctx.out.print("RET");
        break;
      ...
      case I_LPMreg:
        ...
        break;
      case I_LDY_DISPregimm:
        ctx.out.print("LD r"); ctx.out.print(i.reg0); ctx.out.print(",Y+"); ctx.out.print(i.iPar1);
        break;
      case I_STY_DISPregimm:
        ctx.out.print("ST Y+"); ctx.out.print(i.iPar1); ctx.out.print(",r"); ctx.out.print(i.reg0); 
        break;
      case I_LDSregimm:
        ctx.out.print("LDS r"); ctx.out.print(i.reg0); ctx.out.print(",0x"); ctx.out.printHexFix(i.iPar1, 4);
        break;
      case I_STSimmreg:
        ctx.out.print("STS 0x"); ctx.out.printHexFix(i.iPar1, 4); ctx.out.print(",r"); ctx.out.print(i.reg0);
        break;
      case I_CBIimm:
        ctx.out.print("CBI 0x"); ctx.out.printHexFix(i.iPar1>>>3, 2); ctx.out.print(","); ctx.out.print(i.iPar1&0x7);
        break;
      case I_SBIimm:
        ctx.out.print("SBI 0x"); ctx.out.printHexFix(i.iPar1>>>3, 2); ctx.out.print(","); ctx.out.print(i.iPar1&0x7);
        break;
      case I_SBRCregimm:
        ctx.out.print("SBRC r"); ctx.out.print(i.reg0); ctx.out.print(","); ctx.out.print(i.iPar1);
        break;
      case I_PUSHip:
      ...
      case I_CLI:
        ...
        break;
      case I_CALLpatched:
        ctx.out.print("CALLpatched to "); i.refMthd.owner.printNameWithOuter(ctx.out); ctx.out.print('.'); i.refMthd.printNamePar(ctx.out);
        break;
      case I_ADDpatched:
        ctx.out.print("ADDpatched i"); ctx.out.print(i.jDest.instrNr);
        break;
      case I_IHELPER:
        ctx.out.print("--ihelper i"); ctx.out.print(i.jDest.instrNr);
        return 0; //not a real instruction
      case I_BSHOPHINT:
        ctx.out.print("--binophint ");
        switch (i.iPar1) {
          case Ops.B_SHL:  ctx.out.print("shl"); break;
          case Ops.B_SHRL: ctx.out.print("shrl"); break;
          case Ops.B_SHRA: ctx.out.print("shra"); break;
        }
        ctx.out.print(" m"); ctx.out.printHexFix(i.reg0, 8); ctx.out.print("/r"); ctx.out.print(i.reg1);
        ctx.out.print(" until i"); ctx.out.print(i.jDest.instrNr);
        return 0; //not a real instruction
      case I_STEXreg:
      	ctx.out.print("STEX r"); ctx.out.print(i.reg0);
      	break;
      case I_JUMP:
        switch (i.iPar1) {
          case 0:        ctx.out.print("JMP"); break; //unconditionally
          case Ops.C_GE: ctx.out.print("JGE"); break; //jump if greater or equal (signed)
          case Ops.C_LW: ctx.out.print("JLW"); break; //jump if lower (signed)
          case Ops.C_BO: ctx.out.print("JBO"); break; //jump if below (unsigned)
          case Ops.C_NE: ctx.out.print("JNE"); break; //jump if not equal
          case Ops.C_EQ: ctx.out.print("JEQ"); break; //jump if equal
        }
        ctx.out.print(" to i"); ctx.out.print(i.jDest.instrNr);
        break;
      case I_MAGC:
        ctx.out.print("MAGIC");
        if ((i.size&1)!=0) ctx.out.print(" INVSIZE!");
        for (cnt=0; cnt<i.size; cnt+=2) {
          ctx.out.print(" 0x");
          ctx.out.printHexFix((int)i.code[cnt+1], 2);
