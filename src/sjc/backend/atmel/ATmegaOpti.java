...
public class ATmegaOpti extends ATmega {
  ...
  public void finalizeInstructions(Instruction first) {
    ...
    now=first;
    while (now!=null) {
      if (now.type>0) code(now); //do not code jumps and special instructions here
      now=now.next;
    }
    ...
    do {
      ...
      redo=false;
      while (now!=null) {
        if (now.type==I_JUMP) redo|=fixJump(now);
    ...
    if ((mthdContainer.marker&Marks.K_PRCD)!=0 || ctx.printCode) printCode(first, "postOpt");
  }
  ...
  protected boolean fixJump(Instruction jump) {
    ...
    if ((mthdContainer.marker&Marks.K_NOPT)==0 //optimization permitted
        && jump.iPar1!=0 && (next=getNextRealIns(jump.next))!=null && next.type==I_JUMP && next.iPar1==0
        && noJumpDestBetween(jump.next, next) && getNextRealIns(next.next)==getNextRealIns(jump.jDest)
        && (nextRelative=getJumpRel(next, next.jDest))>=-64 && nextRelative<63) { //conditional jump over unconditional jump in small range 
  ...
  protected void set(Instruction i, int type, int reg0, int reg1, int imm) {
    ...
    switch (type) {
      ...
      case I_SBCregreg:
        ...
        return;
      case I_EORregreg:
        if (reg0!=reg1) i.lPar=writeReg(reg0)|readReg(reg0)|readReg(reg1);
        else i.lPar=writeReg(reg0);
      ...
      case I_RET:
        switch (curMthd.retRegType) {
          case 0:
            i.lPar=0l;
            break;
          case StdTypes.T_LONG: case StdTypes.T_DBL:
            i.lPar=readReg(R_PRIM_START+7)|readReg(R_PRIM_START+6)|readReg(R_PRIM_START+5)|readReg(R_PRIM_START+4);
          case StdTypes.T_INT: case StdTypes.T_FLT: case StdTypes.T_DPTR:
            i.lPar|=readReg(R_PRIM_START+3)|readReg(R_PRIM_START+2);
          case StdTypes.T_SHRT: case StdTypes.T_CHAR: case StdTypes.T_PTR:
  ...
  protected void optimize(Instruction first, Instruction last) {
    ...
    if ((mthdContainer.marker&Marks.K_NOPT)!=0) return; //optimizations prohibited
    ...
    do {
      ...
      while (now!=null) {
        if ((iList=(iTmp=((ATmegaOptInstr)now)).jSource)!=null) {
          ...
          iListTmp=iList;
          while (iListTmp.next!=null) iListTmp=iListTmp.next; //search end of current instrlist
      ...
      while (now!=null) {
        switch (now.type) {
          case I_JUMP:
            if (now.iPar1==0) now.iPar2=0; //unconditional jump
            else { //conditional jump
              switch (now.iPar1) {
                case Ops.C_GE: case Ops.C_LW: now.iPar2=SE_READ_FLAG_SIGNED; break;//jump if greater or equal / lower (signed)
                case Ops.C_BO:                now.iPar2=SE_READ_FLAG_CARRY; break; //jump if below (unsigned)
                case Ops.C_NE: case Ops.C_EQ: now.iPar2=SE_READ_FLAG_ZERO; break; //jump if not equal / equal
                default:
      ...
      now=first;
      while (now!=null) {
        if (now.type>0 && (now.iPar2&(SE_WRITE_STACK|SE_WRITE_MEMORY|SE_NO_REMOVE))==0 
            && ((now.iPar2&(SE_WRITE_FLAG_CARRY|SE_WRITE_FLAG_SIGNED|SE_WRITE_FLAG_ZERO))==0
                || flagRelevance(now.next, now.iPar2)==0)) { //normal instruction without side-effect
          hasRead=false;
          for (i=0; i<32; i++) {
            if (isWrite(now, i) && searchReadStartingAt(now.next, i)!=null) hasRead=true;
          }
          if (!hasRead) { //output of this instruction is never used
            kill(now);
            redo=true;
          }
        }
...
INSTR_SW:
        switch (now.type) {
          case I_JUMP:
            if (now.iPar1!=0) switch (searchFlagSetting(now, now.iPar2<<1)) { //write is read<<1
              case 0: //flag is cleared
                if ((now.iPar1&Ops.INVCBIT)!=0) now.iPar1=0; //always jump
                else {
                  kill(now); //never jump
                  redo=true;
                }
          ...
          case I_MOVregreg:
            ...
            else if ((tmp=searchWriteBefore(now, now.reg1, false))!=null && tmp.type>0 && !isRead(tmp, now.reg1)
                && (tmp.iPar2&SE_DEST_REGS)==0 && (now.reg0>=16 || (tmp.iPar2&SE_ONLY_UPPER_REGS)==0)
                && noReadWriteUntil(tmp.next, now, now.reg1, true, true)
                && noReadWriteUntil(tmp.next, now, now.reg0, true, true)
                && searchReadStartingAt(now.next, now.reg1)==null) {
              if (tmp.reg0==now.reg1) tmp.reg0=now.reg0;
              if (tmp.reg1==now.reg1) tmp.reg1=now.reg0;
          ...
          case I_MOVWregreg:
            if (now.reg0==now.reg1) {
            ...
            else if ((vInfo=searchPointerSetting(now, now.reg1))!=null && vInfo.type==ValueInfo.VI_MEMABS
                && now.reg0>=16) {
              ...
              redo=true;
                ...
                && (tmp.iPar2&SE_DEST_REGS)==0 && (now.reg0>=16 || (tmp.iPar2&SE_ONLY_UPPER_REGS)==0)
                && noReadWriteUntil(tmp.next, now.prev, now.reg1, true, true)
                && noReadWriteUntil(tmp.next, now.prev, now.reg0, true, true)
                && searchReadStartingAt(now.next, now.reg1)==null
                && (tmp2=searchWriteBefore(now, now.reg1+1, false))!=null && tmp2.type>0 && !isRead(tmp2, now.reg1+1)
                && (tmp2.iPar2&SE_DEST_REGS)==0 && (now.reg0+1>=16 || (tmp2.iPar2&SE_ONLY_UPPER_REGS)==0)
                && noReadWriteUntil(tmp2.next, now.prev, now.reg1+1, true, true)
                && noReadWriteUntil(tmp2.next, now.prev, now.reg0+1, true, true)
                && searchReadStartingAt(now.next, now.reg1+1)==null) {
              if (tmp.reg0==now.reg1) tmp.reg0=now.reg0;
              if (tmp.reg1==now.reg1) tmp.reg1=now.reg0;
              if (tmp2.reg0==now.reg1+1) tmp2.reg0=now.reg0+1;
              if (tmp2.reg1==now.reg1+1) tmp2.reg1=now.reg0+1;
              set(tmp, tmp.type, tmp.reg0, tmp.reg1, tmp.iPar1); //call "set" on tmp to build additional info
              set(tmp2, tmp2.type, tmp2.reg0, tmp2.reg1, tmp2.iPar1); //call "set" on tmp2 to build additional info
              kill(now);
              redo=true;
            }
            else if ((tmp=searchWriteBefore(now, now.reg1, false))!=null
                && tmp.type==I_ADDregreg && tmp.next.type==I_ADCregreg
                && tmp.reg1==now.reg0 && tmp.next.reg1==now.reg0+1
                && searchReadStartingAt(now.next, now.reg1)==null
                && searchReadStartingAt(now.next, now.reg1+1)==null
                && noReadWriteUntil(tmp.next, now.prev, now.reg0, true, true)
                && noReadWriteUntil(tmp.next.next, now.prev, now.reg0+1, true, true)) {
            ...
            break;
          case I_CLC:
            if ((tmp=getNextRealIns(now.next))!=null && tmp.type==I_CPCregreg
                && noJumpDestBetween(now, tmp)) {
              kill(now);
          ...
          case I_CPregreg:
            if ((vInfo=searchValueSetting(now, now.reg1))!=null) {
              if (now.reg1!=R_ZERO && vInfo.type==ValueInfo.VI_ABS) {
                if (vInfo.imm==0) { //replace 0-compare with special register r11
                  set(now, I_CPregreg, now.reg0, R_ZERO, 0);
                  redo=true;
                }
                else if (now.reg0>=16) { //replace ldi,cp with cpi
          ...
          case I_EORregreg:
            if (now.reg0==now.reg1 && flagRelevance(now.next, now.iPar2)==0) {
              if ((vInfo=searchValueSetting(now, now.reg0))!=null) {
                if (vInfo.type==ValueInfo.VI_ABS && vInfo.imm==0) { //avoid clearing again
                  kill(now);
                  redo=true;
                }
              }
          ...
          case I_LDIregimm:
            if (now.iPar1==0) {
              ...
              if (vInfo.type==ValueInfo.VI_ABS) {
                if (vInfo.imm==now.iPar1) { //avoid setting the same value
                  kill(now);
                  redo=true;
                }
              }
          ...
          case I_ADDregreg:
            if ((vInfo=searchValueSetting(now, now.reg1))!=null && vInfo.type==ValueInfo.VI_ABS) {
              switch (vInfo.imm) {
                case 0:
                  switch (flagRelevance(now.next, now.iPar2)) {
                    case 0: kill(now); redo=true; break INSTR_SW;
                    case SE_WRITE_FLAG_CARRY: set(now, I_CLC, 0, 0, 0); redo=true; break INSTR_SW;
                  }
                  break;
                case 1:
              ...
              if (now.reg0>=16) {
                ...
                immediateBuffer=(long)vInfo.imm;
            ...
            break;
          case I_SBCregreg:
            if (now.reg0>=16 && (vInfo=searchValueSetting(now, now.reg1))!=null
                && vInfo.type==ValueInfo.VI_ABS) {
              set(now, I_SBCIregimm, now.reg0, 0, vInfo.imm);
              redo=true;
            }
            break;
          case I_ADDIWregimm:
            if ((tmp=getNextRealIns(now.next))!=null && (tmp2=getNextRealIns(tmp.next))!=null
                && tmp.type==I_SUBIregimm && tmp2.type==I_SBCIregimm
                && tmp.reg0==now.reg0 && tmp2.reg0==now.reg0+1
                && flagRelevance(tmp2.next, SE_WRITE_FLAG_CARRY)==0) {
          ...
          case I_LDZ_DECreg:
          case I_LDZ_INCreg:
            if (searchReadStartingAt(now.next, R_ZL)!=null) break; //changed Z is needed later
            if ((vInfo=searchPointerSetting(now, R_ZL))!=null) {
              if (vInfo.type==ValueInfo.VI_MEMREL && vInfo.memreg==R_YL
                  && vInfo.imm>=0 && vInfo.imm<64) {
                //replace current Z-based addressing with Y-based addressing
                set(now, now.type==I_STZ_DECreg || now.type==I_STZ_INCreg
                    ? I_STY_DISPregimm : I_LDY_DISPregimm, now.reg0, 0,
                    now.type==I_STZ_DECreg || now.type==I_LDZ_DECreg
                    ? vInfo.imm-1 : vInfo.imm);
                redo=true;
              }
                ...
                if (vInfo.imm>=0x20 && vInfo.imm<0x60) {
                  ...
                  set(now, now.type==I_STZ_DECreg || now.type==I_STZ_INCreg
                      ? I_OUTimmreg : I_INregimm, now.reg0, 0,
                      (now.type==I_STZ_DECreg || now.type==I_LDZ_DECreg
                      ? vInfo.imm-1 : vInfo.imm)-0x20);
                  redo=true;
                }
          ...
          case I_LDY_DISPregimm:
            ...
            if ((tmp=searchStackFrameWriteBefore(now, now.iPar1))!=null
                && isRead(tmp, tmp.reg0) && noReadWriteUntil(tmp.next, now.prev, tmp.reg0, false, true)) {
          ...
          case I_STY_DISPregimm:
            ...
            else if (now.reg0!=R_ZERO && (vInfo=searchValueSetting(now, now.reg0))!=null
                && vInfo.type==ValueInfo.VI_ABS && vInfo.imm==0) {
          ...
          case I_POPreg:
            if ((tmp=searchCorrespPush(now, 0))!=null) {
              ...
              if ((vInfo=searchValueSetting(tmp, tmp.reg0))!=null && vInfo.type==ValueInfo.VI_ABS
                  && noReadWriteUntil(tmp.next, now.prev, now.reg0, true, false)) {
                if (vInfo.imm==0) {
                  kill(tmp);
                  set(now, I_EORregreg, now.reg0, now.reg0, 0);
                  redo=true;
          ...
          case I_BSHOPHINT:
            switch (now.iPar1) {
              case Ops.B_SHL: break; //ok
              case Ops.B_SHRL: break; //ok
              case Ops.B_SHRA: break INSTR_SW; //TODO arithmetic shift right (not supported yet)
              default:
                ctx.out.println("invalid BSHOPHINT");
                return;
            }
            if ((vInfo=searchValueSetting(now, now.reg1))!=null) {
              if (vInfo.type==ValueInfo.VI_ABS && (vInfo.imm&7)==0 && (i=vInfo.imm>>>3)>=0 && i<8) { //get register shift
                ...
                if (i!=0) { //not a no-shifter
                  if (j<i) { //not enough space?!?
                  ...
                  if (now.iPar1==Ops.B_SHL) {
                    while (j>i) {
                      if ((reg0=getReg(j, now.reg0, now.iPar3, false))==-1
                          || (reg1=getReg(j-i, now.reg0, now.iPar3, false))==-1) return;
                      set(tmp, I_MOVregreg, reg0, reg1, 0);
                      tmp=tmp.next;
                      j--;
                    }
                    while (j>=1) {
                      if ((reg0=getReg(j, now.reg0, now.iPar3, false))==-1) return;
                    ...
                    while (k<=(j-i)) {
                      if ((reg0=getReg(k, now.reg0, now.iPar3, false))==-1
                          || (reg1=getReg(k+i, now.reg0, now.iPar3, false))==-1) return;
                      set(tmp, I_MOVregreg, reg0, reg1, 0);
                      tmp=tmp.next;
                      k++;
                    }
                    while (k<=j) {
                      if ((reg0=getReg(k, now.reg0, now.iPar3, false))==-1) return;
                      set(tmp, I_EORregreg, reg0, reg0, 0);
                      tmp=tmp.next;
                      k++;
                    }
              ...
              else if (vInfo.type==ValueInfo.VI_ABS && vInfo.imm==1) { //get bit shift
                ...
                while (tmp.type!=I_DECreg) {
                  if ((tmp.type!=I_NONE && tmp.type!=I_JUMP) || tmp.prev==null) break INSTR_SW;
                  tmp=tmp.prev;
                }
                tmp=tmp.next; //do not delete DEC as the target register may be used later on
                while (tmp!=now.jDest) { //delete all jumps of the binary shift after the dec
                  kill(tmp);
                  tmp=tmp.next;
                }
                kill(now);
          ...
          case I_ANDIregimm:
          case I_ORIregimm:
            if ((tmp=searchWriteBefore(now, now.reg0, false))!=null && tmp.type==I_INregimm
                && (tmp2=getNextRealIns(now.next))!=null && noJumpDestBetween(now, tmp2) && tmp2.type==I_OUTimmreg
                && tmp.iPar1==tmp2.iPar1 && tmp.iPar1<0x20 && searchReadStartingAt(tmp2.next, now.reg0)==null) {
              ...
              j=-1;
              for (i=0; i<8; i++) if ((k&(1<<i))!=0) {
                if (j==-1) j=i; //first bit set
                else { //second bit set
                  j=-1;
                  break; //break for loop after invalidating j
                }
              }
              if (j!=-1) { //reducable, j contains number of bit to set/clear
          ...
          case I_SBRCregimm:
            if ((vInfo=searchValueSetting(now, now.reg0))!=null && vInfo.type==ValueInfo.VI_ABS) {
              if ((vInfo.imm&(1<<now.iPar1))==0) { //always skip
                now.type=I_JUMP;
        ...
        if (vInfo!=null) ungetValueInfo(vInfo);
        if (vInfo2!=null) ungetValueInfo(vInfo2);
      }
    } while (redo);
  }
  ...
  public void kill(Instruction i) {
    ...
    while (before!=null && before.prev!=null && before.type==I_NONE) before=before.prev;
  ...
  private void markReachableCode(Instruction now) {
    while (now!=null) {
      if (now.isDest) return; //already marked
      now.isDest=true;
      switch (now.type) {
        case I_JUMP:
          markReachableCode(now.jDest);
          if (now.iPar1==0) return; //do not continue after unconditional jump
          break;
        case I_ADDpatched:
  ...
  private void handleJumpDest(Instruction now) {
    ...
    iList=iDest.jSource;
    while (iList!=null) {
      if (iList.instr==now) break; //avoid inserting the same instruction again
      iList=iList.next;
    }
    if (iList==null) { //insert instruction
      if (emptyInstrLists==null) iList=new InstrList();
      else {
  ...
  private ValueInfo getEmptyValueInfo() {
    ...
    if (emptyVInfos==null) return new ValueInfo();
  ...
  private int flagRelevance(Instruction i, int flagMask) { //returns 0 for none, 1 for carry, 2 for carry/zero
    int relevantFlags=0;
    
    flagMask&=SE_WRITE_FLAG_CARRY|SE_WRITE_FLAG_SIGNED|SE_WRITE_FLAG_ZERO;
    while (i!=null) {
      if (i.isDest) break; //avoid endless loop, caller will reset isDest
      ...
      if ((flagMask&=~i.iPar2)==0) break; //remove written flags, done if all flags written afterwards
      if (i.type==I_JUMP) { //follow jump branch
        i.isDest=true;
        relevantFlags|=flagRelevance(i.jDest, flagMask);
        i.isDest=false;
        if (i.iPar1==0) break; //unconditional jump already checked
      }
      i=i.next;
    }
  ...
  private boolean noReadWriteUntil(Instruction from, Instruction to, int reg, boolean noRead, boolean noWrite) {
    if (from.instrNr>to.instrNr) return true;
    while (true) {
      if (from==null || from.isDest) return true; //end of method or already looked here and nothing found
      if ((noRead && isRead(from, reg)) || (noWrite && isWrite(from, reg))) return false;
      if (from.type==I_JUMP) {
        from.isDest=true;
        ...
        from.isDest=false;
        if (from.iPar1==0) return true; //unconditional jump ends here
      }
      if (from==to) return true; //no read or write found
      from=from.next;
    }
  }
  ...
  private boolean tryReplaceRegisterReading(Instruction i, Instruction end, int oldReg, int newReg) {
    ...
    if ((tmp=searchReadStartingAt(i, oldReg))==null
    		|| tmp.instrNr>end.instrNr) return true; //no read or read is after end => done
    if ((tmp.iPar2&SE_DEST_REGS)!=0 || (newReg<16 && (tmp.iPar2&SE_ONLY_UPPER_REGS)!=0)
        || !noReadWriteUntil(i, tmp.prev, oldReg, true, true)
        || !noReadWriteUntil(i, tmp.prev, newReg, false, true)) {
      return false; //no replacement possible
    }
    //try to replace registers
    if ((old0=tmp.reg0)==oldReg) new0=newReg;
    else new0=old0;
    if ((old1=tmp.reg1)==oldReg) new1=newReg;
    else new1=old1;
    ...
    if (!tryReplaceRegisterReading(i, end, oldReg, newReg)) { //register replace failed
      set(tmp, tmp.type, old0, old1, tmp.iPar1);
      return false;
    }
  ...
  private boolean tryReplaceRegImmADC(Instruction i) { //returns -1 on failure, 0 if carry cleared and 1 if carry set
    ValueInfo vInfo;
    boolean ret=false;
    
    if (((ATmegaOptInstr)i).jSource!=null) return false;
    if (i.type!=I_ADCregreg) {
      if (flagRelevance(i, SE_WRITE_FLAG_CARRY)!=0) return false;
      immediateBuffer=-immediateBuffer; //add+adc+adc+... is invertable to sub+sbc+sbc+... => invert sign
      return true;
    }
    if (i.reg0<16 || (vInfo=searchValueSetting(i, i.reg1))==null) return false;
    if (vInfo.type!=ValueInfo.VI_ABS) {
  ...
  private ValueInfo searchPointerSetting(Instruction i, int reg) {
    ...
    //check normally
    if ((now=searchWriteBefore(i, reg, false))==null) return null;
    switch (now.type) {
      case I_EORregreg:
        if (now.reg0!=now.reg1) break;
        //no break, do the following I_LDIregimm too
      case I_LDIregimm:
        if ((now.next.type!=I_LDIregimm && (now.next.type!=I_EORregreg || now.next.reg0!=now.next.reg1))
            || now.next.reg0!=now.reg0+1) break;
        if (now.type==I_LDIregimm) value=now.iPar1;
        else value=0;
        if (now.next.type==I_LDIregimm) value|=now.next.iPar1<<8;
        (ret=getEmptyValueInfo()).set(ValueInfo.VI_MEMABS, value);
        break;
      case I_MOVWregreg:
        //check special registers
        if (now.reg1==R_YL) { //stack base pointer relative
          ret=getEmptyValueInfo().set(ValueInfo.VI_MEMREL, 0, R_YL);
        }
        else if ((ret=searchPointerSetting(now, now.reg1))==null) { //also not other register-pair used as pointer
          ...
          else { //did not find setting of registers
            if (ret!=null) {
              ungetValueInfo(ret);
              ret=null;
            }
      ...
      case I_ADDIWregimm:
        ...
        break;
      case I_SUBIregimm:
        if (now.next==null || now.next.type!=I_SBCIregimm || now.next.reg0!=now.reg0+1) break;
        if ((ret=searchPointerSetting(now, now.reg0))!=null) {
          ret.imm-=(int)((short)(now.iPar1|(now.next.iPar1<<8)));
        }
        break;
      case I_ADDregreg:
        if (now.next==null || now.next.type!=I_ADCregreg || now.next.reg0!=now.reg0+1) break;
        ...
        break;
      case I_LDZreg:
      	if (reg!=R_ZL || now.reg0==R_ZL) break;
  ...
  private ValueInfo searchValueSetting(Instruction i, int reg) {
    ...
    switch (now.type) {
      ...
      case I_MOVregreg:
        ...
        break;
      case I_MOVWregreg:
        if (reg==now.reg0) ret=searchValueSetting(now, now.reg1);
        else if (reg==now.reg0+1) ret=searchValueSetting(now, now.reg1+1);
        else fatalError("invalid searchValueSetting for I_MOVWregreg");
        break;
      case I_ANDregreg:
        if ((tmp1=searchValueSetting(now, now.reg0))!=null && (tmp2=searchValueSetting(now, now.reg1))!=null  
            && tmp1.type==ValueInfo.VI_ABS && tmp2.type==ValueInfo.VI_ABS) {
      ...
      case I_ORIregimm:
        ...
        break;
      case I_EORregreg:
        if (now.reg0==now.reg1) ret=getEmptyValueInfo().set(ValueInfo.VI_ABS, 0);
        else if ((tmp1=searchValueSetting(now, now.reg0))!=null && (tmp2=searchValueSetting(now, now.reg1))!=null  
            && tmp1.type==ValueInfo.VI_ABS && tmp2.type==ValueInfo.VI_ABS) {
          ret=tmp1.set(ValueInfo.VI_ABS, tmp1.imm^tmp2.imm);
        }
        break;
      ...
      case I_NEGreg:
        ...
        break;
      case I_ADDIWregimm:
        if ((tmp1=searchValueSetting(now, now.reg0))!=null && (tmp2=searchValueSetting(now, now.reg0+1))!=null
            && tmp1.type==tmp2.type && tmp1.type==ValueInfo.VI_ABS) {
          value=(tmp1.imm|(tmp2.imm<<8)&0xFFFF)+now.iPar1;
          ret=tmp1;
          if (reg==now.reg0) ret.set(ValueInfo.VI_ABS, value&0xFF);
          else if (reg==now.reg0+1) ret.set(ValueInfo.VI_ABS, value>>>8);
          else fatalError("invalid searchValueSetting for I_ADDIWregimm");
        }
        break;
    ...
    //clean up
    if (ret!=tmp1 && tmp1!=null) ungetValueInfo(tmp1);
    if (ret!=tmp2 && tmp2!=null) ungetValueInfo(tmp2);
    return ret;
  }
  ...
  private int searchFlagSetting(Instruction i, int flagWriteMask) {
    ...
    while (i!=null) {
      if (((ATmegaOptInstr)i).jSource!=null) return -1; //don't follow different branches at the moment
      if ((i.iPar2&flagWriteMask)!=0) { //instruction writes requested flag
        switch (i.type) {
          case I_CPregreg:
            if ((tmp1=searchValueSetting(i, i.reg0))!=null && (tmp2=searchValueSetting(i, i.reg1))!=null
                && tmp1.type==ValueInfo.VI_ABS && tmp2.type==ValueInfo.VI_ABS) {
        ...
        if (tmp1!=null) ungetValueInfo(tmp1);
        if (tmp2!=null) ungetValueInfo(tmp2);
        return ret;
      }
      i=i.prev;
    }
  ...
  private Instruction searchWriteBefore(Instruction i, int reg, boolean includeInstruction) {
    ...
    while (i!=null) {
      if (i.isDest) return null; //already looked here, nothing found so far
      if (includeInstruction && isWrite(i, reg)) break;
      includeInstruction=true;
      if ((iList=((ATmegaOptInstr)i).jSource)!=null) {
        hadSideLine=true;
        i.isDest=true;
        while (iList!=null) {
          if (((sideReturn=searchWriteBefore(iList.instr, reg, true))==null)
              || (lastSideReturn!=null && !sameInstruction(sideReturn, lastSideReturn))) {
            i.isDest=false;
            return null;
          }
    ...
    if (hadSideLine && (lastSideReturn==null || !sameInstruction(lastSideReturn, i))) return null;
    return i;
  }
  ...
  private ValueInfo searchLocalSetting(Instruction i, int imm) {
    ...
    i=i.prev;
    while (i!=null) {
      if (((ATmegaOptInstr)i).jSource!=null) return null;
      switch (i.type) {
        case I_PUSHreg:
          if (i.reg0==R_YL) {
            if (foundFrameStart) return null; //do not search over multiple frames
            foundFrameStart=true;
          }
          if (foundFrameStart && ++pushCnt==imm) return searchValueSetting(i, i.reg0);
          break;
        case I_POPreg:
          if (i.reg0==R_YL) return null; //do not search over multiple inline frames
          break;
        case I_JUMP:
        case I_STZ_INCreg:
        case I_STZ_DECreg:
        case I_STX_DECreg:
          return null;
        case I_STY_DISPregimm:
          if (foundFrameStart) return null;
          if (i.iPar1==imm) return searchValueSetting(i, i.reg0);
          break;
      }
      i=i.prev;
    }
  ...
  private Instruction searchStackFrameReadAfter(Instruction i, int imm) {
    ...
    while (i!=null) {
      if (i.isDest) return null; //already checked here
      switch (i.type) {
        case I_JUMP:
          i.isDest=true;
          ...
          i.isDest=false;
          if (i.iPar1==0) return null; //unconditional jump ends here
          break;
        case I_LDX_INCreg:
        case I_LDZreg:
        case I_LDZ_INCreg:
        ...
        case I_LDY_DISPregimm:
          ...
          break;
        default:
          if ((i.iPar2&SE_READ_STACK)!=0) return i;
      }
      i=i.next;
    }
    return null;
  ...
  private Instruction searchCorrespPush(Instruction i, int popCnt) { //initially popCnt has to be 0
    ...
    while (i!=null) {
      ...
      if ((iList=((ATmegaOptInstr)i).jSource)!=null) {
        i.isDest=true;
        while (iList!=null) {
          if (((setter=searchCorrespPush(iList.instr, popCnt))==null)
              || (lastSetter!=null && setter!=lastSetter)) {
            i.isDest=false;
            return null;
          }
          lastSetter=setter;
      ...
      switch (i.type) {
        ...
        case I_ICALL:
        case I_CALLpatched:
          if (popCnt<=i.iPar3) return null; //can not reduce push/pop for call
          break;
        case I_OUTimmreg:
          if (i.iPar1==0x3D) return null; //writing stack pointer is not handled
          break;
        case I_PUSHip:
          if ((popCnt-=2)<=0) return null; //can not split PUSHip
          break;
        case I_PUSHreg:
          if (--popCnt<=0) {
            if (popCnt==0) return i;
            return null;
          }
          break;
        case I_POPreg:
        ...
        case I_BSHOPHINT:
          break;
        default:
          if (i.type<0 || (i.iPar2&SE_WRITE_STACK)!=0 || isWrite(i, R_YL) || isWrite(i, R_YL)) return null;
      }
      i=i.prev;
      if (i!=null && i.type==I_JUMP && i.iPar1==0) return lastSetter; //unconditional jump ends linear search
    }
    return null;
  }
  ...
  private Instruction searchReadStartingAt(Instruction i, int reg) {
    ...
    while (i!=null) {
      ...
      if (i.type==I_JUMP) {
        ...
        i.isDest=false;
        if (i.iPar1==0) return null; //unconditional jump with checked jDest
      }
      if (isRead(i, reg)) return i;
      if (isWrite(i, reg)) return null;
      i=i.next;
    }
    return null;
  }
  ...
  private boolean sameInstruction(Instruction a, Instruction b) {
    ...
    return (a==b || (a.type!=I_MAGC && a.type==b.type //implies b.type!=I_MAGC
        ...
        && a.iPar1==b.iPar1 && a.iPar2==b.iPar2 && a.iPar3==b.iPar3 && a.lPar==b.lPar
        && a.refMthd==b.refMthd && getNextRealIns(a.jDest)==getNextRealIns(b.jDest)));
  }
  
  private Instruction getNextRealIns(Instruction i) {
    while (i!=null && i.type==I_NONE) i=i.next;
    return i;
  }
