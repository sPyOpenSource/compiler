...
public class IA32 extends X86Base {
  ...
  protected int storeReg(int regs) {
    ...
    if ((regs&RegA)!=0) { ins(I_PUSHreg, R_EAX); stored|=RegA; }
    if ((regs&RegB)!=0) { ins(I_PUSHreg, R_EBX); stored|=RegB; }
    if ((regs&RegC)!=0) { ins(I_PUSHreg, R_ECX); stored|=RegC; }
    if ((regs&RegD)!=0) { ins(I_PUSHreg, R_EDX); stored|=RegD; }
    return stored;
  }
  ...
  protected void restoreReg(int regs) {
    usedRegs|=regs;
    writtenRegs|=regs;
    if ((regs&RegD)!=0) ins(I_POPreg, R_EDX);
    if ((regs&RegC)!=0) ins(I_POPreg, R_ECX);
    if ((regs&RegB)!=0) ins(I_POPreg, R_EBX);
  ...
  public void codeEpilog(Mthd outline) {
    ...
    if (curInlineLevel>0) { //end of method inlining
      ...
      else if (noStackOptimization) ins(I_POPreg, R_EBP); //no stack optimization - clean up stack frame
    ...
    if (var!=0) {
      if (var<=8) {
        ins(I_POPdummy);
        if (var==8) ins(I_POPdummy);
      }
      else ins(I_ADDregimm, R_ESP, 0, 0, var);
    }
    ins(I_POPreg, R_EBP);
    if (par>32767) {
    ...
    if ((curMthd.marker&Marks.K_INTR)!=0) {
      ins(I_POPA);
      if (par!=0) ins(I_ADDregimm, R_ESP, 0, 0, 4);
      ins(I_IRET);
    }
  ...
  public void genLoadVarAddr(int dst, int src, Object loc, int off) {
    ...
    if (pos==0 && src==dst) /*nothing to do*/ return;
  ...
  public void genLoadVarVal(int dstR, int src, Object loc, int off, int type) {
    ...
    switch (type) {
      case StdTypes.T_BOOL: ins(I_MOVregmem, dst&~(RS_E&~RS_L), src, pos); return;
      case StdTypes.T_BYTE: ins(I_MOVSXregmem, dst, src, pos, 0, 1); return;
      case StdTypes.T_SHRT: ins(I_MOVSXregmem, dst, src, pos, 0, 2); return;
      case StdTypes.T_CHAR: ins(I_MOVZXregmem, dst, src, pos, 0, 2); return;
      case StdTypes.T_INT:  case StdTypes.T_PTR:  ins(I_MOVregmem, dst, src, pos); return;
      case StdTypes.T_LONG: case StdTypes.T_DPTR:
        if ((dstL=getReg(2, dstR, type, true))==0) return;
        if (dst!=src) { //first move dst, then dstL
  ...
  public void genConvertVal(int dstR, int srcR, int toType, int fromType) {
    ...
    if (toType==StdTypes.T_FLT || toType==StdTypes.T_DBL) {
      ...
      switch (fromType) {
        case StdTypes.T_BYTE: case StdTypes.T_SHRT: case StdTypes.T_CHAR: case StdTypes.T_INT:
          if ((src=getReg(1, srcR, StdTypes.T_INT, false))==0) return;
          ins(I_PUSHreg, src);
          ins(I_FILDmem, R_ESP, 0, 0, 0, FPU32);
          if (useOnly487) ins(I_FWAIT);
          ins(I_POPreg, src);
          return;
        case StdTypes.T_LONG:
          if ((src=getReg(1, srcR, StdTypes.T_LONG, false))==0
              || (srcL=getReg(2, srcR, StdTypes.T_LONG, false))==0) return;
          ins(I_PUSHreg, srcL);
          ins(I_PUSHreg, src);
    ...
    if (fromType==StdTypes.T_FLT || fromType==StdTypes.T_DBL) {
      ...
      switch (toType) {
        case StdTypes.T_BYTE: case StdTypes.T_SHRT: case StdTypes.T_CHAR: case StdTypes.T_INT:
          if ((dst=getReg(1, dstR, toType, false))==0) return;
          ins(I_PUSHreg, dst|RS_E);
          if (useFISTTP) ins(I_FISTTPmem, R_ESP, 0, 0, 0, FPU32); //introduced with P4-SSE3
          else {
        ...
        case StdTypes.T_LONG:
          if ((dst=getReg(1, dstR, StdTypes.T_LONG, false))==0
              || (dstL=getReg(2, dstR, StdTypes.T_LONG, false))==0) return;
          ins(I_PUSHreg, dstL);
          ins(I_PUSHreg, dst);
          if (useFISTTP) ins(I_FISTTPmem, R_ESP, 0, 0, 0, FPU64); //introduced with P4-SSE3
          else {
            ins(I_FNSTCWmem, R_ESP);                 //get FCW
            if (useOnly487) ins(I_FWAIT);
            ins(I_POPreg, dst|RS_E);                 //get value from stack
            ins(I_PUSHreg, dst|RS_E);                //save value to stack again
            ins(I_PUSHreg, dst|RS_E);                //save value to stack restore allocated space
            ins(I_ORregimm, (dst&RS_X)|RS_X, 0, 0, 0x0C00); //set truncate mode
            ins(I_PUSHreg, dst|RS_E);                //save value to stack
            ins(I_FLDCWmem, R_ESP);                  //set control word
            if (useOnly487) ins(I_FWAIT);
            ins(I_POPreg, dst|RS_E);                 //remove changed FCW
    ...
    if ((dst=getReg(1, dstR, StdTypes.T_INT, true))==0  || (src=getReg(1, srcR, toType, false))==0) return;
    ...
    if (toType==StdTypes.T_PTR || fromType==StdTypes.T_PTR) {
      ...
      if (dst!=src) ins(I_MOVregreg, dst, src);
      return;
    }
    //normal conversions
    if (toType==StdTypes.T_LONG) { //to long is always the same
      if ((dstL=getReg(2, dstR, toType, true))==0) return;
      if (fromType==StdTypes.T_LONG) {
        if ((srcL=getReg(2, srcR, toType, false))==0) return;
        if (dstR==srcR) /*nothing to do for long->long*/ return;
        if (dst==srcL) {
      ...
      if ((src=getReg(1, srcR, StdTypes.T_INT, false))==0) return;
      if (src!=dst) ins(I_MOVregreg, dst, src);
      if (dst==R_EAX && dstL==R_EDX) ins(I_CDQ);
      else {
  ...
  public void genPush(int srcR, int type) {
    ...
    switch (type) {
      ...
      case StdTypes.T_FLT:
        ...
        if (isDouble) ins(I_PUSHreg, R_EAX);
        ...
        return;
      case StdTypes.T_LONG: case StdTypes.T_DPTR:
        if ((src=getReg(2, srcR, StdTypes.T_LONG, false))==0) return;
        ins(I_PUSHreg, src);
  ...
  public void genPop(int dstR, int type) {
    ...
    ins(I_POPreg, dst);
    if (type==StdTypes.T_LONG || type==StdTypes.T_DPTR) {
      if ((dst=getReg(2, dstR, StdTypes.T_LONG, true))==0) return;
      ins(I_POPreg, dst);
    }
  }
      ...
      case StdTypes.T_INT:
        switch (opType) {
          ...
          case Ops.S_BSH:
            ...
            switch (opPar) {
              case Ops.B_SHL:  ins(I_SHLregreg, dst); break; //second regiser is fixed to cl
              case Ops.B_SHRL: ins(I_SHRregreg, getReg(1, dstR, type, false)); break; //second regiser is fixed to cl, get real operand size
              case Ops.B_SHRA: ins(I_SARregreg, dst); break; //second regiser is fixed to cl
              default:
                fatalError(ERR_UNSOP_GENBINOP);
                return;
            } 
        ...
        return;
      case StdTypes.T_LONG:
        if ((dstL=getReg(2, dstR, StdTypes.T_LONG, false))==0) return;
        switch (opType) {
          case Ops.S_ARI:
            if ((src2L=getReg(2, src2R, StdTypes.T_LONG, false))==0) return;
            switch (opPar) {
              case Ops.A_AND:
              ...
              case Ops.A_MOD: //mostly the same code for div/mod
                ...
                if (opPar==Ops.A_DIV) ins(I_XORregimm, R_ESI, 0, 0, 1); //neg has to be reversed for div
                ins(I_NEGreg, R_ECX);
                ins(I_NEGreg, R_EBX);
  ...
  public void genBinOpConstRi(int dstR, int src1R, int val, int op, int type) {
    ...
    switch (type) {
        ...
        switch (opType) {
          ...
          case Ops.S_BSH:
            switch (opPar) {
              ...
              case Ops.B_SHRL: ins(I_SHRregimm, getReg(1, dstR, type, false), 0, 0, val); break; //use real operand size
      ...
      case StdTypes.T_LONG:
        switch (opType) {
          case Ops.S_BSH:
            if (val!=32) break; //optimize only if a complete register may be shifted
            if ((dst=getReg(1, dstR, StdTypes.T_LONG, false))==0
                || (dstL=getReg(2, dstR, StdTypes.T_LONG, false))==0) return;
            switch (opPar) {
              case Ops.B_SHL:
  ...
  public void genUnaOp(int dstR, int srcR, int op, int type) {
    ...
    if (type==StdTypes.T_FLT || type==StdTypes.T_DBL) {
      ...
      if ((dst2=getReg(2, dstR, StdTypes.T_LONG, false))==0) return;
      switch (op&0xFFFF) {
        case Ops.A_CPL:
          ins(I_NOTreg, dst);
          ins(I_NOTreg, dst2);
  ...
  public void genIncMem(int dst, int type) {
    ...
    if ((dst=getReg(1, dst, StdTypes.T_PTR, false))==0) return;
    switch (type) {
      case StdTypes.T_BYTE:                       ins(I_INCmem, dst, 0, 0, 0, 1); return;
      case StdTypes.T_SHRT: case StdTypes.T_CHAR: ins(I_INCmem, dst, 0, 0, 0, 2); return;
      case StdTypes.T_INT:                        ins(I_INCmem, dst, 0, 0, 0, 4); return;
      case StdTypes.T_LONG:
  ...
  public void genDecMem(int dst, int type) {
    ...
    if ((dst=getReg(1, dst, StdTypes.T_PTR, false))==0) return;
    switch (type) {
      case StdTypes.T_BYTE:                       ins(I_DECmem, dst, 0, 0, 0, 1); return;
      case StdTypes.T_SHRT: case StdTypes.T_CHAR: ins(I_DECmem, dst, 0, 0, 0, 2); return;
      case StdTypes.T_INT:                        ins(I_DECmem, dst, 0, 0, 0, 4); return;
      case StdTypes.T_LONG:
  ...
  public void genCallIndexed(int intfReg, int off, int parSize) {
    ...
    if (ctx.codeStart!=0) ins(I_LEAregmem, R_EAX, R_EAX, ctx.codeStart);
    ins(I_CALLreg, R_EAX, 0, 0, 0, parSize);
  }
  ...
  public int genComp(int src1R, int src2R, int type, int cond) {
    ...
    if (type==StdTypes.T_FLT || type==StdTypes.T_DBL) {
      ...
      if (useOnly487) {
        ...
        if ((usedRegs&RegA)!=0) ins(I_PUSHreg, R_EAX);
        ins(I_FCOMPP);
        ins(I_FSTSW);
        ins(I_FWAIT);
        ins(I_SAHF);
        if ((usedRegs&RegA)!=0) ins(I_POPreg, R_EAX);
      }
      else ins(I_FCOMIP);
    ...
    nextJumpsUnsigned=false;
    if ((src1=getReg(1, src1R, StdTypes.T_INT, false))==0
        || (src2=getReg(1, src2R, StdTypes.T_INT, false))==0) return 0;
    switch (type) {
      case StdTypes.T_BOOL:
      ...
      case StdTypes.T_INT:  case StdTypes.T_PTR:
        ...
        return cond;
      case StdTypes.T_LONG:
        if ((src1L=getReg(2, src1R, StdTypes.T_LONG, false))==0
            || (src2L=getReg(2, src2R, StdTypes.T_LONG, false))==0) return 0;
  ...
  public int genCompValToConstDoubleOrLongVal(int src, long val, boolean asDouble, int cond) {
    ...
    nextJumpsUnsigned=false;
    if ((reg1=getReg(1, src, StdTypes.T_LONG, false))==0
        || (reg2=getReg(2, src, StdTypes.T_LONG, false))==0) return 0;
  ...
  public void genLoadDerefAddr(int destReg, int objReg, int ind, int baseOffset, int entrySize) {
    ...
    switch (entrySize) {
      ...
      default:
        ...
        if (destReg!=objReg) ins(I_MOVregreg, destReg, objReg);
        if (baseOffset!=0) ins(I_ADDregimm, destReg, 0, 0, baseOffset);
  ...
  public void genMoveToPrimary(int srcR, int type) {
    ...
    if (type==StdTypes.T_LONG || type==StdTypes.T_DPTR) {
      if ((reg=getReg(2, srcR, StdTypes.T_LONG, false))==0) return;
      if (reg!=R_EDX) ins(I_MOVregreg, R_EDX, reg);
    }
  }
  ...
  public void genMoveFromPrimary(int dstR, int type) {
    ...
    if (type==StdTypes.T_LONG || type==StdTypes.T_DPTR) {
      if ((reg=getReg(2, dstR, StdTypes.T_LONG, true))==0) return;
      if (reg!=R_EDX) ins(I_MOVregreg, reg, R_EDX);
    }
  ...
  public void genRestPrimary(int type) {
    ...
    if (type==StdTypes.T_LONG || type==StdTypes.T_DPTR) ins(I_POPreg, R_EDX);
    ins(I_POPreg, R_EAX);
  }
  ...
  public void genCallNative(Object loc, int off, boolean relative, int parSize, boolean noCleanUp) {
    ...
    else ins(I_CALLmem, 0, 0, mem.getAddrAsInt(loc, off), 0, parSize);
  ...
  public void genStoreNativeParameter(int offset, int src, int type) {
    ...
    if ((reg=getReg(1, src, StdTypes.T_INT, false))==0) return;
    ins(I_MOVmemreg, R_ESP, reg, offset);
    if (type==StdTypes.T_LONG || type==StdTypes.T_DPTR) {
      if ((reg=getReg(2, src, StdTypes.T_LONG, false))==0) return;
  ...
  protected void code(Instruction i, int type, int reg0, int reg1, int disp, int imm, int par) {
    ...
    switch (type) {
      ...
      case I_MOVmemimm:
        ...
        return;
      case I_MOVSXregreg:
        if ((reg0&0x0F)==RS_X) i.putByte(0x66); //to 16 bit instead of 32
        i.putByte(0x0F);
      ...
      case I_PUSHmem:
        ...
        return;
      case I_PUSHimm:
        if (imm>=-128 && imm<128) { //8 bits immediate
          i.putByte(0x6A);
          i.putByte(imm);
        }
  ...
  private void codeShiftMemImm(Instruction i, int code, int reg0, int disp, int imm, int par) {
    ...
    if (par==2) { i.putByte(0x66); wordflag=1; }
    else if (par==4) wordflag=1;
    if (imm==1) {
      i.putByte(0xD0|wordflag);
      putMem(i, code, reg0, disp);
    }
