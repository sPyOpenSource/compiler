...
public class AMD64 extends X86Base {
  ...
  public void putCodeRef(Object loc, int offset, Object ptr, int ptrOff) {
    ...
    else mem.putInt(loc, offset, (int)destPtr);
  }
  
  //register allocation and de-allocation
  private int getFreeReg(int avoidRegs, int type) { //allow tmp-Regs, so we always find something
    int ret;
  ...
  protected int storeReg(int regs) {
    ...
    if ((regs&RegA ) != 0) { ins(I_PUSHreg, REX|R_EAX); stored|=RegA; }
    if ((regs&RegB ) != 0) { ins(I_PUSHreg, REX|R_EBX); stored|=RegB; }
    if ((regs&RegC ) != 0) { ins(I_PUSHreg, REX|R_ECX); stored|=RegC; }
    if ((regs&RegD ) != 0) { ins(I_PUSHreg, REX|R_EDX); stored|=RegD; }
    if ((regs&Reg8 ) != 0) { ins(I_PUSHreg, REX|R_8D ); stored|=Reg8; }
    if ((regs&Reg9 ) != 0) { ins(I_PUSHreg, REX|R_9D ); stored|=Reg9; }
    if ((regs&Reg10) != 0) { ins(I_PUSHreg, REX|R_10D); stored|=Reg10; }
    if ((regs&Reg11) != 0) { ins(I_PUSHreg, REX|R_11D); stored|=Reg11; }
    if ((regs&Reg12) != 0) { ins(I_PUSHreg, REX|R_12D); stored|=Reg12; }
    if ((regs&Reg13) != 0) { ins(I_PUSHreg, REX|R_13D); stored|=Reg13; }
    if ((regs&Reg14) != 0) { ins(I_PUSHreg, REX|R_14D); stored|=Reg14; }
    if ((regs&Reg15) != 0) { ins(I_PUSHreg, REX|R_15D); stored|=Reg15; }
    return stored;
  }
  ...
  public void codeProlog() {
    ...
    switch (curMthd.varSize) {
      case 0: break;
      case 8: ins(I_PUSHimm); break;
      default:
        ins(I_XORregreg, R_EBX, R_EBX);
        for (i=curMthd.varSize; i>0; i-=8) ins(I_PUSHreg, REX|R_EBX);
    }
  }
  ...
  public void codeEpilog(Mthd outline) {
    ...
    var=curMthd.varSize;
    par=curMthd.parSize;
    if (curInlineLevel>0) { //end of method inlining
      if (var!=0 || par!=0) {
        if (var==0) { //par!=0
        ...
        else { //neither var==0 nor par==0
          ins(I_MOVregmem, REX|R_EBP, REX|R_EBP);
          ins(I_ADDregimm, REX|R_ESP, 0, 0, var+par+8);
        }
      }
      else if (noStackOptimization) ins(I_POPreg, REX|R_EBP); //no stack optimization - clean up stack frame
      if (--curInlineLevel==0) curVarOffParam=(outline.marker&Marks.K_INTR)!=0 ? VAROFF_PARAM_INT : VAROFF_PARAM_NRM;
      curMthd=outline;
      return;
    }
    ...
    if (var!=0) ins(I_ADDregimm, REX|R_ESP, 0, 0, var);
    ins(I_POPreg, REX|R_EBP);
    if (par>32767) {
    ...
    if ((curMthd.marker&Marks.K_INTR)!=0) {
      ...
      if (par!=0) ins(I_ADDregimm, REX|R_ESP, 0, 0, 8);
      ins(I_IRET);
    }
    else ins(I_RETimm, 0, 0, 0, (curMthd.marker&Marks.K_NTCB)==0 ? par : 0);
    curMthd=null;
  }
  ...
  public void genLoadConstDoubleOrLongVal(int dst, long val, boolean asDouble) {
    if (asDouble) {
      ...
      else ins(I_MOVregimmL, dst, 0, 0, 0, val, 0);
    }
  }
  ...
  public void genLoadVarAddr(int dst, int src, Object loc, int off) {
    ...
    if (src==0) {
      long val;
      if (((val=mem.getAddrAsLong(loc, off))&0xFFFFFFFF00000000l)!=0l) ins(I_MOVregimmL, dst, 0, 0, 0, val, 0);
      else ins(I_MOVregimm, dst&~REX, 0, 0, (int)val);
      return;
    }
    if (src==regBase && pos>=0) if (pos>=0) pos+=curVarOffParam;
    if ((src=getReg(1, src, StdTypes.T_PTR, true))==0) return;
    if (dst==src && pos==0) return;
  ...
  public void genLoadVarVal(int dstR, int src, Object loc, int off, int type) {
    ...
    if (src==regBase && pos>=0) if (pos>=0) pos+=curVarOffParam;
    if (src!=0 && (src=getReg(1, src, StdTypes.T_PTR, false))==0) return;
    if (src==0) {
      //TODO
    ...
    if ((dst=getReg(1, dstR, StdTypes.T_INT, true))==0) return;
    switch (type) {
      case StdTypes.T_BOOL: case StdTypes.T_BYTE: ins(I_MOVSXregmem, dst, src, pos, 0, 0l, 1); return;
      case StdTypes.T_SHRT:                       ins(I_MOVSXregmem, dst, src, pos, 0, 0l, 2); return;
      case StdTypes.T_CHAR:                       ins(I_MOVZXregmem, dst, src, pos, 0, 0l, 2); return;
      case StdTypes.T_INT:                        ins(I_MOVregmem, dst, src, pos); return;
      case StdTypes.T_LONG: case StdTypes.T_PTR:  ins(I_MOVregmem, REX|dst, src, pos); return;
      case StdTypes.T_DPTR:
        if ((dst2=getReg(2, dstR, StdTypes.T_DPTR, true))==0) return;
        if ((dst|=REX)!=(src|=REX)) { //first move dst, then dst2
  ...
  public void genConvertVal(int dst, int src, int toType, int fromType) {
    ...
    if (fromType==StdTypes.T_FLT || fromType==StdTypes.T_DBL) {
      ...
      switch (toType) {
        case StdTypes.T_BYTE: case StdTypes.T_SHRT: case StdTypes.T_CHAR: case StdTypes.T_INT:
          if ((dst=getReg(1, dst, toType, false))==0) return;
          ins(I_PUSHreg, dst|RS_E);
          ins(I_FISTTPmem, REX|R_ESP, 0, 0, 0, 0l, FPU32);
          ins(I_POPreg, dst|RS_E);
          if (toType!=StdTypes.T_INT) ins(I_MOVSXregreg, dst|RS_E, dst);
          dupFPUIns=null;
          popFPUDone++;
          return;
        case StdTypes.T_LONG:
          if ((dst=getReg(1, dst, StdTypes.T_LONG, false))==0) return;
          ins(I_PUSHreg, dst);
    ...
    if (toType==StdTypes.T_LONG || toType==StdTypes.T_PTR) { //to long/ptr is always the same
      if ((dst=getReg(1, dst, StdTypes.T_LONG, true))==0) return; //next line: pointers do not need sign extension
      if (fromType==StdTypes.T_LONG || fromType==StdTypes.T_PTR || toType==StdTypes.T_PTR) {
        if ((src=getReg(1, src, StdTypes.T_LONG, false))==0) return;
        if (dst==src) /*nothing to do*/return;
      ...
      if ((src=getReg(1, src, StdTypes.T_INT, false))==0) return;
      if (dst==(REX|R_EAX)) {
        if (src!=R_EAX) ins(I_MOVregreg, R_EAX, src);
        ins(I_CDQE);
      }
      else {
        if ((usedRegs&RegA)!=0) { //RegA is in use
          ins(I_XCHGregreg, REX|R_EAX, REX|src);
          ins(I_CDQE);
          ins(I_XCHGregreg, REX|R_EAX, REX|src);
          if (dst!=(REX|src)) ins(I_MOVregreg, dst, REX|src);
        }
        else { //silently destroy RegA
    ...
    if ((dst=getReg(1, dst, StdTypes.T_INT, true))==0 || (src=getReg(1, src, toType, false))==0) return;
    switch (fromType) {
      case StdTypes.T_BYTE:
        switch (toType) {
          case StdTypes.T_BYTE: case StdTypes.T_SHRT: case StdTypes.T_CHAR: case StdTypes.T_INT:
            //nothing to do for byte->byte/short/char/int
            if (dst!=(src|RS_E)) ins(I_MOVregreg, dst, src);
            //else: nothing to do
            return;
        }
        break;
      case StdTypes.T_SHRT: case StdTypes.T_CHAR:
        switch (toType) {
          case StdTypes.T_BYTE:
            ...
            return;
          case StdTypes.T_SHRT: case StdTypes.T_CHAR: case StdTypes.T_INT:
      ...
      case StdTypes.T_INT: case StdTypes.T_LONG:
        switch (toType) {
          case StdTypes.T_BYTE: case StdTypes.T_SHRT:
            ins(I_MOVSXregreg, dst, src);
            return;
          case StdTypes.T_CHAR:
  ...
  public void genDup(int dstR, int srcR, int type) {
    ...
    if ((dst=getReg(1, dstR, type, true))==0 || (src=getReg(1, srcR, type, false))==0) return;
    switch (type) {
      case StdTypes.T_BOOL: case StdTypes.T_BYTE:
      case StdTypes.T_SHRT: case StdTypes.T_CHAR:
      case StdTypes.T_INT:
      ...
      case StdTypes.T_DPTR:
        ins(I_MOVregreg, dst, src);
        if ((dst=getReg(2, dstR, StdTypes.T_DPTR, true))==0 || (src=getReg(2, srcR, StdTypes.T_DPTR, false))==0) return;
        ins(I_MOVregreg, dst, src);
        return;
    }
  ...
  public void genPushConstDoubleOrLongVal(long val, boolean asDouble) {
    ...
    if ((reg=getReg(1, regR, StdTypes.T_LONG, false))==0) return;
  ...
  public void genPush(int srcR, int type) {
    ...
    switch (type) {
      ...
      case StdTypes.T_NULL:
        ...
        return;
      case StdTypes.T_DPTR:
        if ((src=getReg(2, srcR, StdTypes.T_DPTR, false))==0) return;
        ins(I_PUSHreg, src);
        //no return, has to do the following, too
    }
    if (type==StdTypes.T_BOOL || type==StdTypes.T_BYTE || type==StdTypes.T_SHRT 
        || type==StdTypes.T_CHAR) type=StdTypes.T_INT;
    if ((src=getReg(1, srcR, type, false))==0) return;
    ins(I_PUSHreg, src);
  }
  ...
  public void genPop(int dstR, int type) {
    ...
    if (type==StdTypes.T_BOOL || type==StdTypes.T_BYTE || type==StdTypes.T_SHRT 
        || type==StdTypes.T_CHAR) dummyType=StdTypes.T_INT;
    else dummyType=type;
    if ((dst=getReg(1, dstR, dummyType, true))==0) return;
    ins(I_POPreg, dst);
    if (type==StdTypes.T_DPTR) {
      if ((dst=getReg(2, dstR, StdTypes.T_DPTR, true))==0) return;
      ins(I_POPreg, dst);
    }
  }
  ...
  public void genAssign(int dst, int srcR, int type) {
    ...
    if ((dst=getReg(1, dst, StdTypes.T_PTR, false))==0 || (src=getReg(1, srcR, type, false))==0) return;
  ...
  public void genStoreVarVal(int objReg, Object loc, int off, int src, int type) {
    ...
    if ((srcR=getReg(1, src, type, false))==0) return;
    if (objReg==0) {
      //TODO
      if ((mem.getAddrAsLong(loc, off)&0xFFFFFFFF00000000l)!=0l) {
    ...
    if (type==StdTypes.T_DPTR) {
      if ((srcR=getReg(2, src, type, false))==0) return;
  ...
  public void genStoreVarConstVal(int objReg, Object loc, int off, int val, int type) {
    ...
    switch (type) {
      ...
      case StdTypes.T_INT: case StdTypes.T_FLT:
    ...
    switch (type) {
      case StdTypes.T_FLT: case StdTypes.T_DBL:
        ...
        switch (opPar) {
          ...
          case Ops.A_MINUS: ins(I_FSUBP); return;
          case Ops.A_MUL:   ins(I_FMULP); return;
          case Ops.A_DIV:   ins(I_FDIVP); return;
        }
      ...
      case StdTypes.T_SHRT: case StdTypes.T_CHAR:
      case StdTypes.T_INT:
        if ((dst=getReg(1, dstR, StdTypes.T_INT, false))==0 || (src1=getReg(1, src1, StdTypes.T_INT, false))==0
            || (src2=getReg(1, src2, StdTypes.T_INT, false))==0) return;
        switch (opType) {
          case Ops.S_ARI:
            switch (opPar) {
          ...
          case Ops.S_BSH:
            ...
            if (xchg) ins(I_XCHGregreg, dst, src2);
            restoreReg(usedMask);
      ...
      case StdTypes.T_LONG:
        if ((dst=getReg(1, dstR, StdTypes.T_LONG, false))==0
            || (src2=getReg(1, src2, StdTypes.T_LONG, false))==0) return;
        switch (opType) {
          case Ops.S_ARI:
            switch (opPar) {
              case Ops.A_AND:   ins(I_ANDregreg, dst, src2); return;
              case Ops.A_XOR:   ins(I_XORregreg, dst, src2); return;
              case Ops.A_OR:    ins(I_ORregreg, dst, src2);  return;
              case Ops.A_PLUS:  ins(I_ADDregreg, dst, src2); return;
              case Ops.A_MINUS: ins(I_SUBregreg, dst, src2); return;
              case Ops.A_MUL:
                if (dst!=(REX|R_EAX) && src2!=(REX|R_EAX)) saveRegs=RegA;
                else saveRegs=0;
                if (dst!=(REX|R_EDX) && src2!=(REX|R_EDX)) saveRegs|=RegD;
                usedMask=storeReg(saveRegs);
                if (dst!=(REX|R_EAX)) {
                  if (src2!=(REX|R_EAX)) ins(I_MOVregreg, REX|R_EAX, dst);
                  else src2=dst; //we don't need the original src2 anymore
                }
                //else: dstsrc1==REX|R_EAX && src2!=REX|R_EAX, nothing needed
                ins(I_IMULreg, src2);
                if (dst!=(REX|R_EAX)) ins(I_MOVregreg, dst, REX|R_EAX);
                restoreReg(usedMask);
                return;
              case Ops.A_DIV:
              case Ops.A_MOD:
                if (dst!=(REX|R_EAX)) { //dstsrc1!=REX|R_EAX
                  if (src2==(REX|R_EAX)) { //dstsrc1!=REX|R_EAX && src2==REX|R_EAX
                    if (dst==(REX|R_EDX)) { //dstsrc1==REX|R_EDX && src2==REX|R_EAX
                      usedMask=storeReg(RegB);
                      ins(I_MOVregreg, REX|R_EBX, src2);
                      ins(I_MOVregreg, REX|R_EAX, dst);
                      src2=REX|R_EBX;
                    }
                    else { //dstsrc1!=REX|R_EAX && dstsrc1!=REX|R_EDX && src2==REX|R_EAX
                      usedMask=storeReg(RegD);
                      ins(I_XCHGregreg, dst, src2);
                      src2=dst;
                    }
                  }
                  else { //dstsrc1!=REX|R_EAX && src2!=REX|R_EAX
                    if (dst==(REX|R_EDX)) { //dstsrc1==REX|R_EDX && src2!=REX|R_EAX (&& src2!=R_EDX)
                      usedMask=storeReg(RegA);
                      ins(I_MOVregreg, REX|R_EAX, dst);
                    }
                    else { //dstsrc1!=REX|R_EAX && dstsrc1!=REX|R_EDX && src2!=REX|R_EAX
                      if (src2==(REX|R_EDX)) { //dstsrc1!=REX|R_EAX && dstsrc1!=REX|R_EDX && src2==REX|R_EDX
                        usedMask=storeReg(RegA);
                        ins(I_MOVregreg, REX|R_EAX, dst);
                        ins(I_MOVregreg, dst, src2);
                        src2=dst;
                      }
                      else { //dstsrc1!=REX|R_EAX && dstsrc1!=REX|R_EDX && src2!=REX|R_EAX && src2!=R_REX|EDX
                ...
                else if (src2==(REX|R_EDX)) { //dstsrc1==REX|R_EAX && src2==REX|R_EDX
                  usedMask=storeReg(RegB);
                  ins(I_MOVregreg, REX|R_EBX, src2);
                  src2=REX|R_EBX;
                }
                ...
                if (opPar==Ops.A_DIV) {
                  ...
                  if (dst!=(REX|R_EDX)) ins(I_MOVregreg, dst, REX|R_EDX);
                }
                restoreReg(usedMask);
                return;
            }
            ...
            return;
          case Ops.S_BSH:
            if (dst==(REX|R_ECX)) { //src2!=REX|R_ECX
  ...
  public void genBinOpConstRi(int dstR, int src1R, int val, int op, int type) {
    ...
    switch (type) {
      ...
      case StdTypes.T_SHRT: case StdTypes.T_CHAR:
      case StdTypes.T_INT:
        if ((dst=getReg(1, dstR, StdTypes.T_INT, false))==0) return;
        switch (opType) {
          case Ops.S_ARI:
            switch (opPar) {
              case Ops.A_AND:   ins(I_ANDregimm, dst, 0, 0, val); break;
              case Ops.A_XOR:   ins(I_XORregimm, dst, 0, 0, val); break;
              case Ops.A_OR:    ins(I_ORregimm, dst, 0, 0, val); break;
              case Ops.A_PLUS:  ins(I_ADDregimm, dst, 0, 0, val); break;
              case Ops.A_MINUS: ins(I_SUBregimm, dst, 0, 0, val); break;
              default:
          ...
          case Ops.S_BSH:
            switch(opPar) {
              case Ops.B_SHL:  ins(I_SHLregimm, dst, 0, 0, val); break;
              case Ops.B_SHRL: ins(I_SHRregimm, getReg(1, dstR, type, false), 0, 0, val); break;
              case Ops.B_SHRA: ins(I_SARregimm, dst, 0, 0, val); break;
              default:
                super.genBinOpConstRi(dstR, src1R, val, op, type);
                return;
            }
  ...
  public void genUnaOp(int dstR, int srcR, int op, int type) {
    ...
    switch (type) {
      case StdTypes.T_FLT: case StdTypes.T_DBL:
        ...
        break;
      case StdTypes.T_LONG:
        if ((dst=getReg(1, dstR, StdTypes.T_LONG, false))==0) return;
        switch (op&0xFFFF) {
          case Ops.A_CPL:
            ins(I_NOTreg, dst);
            return;
  ...
  public void genIncMem(int dst, int type) {
    if ((dst=getReg(1, dst, StdTypes.T_PTR, false))==0) return;
    switch (type) {
      case StdTypes.T_BYTE:                       ins(I_INCmem, dst, 0, 0, 0, 0l, 1); return;
      case StdTypes.T_SHRT: case StdTypes.T_CHAR: ins(I_INCmem, dst, 0, 0, 0, 0l, 2); return;
      case StdTypes.T_INT:                        ins(I_INCmem, dst, 0, 0, 0, 0l, 4); return;
      case StdTypes.T_LONG:                       ins(I_INCmem, dst, 0, 0, 0, 0l, 8); return;
    }
  ...
  public void genDecMem(int dst, int type) {
    if ((dst=getReg(1, dst, StdTypes.T_PTR, false))==0) return;
    switch (type) {
      case StdTypes.T_BYTE:                       ins(I_DECmem, dst, 0, 0, 0, 0l, 1); return;
      case StdTypes.T_SHRT: case StdTypes.T_CHAR: ins(I_DECmem, dst, 0, 0, 0, 0l, 2); return;
      case StdTypes.T_INT:                        ins(I_DECmem, dst, 0, 0, 0, 0l, 4); return;
      case StdTypes.T_LONG:                       ins(I_DECmem, dst, 0, 0, 0, 0l, 8); return;
    }
    fatalError(ERR_UNSTYPE_GENINCDECMEM);
  }
  ...
  public void genLoadConstUnitContext(int dst, Object unitLoc) {
    ...
    if ((dst=getReg(1, dst, StdTypes.T_PTR, true))==0) return;
    if ((unitAddr&0xFFFFFFFF00000000l)==0l) ins(I_MOVregimm, dst&~REX, 0, 0, (int)unitAddr);
  ...
  public void genCallIndexed(int intfReg, int off, int parSize) {
    ...
    if (ctx.codeStart!=0) ins(I_LEAregmem, REX|R_EAX, REX|R_EAX, ctx.codeStart);
    ins(I_CALLreg, REX|R_EAX);
  }
  ...
  public int genComp(int src1, int src2, int type, int cond) {
    ...
    if ((src1=getReg(1, src1, type, false))==0 || (src2=getReg(1, src2, type, false))==0) return 0;
  ...
  public int genCompValToConstVal(int src, int val, int type, int cond) {
    ...
    nextJumpsUnsigned=false;
    if ((src=getReg(1, src, StdTypes.T_INT, false))==0) return 0;
    if (val==0) {
  ...
  public int genCompValToConstDoubleOrLongVal(int srcR, long val, boolean asDouble, int cond) {
    ...
    nextJumpsUnsigned=false;
    if ((src=getReg(1, srcR, StdTypes.T_LONG, false))==0) return 0;
    if (val==0l) ins(I_ORregreg, src, src);
    else {
      tmpR=getFreeReg(srcR, StdTypes.T_LONG);
      if ((tmp=getReg(1, tmpR, StdTypes.T_LONG, false))==0) return 0;
  ...
  public void genLoadDerefAddr(int destReg, int objReg, int ind, int baseOffset, int entrySize) {
    ...
    if ((destReg=getReg(1, destReg, StdTypes.T_PTR, true))==0
        || (objReg=getReg(1, objReg, StdTypes.T_PTR, false))==0
        || (indReg=getReg(1, ind, StdTypes.T_INT, false))==0) return;
    if (entrySize<0) {
    ...
    switch (entrySize) {
      case 1: case 2: case 4: case 8:
        if (destReg!=objReg) ins(I_MOVregreg, destReg, objReg);
      ...
      default:
        ...
        if (destReg!=objReg) ins(I_MOVregreg, destReg, objReg);
        if (baseOffset!=0) ins(I_ADDregimm, destReg, 0, 0, baseOffset, 0l, 0);
  ...
  public void genMoveToPrimary(int srcR, int type) {
    ...
    switch (type) {
      case StdTypes.T_FLT:  case StdTypes.T_DBL:
        if (srcR!=curFPUReg) errorFPU();
        dupFPUIns=null;
        popFPUDone++; //do not destroy result
        return; //leave result on top of stack
      case StdTypes.T_LONG: case StdTypes.T_PTR:
        if ((reg=getReg(1, srcR, StdTypes.T_LONG, false))==0) return;
        if (reg!=(REX|R_EAX)) ins(I_MOVregreg, REX|R_EAX, reg);
        //else: nothing to do
        return;
      case StdTypes.T_DPTR:
        if ((reg=getReg(1, srcR, StdTypes.T_DPTR, false))==0) return;
        if (reg!=(REX|R_EAX)) ins(I_MOVregreg, REX|R_EAX, reg);
        if ((reg=getReg(2, srcR, StdTypes.T_DPTR, false))==0) return;
        if (reg!=(REX|R_EDX)) ins(I_MOVregreg, REX|R_EDX, reg);
        return;
    }
    //default
    if ((reg=getReg(1, srcR, StdTypes.T_INT, false))==0) return;
    if (reg!=R_EAX) ins(I_MOVregreg, R_EAX, reg);
    //else: nothing to do
  }
  ...
  public void genMoveFromPrimary(int dstR, int type) {
    ...
    switch (type) {
      case StdTypes.T_FLT:  case StdTypes.T_DBL:
        if (dstR!=curFPUReg) errorFPU();
        return; //result already on top of stack
      case StdTypes.T_LONG: case StdTypes.T_PTR:
        if ((reg=getReg(1, dstR, StdTypes.T_LONG, true))==0) return;
        if (reg!=(REX|R_EAX)) ins(I_MOVregreg, reg, REX|R_EAX);
        //else: nothin to do
        return;
      case StdTypes.T_DPTR:
        if ((reg=getReg(2, dstR, StdTypes.T_DPTR, true))==0) return;
        if (reg!=(REX|R_EDX)) ins(I_MOVregreg, REX|R_EDX, reg);
        if ((reg=getReg(1, dstR, StdTypes.T_DPTR, true))==0) return;
        if (reg!=(REX|R_EAX)) ins(I_MOVregreg, REX|R_EAX, reg);
        return;
    }
    //default
    if ((reg=getReg(1, dstR, StdTypes.T_INT, true))==0) return;
  ...
  public void genSavePrimary(int type) {
    ...
    switch (type) {
      case StdTypes.T_PTR: case StdTypes.T_LONG:
        ins(I_PUSHreg, REX|R_EAX);
        return;
      case StdTypes.T_DPTR:
        ins(I_PUSHreg, REX|R_EAX);
  ...
  public void genCallNative(Object loc, int off, boolean relative, int parSize, boolean noCleanUp) { //valid only for linux 64 bit
    long destAddr;
    if (relative) ins(I_MOVregmem, REX|R_EBX, REX|R_EDI, off);
    if (parSize>0) {
    ...
    if (relative) ins(I_CALLreg, REX|R_EBX);
    else {
      if (((destAddr=mem.getAddrAsLong(loc, off))&0xFFFFFFFF00000000l)==0l) ins(I_CALLmem, 0, 0, (int)destAddr);
      else {
        ins(I_MOVregimmL, REX|R_EBX, 0, 0, 0, destAddr, 0);
        ins(I_CALLreg, REX|R_EBX);
      }
    }
    if (!noCleanUp && parSize>0) ins(I_ADDregimm, REX|R_ESP, 0, 0, parSize);
  }
  
  public void genReserveNativeStack(int size) {
    if (size>0) ins(I_SUBregimm, REX|R_ESP, 0, 0, size);
  }
  ...
  public void genStoreNativeParameter(int offset, int src, int type) {
    ...
    if ((reg=getReg(1, src, type, false))==0) return;
    ins(I_MOVmemreg, REX|R_ESP, reg, offset);
    if (type==StdTypes.T_DPTR) {
      if ((reg=getReg(2, src, StdTypes.T_DPTR, false))==0) return;
  ...
  protected String getRegName(int reg) {
    switch (reg) {
      ...
      case REX|R_EBP: return "RBP";
      case REX|R_EBX: return "RBX";
      case REX|R_ECX: return "RCX";
      case REX|R_EDI: return "RDI";
      case REX|R_EDX: return "RDX";
      case REX|R_ESI: return "RSI";
      case REX|R_ESP: return "RSP";
      case R_8B: return "R8B";
      case R_8W: return "R8W";
      case R_8D: return "R8D";
      case REX|R_8D: return "R8";
      case R_9B: return "R9B";
      case R_9W: return "R9W";
      case R_9D: return "R9D";
      case REX|R_9D: return "R9";
      case R_10B: return "R10B";
      case R_10W: return "R10W";
      case R_10D: return "R10D";
      case REX|R_10D: return "R10";
      case R_11B: return "R11B";
      case R_11W: return "R11W";
      case R_11D: return "R11D";
      case REX|R_11D: return "R11";
      case R_12B: return "R12B";
      case R_12W: return "R12W";
      case R_12D: return "R12D";
      case REX|R_12D: return "R12";
      case R_13B: return "R13B";
      case R_13W: return "R13W";
      case R_13D: return "R13D";
      case REX|R_13D: return "R13";
      case R_14B: return "R14B";
      case R_14W: return "R14W";
      case R_14D: return "R14D";
      case REX|R_14D: return "R14";
      case R_15B: return "R15B";
      case R_15W: return "R15W";
      case R_15D: return "R15D";
      case REX|R_15D: return "R15";
    }
  ...
  protected Instruction ins(int type, int reg0, int reg1, int disp, int imm, long immL, int par) {
    ...
    switch (type) {
      ...
      case I_MOVSXregreg:
        ...
        i.putByte(0x0F);
        if ((reg1&0x0F)==RS_X) i.putByte(0xBF); //from AX, BX, ...
        else i.putByte(0xBE); //from AL, BL, ...
        putRegReg(i, reg0, reg1);
        break;
      case I_MOVSXregmem:
        putPrefix(i, sizeprefix, rex0, nrg0, nrg1); //also handles "to 16 bit instead of 32"
        i.putByte(0x0F);
        if (par==2) i.putByte(0xBF); //from 16 bit source
        else i.putByte(0xBE); //from 8 bit source
        putMem(i, reg0, reg1, disp);
        break;
      case I_MOVZXregreg:
        putPrefix(i, sizeprefix, rex0, nrg0, nrg1); //also handles "to 16 bit instead of 32"
        i.putByte(0x0F);
        if ((reg1&0x0F)==RS_X) i.putByte(0xB7); //from AX, BX, ...
        else i.putByte(0xB6); //from AL, BL, ...
        putRegReg(i, reg0, reg1);
        break;
      case I_MOVZXregmem:
        putPrefix(i, sizeprefix, rex0, nrg0, nrg1); //also handles "to 16 bit instead of 32"
        i.putByte(0x0F);
        if (par==2) i.putByte(0xB7); //from 16 bit source
        else i.putByte(0xB6); //from 8 bit source
        putMem(i, reg0, reg1, disp);
        break;
      case I_LEAregmem:
        if (!rex1) i.putByte(0x67);
        putPrefix(i, sizeprefix, rex0, nrg0, nrg1);
        i.putByte(0x8D);
        putMem(i, reg0, reg1, disp);
        break;
      case I_PUSHreg:
        ...
        break;
      case I_PUSHimm:
        if (imm>=-128 && imm<128) { //8 bit immediate
          i.putByte(0x6A);
          i.putByte(imm);
        }
      ...
      case I_INCmem:
        ...
        if (par>1) {
          wordflag=1;
          if (par==8) prefix=0x48;
          else if (par==2) i.putByte(0x66);
        }
        else wordflag=0;
        if (nrg0) prefix|=0x41;
        if (prefix!=0) i.putByte(prefix);
  ...
  private void codeAriRegImm(Instruction i, int code, boolean sizeprefix, int wordflag,
      int reg0, boolean rex0, boolean nrg0, int imm) {
    int sizeExt=0;
    
    putPrefix(i, sizeprefix, rex0, false, nrg0);
    if (wordflag!=0 && imm>=-128 && imm<128) sizeExt=2;
  ...
  private void codeShiftRegImm(Instruction i, int code, boolean sizeprefix, int wordflag,
      int reg0, boolean rex0, boolean nrg0, int imm) {
  ...
  private void putMem(Instruction i, int reg0, int reg1, int disp) {
    ...
    if (reg1==0) { //absolute
      ...
      if (reg1ex==R_ESP) { //2-byte-address-encoding for esp
        putRegMemInx(i, reg0, reg1, R_ESP, 1, disp);
      }
      else if (disp==0 && reg1ex!=R_EBP) { //no displacement
  ...
  private void putImm(Instruction i, int reg0, int imm) {
    switch (reg0&0xF) {
      case 0x1: case 0x2: i.putByte(imm); break;
      case 0x3:           i.putShort(imm); break;
      case 0x7:           i.putInt(imm); break;
      default:
  ...
  private void putRegMemInx(Instruction i, int reg0, int reg1, int reg2, int scale, int disp) {
    ...
    switch (scale) {
      case 0: case 1: break; //ok, scaleBits==0
      case 2: scaleBits=0x40; break; //ok, scaleBits for *2
      case 4: scaleBits=0x80; break; //ok, scaleBits for *4
      case 8: scaleBits=0xC0; break; //ok, scaleBits for *8
      default:
