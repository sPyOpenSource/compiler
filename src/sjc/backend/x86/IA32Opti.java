...
public class IA32Opti extends IA32 {
  ...
  public void insPatchedCall(Mthd refMthd, int par) {
    ...
    i.reg2=writeReg(R_EAX)|writeReg(R_EDX)|writeReg(R_EBX)|writeReg(R_ECX)
      |readReg(R_ESI)|readReg(R_EDI)|readReg(R_ESP)|readReg(R_EBP);
    i.iPar3=par;
  }
  
  private int readReg(int r) {
    if (r==0) return 0;
    if ((r&RS_E)==RS_H) r-=0x40;
  ...
  private void set(Instruction i, int type, int reg0, int reg1, int disp, int imm, int par) {
    ...
    switch (i.type=type) {
      ...
      case I_CALLmem:
        i.reg2=writeReg(R_EAX)|writeReg(R_EDX)|writeReg(R_EBX)|writeReg(R_ECX)
          |readReg(reg0)|readReg(R_ESI)|readReg(R_EDI)|readReg(R_ESP)|readReg(R_EBP);
        break;
      case I_PUSHreg:
      ...
      case I_MOVindexed: //will result in mov reg0,[reg0+reg1] (only for pointers!)
        ...
        break;
      case I_XORregreg:
        if (reg0==reg1) i.reg2=writeReg(reg0);
        else i.reg2=writeReg(reg0)|readReg(reg0)|readReg(reg1);
        break;
      case I_ADDregimm:
      case I_SUBregimm:
      case I_SBBregimm:
  ...
  private void optimize(Instruction first) {
    ...
    while (redo) {
      ...
      now=first;
      while (now!=null) {
        if (now.type==I_JUMP && isUntilNextRealIns(now.next, now.jDest)) { //do not jump to next instruction
          kill(now);
          redo=true;
        }
        now.isDest=false;
      ...
      now=first;
      while (now!=null) {
        if (now.type==I_JUMP) now.jDest.isDest=true;
        now=now.next;
      }
      ...
      while (now!=null) {
        if (now.type!=I_NONE) {
          ...
          else if (now.type==I_POPreg && (tmp=searchCorrespPush(now))!=null
              && tryReplacePushPop(tmp, now)) redo=true;
        }
        now=now.next;
      }
        ...
        switch (now.type) {
          case I_MOVregimm:
            ...
            switch (tmp.type) {
              case I_MOVmemreg:
                ...
                break;
              case I_MOVregreg: case I_ADDregreg: case I_SUBregreg:
              case I_ANDregreg: case I_XORregreg: case I_ORregreg:
                if (tmp.reg1!=now.reg0) break;
                set(tmp, (tmp.type&~IM_P1)|I_imm1, tmp.reg0, 0, 0, now.iPar2, 0);
                kill(now);
                break;
              case I_SHLregreg: case I_SHRregreg: case I_SARregreg:
                if (now.reg0!=R_ECX) break;
                set(tmp, (tmp.type&~IM_P1)|I_imm1, tmp.reg0, 0, 0, now.iPar2, 0);
                kill(now);
                break;
              case I_PUSHreg:
            ...
            break;
          case I_MOVregmem:
            if ((now.reg0&RS_E)==RS_E && searchCorrespReadAfter(now, now.reg1, false)==null
                && (tmp=searchCorrespImmWriteBefore(now, now.reg1))!=null && tmp.type==I_MOVregreg) {
          ...
          case I_MOVregreg:
            if ((now.reg0&RS_E)==RS_E) {
              ...
              else if ((tmp=searchCorrespImmWriteBefore(now, now.reg1))!=null) switch (tmp.type) {
                case I_MOVregreg: case I_MOVregmem: case I_MOVregimm:
                  //tmp is mov reg,*
                  if ((tmp2=searchCorrespReadAfter(now, now.reg1, true))==null) {
                  ...
                  else if (!tmp2.isDest && tmp2.type!=I_JUMP && tmp2.type!=I_MAGC
                      && !isRegisterFixed(tmp2) && searchCorrespReadAfter(tmp2, now.reg1, true)==null
                      && searchCorrespWriteBefore(tmp2, now.reg0)==now) {
                    ...
                    if (tmp2.reg0==tmp.reg0) tmp2.reg0=now.reg0;
                    if (tmp2.reg1==tmp.reg0) tmp2.reg1=now.reg0;
                    set(tmp, tmp.type, now.reg0, tmp.reg1, tmp.iPar1, tmp.iPar2, tmp.iPar3);
            ...
            break;
          case I_MOVSXregreg: case I_MOVZXregreg:
            if ((tmp=searchCorrespReadAfter(now, now.reg0, false))==null
                || tmp.type!=I_MOVmemreg || hasReadAfter(tmp, now.reg0)
                || (tmp.reg1&RS_E)!=(now.reg1&RS_E)) break;
            ...
            break;
          case I_MOVSXregmem:
            if ((tmp=searchCorrespReadAfter(now, now.reg0, false))==null) break;
            switch (tmp.type) {
              case I_MOVmemreg:
              ...
              case I_MOVSXregreg:
                if ((tmp.reg0&~RS_E)==(tmp.reg1&~RS_E)) {
                  if (tmp.iPar3==1) set(now, I_MOVSXregmem, now.reg0, now.reg1, now.iPar1, 0, 1);
                  kill(tmp);
                  redo=true;
                }
                break;
            ...
            break;
          case I_PUSHreg:
            if ((now.reg0&RS_E)!=RS_E || (tmp=searchCorrespImmWriteBefore(now, now.reg0))==null
                || hasReadAfter(now, now.reg0)) break;
            switch (tmp.type) {
              case I_POPreg:
                kill(now);
                kill(tmp);
          ...
          case I_PUSHmem:
            ...
            break;
          case I_INCmem: case I_DECmem:
            if ((tmp=searchCorrespImmWriteBefore(now, now.reg0))==null || tmp.type!=I_LEAregmem
                || hasReadAfter(now, now.reg0)) break;
            set(now, now.type, tmp.reg1, 0, tmp.iPar1, 0, now.iPar3);
            kill(tmp);
            redo=true;
            break;
          case I_ADDregimm: case I_SUBregimm: case I_XORregimm: case I_ORregimm:
          case I_SARregimm: case I_SHRregimm: case I_SHLregimm:
            if (tryMergeAriImm(now)) redo=true;
            break;
          case I_ADDregreg: case I_SUBregreg: case I_XORregreg: case I_ANDregreg:
            if (tryMergeAriReg(now)) redo=true;
            break;
          case I_ANDregimm:
            if (tryMergeAriImm(now)) redo=true;
            else {
              if ((flag=now.iPar2!=0xFF) && now.iPar2!=0xFFFF) break;
              if ((tmp=searchCorrespImmWriteBefore(now, now.reg0))==null) break;
              if (tmp.type==I_MOVSXregmem) { //need closer look
                if (flag && tmp.iPar2==1) break; //break if source is 8 bit and dest 16 bit
              }
              else if (tmp.type!=I_MOVregmem) break;
              if (flag) set(now, I_MOVZXregmem, now.reg0, tmp.reg1, tmp.iPar1, 0, 2); //16 bit 
              else set(now, I_MOVZXregmem, now.reg0, tmp.reg1, tmp.iPar1, 0, 1); //8 bit
              kill(tmp);
              redo=true;
            }
            break;
          case I_ORregreg:
            if (now.reg0!=now.reg1) {
              ...
              if ((tmp=searchCorrespImmWriteBefore(now, now.reg0))!=null) switch (tmp.type&IM_OP) {
                case I_AND: case I_OR: case I_XOR: case I_ADD: case I_SUB:
                  //flag setting already done, just kill opcode before
                    kill(now);
                    redo=true;
                  break;
                default:
                  if (now.next.type!=I_JUMP || hasReadAfter(now.next, now.reg0)) break;
                  switch (tmp.type) {
                    case I_MOVregmem:
          ...
          case I_LEAregmem:
            if ((tmp=searchCorrespReadAfter(now, now.reg0, false))==null ||
                mayBeWritten(now, tmp, now.reg1)!=null || 
                hasReadAfter(tmp, now.reg0)) break;
            switch (tmp.type) {
              case I_MOVmemreg:
              ...
              case I_MOVregmem:
                ...
                break;
              case I_MOVmemimm: case I_ADDmemimm: case I_SUBmemimm:
              case I_ANDmemimm: case I_XORmemimm: case I_ORmemimm:
              ...
              case I_LEAregmem:
                ...
                break;
              case I_FLDmem: case I_FILDmem: case I_FSTPmem: case I_FISTTPmem:
            ...
            break;
          case I_FILDmem:
            if (now.reg0==R_ESP && now.iPar1==0 && now.iPar3==FPU32 && (tmp=now.prev)!=null
                && now.prev.type==I_PUSHmem && now.next!=null && now.next.type==I_POPreg) {
            ...
            break;
          case I_FXCH:
            if (now.prev!=null && now.prev.type==I_FLDmem && (tmp=now.prev.prev)!=null
                && tmp.type==I_FLDmem) {
  ...
  private boolean tryMergeAriReg(Instruction now) {
    ...
    if ((tmp=searchCorrespImmWriteBefore(now, now.reg1))==null
        || tmp.type!=I_MOVregmem || hasReadAfter(now, now.reg1))
      return false;
  ...
  private boolean tryMergeAriImm(Instruction now) {
    ...
    if ((tmp1=searchCorrespImmWriteBefore(now, now.reg0))==null || tmp1.type!=I_MOVregmem
        || (tmp2=searchCorrespReadAfter(now, now.reg0, false))==null) return false;
    flag=hasReadAfter(tmp2, now.reg0);
    if (tmp2.type!=I_MOVmemreg || tmp1.reg0!=tmp2.reg1 || tmp1.reg0!=now.reg0
        || tmp1.reg1!=tmp2.reg0 || tmp1.iPar1!=tmp2.iPar1) return false;
    set(now, destType, tmp1.reg1, 0, tmp1.iPar1, now.iPar2, 4);
    if (flag) set(tmp2, tmp1.type, tmp1.reg0, tmp1.reg1, tmp1.iPar1, tmp1.iPar2, tmp1.iPar3);
    else kill(tmp2);
    kill(tmp1);
    return true;
  }
  ...
  private Instruction searchCorrespPush(Instruction now) {
    ...
    while (now!=null) {
      if (now.isDest || now.type<0) return null;
      switch (now.type) {
        case I_CALLimm: case I_CALLreg: case I_CALLmem:
          popCnt+=now.iPar3>>>2;
          break;
        case I_PUSHreg: case I_PUSHimm: case I_PUSHmem: case I_PUSHip:
          if (--popCnt<=0) {
            if (popCnt==0) return now;
            return null;
          }
          break;
        case I_POPreg:
          ...
          break;
        default:
          if (isRead(now, R_ESP) || isWrite(now, R_ESP)) return null;
      }
      now=now.prev;
    }
    return null;
  ...
  private boolean tryReplacePushPop(Instruction push, Instruction pop) {
    ...
    switch (push.type) {
      case I_PUSHreg:
        if (push.reg0!=pop.reg0) break;
        if ((writer=mayBeWritten(push, pop, pop.reg0))==null) { //no writer between push and pop
          kill(pop);
          kill(push);
          return true;
        }
        switch (writer.type) { //try to replace
          case I_CALLreg: case I_CALLmem: case I_CALLimm:
            if (hasReadAfter(writer, pop.reg0)
                || (writer=searchCorrespWriteBefore(push, push.reg0))==null
                || writer.type!=I_LEAregmem || writer.reg0!=push.reg0
                || (writer.reg1!=R_EBP && writer.reg1!=R_ESI
                    && writer.reg1!=R_EDI))
              break;
            kill(push);
  ...
  private Instruction searchCorrespImmWriteBefore(Instruction i, int reg) {
    if (i.isDest || i.type<0) return null;
    i=i.prev;
    while (i!=null) {
      if (i.isDest || i.type<0) return null;
      switch (i.type) {
        case I_NONE: break;
        case I_CALLimm: case I_CALLreg: case I_CALLmem: return null;
        default:
          if (isWrite(i, reg)) return i;
          return null;
      }
      i=i.prev;
    }
  ...
  private Instruction searchCorrespReadAfter(Instruction i, int reg, boolean objOnBreaker) {
    i=i.next;
    while (i!=null) {
      if (i.isDest) return objOnBreaker ? i : null;
      switch (i.type) {
        case I_NONE: break;
  ...
  private boolean hasReadAfterRecursive(Instruction i, int reg) {
    if (++hasReadAfterCalls>100) return true; //limit too deep analysis
    i=i.next;
    while (i!=null) {
      switch (i.type) {
        case I_JUMP:
          //don't follow backward jumps, on conditional jumps also check following instructions
          if (i.jDest.instrNr<=i.instrNr || (i.iPar1!=0 && hasReadAfterRecursive(i, reg))) return true;
          //follow the jump
          i=i.jDest;
          break;
        case I_MAGC:
        ...
        default:
          //check read/write
          if (isRead(i, reg)) return true;
          if (isWrite(i, reg)) return false;
  ...
  private boolean effectlessWrite(Instruction now) {
    ...
    regMask=now.reg2>>>16;
    if (now.isDest || now.type<0 || regMask==0) return false;
    switch (now.type) {
      case I_CALLimm: case I_CALLreg: case I_CALLmem:
      case I_RETimm: case I_IRET:
        return false;
    }
    now=now.next;
    while (now!=null) {
      if ((now.reg2&regMask)!=0 || now.isDest || now.type<0) return false;
      switch (now.type) {
        case I_CALLimm: case I_CALLreg: case I_CALLmem:
          return true;
        case I_RETimm: case I_IRET:
  ...
  private Instruction mayBeWritten(Instruction now, Instruction end, int reg) {
    if (now.isDest || now.type<0) return now;
    switch (now.type) {
      case I_RETimm: case I_IRET:
        return now;
    }
    now=now.next;
    while (now!=end) {
      if (isWrite(now, reg) || now.isDest || now.type<0) return now;
      switch (now.type) {
        case I_RETimm: case I_IRET:
          return now;
      }
