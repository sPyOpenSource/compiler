...
public abstract class X86Base extends Architecture{
  ...
  //common methods
  protected Instruction ins(int type) { return ins(type, 0, 0, 0, 0); }
  protected Instruction ins(int type, int reg0) { return ins(type, reg0, 0, 0, 0); }
  protected Instruction ins(int type, int reg0, int reg1) { return ins(type, reg0, reg1, 0, 0); }
  protected Instruction ins(int type, int reg0, int reg1, int disp) { return ins(type, reg0, reg1, disp, 0); }
  ...
  public int prepareFreeReg(int avoidReg1, int avoidReg2, int reUseReg, int type) {
    ...
    if (type==StdTypes.T_FLT || type==StdTypes.T_DBL) {
      if (reUseReg<0) { //only fpu-registers
        if (reUseReg!=curFPUReg) errorFPU();
        else nextAllocReg=curFPUReg;
      }
      else nextAllocReg=(curFPUReg+=FPUREGINC);
      return 0;
    }
    //mask out fpu- and special registers
    reUseReg&=rAll;
    if (avoidReg1<0) avoidReg1=0;
    if (avoidReg2<0) avoidReg2=0;
    if (reUseReg<0) reUseReg=0;
  ...
  public void deallocRestoreReg(int deallocRegs, int keepRegs, int restore) {
    if (deallocRegs<0) { //FPU-register
      if (deallocRegs!=keepRegs) {
        if (deallocRegs!=curFPUReg) errorFPU();
  ...
  protected int getReg(int nr, int reg, int type, boolean firstWrite) {
    ...
    if (nr==1) switch (reg) {
      case RegClss: return rClss;
      case RegInst: return rInst;
      case RegBase: return rBase;
    }
  ...
  protected int bitSearch(int value, int hit, int prefere) {
    ...
    prefere&=value;
    if (prefere!=0 && prefere!=value) for (i=0; i<32; i++) { //search here only if there is a difference
      j=1<<i;
      if ((prefere&j)!=0) {
        if (--hit==0) return j;
      }
    ...
    switch (type) {
      case StdTypes.T_BOOL:
        if (opPar==Ops.L_NOT) { //do not use I_NOT, would change 0x01 to 0xFE instead of 0x00
          if ((dst=getReg(1, dstR, StdTypes.T_BYTE, false))==0) return;
      ...
      case StdTypes.T_INT:
        if ((dst=getReg(1, dstR, type, false))==0) return;
        switch (opPar) {
          case Ops.A_CPL:   ins(I_NOTreg, dst); return;
          case Ops.A_MINUS: ins(I_NEGreg, dst); return;
          case Ops.A_PLUS:  /*nothing to do*/ return;
        }
        fatalError(ERR_UNSOP_GENUNAOP);
        return;
  ...
  public void genWriteIO(int addrR, int valR, int type, int memLoc) {
    ...
    if ((type!=StdTypes.T_BYTE && type!=StdTypes.T_SHRT && type!=StdTypes.T_CHAR
        && type!=StdTypes.T_INT) || memLoc!=0) {
      curMthd.printPos(ctx, ERR_UNSTYPE_GENWRITEIO);
      fatalError(null);
      return;
    }
    if ((addr=getReg(1, addrR, StdTypes.T_SHRT, false))==0 || (val=getReg(1, valR, type, false))==0) return;
    ...
    if (addrR==RegD) {
      ...
      else { //dstR!=RegD && srcR!=RegA
        if (addrR==RegA) { //dstR==RegA
          if (valR==RegD) { //dstR==RegA && srcR==RegD
            ins(I_XCHGregreg, R_EAX, R_EDX);
          }
          else { //dstR==RegA && src!=RegD
            restore1=storeReg(RegD);
            ins(I_MOVregreg, R_DX, addr);
            ins(I_MOVregreg, typedRegA, val);
          }
  ...
  public void genReadIO(int valR, int addrR, int type, int memLoc) {
    ...
    if ((addr=getReg(1, addrR, StdTypes.T_SHRT, false))==0 || (val=getReg(1, valR, StdTypes.T_INT, true))==0) return;
    typedRegA=getReg(1, RegA, type, true);
    if (addrR==RegD) {
      if (valR!=RegA) restore1=storeReg(RegA);
      //else: nothing to prepare
    }
    else { //dstR!=RegD
      if (valR==RegA) {
    ...
    else if (valR!=RegA) ins(I_MOVregreg, val, R_EAX);
    restoreReg(restore2);
    restoreReg(restore1);
  }
  ...
  public void insJump(Instruction dest, int cond) {
    ...
    me.type=I_JUMP;
    if (nextJumpsUnsigned) switch (cond) {
      case Ops.C_LW: cond=Ops.C_BO; break;
      case Ops.C_LE: cond=SC_BE; break;
      case Ops.C_GE: cond=SC_AE; break;
      case Ops.C_GT: cond=SC_AB; break;
    }
    me.iPar1=cond;
    me.jDest=dest;
    dest.isDest=true;
  ...
  protected boolean fixJump(Instruction me) { //returns true if size of instruction has changed -> redo for already done jumps
    ...
    if (me.instrNr>=dest.instrNr) { //get destination before us
      ...
      if (isUntilNextRealIns(dummy=me.next, dest)) { //do not jump to next instruction
        me.type=I_NONE;
        me.size=0;
        return true;
      }
    ...
    if (me.iPar1==0) { //jump unconditionally
      if (relative>=-128 && relative<=127) { //always a short-jump
        me.code[0]=(byte)0xEB;
        me.code[1]=(byte)relative;
        if (me.size==2) return false;
        me.size=2;
        return true;
      }
      if (relative>=-131 && relative<=-129 && me.size==5) { //re-encoding far jump as short jump allows short jump
      ...
      switch (me.iPar1) { //contains condition
        ...
        case SC_AB: bbbb=0x7; break; //jump if above
        case SC_US: bbbb=0x9; break; //jump if unsigned
        case Ops.C_LW: bbbb=0xC; break; //jump if less
        case Ops.C_LE: bbbb=0xE; break; //jump if less or equal
        case Ops.C_EQ: bbbb=0x4; break; //jump if equal
        case Ops.C_GE: bbbb=0xD; break; //jump if greater or equal (==not less)
        case Ops.C_GT: bbbb=0xF; break; //jump if greater (==not less or equal)
        case Ops.C_NE: bbbb=0x5; break; //jump if not equal
        case Ops.C_BO: bbbb=0x2; break; //jump if carry (==below ==not above or equal)
        default:
    ...
    for (i=1; i<fullIPChangeBytes; i++) me.code[pos++]=(byte)(relative=relative>>>8);
    return false;
  }
  ...
  protected boolean fixAdd(Instruction me) {
    ...
    from=me.jDest;
    to=me.next.jDest;
    if (from.instrNr>=to.instrNr) { //invalid: to before from
      fatalError(ERR_JUMPDESTNOTFOUND_FIXJUMP);
      return false;
    }
    ...
    if (patchedAddPrefix!=0) me.putByte(patchedAddPrefix);
    if (relative<=127) byteAdd=2; //used as bit and flag
    me.putByte(0x81+byteAdd);
    if (me.iPar1==0) { //add to (e/r)bx
      me.putByte(0xC3);
    }
    else { //add to [(e/r)bp]
      me.putByte(0x45);
    ...
    me.putByte(relative);
    if (byteAdd==0) for (i=1; i<fullIPChangeBytes; i++) me.putByte(relative=relative>>>8);
  ...
  protected void printMem(TextPrinter v, int reg, int disp, boolean asmMode) {
    v.print('[');
    if (asmMode && reg==0 && disp>=ctx.mem.getBaseAddress()
        && disp<ctx.mem.getBaseAddress()+ctx.getMaximumImageSize()) {
      v.print("$_A_");
  ...
  protected void printSize(TextPrinter v, int par) {
    switch (par) {
      case 1: v.print("byte "); break;
      case 2: v.print("word "); break;
      case 4: v.print("dword "); break;
      case 8: v.print("qword "); break;
      default: v.print("invSz ");
    }
  }
    ...
    switch (i.type) {
      case I_MAGC:
        ...
        for (cnt=0; cnt<i.size; cnt++) {
          if (cnt>0) v.print(',');
          v.print(" 0x");
          v.printHexFix((int)i.code[cnt], 2);
        }
        return cnt;
      case I_JUMP:
        switch (i.iPar1) { //contains condition
          case 0: v.print("JMP"); break; //jump unconditionally
          case SC_BE: v.print("JBE"); break; //jump if below or equal
          case SC_AE: v.print("JAE"); break; //jump if above or equal
          case SC_AB: v.print("JA"); break; //jump if above
          case SC_US: v.print("JNS"); break; //jump if unsigned
          case Ops.C_LW: v.print("JL"); break; //jump if less
          case Ops.C_LE: v.print("JLE"); break; //jump if less or equal
          case Ops.C_EQ: v.print("JE"); break; //jump if equal
          case Ops.C_GE: v.print("JGE"); break; //jump if greater or equal (==not less)
          case Ops.C_GT: v.print("JG"); break; //jump if greater (==not less or equal)
          case Ops.C_NE: v.print("JNE"); break; //jump if not equal
          case Ops.C_BO: v.print("JB"); break; //jump if carry (==below ==not above or equal)
          default: v.println(ERR_CONDJUMP_FIXJUMP);
        }
        if (asmMode) {
          v.print(' ');
          if (i.size<1+relocBytes) v.print("short ");
      ...
      case I_CALLimm:
        v.print("CALL ");
        if (asmMode) ctx.printUniqueMethodName(v, i.refMthd);
        else {
      ...
      case I_LEAarray: //will result in lea reg0,[reg0+reg1*par+disp] (only for pointers!)
        v.print("LEA "); printReg(v, i.reg0); 
        v.print(",["); printReg(v, i.reg0);
        v.print('+'); 
      ...
      case I_MOVindexed: //will result in mov reg0,[reg0+reg1] (only for pointers!)
        v.print("MOV "); printReg(v, i.reg0);
        v.print(",["); printReg(v, i.reg0);
        v.print('+'); printReg(v, i.reg1);
        v.print(']');
        break;
      case I_INreg:
        v.print("IN ");
      ...
      case I_MARKER:
        v.print("NOP");
        if (asmMode) v.print(" ;marker");
        break;
      case I_STEXreg:
        v.print("STEX");
        break;
      case I_POPdummy:
        if (asmMode) v.print("POP ECX ;pop dummy"); //POPdummy is used only in IA32 and equals "POP ECX"
        else v.print("POPdummy");
        break;
      case I_ADDpatched:
        v.print("ADD ");
        if (i.iPar1==0) printReg(v, getReg(1, RegB, StdTypes.T_PTR, false)); //add to (e/r)bx
        else {
      ...
      default:
        ...
        switch (i.type&IM_OP) {
          case I_MOV:
            v.print("MOV ");
            if ((i.type&IM_P0)==I_mem0 && (i.type&IM_P1)==I_imm1) printMemSize=true;
            break;
          case I_MOVSX:
            v.print("MOVSX ");
            if ((i.type&IM_P0)==I_mem0 || (i.type&IM_P1)==I_mem1) printMemSize=true;
            break;
          case I_MOVZX:
            v.print("MOVZX ");
            if ((i.type&IM_P0)==I_mem0 || (i.type&IM_P1)==I_mem1) printMemSize=true;
        ...
        switch (i.type&IM_P0) {
          case I_reg0:
            ...
            break;
          case I_mem0:
            if (printMemSize) printSize(v, i.iPar3);
          ...
          case I_imm0:
            ...
            v.print("0x"); v.printHexFix(tmp, fullIPChangeBytes<<1);
            break;
          case I_iml0:
            v.print("0x");
