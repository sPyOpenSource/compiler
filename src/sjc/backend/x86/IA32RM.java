...
public class IA32RM extends X86Base {
  ...
  public Mthd prepareMethodCoding(Mthd mthd) {
    ...
    else mthdContainer=mthd; //remember outest level method
    usedRegs=0;
    curMthd=mthd;
    return lastMthd;
  }
  ...
  public void codeProlog() {
    ...
    if ((curMthd.marker&Marks.K_INTR)==0) {
      ...
      if (curMthd.varSize==0 && curMthd.parSize==0) return; //no intr, no parameters, no local vars -> no ebp
    }
    else {
      if (curInlineLevel>0) {
    ...
    switch (curMthd.varSize) {
      case 0: break;
      case 2: ins(I_PUSHimm); break;
      default:
        ins(I_XORregreg, R_BX, R_BX);
        for (i=curMthd.varSize; i>0; i-=2) ins(I_PUSHreg, R_BX);
    }
  }
  ...
  public void genLoadConstVal(int dst, int val, int type) {
    ...
    if ((dst=getReg(1, dst, type, true))==0) return;
    if (val==0) ins(I_XORregreg, dst, dst);
    else ins(I_MOVregimm, dst, 0, 0, val);
  }
  ...
  public void genLoadConstDoubleOrLongVal(int dst, long val, boolean asDouble) {
    ...
    if ((reg1=getReg(1, dst, StdTypes.T_LONG, true))==0 || (reg2=getReg(2, dst, StdTypes.T_LONG, true))==0) return;
  ...
  public void genLoadVarVal(int dstR, int src, Object loc, int off, int type) {
    ...
    if ((dst=getReg(1, dstR, type, true))==0) return;
    if (src==regBase && pos>=0) pos+=curVarOffParam;
    ...
    switch (type) {
      ...
      case StdTypes.T_INT:
        ...
        return;
      case StdTypes.T_DPTR:
        if ((dstL=getReg(2, dstR, StdTypes.T_DPTR, true))==0) return;
        if ((dst|RS_E)!=(src|RS_E)) { //first move dst, then dstL
          ins(I_MOVregmem, dst, src, pos);
        ...
        return;
      case StdTypes.T_LONG:
        if ((dstL=getReg(2, dstR, StdTypes.T_LONG, true))==0) return;
        if ((dst|RS_E)!=(src|RS_E)) { //first move dst, then dstL
          ins(I_MOVregmem, dst, src, pos);
  ...
  public void genConvertVal(int dstR, int srcR, int toType, int fromType) {
    ...
    if ((dst=getReg(1, dstR, toType, true))==0 || (src=getReg(1, srcR, fromType, false))==0) return;
    ...
    if (toType==StdTypes.T_PTR || fromType==StdTypes.T_PTR) {
      if (fromType!=StdTypes.T_INT && toType!=StdTypes.T_INT
          && fromType!=StdTypes.T_SHRT && toType!=StdTypes.T_SHRT) {
    ...
    if (toType==StdTypes.T_LONG) { //to long is always the same
      ...
      if ((src=getReg(1, srcR, fromType, false))==0) return;
      if (fromType!=StdTypes.T_INT) { //convert 8/16-bit to 32 bit
        ins(I_MOVSXregreg, dst, src);
        src|=RS_E; //use as E* register
      }
      else if (src!=dst) ins(I_MOVregreg, dst, src);
      if (dst==R_EAX && dstL==R_EDX) ins(I_CWDE);
      else {
    ...
    switch (fromType) {
      case StdTypes.T_BYTE:
        switch (toType) {
          case StdTypes.T_BYTE:
            ...
            //else: nothing to do
            return;
          case StdTypes.T_SHRT: case StdTypes.T_INT:
            ins(I_MOVSXregreg, dst, src);
            return;
      ...
      case StdTypes.T_SHRT: case StdTypes.T_CHAR:
        switch (toType) {
          case StdTypes.T_BYTE: case StdTypes.T_SHRT: case StdTypes.T_CHAR:
  ...
  public void genDup(int dstR, int srcR, int type) {
    ...
    switch (type) {
      ...
      case StdTypes.T_LONG: case StdTypes.T_DPTR:
        ins(I_MOVregreg, dst, src);
        if ((dst=getReg(2, dstR, type, true))==0 || (src=getReg(2, srcR, type, false))==0) return;
        ins(I_MOVregreg, dst, src);
        return;
    }
  ...
  public void genPush(int srcR, int type) {
    ...
    if (type==StdTypes.T_LONG || type==StdTypes.T_DPTR) {
      if ((src=getReg(2, srcR, type, false))==0) return;
      ins(I_PUSHreg, src);
    }
  ...
  public void genPop(int dstR, int type) {
    ...
    ins(I_POPreg, dst);
    if (type==StdTypes.T_LONG || type==StdTypes.T_DPTR) {
      if ((dst=getReg(2, dstR, type, true))==0) return;
      ins(I_POPreg, dst);
    }
  }
  ...
  public void genBinOp(int dstR, int src1R, int src2R, int op, int type) { //may destroy src2R
    ...
    boolean xchg;
    if ((dst=getReg(1, dstR, type, false))==0 || (src2=getReg(1, src2R, type, false))==0) return;
  ...
  public void genCall(int off, int clssReg, int parSize) {
    ...
    if (ctx.codeStart==0) ins(I_CALLmem, R_DI, 0, off);
    else {
  ...
  public void genCallIndexed(int intfReg, int off, int parSize) {
    ...
    if (ctx.codeStart!=0) ins(I_LEAregmem, R_BX, R_BX, ctx.codeStart);
    ins(I_CALLreg, R_BX);
  }
  ...
  public int genComp(int src1R, int src2R, int type, int cond) {
    ...
    if ((src1=getReg(1, src1R, type, false))==0 || (src2=getReg(1, src2R, type, false))==0) return 0;
    switch (type) {
      case StdTypes.T_BOOL: case StdTypes.T_BYTE:
      case StdTypes.T_SHRT: case StdTypes.T_CHAR:
      case StdTypes.T_INT:  case StdTypes.T_PTR:
  ...
  public int genCompValToConstVal(int src, int val, int type, int cond) { //type not of interest here
    if ((src=getReg(1, src, type, false))==0) return 0;
    if (val==0) {
  ...
  public void genLoadDerefAddr(int destReg, int objReg, int indReg, int baseOffset, int entrySize) {
    if ((destReg=getReg(1, destReg, StdTypes.T_PTR, true))==0
        ...
        || (indReg=getReg(1, indReg, StdTypes.T_INT, false))==0) return;
    if (entrySize<0) {
      ins(I_NEGreg, indReg);
      entrySize=-entrySize;
    }
  ...
  public void genMoveToPrimary(int srcR, int type) {
    int reg;
    if ((reg=getReg(1, srcR, type, false))==0) return;
    if (srcR!=RegA) ins(I_MOVregreg, getReg(1, RegA, type, false), reg);
    if (type==StdTypes.T_LONG || type==StdTypes.T_DPTR) {
      if ((reg=getReg(2, srcR, type, false))==0) return;
      if (srcR!=RegD) ins(I_MOVregreg, getReg(2, RegD, type, false), reg);
    }
  }
  ...
  public void genMoveFromPrimary(int dstR, int type) {
    int reg;
    if (type==StdTypes.T_LONG || type==StdTypes.T_DPTR) {
      if ((reg=getReg(2, dstR, type, true))==0) return;
      if (dstR!=RegD) ins(I_MOVregreg, reg, getReg(2, RegD, type, false));
    }
    if ((reg=getReg(1, dstR, type, true))==0) return;
    if (dstR!=RegA) ins(I_MOVregreg, reg, getReg(1, RegA, type, false));
  }
  ...
  private void insMem(int type, boolean sizeprefix, int wordflag,
      int reg, int mem, int disp, int imm, int par) {
    Instruction i;
    int popReg=0;
    ...
    switch (type) {
      ...
      case I_MOVmemimm:
        ...
        break;
      case I_MOVSXregmem:
        if ((reg&0x0F)==RS_E) i.putByte(0x66); //to 32 bit instead of 16
        i.putByte(0x0F);
    ...
    if (popReg!=0) ins(I_POPreg, popReg, 0, 0, 0, 0);
  }
  ...
  private Instruction ins(int type, int reg0, int reg1, int disp, int imm, int par) {
    ...
    switch (type) {
      ...
      case I_MOVSXregreg:
        tmp=reg0&0x0F;
        if (tmp==RS_E) i.putByte(0x66); //to 32 bit instead of 16
        i.putByte(0x0F);
      ...
      case I_PUSHimm:
        if (imm>=-128 && imm<128) { //8 bits immediate
          if (par==StdTypes.T_INT) i.putByte(0x66);
          i.putByte(0x6A);
          i.putByte(imm);
        }
        else if (par!=StdTypes.T_INT){ //16 bit immediate
          i.putByte(0x68);
          i.putShort(imm);
        }
  ...
  private void putMem(Instruction i, int reg0, int reg1, int disp) {
    ...
    if (reg1==0) { //absolute
      ...
      switch (reg1) {
        case R_SI: reg1code=0x4; break;
        case R_DI: reg1code=0x5; break;
        case R_BP: reg1code=0x6; break;
        case R_BX: reg1code=0x7; break;
        default:
          fatalError("IA32RM: invalid2 reg1 in putMem");
          return;
      }
      if (disp==0 && reg1!=R_BP) { //no displacement
