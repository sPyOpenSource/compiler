...
public abstract class Architecture {
  ...
  public boolean mayInline() { //may be overwritten if architecture does not support method inling
    if (curInlineLevel<maxInlineLevels) return true;
    if (ctx.verbose) {
      if (!printedInlineExceed) ctx.out.println("warning: exceeded inline level maximum");
  ...
  public void disposeMethodCode() { //needed only by multi-stage-optimizer
    Instruction ins=firstInstr.next, last;
  ...
  protected void addToCodeRefFixupList(Instruction i, int offset) {
    ...
    if (emptyList==null) dest=new InstrList();
    else {
      dest=emptyList;
  ...
  protected boolean isUntilNextRealIns(Instruction i, Instruction until) {
    while (i!=until) {
      if (i==null || i.type!=I_NONE) return false;
      i=i.next;
    }
    return true;
  }
