...
public class SSA extends Architecture {
  ...
  public int ensureFreeRegs(int ignoreReg1, int ignoreReg2, int keepReg1, int keepReg2) {
    ...
    while (reg!=null) {
      if (reg.saveStopper>0) break;
      if (reg.nr!=ignoreReg1 && reg.nr!=ignoreReg2) {
        sc=1;
        reg.saved++;
      }
      else {
        if (reg.saved!=0) fatalError("SSA: can not ignore already saved register");
        sc=0;
      }
    ...
    if (lSaved!=null) lSaved.saveStopper++;
    return cnt;
  }
  ...
  public void deallocRestoreReg(int deallocRegs, int keepRegs, int restore) {
    ...
    if (restore!=0) {
      cmp=lReg;
      //search saveStopper
      while (cmp!=null && cmp.saveStopper==0) cmp=cmp.prev;
      if (cmp==null) fatalError("SSA: invalid saveStopper (null)");
        ...
        if (restore!=0) fatalError("SSA: invalid saveStopper (restore)");
      }
    }
    ...
    if (keepRegs!=deallocRegs && deallocRegs!=0) {
      ...
      //free register
      if (lReg.nr!=deallocRegs) fatalError("SSA: not last register in dealloc");
      else if (lReg.saved>0 || lReg.saveStopper>0) fatalError("SSA: dealloc for saved/saveStopper register");
      else {
        cmp=lReg;
        lReg=lReg.prev;
        if (lReg!=null) lReg.next=null;
        else fReg=null; //no last register => no first register
        cmp.next=eReg;
        eReg=cmp;
      }
    }
  ...
  public void insertKillHint(int deallocRegs) {
    if (deallocRegs!=0) ins(SSADef.I_KILLOJMP, deallocRegs);
  }
  
  public int insertFlowHint(int hint, int id) {
    if (hint==F_BLOCKSTART) id=++blockInMthdID;
  ...
  public void insertZeroHint(int objReg, int offset, int type) {
    ...
    switch (type) {
      ...
      case StdTypes.T_FLT: case StdTypes.T_PTR: case StdTypes.T_DPTR: case StdTypes.T_BOOL:
  ...
  private int getTypeID(int type) {
    switch (type) {
      ...
      case StdTypes.T_SHRT: return 2;
      case StdTypes.T_CHAR: return 3;
      case StdTypes.T_INT:  return 4;
      case StdTypes.T_FLT:  return 5;
      case StdTypes.T_LONG: return 8;
      case StdTypes.T_DBL:  return 9;
      case StdTypes.T_PTR:  return -1; //return relocBytes;
      case StdTypes.T_DPTR: return -2; //return relocBytes*2;
  ...
  public void prepareFinalization(Instruction first) {
    ...
    now=first;
    while (now!=null) { //isDest is initialized to false
      if (now.type==SSADef.I_JUMP) now.jDest.isDest=true;
      now=now.next;
    }
    optimize(first);
  }
  ...
  public void finalizeInstructions(Instruction first) {
    ...
    while (now!=null) {
      switch (now.type) {
        case SSADef.I_JUMP:
        case SSADef.I_TFBUILD: case SSADef.I_TFUPD1: case SSADef.I_TFUPD2:
          fixJDest(now);
          break;
    ...
    now=first;
    while (now!=null && now.type!=SSADef.I_MARKER) now=now.next; //search first marker
    while (now!=null && (now.type!=SSADef.I_LEAVE || now.next.type!=SSADef.I_MARKER)) { //add all instructions until we reach a leave
      diff+=now.size;
      now=now.next;
    }
    if (now==null) {
    ...
    if (ctx.printCode || ((mthdContainer.marker&Marks.K_PRCD)!=0)) printCode(first);
  }
  ...
  public void codeEpilog(Mthd outline) {
    if ((curMthd.marker&Marks.K_THRW)!=0) insertFlowHint(F_THRWMTHD, SSADef.F_DOTHROW);
    if (outline==null) {
  ...
  public void genLoadConstVal(int dst, int val, int type) {
    switch (type) {
      case StdTypes.T_PTR:
        if (val==0) ins(SSADef.I_LOADnp, dst, 0, 0, getTypeID(StdTypes.T_PTR));
        else ins(SSADef.I_LOADim_p, dst, 0, 0, 0, 0l, val);
        break;
  ...
  public void genLoadVarAddr(int dst, int src, Object loc, int off) {
    ...
    if (src==0) { //absolute addressing mode
  ...
  public void genLoadVarVal(int dst, int src, Object loc, int off, int type) {
    ...
    if (pos==0 && src!=SSADef.R_BASE) ins(SSADef.I_LOADval, dst, src, 0, getTypeID(type)); //address already existing and not in stack frame
    else { //calculate address, may be it will be used
  ...
  public void genConvertVal(int dst, int src, int toType, int fromType) {
    ...
    else ins(SSADef.I_CONV, dst, src, 0, getTypeID(toType), 0l, getTypeID(fromType));
  }
  ...
  public void genPushConstVal(int val, int type) {
    if (type==StdTypes.T_PTR || type==StdTypes.T_DPTR) {
      if (val!=0) fatalError("genPushNull with invalid type");
      else {
        ins(SSADef.I_PUSHnp);
        if (type==StdTypes.T_DPTR) ins(SSADef.I_PUSHnp);
      }
    }
    else ins(SSADef.I_PUSHim_i, 0, 0, 0, getTypeID(type), 0l, val);
  }
  ...
  public void genBinOp(int dst, int src1, int src2, int op, int type) { //may destroy src2R
    ...
    switch (opPar) {
      case Ops.A_AND:   ins(SSADef.I_AND, dst, src1, src2, typeID); return;
      case Ops.A_XOR:   ins(SSADef.I_XOR, dst, src1, src2, typeID); return;
      case Ops.A_OR:    ins(SSADef.I_OR, dst, src1, src2, typeID); return;
      case Ops.A_PLUS:  ins(SSADef.I_ADD, dst, src1, src2, typeID); return;
      case Ops.A_MINUS: ins(SSADef.I_SUB, dst, src1, src2, typeID); return;
      case Ops.A_MUL:   ins(SSADef.I_MUL, dst, src1, src2, typeID); return;
      case Ops.A_DIV:   ins(SSADef.I_DIV, dst, src1, src2, typeID); return;
      case Ops.A_MOD:   ins(SSADef.I_MOD, dst, src1, src2, typeID); return;
      case Ops.B_SHL:   ins(SSADef.I_SHL, dst, src1, src2, typeID); return;
      case Ops.B_SHRL:  ins(SSADef.I_SHRL, dst, src1, src2, typeID); return;
      case Ops.B_SHRA:  ins(SSADef.I_SHRA, dst, src1, src2, typeID); return;
    }
  ...
  public void genUnaOp(int dst, int src, int op, int type) {
    ...
    if (type==StdTypes.T_BOOL) {
      if (opPar==Ops.L_NOT) { //do not use I_NOT, would change 0x01 to 0xFE instead of 0x00
    ...
    typeID=getTypeID(type);
    switch (opPar) {
      case Ops.A_CPL:   ins(SSADef.I_NOT, dst, src, 0, typeID); return;
      case Ops.A_MINUS: ins(SSADef.I_NEG, dst, src, 0, typeID); return;
      case Ops.A_PLUS:  ins(SSADef.I_COPY, dst, src, 0, typeID); return;
    }
  ...
  public void genCall(int off, int clssReg, int parSize) {
    ...
    if (lReg!=null) ins(SSADef.I_REGRANGE, fReg.nr, lReg.nr);
    else ins(SSADef.I_REGRANGE, 0, 0);
  }
  ...
  private Instruction ins(int type) { return ins(type, 0, 0, 0, 0, 0l, 0); }
  private Instruction ins(int type, int reg0) { return ins(type, reg0, 0, 0, 0, 0l, 0); }
  private Instruction ins(int type, int reg0, int reg1) { return ins(type, reg0, reg1, 0, 0, 0l, 0); }
  private Instruction ins(int type, int reg0, int reg1, int reg2, int iPar1) { return ins(type, reg0, reg1, reg2, iPar1, 0l, 0); }
  ...
  private void codeInstruction(Instruction i) {
    ...
    switch (i.type) {
      ...
      case SSADef.I_JUMP: //jump-instruction
        ...
        if (i.isDest) pre|=SSADef.IPJMPDEST;
        i.code[0]=(byte)pre;
      ...
      case I_MAGC: //magic instruction
        for (pos=i.size-1; pos>=0; pos--) i.code[pos+2]=i.code[pos];
    ...
    if (i.isDest) i.code[0]=(byte)((opts>>>SSADef.IPOPTSHFT)|SSADef.IPJMPDEST);
    else i.code[0]=(byte)(opts>>>SSADef.IPOPTSHFT);
    i.code[1]=(byte)(i.type&SSADef.ICODEMASK);
    pos=2;
    if ((opts&SSADef.IP_reg0)!=0) { i.replaceInt(pos, i.reg0); pos+=4; }
    if ((opts&SSADef.IP_reg1)!=0) { i.replaceInt(pos, i.reg1); pos+=4; }
    if ((opts&SSADef.IP_reg2)!=0) { i.replaceInt(pos, i.reg2); pos+=4; }
    switch (opts&SSADef.IP_iPar1) {
      case 0: break; //no iPar1
    ...
    if ((opts&SSADef.IP_para)!=0) { i.replaceInt(pos, i.iPar2); pos+=4; }
    if ((opts&SSADef.IP_im_l)!=0) { i.replaceLong(pos, i.lPar); pos+=8; }
    i.size=pos;
  }
  ...
  private void fixJDest(Instruction me) {
    ...
    //get offset
    if (me.instrNr>=dest.instrNr) { //get destination before us
  ...
  public int print(Instruction i) {
    ...
    switch (i.type) {
      ...
      case SSADef.I_JUMP:
        ctx.out.print("JUMP ");
        switch (i.iPar1) {
          case SSADef.CC_AL: ctx.out.print("always j"); break;
          case SSADef.CC_LW: ctx.out.print("ifless j"); break; //jump if less
          case SSADef.CC_LE: ctx.out.print("ifleeq j"); break; //jump if less or equal
          case SSADef.CC_EQ: ctx.out.print("ifequa j"); break; //jump if equal
          case SSADef.CC_GE: ctx.out.print("ifeqgr j"); break; //jump if greater or equal (==not less)
          case SSADef.CC_GT: ctx.out.print("ifgrea j"); break; //jump if greater (==not less or equal)
          case SSADef.CC_NE: ctx.out.print("ifuneq j"); break; //jump if not equal
          case SSADef.CC_BO: ctx.out.print("ifbdok j"); break; //jump if bound ok
          default: ctx.out.print("###invcond j");
        }
    ...
    if ((opts&SSADef.IP_iPar1)==SSADef.IP_size) { //byte parameter: size of operand
      ctx.out.print(' ');
      switch (i.iPar1) {
        case -2: ctx.out.print("dptr"); break;
        case -1: ctx.out.print("ptr"); break;
        case 1: ctx.out.print("byte"); break;
        case 2: ctx.out.print("short"); break;
        case 4: ctx.out.print("int"); break;
        case 5: ctx.out.print("float"); break;
        case 8: ctx.out.print("long"); break;
        case 9: ctx.out.print("double"); break;
        default: ctx.out.print("us"); ctx.out.print(i.iPar1);
      }
    }
    if ((opts&SSADef.IP_reg0)!=0) { ctx.out.print(" r"); ctx.out.print(i.reg0); }
    if ((opts&SSADef.IP_reg1)!=0) { ctx.out.print(" r"); ctx.out.print(i.reg1); }
    if ((opts&SSADef.IP_reg2)!=0) { ctx.out.print(" r"); ctx.out.print(i.reg2); }
    switch (opts&SSADef.IP_iPar1) {
      case 0: break; //no iPar1
    ...
    if ((opts&SSADef.IP_para)!=0) { ctx.out.print(' '); ctx.out.print(i.iPar2); }
    if ((opts&SSADef.IP_im_l)!=0) { ctx.out.print(' '); ctx.out.print(i.lPar); }
    return 1;
  }
}
