...
public class ARM7 extends Architecture {
  ...
  protected int bitSearch(int value, int hit, int prefere) {
    ...
    prefere&=value;
    if (prefere!=0 && prefere!=value) for (i=0; i<16; i++) { //search here only if there is a difference
      j=1<<i;
      if ((prefere&j)!=0) {
        if (--hit==0) return j;
      }
  ...
  protected int storeReg(int regs) {
    ...
    if (regs!=0) insBlockTransfer(IC_AL, IOBT_STMFDu, RegSP, regs);
    return regs;
  }
  ...
  protected void restoreReg(int regs) {
    usedRegs|=regs;
    writtenRegs|=regs;
    if (regs!=0) insBlockTransfer(IC_AL, IOBT_LDMFDu, RegSP, regs);
  }
  ...
  public void codeEpilog(Mthd outline) {
    if (curMthd.varSize>0) insDataImm(IC_AL, IOD_ADD, RegSP, RegSP, curMthd.varSize, 0);
    insBlockTransfer(IC_AL, IOBT_LDMFDu, RegSP, (1<<RegBASE)|(1<<RegLR));
    if (curMthd.parSize>0) {
      if (curMthd.parSize>0xFF) fatalError("too many parameters");
  ...
  public void genLoadConstDoubleOrLongVal(int dst, long val, boolean asDouble) {
    int dst1, dst2;
    if ((dst1=getReg(1, dst, true))==-1 || (dst2=getReg(2, dst, true))==-1) return;
  ...
  public void genLoadVarAddr(int dst, int src, Object loc, int off) {
    ...
    if ((dstReg=getReg(1, dst, false))==-1) return;
    if (src==0) genLoadConstVal(dst, off, StdTypes.T_INT);
    else {
      if (src==1<<RegBASE && off>=0) off+=8; //skip old base pointer and old pc
      if ((srcReg=getReg(1, src, false))==-1) return;
      if (off==0) {
        if (src==dst) return; //nothing to do
        insDataReg(IC_AL, IOD_MOV, dstReg, 0, srcReg);
      }
      else { //off!=0
        if (off>0) insDataImm(IC_AL, IOD_ADD, dstReg, srcReg, off, 0);
        else insDataImm(IC_AL, IOD_SUB, dstReg, srcReg, -off, 0);
      }
    }
  }
  ...
  public void genLoadVarVal(int dstReg, int src, Object loc, int off, int type) {
    ...
    if (src==0) { //absolute address
      ...
      if (src==1<<RegBASE && off>=0) off+=8; //go behind old base pointer and old pc on stack
      if ((srcReg=getReg(1, src, true))==-1) return;
    }
    switch (type) {
      case StdTypes.T_BOOL:
        if ((dstReg=getReg(1, dstReg, true))==-1) return;
      ...
      case StdTypes.T_LONG:
        if ((dst2Reg=getReg(2, dstReg, true))==-1) return;
  ...
  public void genConvertVal(int dst, int src, int toType, int fromType) {
    ...
    switch (toType) {
      case StdTypes.T_LONG:
        switch (fromType) {
          case StdTypes.T_LONG:
            if ((dst2Reg=getReg(2, dst, true))==-1 || (dstReg=getReg(2, dst, true))==-1
                || (src2Reg=getReg(2, src, false))==-1 || (srcReg=getReg(1, src, false))==-1) return;
            if (dstReg!=srcReg) insDataReg(IC_AL, IOD_MOV, dstReg, 0, srcReg);
            if (dst2Reg!=src2Reg) insDataReg(IC_AL, IOD_MOV, dst2Reg, 0, src2Reg);
            return;
          case StdTypes.T_BYTE: case StdTypes.T_SHRT: case StdTypes.T_CHAR: case StdTypes.T_INT:
            if ((dst2Reg=getReg(2, dst, true))==-1 || (dstReg=getReg(2, dst, true))==-1 || (srcReg=getReg(1, src, false))==-1) return;
            if (srcReg!=dstReg) insDataReg(IC_AL, IOD_MOV, dstReg, 0, srcReg);
      ...
      case StdTypes.T_INT:
        switch (fromType) {
          case StdTypes.T_LONG: case StdTypes.T_INT: case StdTypes.T_PTR:
      ...
      case StdTypes.T_SHRT:
        switch (fromType) {
          case StdTypes.T_LONG: case StdTypes.T_BYTE: case StdTypes.T_SHRT:
            if ((dstReg=getReg(1, dst, true))==-1 || (srcReg=getReg(1, src, false))==-1) return;
            if (dstReg!=srcReg) insDataReg(IC_AL, IOD_MOV, dstReg, 0, srcReg);
            return;
          case StdTypes.T_CHAR: case StdTypes.T_INT:
            if ((dstReg=getReg(1, dst, true))==-1 || (srcReg=getReg(1, src, false))==-1) return;
      ...
      case StdTypes.T_CHAR:
        switch (fromType) {
          case StdTypes.T_CHAR:
            ...
            if (dstReg!=srcReg) insDataReg(IC_AL, IOD_MOV, dstReg, 0, srcReg);
            return;
          case StdTypes.T_LONG: case StdTypes.T_BYTE: case StdTypes.T_SHRT: case StdTypes.T_INT:
            if ((dstReg=getReg(1, dst, true))==-1 || (srcReg=getReg(1, src, false))==-1) return;
            insDataReg(IC_AL, IOD_MOV, dstReg, 0, srcReg, 16, IOD_M_LSLimm);
      ...
      case StdTypes.T_BYTE:
        switch (fromType) {
          case StdTypes.T_BYTE:
            ...
            if (dstReg!=srcReg) insDataReg(IC_AL, IOD_MOV, dstReg, 0, srcReg);
            return;
          case StdTypes.T_LONG: case StdTypes.T_SHRT: case StdTypes.T_CHAR: case StdTypes.T_INT:
            if ((dstReg=getReg(1, dst, true))==-1 || (srcReg=getReg(1, src, false))==-1) return;
  ...
  public void genDup(int dst, int src, int type) {
    ...
    switch (type) {
      case StdTypes.T_LONG: case StdTypes.T_DPTR:
        if ((dstReg=getReg(2, dst, true))==-1 || (srcReg=getReg(2, src, false))==-1) return;
        if (dstReg!=srcReg) insDataReg(IC_AL, IOD_MOV, dstReg, 0, srcReg);
    }
  }
  ...
  public void genBinOp(int dst, int src1, int src2, int op, int type) {
    ...
    switch (type) {
      case StdTypes.T_BOOL:
      case StdTypes.T_BYTE: case StdTypes.T_SHRT: case StdTypes.T_CHAR: case StdTypes.T_INT:
        if ((dst=getReg(1, dst, true))==-1 || (src1=getReg(1, src1, false))==-1
            || (src2=getReg(1, src2, false))==-1) return;
        switch (op) {
        ...
        return;
      case StdTypes.T_LONG:
        if ((dstHi=getReg(2, dst, true))==-1 || (dst=getReg(1, dst, true))==-1
            || (src1Hi=getReg(2, src1, false))==-1 || (src1=getReg(1, src1, false))==-1) return;
        if ((op>>>16)!=Ops.S_BSH && (src2Hi=getReg(2, src2, false))==-1) return;
        if ((src2=getReg(1, src2, false))==-1) return;
        switch (op) {
          case (Ops.S_ARI<<16)|Ops.A_PLUS:
  ...
  public void genUnaOp(int dst, int src, int op, int type) {
    ...
    switch (type) {
      case StdTypes.T_BOOL:
        if ((dst=getReg(1, dst, true))==-1 || (src=getReg(1, src, false))==-1) return;
        switch (op) {
          case (Ops.S_LOG<<16)|Ops.L_NOT:   insDataImm(IC_AL, IOD_XOR, dst, src, 1, 0); return;
        }
        break;
      case StdTypes.T_BYTE: case StdTypes.T_SHRT: case StdTypes.T_CHAR: case StdTypes.T_INT:
        if ((dst=getReg(1, dst, true))==-1 || (src=getReg(1, src, false))==-1) return;
        switch (op) {
          case (Ops.S_ARI<<16)|Ops.A_CPL:   insDataReg(IC_AL, IOD_MVN, dst, 0, src); return;
          case (Ops.S_ARI<<16)|Ops.A_MINUS: insDataImm(IC_AL, IOD_RSB, dst, src, 0, 0); return;
          case (Ops.S_ARI<<16)|Ops.A_PLUS:  if (dst!=src) insDataReg(IC_AL, IOD_MOV, dst, 0, src); return;
        }
        break;
      case StdTypes.T_LONG:
        if ((dstHi=getReg(2, dst, true))==-1 || (srcHi=getReg(2, src, false))==-1
            || (dst=getReg(1, dst, true))==-1 || (src=getReg(1, src, false))==-1) return;
        switch (op) {
          ...
          case (Ops.S_ARI<<16)|Ops.A_MINUS:
            ...
            return;
          case (Ops.S_ARI<<16)|Ops.A_PLUS:
            if (dst!=src) insDataReg(IC_AL, IOD_MOV, dst, 0, src);
            if (dstHi!=srcHi) insDataReg(IC_AL, IOD_MOV, dstHi, 0, srcHi);
            return;
        }
        break;
    }
  ...
  public void genCondJmp(Instruction dest, int condHnd) {
    int cond;
    switch (condHnd) {
      case Ops.C_EQ: cond=IC_EQ; break; //"=="
      case Ops.C_NE: cond=IC_NE; break; //"!="
      case Ops.C_LW: cond=IC_LT; break; //"<"
      case Ops.C_GE: cond=IC_GE; break; //">="
      case Ops.C_LE: cond=IC_LE; break; //"<="
      case Ops.C_GT: cond=IC_GT; break; //">"
      case Ops.C_BO: cond=IC_CC; break; //unsigned "<"
      default:
  ...
  public int genComp(int src1, int src2, int type, int cond) {
    ...
    switch (type) {
      ...
      case StdTypes.T_SHRT: case StdTypes.T_CHAR:
      case StdTypes.T_INT:  case StdTypes.T_PTR:
        if ((src1=getReg(1, src1, false))==-1 || (src2=getReg(1, src2, false))==-1) return 0;
        insDataReg(IC_AL, IOD_CMPupd, 0, src1, src2);
        return cond;
      case StdTypes.T_LONG:
        if ((src1Hi=getReg(2, src1, false))==-1 || (src2Hi=getReg(2, src2, false))==-1
            || (src1=getReg(1, src1, false))==-1 || (src2=getReg(1, src2, false))==-1) return 0;
  ...
  public int genCompValToConstVal(int src, int val, int type, int cond) {
    ...
    if ((src=getReg(1, src, false))==-1) return 0;
    insMoveConst(IC_AL, RegHLP, val, 0);
  ...
  public int genCompValToConstDoubleOrLongVal(int src, long val, boolean asDouble, int cond) {
    ...
    if ((srcHi=getReg(2, src, false))==-1 || (srcLo=getReg(1, src, false))==-1) return 0;
    insMoveConst(IC_AL, RegLR, (int)(val>>>32), 0);
  ...
  public void genMoveToPrimary(int srcR, int type) {
    ...
    if (reg!=0) insDataReg(IC_AL, IOD_MOV, 0, 0, reg);
    if (type==StdTypes.T_LONG || type==StdTypes.T_DPTR) {
      if ((reg=getReg(2, srcR, false))==-1) return;
      if (reg!=1) insDataReg(IC_AL, IOD_MOV, 1, 0, reg);
    }
  }
  ...
  public void genMoveFromPrimary(int dstR, int type) {
    int reg;
    if (type==StdTypes.T_LONG || type==StdTypes.T_DPTR) {
      if ((reg=getReg(2, dstR, false))==-1) return;
      if (reg!=1) insDataReg(IC_AL, IOD_MOV, reg, 0, 1);
    }
    if ((reg=getReg(1, dstR, false))==-1) return;
  ...
  public void genLoadDerefAddr(int destReg, int objReg, int indReg, int baseOffset, int entrySize) {
    int op, shift;
    if ((destReg=getReg(1, destReg, true))==-1
        || (objReg=getReg(1, objReg, false))==-1
        || (indReg=getReg(1, indReg, false))==-1) return;
    if (entrySize<0) {
      entrySize=-entrySize;
      op=IOD_SUB;
    }
    ...
    if (baseOffset!=0) {
      if (baseOffset>=1 && baseOffset<=255) insDataImm(IC_AL, IOD_ADD, destReg, destReg, baseOffset, 0);
      else if (baseOffset<=-1 && baseOffset>=-255) insDataImm(IC_AL, IOD_SUB, destReg, destReg, -baseOffset, 0);
      else fatalError("unsupported baseOffset in genLoadDerefAddr");
    }
  }
  ...
  public void genThrowFrameBuild(int globalAddrReg, Instruction dest, int throwBlockOffset) {
    ...
    else fatalError("unsupported throwBlockOffset");
  ...
  protected void insDataReg(int cond, int op, int dstReg, int src1Reg, int src2Reg, int shiftRegOrImm, int shiftType) {
    ...
    int shiftPos=7;
    if ((shiftType&0x00000010)!=0) shiftPos=8; //shift is not an immediate value but a register, has to be shifted one bit further
  ...
  protected void insDataImm(int cond, int op, int dstReg, int srcReg, int val, int rotateLeftEven) {
    ...
    if ((val&0xFFFFFF00)!=0) fatalError("value for insDataImm has to be 0..255");
    else ins(cond|0x02000000|op|(srcReg<<16)|(dstReg<<12)|(rotateLeftEven<<(8-1))|val);
  }
  ...
  protected void insBlockTransfer(int cond, int op, int ptrReg, int regMask) {
    ...
    if (ptrReg==RegSP) {
      int regsAffected=0;
      for (int m=0x0001; m!=0x10000; m=m<<1) if ((regMask&m)!=0) regsAffected++;
      if (op==IOBT_LDMFDu) regsAffected=-regsAffected;
  ...
  protected void insMoveConst(int cond, int dstReg, int val, int maxDiff) {
    int bs;
    if ((bs=getEvenByteRol(val))!=-1) insDataImm(cond, IOD_MOV, dstReg, 0, rotateLeft(val, bs), bs); //value is rotated byte, encode immediate load
    else if ((bs=getEvenByteRol(~val))!=-1) insDataImm(cond, IOD_MVN, dstReg, 0, rotateLeft(~val, bs), bs); //value is rotated inverted byte, encode immediate load
    else insLoadLit(cond, dstReg, val, maxDiff);
  }
  ...
  protected void insLoadLit(int cond, int dstReg, int value, int acceptDiff) {
    ...
    lit=literals;
    while (lit!=null) {
      if (lit.iPar1==value) break;
      lit=lit.next;
    }
    if (lit==null) {
      lit=getUnlinkedInstruction();
      if (lastLiteral==null) literals=lit;
      else lastLiteral.next=lit;
      lastLiteral=lit;
      lit.type=IT_LITERAL;
      lit.size=4;
      lit.iPar1=value;
  ...
  protected boolean fixBranch(Instruction me) {
    ...
    if (relative==0) { //do not code jump to next instruction
      me.size=0;
      me.type=I_NONE;
      return true;
    }
  ...
  protected boolean fixAddPatched(Instruction me, boolean doRealEncode) {
    ...
    if ((tmp=getEvenByteRol(relative))!=-1 && me.iPar1==0) { //shorten only if no ping-pong of size detected
      me.size=4; //distance may be encoded as immediate add
      if (doRealEncode) me.replaceInt(0, IC_AL|0x02000000|IOD_ADD|(1<<16)|(1<<12)|(tmp<<(8-1))|rotateLeft(relative, tmp)); //value is rotated byte, encode immediate add
    }
    else {
      if (oldSize==4) me.iPar1=1; //remember that we were better already, do not change back to single instruction to avoid endless loop
