  ...
  public static void main(String[] args) {
    ...
    try {
      while (fis.available() > 0) {
        ...
        if (type == (0xFF & SSADef.I_INLINE)) { //MAGIC.inline
          while (param-- > 0) readByte(); //skip bytes
        }
        else if (type == (0xFF & SSADef.I_JUMP)) { //jump
          readByte(); //step over condition
          readInt(); //step over relative
        } else { //normal instruction
          param = param << SSADef.IPOPTSHFT;
          //fetch parameters
          if ((param & SSADef.IP_reg0) != 0) reg0 = readInt();
          if ((param & SSADef.IP_reg1) != 0) reg1 = readInt();
          if ((param & SSADef.IP_reg2) != 0) reg2 = readInt();
          if ((param & SSADef.IP_size) != 0) iPar1 = readByte();
          else if ((param & SSADef.IP_im_i) != 0) iPar1 = readInt();
          if ((param & SSADef.IP_para) != 0) iPar2 = readInt();
          if ((param & SSADef.IP_im_l) != 0) readLong(); //lPar=readLong();
          //check instruction
          switch (type) {
//          nothing to check
...
//          check write to reg0 and read of reg1, unary operation
            ...
            case 0xFF & SSADef.I_COPY:
              if (reg1 != SSADef.R_PRIR) checkWrite(reg0); //copy after call without reg-alloc
              else usage[reg0] |= R_WRITTEN | R_READ; //R_READ: no error if result of call is not read
              checkRead(reg1);
              checkRegType(reg0, iPar1);
...
//          check write of reg0 and read of reg1, reg2 for binary operation
            ...
            case 0xFF & SSADef.I_STKCHK:
              ...
              break;
            default:
              System.out.println("###unknown instruction param/type: "
                  + param + "/" + type + " in file " + filename);
              fis.close();
              return;
          }
        }
      ...
      fis.close();
      for (i = SSADef.R_GPRS; i < usage.length; i++) checkRegUsed(i);
    } catch (IOException e) {
  ...
  private static void allocReg(int reg, int size) {
    if (reg >= SSADef.R_GPRS) {
      ...
      if (blockBaseRegNr == 0) blockBaseRegNr = reg;
  ...
  private static void restReg(int reg, int rc) {
    ...
    else usage[reg] = usage[reg] | R_ALLOCED;
  }
