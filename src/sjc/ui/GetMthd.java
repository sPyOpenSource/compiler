...
public class GetMthd {
  ...
  public static void main(String[] args) {
    ...
    if (args[1].equals("-")) {
      ...
      String addr;
      try {
        while ((addr=in.readLine())!=null) findIP(addr);
      }
      catch (IOException e) {
  ...
  private static void findIP(String addr) {
    int ip;
    try {
      if (addr.startsWith("0x")) ip=Integer.parseInt(addr.substring(2), 16);
      else ip=Integer.parseInt(addr);
    }
    ...
    try {
      for (int i=0; i<data.size(); i++) {
        ...
        if (line.startsWith("code:")) {
          ...
          if (start<=ip && ip<start+size) { //found method
            if (shortOut) printShortMethodInfo(i, ip-start);
            else printMethodInfo(i, ip-start);
            return;
          }
        }
      }
  ...
  private static void printShortMethodInfo(int entry, int offset) throws IOException,NumberFormatException {
    ...
    while (entry<data.size() && !(line=data.get(entry)).equals("")) {
      if (line.startsWith("mthd:")) mthd=line.substring(6);
    ...
    if (line.equals(lastShortLine)) return;
  ...
  private static void printMethodInfo(int entry, int offset) throws IOException,NumberFormatException {
    ...
    if (slhiLine!=null) System.out.println("slhi: "+slhiLine);
  }
}
