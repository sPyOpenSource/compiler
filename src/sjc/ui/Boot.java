...
public class Boot {
  private static class RawContainer extends BootableImage {
    public boolean init(byte[] data) {
      if ((memBlock = data) == null) return false; //get binary data
      if ((memBlockLen = memBlock.length) != readInt(4)) return false; //get and check length
      if ((readInt(28) & 0xFF0000FF) != 0x550000AA) return false; //check endianess
    ...
    private int readInt(int off) {
      return ((int)memBlock[off] & 0xFF) | (((int)memBlock[off + 1] & 0xFF) << 8)
        | (((int)memBlock[off + 2] & 0xFF) << 16) | (((int)memBlock[off + 3] & 0xFF) << 24);
    }
  }
