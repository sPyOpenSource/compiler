...
public class SCG extends JFrame {
  ...
  private void triggerATmegaEvaluation(Object source) {
    ...
    if (sAT_explicitConv.isSelected()) X_explicitConv.setSelected(true);
    if (sAT_optimEnabled.isSelected()) t_arch.setSelectedItem("atmegaOpt");
    else t_arch.setSelectedItem("atmega");
    if (sAT_skipInlineMthd.isSelected()) k_skipInlineMthd.setSelected(true);
    if (sAT_byteString.isSelected()) y_byteString.setSelected(true);
  ...
  private void triggerExpertEvaluation(Object source) {
    ...
    if (L_flat.isSelected()) appendOption(line, "-L");
    ...
    if (x_stackExtreme.isSelected()) appendOption(line, "-x");
    if (C_skipArrayCheck.isSelected()) appendOption(line, "-C");
    if (B_skipBoundCheck.isSelected()) appendOption(line, "-B");
    if (b_rteBound.isSelected()) appendOption(line, "-b");
    if (n_rteNull.isSelected()) appendOption(line, "-n");
    if (X_explicitConv.isSelected()) appendOption(line, "-X");
    if (d_mthdDebug.isSelected()) appendOption(line, "-n");
    if (g_genAllUnits.isSelected()) appendOption(line, "-g");
    if (G_genAllMthds.isSelected()) appendOption(line, "-G");
    if (j_genIntfParents.isSelected()) appendOption(line, "-j");
    if (q_relManager.isSelected()) appendOption(line, "-q");
    if (Q_srcHint.isSelected()) appendOption(line, "-Q");
    if (W_printCode.isSelected()) appendOption(line, "-W");
    if (F_profiler.isSelected()) appendOption(line, "-F");
    if (f_throwWithoutCatch.isSelected()) appendOption(line, "-f");
    if (R_skipSync.isSelected()) appendOption(line, "-R");
  ...
  private void tryDelItems(JList list) {
    ...
    for (Object o: list.getSelectedValues()) model.removeElement(o); 
  }
