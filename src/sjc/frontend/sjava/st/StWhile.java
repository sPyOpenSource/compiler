...
public class StWhile extends StLoop {
  ...
  protected int innerResolve(int flowCode, Unit unitContext, Mthd mthdContext, Context ctx) {
    ...
    if ((flowAnalysisBuffer&FA_HAS_ENDBLOCK)!=0) flowCode&=~FA_NEXT_IS_UNREACHABLE; //break existing
      ...
      if (cond.getConstIntValue(ctx)==1) flowCode|=FA_NEXT_IS_UNREACHABLE; //endless loop without break
    }
    else flowCode&=~FA_NEXT_IS_UNREACHABLE; //condition existing
    ...
    id=ctx.arch.insertFlowHint(Architecture.F_BLOCKSTART, 0);
     if (!inclusiveWhile) {
      if (cond.calcConstantType(ctx)==StdTypes.T_INT) {
        ...
        if (cond.getConstIntValue(ctx)==0) return; //loop will never be executed
        //else: constant true, no check, just start the loop 
      }
      else ctx.arch.genJmp(contDest); //not constant, first check condition
    }
