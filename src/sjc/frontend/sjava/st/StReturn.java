...
public class StReturn extends Stmt {
	...
	protected int innerResolve(int flowCode, Unit unitContext, Mthd mthdContext, Context ctx) {
	  ...
	  if (retVal!=null) { //the programmer wants us to return something, check type
      ...
      if (retVal.baseType==StdTypes.T_NULL) {
        ...
        else if (res==StdTypes.T_DPTR) retVal.baseType=StdTypes.T_NDPT;
        else {
        ...
        if (res==TypeRef.C_OT && mthdContext.retType.isIntfType()) { //insert implicit conversion of object to interface
	  ...
	  outest=outer;
	  while (outest.outer!=null) outest=outest.outer;
