...
public class StBlock extends StBreakable {
  ...
  protected int innerResolve(int flowEntryCode, Unit unitContext, Mthd mthdContext, Context ctx) {
	...
	//check constructor specials
        if (isOutestInConstr) { //in constructors, first statement has to be call to super
            if (stmts == null || !stmts.isSuperThisCallStmt(ctx)) { //no statement or not super/this-statement
            ...
            if (!stmts.resolveSuperThisCallStmt(unitContext, mthdContext, ctx)) return FA_ERROR;
        ...
        //check statements
        while (stmt != null) {
          if ((flowCode = stmt.resolve(flowCode, unitContext, mthdContext, ctx)) == FA_ERROR) return flowCode; //break on error
        ...
        if (isOutestInConstr && mthdContext.checkInitVars != null && (flowCode & FA_NEXT_IS_UNREACHABLE) == 0
            && !mthdContext.checkVarWriteState(mthdContext, ctx)) return FA_ERROR;
        //update flow code
    if ((flowAnalysisBuffer & FA_HAS_ENDBLOCK) == 0 && (flowCode & FA_NEXT_IS_UNREACHABLE) != 0) flowCode |= FA_NEXT_IS_UNREACHABLE;
  ...
  public void genOutput(Context ctx) {
    ...
    @Override
  public void innerGenOutput(Context ctx) {
    genOutput(ctx, false);
  }
	
    public void genOutput(Context ctx, boolean saveInstContext) {	  
