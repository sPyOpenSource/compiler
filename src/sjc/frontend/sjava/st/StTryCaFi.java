...
public class StTryCaFi extends StBlock {
  ...
  //for description of throw frame, see Architecture
    ...
    VrblStateList preState;
	  ...
	  while (curCatch!=null) {
      if (!(var=curCatch.catchVar).type.resolveType(unitContext, ctx)
          || !var.checkNameAgainstParam(mthdContext.param, ctx)
          || !var.checkNameAgainstVrbl(mthdContext.vars, ctx)
          || !var.enterSize(Vrbl.L_LOCAL, ctx)) return FA_ERROR;
      ...
      if (!var.type.isCheckedExceptionType(ctx)) curCatch.isValid=true; //not checked exceptions are always valid
      curCatch.catchVar.type.qid.unitDest.modifier|=Modifier.MA_ACCSSD;
      if (ctx.dynaMem) curCatch.importedClass=unitContext.getRefUnit(var.type.qid.unitDest, true);
    ...
    globalRes=(singleRes=tryBlock.resolve(flowCode, unitContext, mthdContext, ctx))&FA_ERROR;
	  if ((singleRes&FA_NEXT_IS_UNREACHABLE)!=0) allBlocksHaveNextUnreachable=true;
	  curCatch=catchBlocks;
	  while (curCatch!=null) {
      ...
      if ((singleRes&FA_HAS_ENDBLOCK)!=0 || (singleRes&FA_NEXT_IS_UNREACHABLE)==0) allBlocksHaveNextUnreachable=false;
	  ...
	  if (allBlocksHaveNextUnreachable) globalRes|=FA_NEXT_IS_UNREACHABLE;
	...
	public boolean handlesThrowable(Unit thrown, Context ctx) {
  ...
  @Override
    ...
    if (ctx.noThrowFrames) { //code-optimized skipping of catch-blocks
    ...
    else { //normal and Java-conforming code
      ...
      if ((regRes=buildGlobalAddress(ctx))==0) return;
        ...
        while (curCatch!=null) {
          ...
          ExCheckType.genIsType(regRes, regExc, false,
              curCatch.catchVar.type, ExCheckType.C_ISINST,
              runtimeClass, curCatch.importedClass, ctx);
        ...
        if (finallyBlock!=null) ctx.arch.genJmp(finallyStart);
        else {
        ...
        while (curCatch!=null) {
          ...
          else ctx.arch.genThrowFrameUpdate(catchListStart, finallyStart, excFrameOffset);
          ...
          else ctx.arch.genJmp(afterAll);
          curCatch=curCatch.nextCatchDecl;
        }
  	  }
  	  else ctx.arch.appendInstruction(catchListStart); //dummy-insert, ease conditions above
    ...
    else ctx.arch.genLoadVarAddr(regRes, 0,
        !ctx.embedded ? ctx.rteDynamicRuntime.outputLocation : ctx.ramLoc, ctx.rteThrowFrame);
    return regRes;
	}
}
