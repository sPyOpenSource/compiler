...
public class StVrbl extends Stmt {
  ...
  public StVrbl(Vrbl iv, boolean ifi) {
  ...
  protected int innerResolve(int flowCode, Unit unitContext, Mthd mthdContext, Context ctx) {
    ...
    if (varCount>0) { //if already resolved: reset resolving
      for (int i=1; i<varCount; i++) var=var.nextVrbl;
      var.nextVrbl=null;
      var=varList;
      varCount=0;
    }
    ...
    while (var!=null) {
      if (last!=null) { //already resolved a variable
      ...
      if (!var.type.resolveType(unitContext, ctx) || !var.checkNameAgainstVrbl(mthdContext.vars, ctx)
          || !var.checkNameAgainstParam(mthdContext.param, ctx) || !var.enterSize(Vrbl.L_LOCAL, ctx)) return FA_ERROR;
      if (var.minSize<0) offset+=var.minSize*ctx.arch.relocBytes; //this is a reloc, use size given in ctx
      else offset-=var.minSize; //scalar, take minimum size
      ...
      if (var.init!=null) {
        ...
        if (!SJava.checkVarInitType(var, (mthdContext.marker&Marks.K_EXPC)!=0, unitContext, ctx)) return FA_ERROR;
        var.modifier|=Modifier.MF_ISWRITTEN; //initialized variables are written
      }
      if (origLastNextVrbl!=null) last.nextVrbl=origLastNextVrbl; //restore previously changed nextVrbl
