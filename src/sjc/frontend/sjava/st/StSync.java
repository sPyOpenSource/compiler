...
public class StSync extends StBreakable {
  ...
  public StSync(StBreakable io, int is, int fid, int il, int ic) {
  ...
  public void printBreakableStatement(CodePrinter prnt) {
	...
	protected int innerResolve(int flowCode, Unit unitContext, Mthd mthdContext, Context ctx) {
	  //resolve syncBlock
	  if (((flowCode=syncBlock.resolve(flowCode, unitContext, mthdContext, ctx))&FA_ERROR)!=0) return FA_ERROR;
	  ...
	  switch (syncType) {
	    case SYNC_NORM: //object to synchronize on is explicitly given
	      ...
	      mthdContext.varSize=-(syncObjOffset
	        =(-mthdContext.varSize-ctx.arch.relocBytes)&~ctx.arch.stackClearBits);
	      break;
	  ...
	  //everything ok
    if (!ctx.noSyncCalls && ctx.dynaMem) runtimeClass=unitContext.getRefUnit(ctx.rteDynamicRuntime, true);
	  return flowCode;
	}
	...
	protected void innerGenOutput(Context ctx) {
	  int regSyncObj=0;
	  if (ctx.noSyncCalls) return;
	  //evaluate expression and call runtime for "enter"
	  switch (syncType) {
	    case SYNC_NORM:
	      if (ctx.arch.prepareFreeReg(0, 0, 0, StdTypes.T_PTR)!=0) { //should be 0 always
	    ...
	    case SYNC_INST:
	      ...
	      break;
	    case SYNC_CLSS:
	      if (ctx.dynaMem) genOutputDoSyncCall(ctx.arch.regClss, 0, ctx); //unit context existing
	      else { //unit context must be loaded
          if (ctx.arch.prepareFreeReg(0, 0, 0, StdTypes.T_PTR)!=0) { //should be 0 always
            compErr(ctx, ERR_NOFREEREG);
            return;
          }
    ...
    switch (syncType) {
      ...
      case SYNC_INST:
        ...
        break;
      case SYNC_CLSS:
        if (ctx.dynaMem) genOutputDoSyncCall(ctx.arch.regClss, 1, ctx); //unit context existing
        else { //unit context must be loaded
          if (ctx.arch.prepareFreeReg(0, 0, 0, StdTypes.T_PTR)!=0) { //should be 0 always
            compErr(ctx, ERR_NOFREEREG);
            return;
    ...
    else ctx.arch.genCallConst(ctx.rteDoSyncMd, ctx.rteDoSyncMd.parSize);
	}
}
