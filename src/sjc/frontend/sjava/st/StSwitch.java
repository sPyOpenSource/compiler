...
public class StSwitch extends StBreakable {
	...
	public StSwitch(StBreakable io, StringList ila, int fid, int il, int ic) {
    ...
    preState=ctx.copyVrblListState(mthdContext.vars, mthdContext.checkInitVars);
      ...
      if (def==stmt || (cases!=null && cases.stmt==stmt)) { //only one of def==stmt and cases.stmt==stmt may be true
      ...
      //handle statement
	    ...
	    if ((flowAnalysisBuffer&FA_HAS_SHORTCUT)!=0
	        || ((singleRes&FA_NEXT_IS_UNREACHABLE)==0 && stmt.nextStmt==null)) {
        flowAnalysisBuffer&=~FA_HAS_SHORTCUT;
        if (def!=null) {
          if (sure==null) sure=ctx.copyVrblListState(mthdContext.vars, mthdContext.checkInitVars);
          else ctx.mergeVrblListState(sure, mthdContext.vars, mthdContext.checkInitVars);
        }
      }
    ...
    //last block was potentially not merged into complRes
	  complRes|=flowAnalysisBuffer&FA_HAS_ENDBLOCK;
    if ((complRes&FA_HAS_ENDBLOCK)==0 && (singleRes&FA_NEXT_IS_UNREACHABLE)!=0 && def!=null) flowCode|=FA_NEXT_IS_UNREACHABLE;
    if ((complRes&FA_ERROR)!=0) flowCode|=FA_ERROR;
    //handle variable writing
    ctx.setVrblListState(mthdContext.vars, mthdContext.checkInitVars, preState);
    if (sure!=null) ctx.setVrblStateSure(mthdContext.vars, mthdContext.checkInitVars, sure);
    ctx.recycleVrblStatelist(preState);
    ctx.recycleVrblStatelist(sure);
    ...
    while (cases!=null) {
      ...
      while (cmp!=null) {
        if (constType==StdTypes.T_INT ?
            cases.cond.getConstIntValue(ctx)==cmp.cond.getConstIntValue(ctx)
            : cases.cond.getConstLongValue(ctx)==cmp.cond.getConstLongValue(ctx)) {
    ...
    Instruction defIns=null;
	  ...
	  if (ctx.arch.prepareFreeReg(0, 0, 0, cond.baseType)!=0) { //should be 0 always
    ...
    ctx.arch.insertKillHint(reg);
	  condstmt=caseConds;
	  while (condstmt!=null) {
	    if (constType==StdTypes.T_INT) condHnd=ctx.arch.genCompValToConstVal(reg,
	        condstmt.cond.getConstIntValue(ctx), cond.baseType, Ops.C_EQ);
	    else condHnd=ctx.arch.genCompValToConstDoubleOrLongVal(reg,
	        condstmt.cond.getConstLongValue(ctx), false, Ops.C_EQ);
	  ...
	  else ctx.arch.genJmp(breakDest);
	  stmt=stmts;
    condstmt=caseConds;
    ctx.arch.insertFlowHint(Architecture.F_SWSBSTART, id);
      ...
      if (def==stmt) ctx.arch.appendInstruction(defIns);
	    stmt.genOutput(ctx);
	    stmt=stmt.nextStmt;
	  }
