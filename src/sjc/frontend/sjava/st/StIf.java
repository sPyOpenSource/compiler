...
public class StIf extends Stmt {
    ...
    if (!cond.isBoolType()) {
	  ...
	  if (cond.calcConstantType(ctx)==StdTypes.T_INT) {
	    if (cond.getConstIntValue(ctx)==1) flowFalse|=FA_DEAD_CODE;
	    else flowTrue|=FA_DEAD_CODE;
	    if ((flowCode&FA_DEAD_CODE)==0) flowFilter=~FA_DEAD_CODE; //filter dead code
	  }
    
	  preState=ctx.copyVrblListState(mthdContext.vars, mthdContext.checkInitVars);
	  if ((trRes=trStmt.resolve(flowTrue, unitContext, mthdContext, ctx))==FA_ERROR) return FA_ERROR;
	  if (faStmt!=null) {
	    ...
	    if ((faRes=faStmt.resolve(flowFalse, unitContext, mthdContext, ctx))==FA_ERROR) return FA_ERROR;
	    if (((trRes|faRes)&FA_NEXT_IS_UNREACHABLE)!=0) { //at least one of tr/fa has next_unreachable set
	      if ((trRes&FA_NEXT_IS_UNREACHABLE)!=0) {
	        if ((faRes&FA_NEXT_IS_UNREACHABLE)!=0) ctx.setVrblListState(mthdContext.vars, mthdContext.checkInitVars, preState);
	        //else: nothing to do if only faStmt is relevant
	      }
        else ctx.setVrblListState(mthdContext.vars, mthdContext.checkInitVars, trState);
	    }
	    else ctx.setVrblStateCombined(mthdContext.vars, mthdContext.checkInitVars, trState);
	    ctx.recycleVrblStatelist(trState);
	  }
	  else { //there is no faStmt
	    if ((trRes&FA_NEXT_IS_UNREACHABLE)!=0) ctx.setVrblListState(mthdContext.vars, mthdContext.checkInitVars, preState);
	    else ctx.setVrblStatePotential(mthdContext.vars, mthdContext.checkInitVars, preState);
	  }
    ...
    if (cond.calcConstantType(ctx)==StdTypes.T_INT) {
      if (cond.getConstIntValue(ctx)==1) trStmt.genOutput(ctx);
      else {
        if (faStmt!=null) faStmt.genOutput(ctx);
        //else: nothing to do
      }
      return;
    }
    ...
    ctx.arch.insertFlowHint(Architecture.F_CONDSTART, id);
	  ...
	  else cond.genOutputCondJmp(afterwards, false, trIns, ctx);
