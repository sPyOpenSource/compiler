...
public class StAssert extends Stmt {
    ...
    public StAssert(int fid, int il, int ic) {
        super(fid, il, ic);
    }
	...
	protected void innerGenOutput(Context ctx) {
	  ...
	  if (skipEncode) return; //assert disabled or constant true condition
    afterwards=ctx.arch.getUnlinkedInstruction();
    //check if condition is constant
    if (cond.calcConstantType(ctx)!=StdTypes.T_INT) { //condidional assert
    ...
    if (msg==null) ctx.arch.genPushConstVal(0, StdTypes.T_PTR);
    else {
    ...
    else ctx.arch.genCallConst(ctx.rteAssertFailedMd, ctx.rteAssertFailedMd.parSize);
    ctx.arch.deallocRestoreReg(0, 0, restore);
    ctx.arch.appendInstruction(afterwards);
    if (doCall!=null) ctx.arch.insertFlowHint(Architecture.F_BLOCKEND, id);
	}
}
