/* Copyright (C) 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2015 Stefan Frenz
 * 
...
public class JMthd extends Mthd {
  ...
  public boolean checkNameAndType(Unit inUnit, Context ctx) {
    ...
    marker|=inUnit.marker&(Marks.K_INTR|Marks.K_DEBG|Marks.K_FINL|Marks.K_NINL|Marks.K_PROF|Marks.K_NPRF|Marks.K_PRCD
        |Marks.K_NOPT|Marks.K_SEPC|Marks.K_NSPC|Marks.K_SLHI|Marks.K_EXPC|Marks.K_NWIN|Marks.K_FCDG|Marks.K_ASRT);
    ...
    if ((marker&Marks.K_FCDG)!=0) modifier|=Modifier.M_NDCODE;
    //check return-type
    if (retType==null) {
      ...
      if (!retType.resolveType(inUnit, ctx)) return false;
    }
    //check parameter-types
    parCnt=0;
    par=param;
    while (par!=null) {
      if (!par.type.resolveType(inUnit, ctx) || !param.checkNameAgainstParam(param, ctx)
          || !par.enterSize(Param.L_PARAM, ctx)) {
        ctx.out.print(" in method ");
        printNamePar(ctx.out);
        return false;
      }
      if (par.minSize<0) offset+=par.minSize*ctx.arch.relocBytes; //this is a reloc, use specified pointer-size
      else offset-=(par.minSize+ctx.arch.stackClearBits)&~ctx.arch.stackClearBits; //scalar, minimum size aligned to specified bits
      par.relOff=offset;
    ...
    om=inUnit.mthds;
    while (om!=this) {
      if (matches(om, ctx)) { //name and types identical
        printPos(ctx, "method ");
        printNamePar(ctx.out);
    ...
    while (curThrows!=null) {
      if (!curThrows.qid.resolveAsUnit(inUnit, ctx)
          || !curThrows.qid.unitDest.resolveInterface(ctx)) return false;
  ...
  public boolean resolve(Context ctx) {
    ...
    if ((modifier&Modifier.M_NAT)!=0) {
      ...
      if (!nativeAddress.type.isStructType()) switch (nativeAddress.type.baseType) {
        case StdTypes.T_BYTE: invType=ctx.arch.relocBytes!=1; break;
        case StdTypes.T_SHRT: invType=ctx.arch.relocBytes!=2; break;
        case StdTypes.T_INT: break; //always allowed
        case StdTypes.T_LONG: invType=ctx.arch.relocBytes!=8; break;
        default: invType=true; //all other types are not valid
      }
      if (invType || nativeAddress.type.arrDim!=0 || nativeAddress.type.isObjType()
          || nativeAddress.type.isIntfType()) {
      ...
      mPar=param;
      while (mPar!=null) {
        if ((mPar.type.baseType==StdTypes.T_QID && !mPar.type.isStructType())
            || mPar.type.arrDim!=0) {
    ...
    if (isConstructor) {
      ...
      if (call.id.equals(SJava.KEY_SUPER) && owner.instInitVars==null && call.dest!=null && call.dest.owner.explConstr) {
        ...
        while (cs instanceof StEmpty) cs=cs.nextStmt;
        if (cs==null) { //nothing more than the super-call and empty statements
          mPar=param;
          cPar=call.par;
          while (mPar!=null) {
            if (cPar==null) break; //parameters in call are less than in ourself
            if (cPar.expr instanceof ExVar) {
              ep=(ExVar)cPar.expr;
              if (ep.dest!=mPar) break; //parameter in call is not corresponding to our own one
            }
            else break;
            mPar=mPar.nextParam;
            cPar=cPar.nextParam;
          ...
          if (mPar==null && cPar==null) {
            redirect=call.dest;
            while (redirect.redirect!=null) redirect=redirect.redirect;
          }
        }
      }
    ...
    if (ctx.globalProfiling || (marker&Marks.K_PROF)!=0) {
      ...
      if (ctx.dynaMem) runtimeClass=owner.getRefUnit(ctx.rteDynamicRuntime, true);
    }
    ...
    if (ctx.globalStackExtreme || (marker&Marks.K_SEPC)!=0) {
      ...
      if (ctx.dynaMem && runtimeClass==null) runtimeClass=owner.getRefUnit(ctx.rteDynamicRuntime, true);
    }
    //enter register of return value
    if (retType!=null) retRegType=retType.getRegType(ctx);
    else if (isConstructor && (modifier&Modifier.M_EXINIT)!=0) retRegType=StdTypes.T_PTR;
    else retRegType=0;
    //everything done
    isResolved=true;
    return true;
  ...
  private void setWriteCheckState(boolean state) {
    ...
    while (vrblList!=null) {
      if (state) vrblList.vrbl.modifier|=Modifier.MF_ISWRITTEN;
      else vrblList.vrbl.modifier&=~(Modifier.MF_ISWRITTEN|Modifier.MF_MAYBEWRITTEN);
      vrblList=vrblList.next;
    }
  }
  ...
  public void genInlineOutput(Context ctx) {
    ...
    if (redirect!=null) redirect.genInlineOutput(ctx);
    else {
      if (inGenOutput) {
      ...
      ctx.arch.codeProlog();
      if (block!=null) block.genOutput(ctx);
      if (ctx.err) return;
  ...
  private void genProfilingCall(int id, int mode, Context ctx) {
    ...
    else ctx.arch.genCallConst(ctx.rteDRProfilerMd, ctx.rteDRProfilerMd.parSize);
  }
  ...
  private void genStackExtremeCheck(Context ctx) {
    ...
    else ctx.arch.genLoadVarVal(value, 0,
        !ctx.embedded ? ctx.rteDynamicRuntime.outputLocation : ctx.ramLoc, ctx.rteStackExtreme,
        StdTypes.T_PTR);
    ...
    else ctx.arch.genCallConst(ctx.rteDRStackExtremeErrMd, ctx.rteDRStackExtremeErrMd.parSize);
