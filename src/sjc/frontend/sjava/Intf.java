...
public class Intf extends JUnit {
    ...
    list=extsImplIDList;
    while (list!=null) {
      if (!resolveExtsIplsQID(list.qid, ctx)) return false;
      if (!list.qid.unitDest.resolveInterface(ctx)) return false;
  ...
  public boolean assignOffsets(boolean doClssOff, Context ctx) {
    ...
    if (!ctx.rteSIntfDesc.assignOffsets(true, ctx)) return false;
    ...
    if (ctx.dynaMem) fixDynaAddresses(ctx);
    //everything done
  ...
  public boolean genDescriptor(Context ctx) {
    ...
    if (ctx.genAllUnitDesc || ctx.genIntfParents) modifier|=Modifier.MA_ACCSSD; //parents-listing requires all SIntfDesc to be created
    ...
    else if (!allocateDescriptor(clssScalarTableSize+statScalarTableSize,
        clssRelocTableEntries+statRelocTableEntries,
        ctx.rteSIntfDesc.outputLocation, ctx)) {
      outputError=true;
      return false;
    }
    //outputAddr is now valid and will not be changed anymore
    else if (!ctx.embedded) { //fix dynamic varPos
      checkVrbl=vars;
      while (checkVrbl!=null) {
        switch (checkVrbl.location) {
          case Vrbl.L_CLSSSCL:
    ...
    QualIDList list;
	  ...
	  while (meM!=null) {
	    ...
	    cmpM=mthds;
	    while (cmpM!=meM) {
	      if (meM.matches(cmpM, ctx)) { //name and signature identical
    ...
    while (list!=null) {
	    ...
	    while (meM!=null) {
	      ...
	      if (cmpM!=null) { //meMthod with same name and signature exists
	        if (meM.retType.compareType(cmpM.retType, false, ctx)!=TypeRef.C_EQ //not same return-type
	            || ((cmpM.modifier|meM.modifier)&Modifier.M_PUB)!=(meM.modifier&Modifier.M_PUB)) { //reduces visibility
  	        meM.printPos(ctx, "method ");
            ctx.out.print(meM.name);
	      ...
	      else { //no matching meMethod found -> copy
          if (lastM==null) { //no meMethod until now
	  ...
	  while (meV!=null) {
	    if ((meV.modifier&(Modifier.M_STAT|Modifier.M_FIN))!=(Modifier.M_STAT|Modifier.M_STAT)) {
  	    cmpV=vars;
  	    while (cmpV!=meV) {
  	      if (meV.name.equals(cmpV.name)) { //name
  	  ...
  	  else { //should never happen, parser should have not accepted this variable
  ...
  public IndirUnitMapList enterInheritableReferences(Object objLoc, IndirUnitMapList lastIntf, Context ctx) {
	...
	public boolean genOutput(Context ctx) {
	  ...
	  if (ctx.genIntfParents && extsImplIDList!=null) {
	    ...
	    while (parentsList!=null) { //insert all parents in the array
    ...
    if ((modifier&Modifier.MA_ACCSSD)==0) dbw.markUnitAsNotUsed();
    else {
