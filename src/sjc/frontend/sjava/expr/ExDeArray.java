...
public class ExDeArray extends Expr {
  ...
  public ExDeArray(int fid, int il, int ic) {
    super(fid, il, ic);
  }
  ...
  public boolean resolve(Unit unitContext, Mthd mthdContext, int resolveFlags,
      TypeRef preferredElemType, Context ctx) {
    ...
    if (!le.resolve(unitContext, mthdContext, resolveFlags|RF_CHECKREAD, preferredObjType, ctx)) return false;
    if (le.arrDim<1) {
      printPos(ctx, "left side is not an array");
      return false;
    }
    if (ind!=null) { //ind==null only for internally inserted ExDeArrays
      if (!ind.resolve(unitContext, mthdContext, resolveFlags|RF_CHECKREAD, ctx.intType, ctx)) {
        ctx.out.print(" in array-deref");
        return false;
      }
      if ((mthdContext.marker&Marks.K_EXPC)==0) switch (ind.getBaseTypeConvertible()) {
        case StdTypes.T_BYTE: case StdTypes.T_SHRT: case StdTypes.T_CHAR:
          if ((ind=ExEnc.getConvertedResolvedExpr(ind, ctx.intType, unitContext, ctx))==null) return false;
      }
      if (!ind.isIntType()) {
    ...
    if (ctx.dynaMem && (ctx.runtimeBound || ctx.runtimeNull || ctx.doArrayStoreCheck)
        && ind!=null)
  ...
  public void genOutputAddrFirstStopElem(int firstAddrReg, int stopAddrReg, Context ctx) {
    genOutputAddr(0, stopAddrReg, firstAddrReg, ctx);
  }
  
  public int getElemSize(Context ctx) {
    int regType;
    if (le.typeSpecial>0) { //valid struct array special
      if (baseType>0) return TypeRef.getMinSize(baseType);
      else if (baseType==TypeRef.T_QID) return qid.unitDest.instScalarTableSize;
    }
    //normal element or invalid struct array special (will stop the compiler in genOutputAddr)
    return (regType=getRegType(ctx))==StdTypes.T_PTR || regType==StdTypes.T_DPTR
      ? -ctx.arch.relocBytes : TypeRef.getMinSize(regType);
  }
  ...
  private void genOutputAddr(int lePtr, int addrReg, int elem0Reg, Context ctx) { //if elem0Reg!=0, put addr stop-element in addrReg and addr of first element in elem0Reg
    ...
    if (lePtr==0) { //left side is not needed after outputAddr
      objRestore=ctx.arch.prepareFreeReg(0, 0, elem0Reg==0 ? addrReg : 0, StdTypes.T_PTR);
      obj=ctx.arch.allocReg();
    }
    else obj=lePtr; //remember pointer to array for caller
    ...
    switch (le.typeSpecial) {
      ...
      case TypeRef.S_NOSPECIAL: //instance array (both normal and inline)
        if (ctx.runtimeNull) {
          ...
          else ctx.arch.genCallConst(ctx.rteDRNullExcMd, ctx.rteDRNullExcMd.parSize);
        ...
        if (elem0Reg==0) { //normal de-array
          ind.genOutputVal(index, ctx);
          if (ctx.doBoundCheck) {
            excCheckDone=ctx.arch.getUnlinkedInstruction();
            if (le.typeSpecial==TypeRef.S_FLASHINLARR || le.typeSpecial==TypeRef.S_FLASHREF) ctx.arch.nextValInFlash();
            ...
            else ctx.arch.genNativeBoundException();
            ctx.arch.appendInstruction(excCheckDone);
          }
        }
        else ctx.arch.genLoadVarVal(index, obj, null, lenPos, StdTypes.T_INT); //calculate address of stop element
        if (size>0) { //scalars
          if (ctx.indirScalars) { //movable indirect scalars
            indirBaseObj=obj;
          ...
          else pos=container.instScalarTableSize; //normal scalars
        }
        else pos=-(container.instRelocTableEntries+1)*ctx.arch.relocBytes; //relocs
        ctx.arch.genLoadDerefAddr(addrReg, obj, index, pos, size);
        if (indirBaseObj!=0) { //clean up indir
          ctx.arch.deallocRestoreReg(indirBaseObj, addrReg, indirRestore);
          obj=indirBaseObj;
        }
        if (elem0Reg!=0) { //also get address of first element
          if (ctx.indirScalars && size>0) ctx.arch.genLoadVarVal(elem0Reg, obj, null, ctx.indirScalarAddrOff, StdTypes.T_PTR);
          else ctx.arch.genLoadVarAddr(elem0Reg, obj, null, pos);
        }
      ...
      case TypeRef.S_STRUCTARRNOTSPEC: //struct inline array with unlimited index
        if (baseType>0) size=TypeRef.getMinSize(baseType);
        else if (baseType==TypeRef.T_QID) size=qid.unitDest.instScalarTableSize;
        else {
          compErr(ctx, "ExDeArray.genOutputAddr with invalid struct-deref");
          return;
        }
        if (elem0Reg==0 && ind.calcConstantType(ctx)==StdTypes.T_INT) { //only relevant in normal mode
          ...
          if (elem0Reg==0) { //normal de-array
            ind.genOutputVal(index, ctx);
            if (ctx.doBoundCheck && le.typeSpecial>0) { //insert check for array index
              excCheckDone=ctx.arch.getUnlinkedInstruction();
          ...
          if (elem0Reg!=0) ctx.arch.genDup(elem0Reg, obj, StdTypes.T_PTR); //also get address of first element
  ...
  public void genOutputPrepareAssignTo(int destReg, int newValueReg, Expr newValueEx, Context ctx) {
    ...
    if (ctx.doArrayStoreCheck && (newValueEx.isObjType() || newValueEx.isIntfType())) {
      ...
      else ctx.arch.genCallConst(ctx.rteDRCheckArrayStoreMd, ctx.rteDRCheckArrayStoreMd.parSize);
    ...
    else super.genOutputPrepareAssignTo(destReg, newValueReg, newValueEx, ctx);
  }
  ...
  public void genOutputValOfElemAddr(int reg, int addrReg, Context ctx) {
    if (qid!=null && (qid.unitDest.modifier&Modifier.M_STRUCT)!=0) { //struct de-array
      ctx.arch.genDup(reg, addrReg, StdTypes.T_PTR);
    }
    else if (intfConv!=null) { //interface de-array
      ctx.arch.genLoadVarVal(reg, addrReg, null, 0, StdTypes.T_PTR);
      intfConv.genIsType(reg, reg, true, ctx);
    }
    else { //normal de-array
      if (le.typeSpecial==TypeRef.S_FLASHINLARR || le.typeSpecial==TypeRef.S_FLASHREF) ctx.arch.nextValInFlash();
