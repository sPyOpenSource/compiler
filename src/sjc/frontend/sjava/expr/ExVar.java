...
public class ExVar extends ExAccVrbl {
  ...
  protected boolean varResolveSuper(Unit inUnit, Mthd inMthd, Context ctx) {
    ...
    baseType=T_QID;
    qid=parent.getQIDTo();
    if (ctx.dynaMem) importedClass=inUnit.getRefUnit(parent, true);
    return true;
  }
  
	//has also to resolve units and packets for ExDeRef
	protected boolean varResolve(Pack inPack, Unit inUnit, Mthd inMthd, Unit unitContext, Mthd mthdContext,
	    int resolveFlags, boolean asDeRef, TypeRef preferredType, Context ctx) {
	  Param vParam=null;
    ...
    if (inUnit!=null) impt=inUnit.getImportList();
	  if (asDeRef || (!resolveInVar(vLocal, ctx) && !resolveInParam(vParam, ctx))) {
      if (inUnit!=null && inUnit.innerUnits!=null && resolveInInner(inUnit.innerUnits, ctx)) { //check inner units
        //TODO: is this affected by checkRead?
        return unitContext.minimumAccessLevel(this, id, qid.unitDest, qid.unitDest.modifier,
            inUnit==unitContext, ctx)!=Modifier.M_ERROR;
      }
      //access not through inner class
      if (inUnit==null || !resolveInUnit(inUnit, ctx)) {
        if (!resolveInPackage(inPack, true) && (asDeRef || (inUnit==null
            || (!resolveInImptList(impt, false) && !resolveInPackage(inUnit.pack.packDest, asDeRef)
                && !resolveInImptList(impt, true) && !resolveInPackage(ctx.root, true)
                && !resolveInPackage(ctx.defUnits, false) && !resolveInPackage(ctx.rte, false))))) {
          if (inUnit!=null && inUnit.outerUnit!=null) { //try to resolve in outer unit
            if (varResolve(inPack, inUnit.outerUnit, inMthd, unitContext, mthdContext, resolveFlags,
                asDeRef, preferredType, ctx)) {
              if (dest==null) { //found "variable" which is class name of inner class of shared outer class
                if (qid.unitDest!=null) return true;
              ...
              dvar=(Vrbl)dest;
              if ((dvar.modifier&Modifier.M_STAT)==0) { //access via instance
              ...
              else { //access via class descriptor
                ensureClassContext=true;
                if (ctx.dynaMem) importedClass=unitContext.getRefUnit(dvar.owner, true);
              }
              if (ctx.relations!=null) ctx.relations.addRelation(dest.owner, null, dest, unitContext, mthdContext);
        ...
        if (baseType!=StdTypes.T_PACK) {
          if ((accessLevel=unitContext.minimumAccessLevel(this, id, qid.unitDest, qid.unitDest.modifier, true, ctx))
        ...
        if (dvar.location==AccVar.L_NOTRDY && !dvar.owner.resolveInterface(ctx)) return false; //output already done
        //check access level
        if ((accessLevel=unitContext.minimumAccessLevel(this, dvar.name, dvar.owner, dvar.modifier, inUnit==unitContext, ctx))
            ==Modifier.M_ERROR) return false;
        dvar.modifier|=accessLevel|Modifier.MA_ACCSSD;
        //check if constant
        if (dvar.location==AccVar.L_CONSTDC || dvar.location==AccVar.L_CONSTTR
            || dvar.location==AccVar.L_CONST) {
          ...
          if (dvar.init!=null && (constType=dvar.init.calcConstantType(ctx))==0 &&
              !(constObject=dvar.init.isCompInitConstObject(ctx))) {
            printPos(ctx, "unknown type of constant variable");
            return false;
          }
          ExConstInitObj coi;
          if (constObject && (!dvar.init.isCompInitConstObject(ctx) || (coi=dvar.init.getConstInitObj(ctx))==null
              || (dest=coi.dest)==null)) {
        ...
        //check static access
          ...
          if ((inUnit!=dvar.owner || (inMthd.modifier&Modifier.M_STAT)==0)
              && (constObject || (dvar.modifier&Modifier.M_FIN)==0)) {
            ensureClassContext=true;
  ...
  private boolean resolveInInner(Unit unit, Context ctx) {
    while (unit!=null) {
      ...
      if (resolveInInner(unit.innerUnits, ctx)) return true;
      unit=unit.nextUnit;
    }
    return false;
  }
	...
	private boolean resolveInUnit(Unit unit, Context ctx) {
	  if ((dest=unit.searchVariable(id, ctx))==null) return false;
    qid=unit.getQIDTo();
	  getTypeOf(dest.type);
	  return true;
	}
	  ...
	  while (impt!=null) {
	    if (packagesNotUnits) {
	      if (impt.qid.type==QualID.Q_IMPORTPACK) {
	        if (resolveInPackage(impt.qid.packDest, true)) return true;
	      }
	    }
	    else {
	...
	public void genOutput(Context ctx) { //expression that consists only of this ExVar -> value is not of interest
