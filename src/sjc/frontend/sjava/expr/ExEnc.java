/* Copyright (C) 2005, 2006, 2007, 2008, 2009, 2010, 2012, 2019 Stefan Frenz
 * 
...
public class ExEnc extends ExCheckType {
  ...
  private final static String INSCONV  = "inserted implicit conversion";
    ...
    if (convertTo!=null) resolveFlags|=RF_CHECKREAD;
	  if (!ex.resolve(unitContext, mthdContext, resolveFlags,
	      convertTo==null ? preferredType : convertTo, ctx)) return false;
    return implConvResolve(unitContext, ctx);
	}
	
  public boolean implConvResolve(Unit unitContext, Context ctx) {
    ...
    if (convertTo!=null) {
      ...
      if (isStructType()) {
        ...
        switch (calcConstType) {
          case T_INT: constInt=ex.getConstIntValue(ctx); break;
          case T_LONG: constLong=ex.getConstLongValue(ctx); break;
        }
      }
    ...
    if ((myInt=ex.calcConstantType(ctx))==T_INT || myInt==T_LONG) {
      if (myInt==T_INT) myInt=ex.getConstIntValue(ctx);
      else myLong=ex.getConstLongValue(ctx);
      //is constant
      switch (baseType) { //toType
        case T_BYTE:
          if (getIntOfEnc(myInt, myLong, ctx) && (int)((byte)constInt)!=constInt
              && (constInt&0xFF)!=constInt) {
            printPos(ctx, EXCRANGE);
            ctx.out.println(WARNONLY);
          }
          constInt=(int)((byte)constInt);
          calcConstType=T_INT;
          break;
        case T_SHRT:
          if (getIntOfEnc(myInt, myLong, ctx) && (int)((short)constInt)!=constInt
              && (constInt&0xFFFF)!=constInt) {
            printPos(ctx, EXCRANGE);
            ctx.out.println(WARNONLY);
          }
          constInt=(int)((short)constInt);
          calcConstType=T_INT;
          break;
        case T_CHAR:
	...
	public void genOutputAddr(int reg, Context ctx) {
	  if (convertTo==null) ex.genOutputAddr(reg, ctx);
	  else compErr(ctx, "ExEnc.genOutputAddr is invalid for converted types");
	}
  ...
  public boolean canGenAddr(Unit unitContext, boolean allowSpecialWriteAccess, int resolveFlags, Context ctx) {
	...
	public void genOutputVal(int reg, Context ctx) {
      ...
      case T_INT:
        ...
        break;
      case T_LONG:
        ctx.arch.genLoadConstDoubleOrLongVal(reg, getConstLongValue(ctx),
            getRegType(ctx)==StdTypes.T_DBL);
        break;
      default:
	      if (convertTo==null || isStructType()) ex.genOutputVal(reg, ctx);
	      else {
	        restore=ctx.arch.prepareFreeReg(0, 0, reg, ex.getRegType(ctx));
	        oreg=ctx.arch.allocReg();
	        ex.genOutputVal(oreg, ctx);
	        if (baseType>0 && arrDim==0) ctx.arch.genConvertVal(reg, oreg, baseType, ex.baseType);
	        else genIsType(reg, oreg, true, ctx);
	...
	public void genOutputCondJmp(Instruction jumpDest, boolean isTrue, Instruction elseDest, Context ctx) {
	  if (convertTo!=null) compErr(ctx, "ExEnc.genOutputCondJmp with conversion is invalid");
    else ex.genOutputCondJmp(jumpDest, isTrue, elseDest, ctx);
	}
  ...
  private boolean getFloatOfEnc(int i, long l, Context ctx) {
    switch (ex.baseType) { //fromType
      case T_BYTE: case T_SHRT: case T_CHAR: case T_INT:
        constInt=ctx.arch.real.buildFloat((long)i, 0);
        return false;
      case T_LONG:
  ...
  private boolean getDoubleOfEnc(int i, long l, Context ctx) {
    switch (ex.baseType) { //fromType
      case T_BYTE: case T_SHRT: case T_CHAR: case T_INT:
        constLong=ctx.arch.real.buildDouble((long)i, 0);
        return false;
      case T_LONG:
