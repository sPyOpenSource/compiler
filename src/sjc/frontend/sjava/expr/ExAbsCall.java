...
public abstract class ExAbsCall extends Expr {
    ...
    if (SJava.isSpecialName(name)) {
	  ...
	  pa=par;
	  while (pa!=null) {
	    if (!pa.expr.resolve(unitContext, mthdContext, resolveFlags|RF_CHECKREAD, null, ctx)) return false;
	    pa=pa.nextParam;
	  }
	  ...
	  //search matching method/constructor
	    ...
	    if (inUnit.name.charAt(0)=='$' && name.equals(inUnit.name)) { //check if constructor for anonymous class
      ...
      if ((dest=searchCalledMethod(inUnit, asConstr, (mthdContext.marker&Marks.K_EXPC)==0, name, par, ctx))==null) return false;
      if (!checkImplicitConverts(unitContext, ctx)) return false;
    }
	  //check modifiers
	  ...
	  if ((accessLevel=unitContext.minimumAccessLevel(dest, dest.name,
	      dest.owner, dest.modifier, inUnit==unitContext || partOfSuper, ctx))
	      ==Modifier.M_ERROR) return false;
    ...
    //check throwables
    if ((curThrows=dest.throwsList)!=null) while (curThrows!=null) {
      if (ctx.excChecked.isParent(curThrows.qid.unitDest, ctx)
          && !mthdContext.handlesThrowable(this, curThrows.qid.unitDest, ctx))
        return false;
  ...
  private Mthd searchCalledMethod(Unit inUnit, boolean asConstr, boolean allowBaseTypeConv, String mthdName, FilledParam par, Context ctx) {
    ...
    do {
      while (mthd!=null) {
        if (asConstr==mthd.isConstructor //check only if method/constructor,
            && mthd.parCnt==parCnt && mthdName.equals(mthd.name)) { //name and number of parameters match
          cur=mthd.checkParamConversions(par, allowBaseTypeConv && (mthd.marker&Marks.K_EXPC)==0, ctx);
          if (cur!=-1) { //convertable
            if (best==-1 || cur<best) { //nothing found or better match than match bevor, use current
              dest=mthd;
              if ((outerLevel=curLevel)>0) outerAccessStart=inUnit;
              best=cur;
              found=1;
            }
            else if (cur==best) { //match with same count of conversions, possibly ambiguous
              //check if method is not overloaded => ambiguous
              if (!dest.isOverloadedBy(mthd) && !mthd.isOverloadedBy(dest)) found++;
            }
          }
        }
        mthd=mthd.nextMthd;
      }
      if (dest!=null && (dest.modifier&Modifier.M_STAT)!=0) break; //terminate search: found static destination
      if (curClass.extsID!=null) { //switch to parent
        mthd=(curClass=curClass.extsID.unitDest).mthds;
      }
      else if (lastInner.outerUnit!=null && found==0) { //switch to last known class and switch to outer, if not method found so far
        mthd=(lastInner=curClass=lastInner.outerUnit).mthds;
        curLevel++;
      }
      else break; //terminate search: no other class to search in
    } while (curClass!=null);
    if (found==1) return dest;
    ...
    printPos(ctx, null);
    if (asConstr) ctx.out.print(": constructor ");
    else ctx.out.print(": method ");
    ctx.out.print(mthdName);
    ctx.out.print("(");
    if (par!=null) par.printTypes(ctx.out);
    if (found==0) {
      ctx.out.print(") not found");
      if (asConstr) ctx.out.print(" (has parent an explicit constructor?)");
    }
    else ctx.out.print(") matches multiple times");
    return null;
  }
  
  protected void genOutputPar(Context ctx) { //called from genOutputVal and ExDeref
    FilledParam pa;
    int off, reg, type;
    
    pa=par;
    if ((dest.modifier&Modifier.M_NAT)!=0) { //special case: C-style parameter order
      ...
      while (pa!=null) {
        ...
        pa=pa.nextParam;
        switch (type) { //increase stack offset for next parameter
          case StdTypes.T_DPTR: off+=ctx.arch.relocBytes*2; break;
          case StdTypes.T_PTR: off+=ctx.arch.relocBytes; break;
          default: off+=getMinSize(type);
        }
        off=(off+ctx.arch.stackClearBits)&~ctx.arch.stackClearBits;
      }
    }
    else while (pa!=null) { //normal case: generate and push parameters
  ...
  protected void genOutputCall(int reg, int obj, Context ctx) { //called from ExDeref and ExCall.genOutputVal (latter not for intf)
    int type;
    boolean forceInline;
    
    if ((dest.modifier&Modifier.M_NAT)!=0) { //call native method
      ctx.arch.genCallNative(!ctx.embedded ? dest.nativeAddress.owner.outputLocation : ctx.ramLoc, dest.nativeAddress.relOff, ctx.dynaMem,
          dest.parSize, (dest.marker&Marks.K_NWIN)!=0);
    }
    else if (!callViaIntf) { //normal call via object or class
      if (!implContextSwitch) {
        if (outerLevel>0) loadOuterInstance(ctx); //caller must have saved instance context already
      }
      else if (ctx.dynaMem) {
      ...
      if ((forceInline=(dest.marker&Marks.K_FINL)!=0) && ctx.arch.mayInline() && !dest.inGenOutput) {
        ...
        if (!ctx.dynaMem && ((dest.modifier&(Modifier.M_STAT|Modifier.M_FIN|Modifier.M_PRIV))!=0
            || (dest.modifier&Modifier.M_OVERLD)==0)) ctx.arch.genCallConst(dest, dest.parSize);
        else ctx.arch.genCall(dest.relOff, ctx.arch.regClss, dest.parSize);
      }
    }
    else { //call via interface-object (only from ExDeRef)
      ctx.arch.genCallIndexed(obj, ctx.rteSIntfMap.instScalarTableSize+dest.relOff*4,
          dest.parSize);
    }
    //move result to desired register
    if (reg!=0 && (type=getRegType(ctx))!=0) ctx.arch.genMoveFromPrimary(reg, type);
  ...
  private boolean checkImplicitConverts(Unit unitContext, Context ctx) { //dest and par are valid at this time
    FilledParam fp=par;
    Param dp=dest.param;
    int cmpRes;
    ...
    while (fp!=null) {
      ...
      if (dp.type.baseType!=TypeRef.T_QID && dp.type.arrDim==0) { //this is the case which we dont support
        if (dp.type.baseType!=fp.expr.baseType) { //if we come to this point, implicit base type conversion was enabled
          if ((fp.expr=ExEnc.getConvertedResolvedExpr(fp.expr, dp.type, unitContext, ctx))==null) return false;
        }
      }
      if (fp.expr.baseType==T_NULL) {
        if (dp.type.isIntfType()) fp.expr.baseType=T_NDPT;
        else fp.expr.baseType=T_NNPT;
      }
      else {
        cmpRes=fp.expr.compareType(dp.type, true, ctx); //get type-compare
        if (cmpRes==TypeRef.C_OT && dp.type.isIntfType()) { //insert implicit conversion of object to interface
