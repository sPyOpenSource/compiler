...
import sjc.compbase.expr.ExConstNew;
...
public class ExNew extends ExAbsCall {
  ...
  public boolean resolve(Mthd enclosingMthd, Unit unitContext, Mthd mthdContext, int resolveFlags, TypeRef preferredType, Context ctx) {
    ...
    if (!obj.resolveType(unitContext, ctx)) return false;
    ...
    if (asArray) { //create new array
      canBeDoneAtCompileTime=mthdContext==unitContext.initStat;
      //resolve parameter
      pa=par;
      while (pa!=null) {
        if (pa.expr==null) break; //no more parameters, just for type-setting
        //not empty parameter, resolve it
        if (!pa.expr.resolve(unitContext, mthdContext, resolveFlags|RF_CHECKREAD, ctx.intType, ctx)) return false;
        if (!pa.expr.isIntType()) {
          printPos(ctx, "expression to size an array has to be of type int");
          return false;
        }
        if (pa.expr.calcConstantType(ctx)!=StdTypes.T_INT) canBeDoneAtCompileTime=false; //not a constant dimension
        pa=pa.nextParam;
        if (pa!=null) multArray=true;
      }
      //check type of array
      ...
      if (canBeDoneAtCompileTime && constNew==null && !(constNew=new ExConstNew(fileID, line, col))
          .fillConstNew(unitContext, destTypeUnit, obj, par, resolveFlags, ctx)) return false; //create constNew-container
    }
    else { //no array, must be a new Object
      if  (obj.baseType!=T_QID) {
      ...
      //check access
      if ((accessLevel=unitContext.minimumAccessLevel(destTypeUnit, destTypeUnit.name,
          destTypeUnit, destTypeUnit.modifier, true, ctx))==Modifier.M_ERROR) return false;
      ...
      if (!resolveInMthd(obj.qid.getLastPID(), true, destTypeUnit,
          destTypeUnit.initDyna, unitContext, mthdContext, resolveFlags, false, ctx)) return false;
      ...
      if (dest!=destTypeUnit.initDyna) callExplicitConstr=true;
      //check if object may be allocated at compile-time
      if ((canBeDoneAtCompileTime=(mthdContext==unitContext.initStat && !callExplicitConstr))
          && constNew==null && !(constNew=new ExConstNew(fileID, line, col))
              .fillConstNew(unitContext, destTypeUnit, obj, null, resolveFlags, ctx)) return false; //create constNew-container
    }
  ...
  public void genOutputVal(int reg, int outerClassReg, Context ctx) {
    ...
    if (reg==0) { //allocate register if it is not given by outer expression
      specialRestore=ctx.arch.prepareFreeReg(0, 0, 0, StdTypes.T_PTR);
      reg=ctx.arch.allocReg();
      doSpecialRestore=true;
    }
    if (asArray) { //create array
      if (ctx.dynaMem) ctx.arch.genSaveUnitContext(); //save current class context for call
      ...
      if (obj.baseType!=TypeRef.T_QID) { //basic type
        ...
        if ((obj.qid.unitDest.modifier&Modifier.M_STRUCT)!=0) ctx.arch.genPushConstVal(0, StdTypes.T_PTR); //push null if array of struct
        else if (ctx.dynaMem) ctx.arch.genLoadUnitContext(tmpPtr, importedClass.relOff); //load address of destination type 
        else ctx.arch.genLoadConstUnitContext(tmpPtr, destTypeUnit.outputLocation); //load address of destination type
      ...
      else ctx.arch.genCallConst(ctx.rteDRNewArrayMd, ctx.rteDRNewArrayMd.parSize); //call runtime system
      ...
      if (ctx.dynaMem) ctx.arch.genRestUnitContext(); //save current class context
      if (multArray) {
        p=par.nextParam;
        level=1;
        while (p!=null) { //generate call to rteDRNewMultArray for each parameter==level
          level++;
          if (ctx.dynaMem) ctx.arch.genSaveUnitContext(); //save current instance context for call
          ...
          if (obj.baseType!=TypeRef.T_QID) { //basic type
            ...
            restore2=ctx.arch.prepareFreeReg(0, 0, reg, StdTypes.T_PTR);
            tmpPtr=ctx.arch.allocReg();
            if (ctx.dynaMem) ctx.arch.genLoadUnitContext(tmpPtr, importedClass.relOff); //load address of destination type 
            else ctx.arch.genLoadConstUnitContext(tmpPtr, destTypeUnit.outputLocation); //load address of destination type
            ctx.arch.genPush(tmpPtr, StdTypes.T_PTR); //push that address
            ctx.arch.deallocRestoreReg(tmpPtr, 0, restore2);
          }
          ...
          else ctx.arch.genCallConst(ctx.rteDRNewMultArrayMd, ctx.rteDRNewMultArrayMd.parSize); //call runtime system
          ctx.arch.genMoveFromPrimary(reg, StdTypes.T_PTR); //load result in desired register
          ctx.arch.deallocRestoreReg(0, 0, restore); //restore saved registers
          if (ctx.dynaMem) ctx.arch.genRestUnitContext(); //save current instance context
          p=p.nextParam;
        }
      }
    }
    else if (callExplicitConstr && (dest.modifier&Modifier.M_EXINIT)!=0) { //explicit allocation, no object allocation here
      restore=ctx.arch.ensureFreeRegs(reg, 0, 0, 0); //save registers
    ...
    else { //constructor for normal object
      if (callExplicitConstr) {
        restoreCall=ctx.arch.ensureFreeRegs(reg, 0, outerClassReg, 0); //save registers
        ctx.arch.genSaveInstContext(); //save current instance context for constructor
        genOutputPar(ctx); //generate parameters for explicit call
      }
      else if (ctx.dynaMem) ctx.arch.genSaveUnitContext(); //save current class context for new-call
      ...
      if (!ctx.alternateObjNew) {
        ...
        if (ctx.indirScalars) ctx.arch.genPushConstVal(destTypeUnit.instIndirScalarTableSize, StdTypes.T_INT); //push indirect scalarsize of destination object
      ...
      tmpPtr=ctx.arch.allocReg();
      if (ctx.dynaMem) ctx.arch.genLoadUnitContext(tmpPtr, importedClass.relOff); //load address of destination type indirectly
      else ctx.arch.genLoadConstUnitContext(tmpPtr, destTypeUnit.outputLocation); //load address of destination type directly
      ...
      else ctx.arch.genCallConst(ctx.rteDRNewInstMd, ctx.rteDRNewInstMd.parSize); //call runtime system
  	  ctx.arch.genMoveFromPrimary(reg, StdTypes.T_PTR); //load result in desired register
      ctx.arch.deallocRestoreReg(0, reg, restore); //restore saved registers
      if (destTypeUnit.outerUnit!=null && (destTypeUnit.modifier&Modifier.M_STAT)==0) { //remember current instance as outer instance
      ...
      if (callExplicitConstr) { //generate explicit call, parameters are already on stack
        ctx.arch.genLoadInstContext(reg); //load instance and context
        genOutputCall(0, 0, ctx); //generate call, no output
        if (!doSpecialRestore) ctx.arch.genDup(reg, ctx.arch.regInst, StdTypes.T_PTR); //copy instance pointer to result
      ...
      else if (ctx.dynaMem) ctx.arch.genRestUnitContext(); //restore current class context
    }
    if (doSpecialRestore) ctx.arch.deallocRestoreReg(reg, 0, specialRestore); //restore register environment if required
  }
}
