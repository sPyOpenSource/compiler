...
public class ExUna extends Expr {
    ...
    switch (opType) {
      case Ops.S_ARI:
        ...
        else if ((baseType!=T_BYTE && baseType!=T_SHRT && baseType!=T_INT && baseType!=T_LONG)
            || (opPar!=Ops.A_PLUS && opPar!=Ops.A_MINUS && opPar!=Ops.A_CPL)) {
    ...
    if (calcConstantType(ctx)==0 && (ctx.arch.unaAriCall[baseType]&(1<<(opPar-Ops.MSKBSE)))!=0) {
      ...
      switch (opPar) {
        ...
        case Ops.A_CPL:   ariCallOp='~'; break;
        case Ops.L_NOT:   ariCallOp='!'; break;
        default:
    ...
    par=op&0xFFFF;
	  switch (opType) {
	    case Ops.S_ARI:
	      switch (par) {
          case Ops.A_PLUS: return v;
					case Ops.A_MINUS: return baseType==T_FLT ? ctx.arch.real.negateFloat(v) : -v;
          case Ops.A_CPL: return ~v;
	      }
	      break;
      case Ops.S_LOG:
        switch (par) {
          case Ops.L_NOT: return v^1;
        }
	  }
    ...
    switch (opType) {
      case Ops.S_ARI:
        switch (par) {
          case Ops.A_PLUS: return v;
          case Ops.A_MINUS: return baseType==T_DBL ? ctx.arch.real.negateDouble(v) : -v;
          case Ops.A_CPL: return ~v;
        }
        break;
    }
    ...
    if (genAriCall) genOutAriCall(reg, preVal, ctx);
    else switch (opType) {
      case Ops.S_ARI:
        switch (opPar) {
          case Ops.A_PLUS:
