...
public class ExCall extends ExAbsCall {
  ...
  public void printExpression(CodePrinter codePrnt) {
	...
	public boolean mthdResolve(Unit inUnit, Mthd inMthd, Unit unitContext, Mthd mthdContext, int resolveFlags, boolean partOfSuper, Context ctx) {
	  ...
	  implContextSwitch=((dest.modifier&Modifier.M_STAT)!=0) &&
	    (dest.owner!=unitContext || (mthdContext.modifier&Modifier.M_STAT)==0);
    //get return-type
  ...
  public boolean resolveSuperThisCall(Unit inUnit, Mthd inMthd, Context ctx) {
    ...
    if (callSuper) {
      if (inUnit==ctx.langRoot) { //no parent existing
        effectType=EF_NORM;
        noCall=true;
        return true;
      }
    ...
    else calledConstrUnit=inUnit;
    //resolve call
    if (!resolveInMthd(calledConstrUnit.name, true,
        calledConstrUnit, calledConstrUnit.initDyna, inUnit, inMthd, RF_NONE, true, ctx)) return false;
    if (dest==calledConstrUnit.initDyna) noCall=true; //implicit constructor, no effect
    else {
      if (callSuper) implContextSwitch=true; //always switch to class of parent if call to super
      else if (dest==inMthd) { //check recursion
	...
	public void genOutputVal(int reg, Context ctx) { //not called from ExDeref
	  int restore;
	  
    if (noCall) return; //nothing to do, just return
	  //prepare call
	  restore=ctx.arch.ensureFreeRegs(reg, 0, 0, 0);
	  //check if call needs context switch
    if (outerLevel>0) ctx.arch.genSaveInstContext();
    else if (implContextSwitch && ctx.dynaMem) ctx.arch.genSaveUnitContext();
    ...
    if (outerLevel>0) ctx.arch.genRestInstContext();
    else if (implContextSwitch && ctx.dynaMem) ctx.arch.genRestUnitContext();
	  //restore registers
