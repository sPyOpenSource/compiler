...
package sjc.frontend.sjava.expr;
...
public class ExBin extends ExCheckType {
  ...
  public boolean resolve(Unit unitContext, Mthd mthdContext, int resolveFlags, TypeRef preferredType, Context ctx) {
    ...
    if (!le.resolve(unitContext, mthdContext,
        ((opType!=Ops.S_ASN || opPar!=0) ? resolveFlags : (resolveFlags&~RF_CHECKREAD)), null, ctx)
        || !ri.resolve(unitContext, mthdContext, resolveFlags,
            (opType==Ops.S_ASN || opType==Ops.S_CMP) ? le : null, ctx)) return false;
    //try to get resulting type
    switch (opType) { //resulting type depends on operator
      case Ops.S_ASN: //le has to be var and the resulting type has to match type of le
      case Ops.S_ASNARI: case Ops.S_ASNBSH: //mostly the same checks have to be done
          ...
          if (opType!=Ops.S_ASN) {
            ...
            if (cmpRes==TypeRef.C_NP || cmpRes==TypeRef.C_OT) {
              if ((mthdContext.marker&Marks.K_EXPC)==0) {
                ...
                if (destType!=null) {
                  if ((ri=ExEnc.getConvertedResolvedExpr(ri, destType, unitContext, ctx))==null) return false;
                }
                else { noImplicitConvert(ctx); return false; }
              }
              else { noImplicitConvert(ctx); return false; }
            }
            if (cmpRes==TypeRef.C_TT && le.isIntfType()) { //insert implicit conversion of object to interface
              if ((ri=ExEnc.getConvertedResolvedExpr(ri, le, unitContext, ctx))==null) return false;
            }
          }
        }
        getTypeOf(le); //required for checkAriBasicsAndCall
        if ((le.isObjType() || le.isIntfType())
            && (ctx.assignCall || (ctx.assignHeapCall && !le.isAddrOnStack()))) {
          if (ctx.dynaMem) runtimeClass=unitContext.getRefUnit(ctx.rteDynamicRuntime, true);
          genAssignCall=true;
        }
        else switch (opType) {
          case Ops.S_ASNARI:
            ...
            if (!checkAriBasicsAndCall(opPar, unitContext, ctx)) return false;
            break;
          case Ops.S_ASNBSH:
            if (baseType<0) {
              printPos(ctx, ARINOTOBJ);
            ...
            if (le.baseType!=T_BYTE && le.baseType!=T_SHRT && le.baseType!=T_CHAR
                && le.baseType!=T_INT && le.baseType!=T_LONG) {
      ...
      case Ops.S_CMP: //if types are equal or convertable, the result is a boolean value
        if (opPar==Ops.C_INOF) { //instanceof
          if (le.baseType!=T_QID) {
            printPos(ctx, "only objects can be instanceof a class");
            return false;
          }
          if (!prepareObjectConversion(le, ri, unitContext, ctx)) return false;
        }
        else { //all other compares
          if (le.baseType==T_NULL) {
            if (ri.baseType==T_NULL) { //compare constant null with constant null ?!?
          ...
          if (ri.baseType==T_NULL) { //check null-type
            if (!assignNullType(ctx)) return false;
          }
          else {
            if (le.compareType(ri, true, ctx)==TypeRef.C_NP) {
              if ((mthdContext.marker&Marks.K_EXPC)==0) {
                if (!checkImplicitIntBaseTypeConversion(unitContext, ctx)) return false;
            ...
            if (le.baseType<0 || le.arrDim>0 || ri.baseType<0 || ri.arrDim>0) { //compare pointers, check type of compare
        ...
        if (le.calcConstantType(ctx)!=0 && ri.calcConstantType(ctx)==0) {
          change=false;
          switch (opPar) { //if left side is constant and right side is not, try to change sides
            case Ops.C_GE: opPar=Ops.C_LE; change=true; break;
            case Ops.C_GT: opPar=Ops.C_LW; change=true; break;
            case Ops.C_LE: opPar=Ops.C_GE; change=true; break;
            case Ops.C_LW: opPar=Ops.C_GT; change=true; break;
            case Ops.C_EQ: case Ops.C_NE: change=true; break;
          }
          if (change) {
            tmpEx=le;
            le=ri;
      ...
      case Ops.S_ARI: //types have to be equal or convertable
        if ((mthdContext.marker&Marks.K_EXPC)==0 && !checkImplicitIntBaseTypeConversion(unitContext, ctx)) return false;
        ...
        if (!checkAriBasicsAndCall(opPar, unitContext, ctx)) return false;
        if (le.calcConstantType(ctx)!=0 && ri.calcConstantType(ctx)==0) switch (opPar) { //if left side is constant and right side is not, try to change sides
          case Ops.A_AND: case Ops.A_OR: case Ops.A_XOR:
          case Ops.A_MUL: case Ops.A_PLUS: 
            tmpEx=le;
            le=ri;
            ri=tmpEx;
            break;
      ...
      case Ops.S_BSH:
        if ((mthdContext.marker&Marks.K_EXPC)==0 && ri.arrDim==0 && (ri.baseType==T_BYTE || ri.baseType==T_SHRT || ri.baseType==T_CHAR)) {
          if ((ri=ExEnc.getConvertedResolvedExpr(ri, ctx.intType, unitContext, ctx))==null) return false;
        }
        if (!ri.isIntType()) {
          printPos(ctx, "right operand of shift has to be int");
          return false;
        }
        if (le.arrDim!=0 || (le.baseType!=T_BYTE && le.baseType!=T_SHRT && le.baseType!=T_CHAR
            && le.baseType!=T_INT && le.baseType!=T_LONG)) {
          printPos(ctx, "left operand of shift has to be byte, short, char, int or long");
          return false;
        }
        if ((mthdContext.marker&Marks.K_EXPC)==0 && le.baseType!=T_INT && le.baseType!=T_LONG) {
          if ((le=ExEnc.getConvertedResolvedExpr(le, ctx.intType, unitContext, ctx))==null) return false;
        }
        getTypeOf(le);
        break;
      case Ops.S_LOG: //logical operators need boolean values as operands and have boolean result
        if (!le.isBoolType() || !ri.isBoolType()) { noImplicitConvert(ctx); return false; }
        baseType=T_BOOL;
        break;
      default:
    ...
    if (opType==Ops.S_ASN || opType==Ops.S_ARI) {
      ...
      if ((opPar==Ops.A_DIV || opPar==Ops.A_MOD) 
          && ((rcType==T_INT && ri.getConstIntValue(ctx)==0)
          || (rcType==T_LONG && ri.getConstLongValue(ctx)==0l))) {
  ...
  private boolean checkImplicitIntBaseTypeConversion(Unit unitContext, Context ctx) {
    ...
    if (dest!=null) {
      if (le.baseType!=dest.baseType
          && (le=ExEnc.getConvertedResolvedExpr(le, dest, unitContext, ctx))==null) return false;
      if (ri.baseType!=dest.baseType
          && (ri=ExEnc.getConvertedResolvedExpr(ri, dest, unitContext, ctx))==null) return false;
      return true;
    }
    if (le.baseType==ri.baseType) return true;
    noImplicitConvert(ctx);
    return false;
  }
  ...
  private boolean checkAriBasicsAndCall(int opPar, Unit unitContext, Context ctx) {
    ...
    if (calcConstantType(ctx)==0 && (ctx.arch.binAriCall[baseType]&(1<<(opPar-Ops.MSKBSE)))!=0) {
      if (ctx.dynaMem) runtimeClass=unitContext.getRefUnit(ctx.rteDynamicAri, true);
      genAriCall=true;
      ctx.arch.binAriCall[baseType]|=0x80000000; //set sign-bit to request service
      switch (opPar) {
        case Ops.A_PLUS:  ariCallOp='+'; break;
        case Ops.A_MINUS: ariCallOp='-'; break;
        case Ops.A_MUL:   ariCallOp='*'; break;
        case Ops.A_DIV:   ariCallOp='/'; break;
        case Ops.A_MOD:   ariCallOp='%'; break;
        default:
  ...
  public int calcConstantType(Context ctx) {
    ...
    if (arrDim>0) return 0;
    opType=op>>>16;
    if (opType!=Ops.S_ARI && opType!=Ops.S_BSH
      && opType!=Ops.S_LOG && opType!=Ops.S_CMP) return 0;
    if (le.calcConstantType(ctx)==0 || ri.calcConstantType(ctx)==0) return 0;
    switch (baseType) {
      case T_BYTE: case T_SHRT: case T_INT:
      case T_CHAR: case T_BOOL: case T_FLT:
        return T_INT;
      case T_LONG: case T_DBL:
        return T_LONG;
    }
    return 0;
  }
  ...
  public int getConstIntValue(Context ctx) {
    ...
    if (opType==Ops.S_CMP) {
      if (le.calcConstantType(ctx)==T_INT && ri.calcConstantType(ctx)==T_INT) {
        ...
        if (le.baseType==StdTypes.T_FLT) return ctx.arch.real.compareConstFloat(l, r, par);
        switch (par) {
          case Ops.C_LW: if (l<r) return 1; else return 0;
          case Ops.C_LE: if (l<=r) return 1; else return 0;
          case Ops.C_EQ: if (l==r) return 1; else return 0;
          case Ops.C_GE: if (l>=r) return 1; else return 0;
          case Ops.C_GT: if (l>r) return 1; else return 0;
          case Ops.C_NE: if (l!=r) return 1; else return 0;
        }
      }
      if (le.calcConstantType(ctx)==T_LONG && ri.calcConstantType(ctx)==T_LONG) {
        ll=le.getConstLongValue(ctx);
        lr=ri.getConstLongValue(ctx);
        if (le.baseType==StdTypes.T_DBL) return ctx.arch.real.compareConstDouble(ll, lr, par);
        switch (par) {
          case Ops.C_LW: if (ll<lr) return 1; else return 0;
          case Ops.C_LE: if (ll<=lr) return 1; else return 0;
          case Ops.C_EQ: if (ll==lr) return 1; else return 0;
          case Ops.C_GE: if (ll>=lr) return 1; else return 0;
          case Ops.C_GT: if (ll>lr) return 1; else return 0;
          case Ops.C_NE: if (ll!=lr) return 1; else return 0;
        }
      }
      compErr(ctx, INVCALLGETCONST);
      return 0;
    ...
    l=le.getConstIntValue(ctx);
    r=ri.getConstIntValue(ctx);
    if (baseType==T_FLT) return ctx.arch.real.binOpFloat(l, r, par);
    switch (opType) {
      case Ops.S_ARI:
        switch (par) {
          case Ops.A_AND: return l&r;
          case Ops.A_OR: return l|r;
          case Ops.A_XOR: return r^r;
          case Ops.A_PLUS: return checkBits(l+r);
          case Ops.A_MINUS: return checkBits(l-r);
          case Ops.A_MUL: return checkBits(l*r);
          case Ops.A_DIV: if (r==0) { compErr(ctx, "division by zero"); ctx.out.println(); return 0; } else return checkBits(l/r);
          case Ops.A_MOD: return checkBits(l%r);
        }
        break;
      case Ops.S_BSH:
        switch (par) {
          ...
          case Ops.B_SHRL: return l>>>r;
          case Ops.B_SHRA: return l>>r;
        }
        break;
      case Ops.S_LOG:
        switch (par) {
          case Ops.L_AND: if (l!=0 && r!=0) return 1; else return 0;
          case Ops.L_OR: if (l!=0 || r!=0) return 1; else return 0;
        }
        break;
    }
    compErr(ctx, INVCALLGETCONST);
  ...
  private int checkBits(int v) {
    switch (baseType) {
      case T_BYTE: return (int)((byte)v);
      case T_SHRT: return (int)((short)v);
      case T_CHAR: return (int)((char)v);
    }
    return v;
  }
  ...
  public long getConstLongValue(Context ctx) {
    ...
    if (opType==Ops.S_BSH) {
      ...
      r=ri.getConstIntValue(ctx);
      switch (par) {
        case Ops.B_SHL: return ll<<r;
        case Ops.B_SHRL: return ll>>>r;
        case Ops.B_SHRA: return ll>>r;
      }
    }
    else {
      ...
      lr=ri.getConstLongValue(ctx);
      if (baseType==T_DBL) return ctx.arch.real.binOpDouble(ll, lr, par);
      if (opType==Ops.S_ARI) {
        switch (par) {
          case Ops.A_AND: return ll&lr;
          case Ops.A_OR: return ll|lr;
          case Ops.A_XOR: return lr^lr;
          case Ops.A_PLUS: return ll+lr;
          case Ops.A_MINUS: return ll-lr;
          case Ops.A_MUL: return ll*lr;
          case Ops.A_DIV: if (lr==0l) { printPos(ctx, "division by zero"); ctx.out.println(); } else return ll/lr;
          case Ops.A_MOD: return ll%lr;
        }
      }
    }
    compErr(ctx, INVCALLGETCONST);
  ...
  public void genOutputVal(int reg, Context ctx) {
    ...
    switch (opType) {
      case Ops.S_ASN:
        if (!genAssignCall) le.genOutputAssignTo(reg, ri, ctx);
        else {
          if (reg==0) {
          ...
          if (ctx.dynaMem) ctx.arch.genSaveUnitContext();
          if (isIntfType()) {
          ...
          else ctx.arch.genCallConst(ctx.rteDRAssignMd, ctx.rteDRAssignMd.parSize);
          ctx.arch.deallocRestoreReg(0, reg, restore2);
          ctx.arch.deallocRestoreReg(addr, 0, restore1);
          if (deallocReg) ctx.arch.deallocRestoreReg(reg, 0, restoreReg);
        }
        return;
      case Ops.S_ASNARI:
        if (chkAriErr(ctx)) return;
        if (reg==0) {
      ...
      case Ops.S_ASNBSH:
        ...
        if (riConstType==0) {
          ...
          if (riConstType==T_INT) ctx.arch.genBinOpConstRi(reg, reg1, ri.getConstIntValue(ctx), (Ops.S_BSH<<16)|opPar, baseType);
          else compErr(ctx, "bitshift with constant right but not int type");
        }
        ctx.arch.deallocRestoreReg(reg1, reg, restore2);
        ctx.arch.genAssign(addr, reg, baseType);
        ctx.arch.deallocRestoreReg(addr, 0, restore1);
      ...
      case Ops.S_BSH: //no type-check, already done by exSubResolve
        ...
        else ctx.arch.genBinOpConstRi(reg, reg1, ri.getConstIntValue(ctx), op, baseType);
        ctx.arch.deallocRestoreReg(reg1, reg, restore1);
        return;
    }
  ...
  private void genOutAriCall(int reg, int reg1, int reg2, Context ctx) {
    ...
    ctx.arch.genPushConstVal((int)ariCallOp, T_BYTE);
    if (ctx.dynaMem) {
      ctx.arch.genLoadUnitContext(ctx.arch.regClss, runtimeClass.relOff);
      ctx.arch.genCall(target.relOff, ctx.arch.regClss, target.parSize);
      ctx.arch.genRestUnitContext();
    }
    else ctx.arch.genCallConst(target, target.parSize);
  ...
  public void genOutputCondJmp(Instruction jumpDest, boolean isTrue, Instruction elseDest, Context ctx) {
    ...
    if (calcConstantType(ctx)==T_INT) {
      ...
      if ((val1==1 && isTrue) || (val1==0 && !isTrue)) ctx.arch.genJmp(jumpDest);
      //else: don't jump
      return;
    }
    if (le.baseType<0 || ri.baseType<0 || le.arrDim>0 || ri.arrDim>0) { //compare pointers or check instanceof
      ...
      if (opPar==Ops.C_INOF) {
        ...
        else condHnd=ctx.arch.genCompPtrToNull(addr1, isTrue ? opPar : opPar^Ops.INVCBIT);
        ctx.arch.deallocRestoreReg(addr1, 0, restore1);
        ctx.arch.genCondJmp(jumpDest, condHnd);
      }
      return;
