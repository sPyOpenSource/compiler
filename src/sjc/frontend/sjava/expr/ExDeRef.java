...
import sjc.frontend.sjava.Magic;
...
public class ExDeRef extends Expr {
  ...
  public ExDeRef(int fid, int il, int ic) {
    super(fid, il, ic);
    isSuper=false; //super needs a very special handling
  }
  
  public void printExpression(CodePrinter codePrnt) {
    if (isSuper) codePrnt.exprSuper(ri);
    else if (le.baseType==StdTypes.T_MAGC) magic.printExpression(ri, codePrnt); 
    else codePrnt.exprDeref(le, ri, leftStatic);
  }
  ...
  public boolean resolve(Unit unitContext, Mthd mthdContext, int resolveFlags, TypeRef preferredType, Context ctx) {
    ...
    if (le instanceof ExVar) { //this is the left-out-identifier, only this one has full search-space
      asVar=(ExVar)le;
      if (asVar.id.equals(SJava.KEY_MAGIC)) {
        ...
        isSuper=true;
        if (ctx.dynaMem) leImportedClassIndexed=asVar.importedClass; 
      }
      else if (!asVar.varResolve(null, unitContext, mthdContext, unitContext, mthdContext,
          resolveFlags|RF_CHECKREAD, false, null, ctx)) return false;
    }
    ...
    if (le.arrDim>0) {
      if (le.typeSpecial!=TypeRef.S_FLASHREF && le.typeSpecial!=TypeRef.S_NOSPECIAL) { //STRUCT and inline arrays are forbidden
      ...
      case StdTypes.T_MAGC:
        ...
        if ((ri=(magic=(Magic)config).resolve(ri, unitContext, mthdContext, resolveFlags, preferredType, ctx))==null) return false;
        getTypeOf(ri);
      ...
      case StdTypes.T_PACK:
        ...
        break;
      default:
        if (le.baseType==TypeRef.T_QID) { //this is an instance of an object
          if (le.qid==null || le.qid.unitDest==null) {
            compErr(ctx, ERRQID);
            return false;
          }
    ...
    if (ri instanceof ExCall) {
      ...
      asVar=(ExVar)ri;
      if (asVar.id.equals(SJava.KEY_THIS)) { //special case for (inner) class referencing (outer) instance
      ...
      else { //there is a name to resolve
        if (!asVar.varResolve(inPack, inUnit, inMthd, unitContext, mthdContext, resolveFlags,
            true, preferredType, ctx)) {
          ctx.out.print(ERREXT);
          return false;
        }
        if (inPack!=null) leftStatic=noGenLeftSide=true;
        ...
        if (asVar.dest!=null && (asVar.dest.modifier&Modifier.M_STAT)!=0) leftStatic=true;
      }
    ...
    if (ctx.runtimeNull && ctx.dynaMem) runtimeClass=unitContext.getRefUnit(ctx.rteDynamicRuntime, true);
  ...
  public void genOutputAddr(int reg, Context ctx) {
    ...
    if (le.baseType==StdTypes.T_MAGC) { //left side is "MAGIC", this is an error
      compErr(ctx, "ExDeref.genOutputAddr not valid for MAGIC");
    }
    else if (noGenLeftSide) ri.genOutputAddr(reg, ctx);
    else if (calcConstantType(ctx)!=0) compErr(ctx, "ExDeref.genOutputAddr not valid for constant value");
    else if (ri instanceof ExCall) compErr(ctx, "ExDeref.genOutputAddr not valid for call"); //super is supported only for calls => print same message
    else if (ri instanceof ExVar) {
      asVar=(ExVar)ri;
      if (le.baseType==StdTypes.T_DESC) { //left side just a reference to a unit
        asVar.useResu=true;
        if (ctx.dynaMem) {
        ...
        else asVar.genOutputAddr(reg, ctx);
      }
      else { //left side is something that returns an object
        asVar.useResu=true;
        restore2=ctx.arch.prepareFreeReg(0, 0, reg, le.getRegType(ctx));
        extReg=ctx.arch.allocReg();
    ...
    else compErr(ctx, "ExDeref.genOutputAddr with invalid type of ri");
  }
  ...
  public void genOutputVal(int reg, Context ctx) {
    ...
    if (le.baseType==StdTypes.T_MAGC) magic.genOutput(reg, ri, ctx); //left side is "MAGIC", special handling
    else if (noGenLeftSide || ri.calcConstantType(ctx)!=0) ri.genOutputVal(reg, ctx);
    else {
      if (ri instanceof ExCall) {
        asCall=(ExCall)ri;
        ...
        if (isSuper) {
          ...
          if (reg!=0) {
            if ((type=getRegType(ctx))!=0) ctx.arch.genMoveFromPrimary(reg, type);
          }
        }
        else {
          if (le.baseType==StdTypes.T_DESC) { //left side is a reference to a unit
            if (ctx.dynaMem) {
              ctx.arch.genSaveUnitContext();
              asCall.genOutputPar(ctx);
              ctx.arch.genLoadUnitContext(ctx.arch.regClss, leImportedClassIndexed.relOff);
            }
            else asCall.genOutputPar(ctx);
            ...
            if (ctx.dynaMem) ctx.arch.genRestUnitContext();
          }
          else { //left side is something that returns an object
            restore2=ctx.arch.prepareFreeReg(0, 0, reg, le.getRegType(ctx));
            extReg=ctx.arch.allocReg();
        ...
        if (le.baseType==StdTypes.T_DESC && !ctx.dynaMem) {
          ...
          if (le.baseType==StdTypes.T_DESC) leRegType=StdTypes.T_PTR;
          else leRegType=le.getRegType(ctx);
  ...
  public void genOutputAssignTo(int newValueReg, Expr newValue, Context ctx) {
    ...
    if (le.baseType==StdTypes.T_MAGC) super.genOutputAssignTo(newValueReg, newValue, ctx);
    else if (noGenLeftSide || ri.calcConstantType(ctx)!=0) ri.genOutputVal(newValueReg, ctx);
    else if (ri instanceof ExVar) {
      asVar=(ExVar)ri;
      if (le.baseType==StdTypes.T_DESC && !ctx.dynaMem) {
        //variable access is statically bound, no class context needed
    ...
    else super.genOutputAssignTo(newValueReg, newValue, ctx);
  } 
}
