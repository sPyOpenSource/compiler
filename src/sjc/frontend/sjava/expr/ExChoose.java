...
public class ExChoose extends ExBin {
    ...
    //resolve left sub-expression
	  if (!le.resolve(unitContext, mthdContext, resolveFlags|RF_CHECKREAD, ctx.boolType, ctx)) return false; //left side always has impact on following code
    ...
    if (ce.baseType==StdTypes.T_NULL) {
      if (!nullCopy(ce, ri, ctx)) return false;
    }
    else if (ri.baseType==StdTypes.T_NULL) {
      if (!nullCopy(ri, ce, ctx)) return false;
    }
    switch (ce.compareType(ri, true, ctx)) {
      case TypeRef.C_TT: case TypeRef.C_EQ: getTypeOf(ce); break;
      case TypeRef.C_OT: getTypeOf(ri); break;
      default:
    ...
    if (le.calcConstantType(ctx)==StdTypes.T_INT) {
      if (le.getConstIntValue(ctx)==1) return ce.calcConstantType(ctx);
      else return ri.calcConstantType(ctx);
    }
    return 0;
	}
    ...
    if (le.calcConstantType(ctx)==StdTypes.T_INT) {
      if (le.getConstIntValue(ctx)==1) return ce.getConstIntValue(ctx);
      else return ri.getConstIntValue(ctx);
    }
    return 0;
	}
    ...
    if (le.calcConstantType(ctx)==StdTypes.T_INT) {
      if (le.getConstIntValue(ctx)==1) return ce.getConstLongValue(ctx);
      else return ri.getConstLongValue(ctx);
    }
    return 0l;
	}
    ...
    if (calcConstantType(ctx)==StdTypes.T_INT) {
      ...
      if (res==(isTrue?1:0)) ctx.arch.genJmp(jumpDest);
      return;
    }
    //not a constant condition, generate code
    ceIns=ctx.arch.getUnlinkedInstruction();
