...
public class ExClssMthdName extends Expr {
  ...
  public void printExpression(CodePrinter codePrnt) {
    if (destType!=null) codePrnt.exprClssName(destType);
    else codePrnt.magcClssMthdName(destUnit, destMthd);
  }
  
	public boolean resolve(Unit unitContext, Mthd mthdContext, int resolveFlags, TypeRef preferredType, Context ctx) {
    Mthd mthd;
    
    if (destType!=null) { //used by JParser
      if (!destType.resolveType(unitContext, ctx)) return false;
      if (destType.qid!=null) destUnit=destType.qid.unitDest;
      getTypeOf(destType);
    }
    else if (unitName!=null) { //used by MAGIC
      if (!resolveUnitName(unitContext, ctx)) return false;
      //destUnit already filled
      baseType=T_VOID;
      if (mthdName!=null) { //caller is interested in method, not unit
        if (!resolveMthdName(ctx)) return false;
        mthd=destMthd;
        while (mthd!=null) {
	  ...
	  if (destUnit!=null && doImport && ctx.dynaMem) importedClass=unitContext.getRefUnit(destUnit, true);
	  if (ctx.relations!=null) ctx.relations.addRelation(destUnit, null, null, unitContext, mthdContext);
	  //everything resolved
	  return true;
	}
  ...
  private boolean resolveMthdName(Context ctx) {
    ...
    while (chk!=null) {
      if (chk.name.equals(mthdName)) {
        if (destMthd!=null) { //second match found -> error
    ...
    if (destMthd!=null) return true; //only one match found
