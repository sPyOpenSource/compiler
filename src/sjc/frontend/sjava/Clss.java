...
public class Clss extends JUnit {
  //required fields for resolving
    ...
    if (this!=ctx.langRoot) { //language-root already checked in validateModifier
      if (extsID!=null) { //this class extends another class, extsID unitDest is already resolved (but not its interface!)
        if (!extsID.unitDest.resolveInterface(ctx)) return false;
        if ((extsID.unitDest.modifier&Modifier.M_INDIR)!=0) {
          if (name.charAt(0)=='$') { //anonymous inner class implements interface instead of extending class
    ...
    list=extsImplIDList;
    while (list!=null) {
	    if (!resolveExtsIplsQID(list.qid, ctx) || !list.qid.unitDest.resolveInterface(ctx)) return false;
	    if ((list.qid.unitDest.modifier&Modifier.M_INDIR)==0) {
	      printPos(ctx, "class can not be implemented by class ");
        ctx.out.println(name);
        return false;
	    }
	    if (ctx.relations!=null) ctx.relations.addRelation(list.qid.unitDest, this);
    ...
    if ((modifier&Modifier.M_STRUCT)!=0) {
      ...
      while (checkVrbl!=null) {
        //mark variable
        if ((checkVrbl.modifier&Modifier.M_FIN)==0) checkVrbl.modifier|=Modifier.M_STRUCT;
	...
	private boolean isInlArrCountDefined() {
	  Vrbl v=vars;
	  while (v!=null) {
	    if ((v.modifier&Modifier.M_ARRLEN)!=0) return true;
	    v=v.nextVrbl;
	  }
	  return false;
	}
  ...
  protected boolean checkInstVarInit(Vrbl var, Context ctx) {
    ...
    if (!explConstr) { //insert standard constructor for initialization
      ...
      if (!addedConstr.checkNameAndType(this, ctx)) return false;
    }
    ...
    if (var.init.baseType==StdTypes.T_NULL) return true; //null, do not insert into instInitVars
    //insert variable into list
    if (instInitVars==null) instInitVars=new VrblList(var);
    else {
      lastInit=instInitVars;
      while (lastInit.next!=null) lastInit=lastInit.next;
  ...
  protected boolean resolveMthdExtsIpls(Context ctx) {
    ...
    if (inlArr!=null ^ isInlArrCountDefined()) { //either both active or both inactive
    ...
    if ((modifier&Modifier.MM_FLASH)!=0) {
      ...
      while (checkVrbl!=null) {
        switch (checkVrbl.type.typeSpecial) {
          case TypeRef.S_NOSPECIAL: checkVrbl.type.typeSpecial=TypeRef.S_FLASHREF; break;
          case TypeRef.S_INSTINLARR: checkVrbl.type.typeSpecial=TypeRef.S_FLASHINLARR; break;
          default:
    ...
    if (!explConstr && extsID!=null) { //if we had no explicit constructor, copy state of parent
    ...
    while (checkMthd!=null) {
      if (((checkMthd.modifier&Modifier.M_STAT)==0) && (extsID!=null)) { //check extend if not static
        destMthd=extsID.unitDest.searchMethod(checkMthd, ctx);
      }
      else destMthd=null;
      if (destMthd!=null && (destMthd.modifier&Modifier.M_PRIV)==0) { //this will overwrite another method
        if (checkMthd.retType.compareType(destMthd.retType, false, ctx)!=TypeRef.C_EQ) {
          ...
          else switch (destMthd.modifier&(Modifier.M_PUB|Modifier.M_PROT|Modifier.M_PACP)) {
            case Modifier.M_PUB:  checkMthd.modifier|=Modifier.MA_PUB; break;
            case Modifier.M_PROT: checkMthd.modifier|=Modifier.MA_PROT; break;
            case Modifier.M_PACP: checkMthd.modifier|=Modifier.MA_PACP; break;
          }
    ...
    if (extsID!=null && (extsID.unitDest.modifier&Modifier.M_ABSTR)!=0
        && (modifier&Modifier.M_ABSTR)==0) {
      ...
      while (checkClss!=null && (checkClss.modifier&Modifier.M_ABSTR)!=0) {
        ...
        if (checkClss.extsID!=null) checkClss=checkClss.extsID.unitDest;
        else checkClss=null;
      }
    }
    ...
    chkIntf=extsImplIDList;
	  while (chkIntf!=null) {
	    if (!checkImplementation(chkIntf.qid.unitDest, ctx)) fulfilled=false;
	    else {
	      chkIntfParentList=chkIntf.qid.unitDest.extsImplIDList;
	      while (chkIntfParentList!=null) { //check all parents - implementation is done already, but we need the pointers and indices
    ...
    if (outerUnit!=null && (modifier&Modifier.M_STAT)==0) { //check extends and on demand add internal variable "$outer"
      if ((modifier&Modifier.M_STRUCT)!=0) {
      ...
      else if (extsID!=null && extsID.unitDest.outerUnit!=null) { //check if outer is compatible
	  ...
	  if (extsID!=null) {
	    checkVrbl=vars;
	    while (checkVrbl!=null) {
        if ((tmpVrbl=extsID.unitDest.searchVariable(checkVrbl.name, ctx))!=null
            && (tmpVrbl.modifier&(Modifier.M_PUB|Modifier.M_PROT
                |Modifier.M_PACP|Modifier.M_PRIV))!=Modifier.M_PRIV) {
    ...
    if ((modifier&Modifier.M_STRUCT)!=0) {
      if (extsID==null) instScalarTableSize=0; //this==ctx.structClass, no variable inside
      else {
        ...
        while (checkVrbl!=null) {
          ...
          if (checkVrbl.type.arrDim!=0) {
            if (checkVrbl.type.typeSpecial>0) i=checkVrbl.minSize*checkVrbl.type.typeSpecial;
            else i=0;
          }
          else i=checkVrbl.minSize;
          //assign offset if variable is not final static
          switch (checkVrbl.modifier&(Modifier.M_FIN|Modifier.M_STAT)) {
            case Modifier.M_FIN: case Modifier.M_STAT:
            ...
            case Modifier.M_FIN|Modifier.M_STAT: //constant field, no offset required
              break;
            case 0:
              if (checkVrbl.relOff==AccVar.INV_RELOFF) { //not explicitly assigned offset
                checkVrbl.relOff=instScalarTableSize;
                instScalarTableSize+=i;
              }
              else { //variable had offset-annotation, check if end is behind current end
                if ((i=checkVrbl.relOff+i)>instScalarTableSize) instScalarTableSize=i;
              }
              break;
          }
          //check array special
          if (checkVrbl.type.arrDim!=0 && checkVrbl.type.typeSpecial==TypeRef.S_STRUCTARRNOTSPEC) { //no array size specified
            //print a warning
      ...
      if (fulfilled && sigOK) offsetsAssigned=true;
      else offsetError=true;
    }
	...
	public UnitList getRefUnit(Unit refUnit, boolean insert) {
	  ...
	  if (extsID!=null && (check=extsID.unitDest.getRefUnit(refUnit, false))!=null) return check; //found
	  //check if imported by us
	  check=referenced;
	  last=null;
	  //search if already imported
	  while (check!=null) {
	    if (check.unit==refUnit) return check;
	    last=check;
	    check=check.next;
	  }
	  //not found
	  if (!insert) return null; //we shall not insert
  ...
  public boolean assignOffsets(boolean doClssOff, Context ctx) {
    ...
    if (extsID == null) {
      ...
      if (this==ctx.rteSClassDesc) { //java.rte.SClassDesc
        ...
        //check parent
        if (!parent.assignOffsets(true, ctx)) return false;
    ...
    if (doClssOff) {
      ...
      while (checkMthd!=null) {
        if ((checkMthd.modifier&Modifier.M_NAT)==0) {
          if ((checkMthd.modifier&Modifier.M_ABSTR)==0) {
            omthd=checkMthd;
            if (ctx.genAllMthds) checkMthd.modifier|=Modifier.M_NDCODE;
            else while (omthd!=null) {
            ...
            if ((checkMthd.modifier&Modifier.M_NDCODE)==0) {
              if (ctx.debugCode) checkMthd.modifier|=Modifier.M_NDCODE;
              if (ctx.verbose) {
      ...
      switch (checkVrbl.location) {
        case Vrbl.L_CLSSSCL:
          if (doClssOff) {
            modifier|=Modifier.MA_ACCSSD;
            if (ctx.embedded) {
              if (ctx.arch.needsAlignedVrbls) { //align current ramSize to a multiple of the variable size
        ...
        case Vrbl.L_CLSSREL:
          if (doClssOff) {
            ...
            else checkVrbl.relOff=-(statRelocTableEntries-=checkVrbl.minSize)*ctx.arch.relocBytes;
          }
          break;
        case Vrbl.L_INSTSCL:
          if (doInstOff) {
        ...
        case Vrbl.L_INSTREL:
          if (doInstOff) {
            if (checkVrbl.minSize==-1) instRelocTableEntries++;
            else if (checkVrbl.minSize==-2) instRelocTableEntries+=2;
            else {
        ...
        case Vrbl.L_STRUCT:
          ...
          return false;
        case Vrbl.L_INLARR:
          if (doInstOff) inlArrVrbl=checkVrbl; //remember for second run
          break;
        default:
  ...
  public boolean genDescriptor(Context ctx) {
    ...
    if (outputLocation!=null || (modifier&Modifier.M_STRUCT)!=0) return true;
    if (outputError) return false;
    
    if (ctx.relations!=null && extsID!=null) ctx.relations.addRelation(extsID.unitDest, this);
    
    if (extsID!=null && this!=ctx.rteSClassDesc && !extsID.unitDest.genDescriptor(ctx)) {
      outputError=true;
      return false;
    }
    if (ctx.genAllUnitDesc || (marker&Marks.K_FOCD)!=0) modifier|=Modifier.MA_ACCSSD;
    ...
    if (!allocateDescriptor(clssScalarTableSize+statScalarTableSize,
        clssRelocTableEntries+statRelocTableEntries, ctx.rteSClassDesc.outputLocation, ctx)) {
      outputError=true;
      return false;
    }
    ...
    if (!ctx.embedded) {
      checkVrbl=vars;
      while (checkVrbl!=null) {
        switch (checkVrbl.location) { //L_CLSS* are in fact allocated in stat*, so move them by clss*
          case Vrbl.L_CLSSSCL:
    ...
    while (mthd!=null) {
      ...
      if ((mthd.modifier&Modifier.M_NDCODE)!=0) {
        ...
        if (mthd.relOff!=0 && (mthd.modifier&Modifier.M_STAT)!=0) {
          ...
          if (ctx.dynaMem) ctx.arch.putRef(outputLocation, mthd.relOff+ctx.arch.relocBytes, outputLocation, 0); //put ourselve as owner
        }
      }
      mthd=mthd.nextMthd;
    }
    ...
    if (firstIntf!=null) ctx.arch.putRef(outputLocation, ctx.rteSClassImpl, firstIntf.outputLocation, 0);
    else if (chkIntf!=null) ctx.arch.putRef(outputLocation, ctx.rteSClassImpl, chkIntf.outputLocation, 0);
      ...
      if ((var.modifier&Modifier.M_STAT)!=0 && var.init!=null) {
        switch (var.location) {
          case Vrbl.L_CLSSREL:
            ...
            break;
          case Vrbl.L_CLSSSCL:
            if (var.init.isNullType() && var.type.isStructType()) break; //null==0 is default but needs special care for STRUCTs
            ...
            if (ctx.embedded) addr=ctx.ramInitLoc;
            else addr=outputLocation;
            int type=var.type.baseType;
            if (var.init.isStructType()) type=calcConst;
            switch (type) {
            ...
            break;
          case Vrbl.L_CONST: case Vrbl.L_CONSTDC:
  ...
  public IndirUnitMapList enterInheritableReferences(Object objLoc, IndirUnitMapList lastIntf, Context ctx) { //returns pointer to first interface map
    ...
    if (extsID!=null && (inheritedIntfMaps=extsID.unitDest.enterInheritableReferences(objLoc, null, ctx))!=null
        && lastIntf!=null) {
    ...
    mthd=mthds;
    if (ctx.dynaMem) while (mthd!=null) {
      if (mthd.relOff!=0) { //always put method and owner
    ...
    else while (mthd!=null) {
      if (mthd.relOff!=0 && (mthd.modifier&Modifier.M_STAT)==0) { //only if dynamic call, only address needed
        ctx.arch.putRef(objLoc, mthd.relOff, mthd.outputLocation, 0); //put address of method
      }
      mthd=mthd.nextMthd;
    }
    ...
    if ((refc=referenced)!=null) {
      if (!ctx.dynaMem) { //static compilation must not have filled referenced
    ...
    dbw.startUnit((modifier&Modifier.M_STRUCT)==0 ? "class" : "struct", this);
	  if ((modifier&Modifier.M_STRUCT)==0) dbw.hasUnitOutputLocation(
	      (!ctx.dynaMem && (modifier&Modifier.MA_ACCSSD)==0) ? null : outputLocation);
	  else dbw.hasUnitOutputLocation(null);
    dbw.hasUnitFields(clssRelocTableEntries, clssScalarTableSize,
        statRelocTableEntries, statScalarTableSize,
        instRelocTableEntries, instScalarTableSize, instIndirScalarTableSize);
	...
	private boolean checkImplementation(Unit checkIntf, Context ctx) {
	  ...
	  me=implemented;
	  while (me!=null) {
	    if (me.intf==checkIntf) return true; //this interface is not new to us
	    me=me.next;
	  }
      ...
      if (destMthd!=null) {
        if ((destMthd.modifier&Modifier.M_STAT)!=0) {
          ...
          switch (checkMthd.modifier&(Modifier.M_PUB|Modifier.M_PROT|Modifier.M_PACP)) {
            case Modifier.M_PUB:  destMthd.modifier|=Modifier.MA_PUB; break;
            case Modifier.M_PROT: destMthd.modifier|=Modifier.MA_PROT; break;
            case Modifier.M_PACP: destMthd.modifier|=Modifier.MA_PACP; break;
          }
        }
        checkMthd=checkMthd.nextMthd;
      }
      else if ((modifier&Modifier.M_ABSTR)!=0) { //if class is abstract, silently insert method to fulfill interface
  ...
  private boolean genIntfOutput(IndirUnitMapList current, Context ctx) {
    ...
    for (i=1; i<cnt; i++) ctx.mem.putInt(intfLoc, base+i*4, current.map[i].relOff);
    //set the owner
