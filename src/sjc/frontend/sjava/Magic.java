...
public class Magic extends CtxBasedConfig {
  ...
  protected void genOutput(int reg, Expr ex, Context ctx) {
    ...
    if (ex instanceof ExCall) switch ((call=(ExCall)ex).magicType) {
      ...
      case M_INLINEBK: genOutputInlineBlock(call, ctx); return;
      case M_WMEM64:   genOutputWMem(false, StdTypes.T_LONG, call, ctx); return;
      case M_RMEM64:   genOutputRMem(false, reg, StdTypes.T_LONG, call, ctx); return;
      case M_WMEM32:   genOutputWMem(false, StdTypes.T_INT, call, ctx); return;
      case M_RMEM32:   genOutputRMem(false, reg, StdTypes.T_INT, call, ctx); return;
      case M_WMEM16:   genOutputWMem(false, StdTypes.T_SHRT, call, ctx); return;
      case M_RMEM16:   genOutputRMem(false, reg, StdTypes.T_SHRT, call, ctx); return;
      case M_WMEM8:    genOutputWMem(false, StdTypes.T_BYTE, call, ctx); return;
      case M_RMEM8:    genOutputRMem(false, reg, StdTypes.T_BYTE, call, ctx); return;
      case M_WIOS64:   genOutputWMem(true, StdTypes.T_LONG, call, ctx); return;
      case M_RIOS64:   genOutputRMem(true, reg, StdTypes.T_LONG, call, ctx); return;
      case M_WIOS32:   genOutputWMem(true, StdTypes.T_INT, call, ctx); return;
      case M_RIOS32:   genOutputRMem(true, reg, StdTypes.T_INT, call, ctx); return;
      case M_WIOS16:   genOutputWMem(true, StdTypes.T_SHRT, call, ctx); return;
      case M_RIOS16:   genOutputRMem(true, reg, StdTypes.T_SHRT, call, ctx); return;
      case M_WIOS8:    genOutputWMem(true, StdTypes.T_BYTE, call, ctx); return;
      case M_RIOS8:    genOutputRMem(true, reg, StdTypes.T_BYTE, call, ctx); return;
      case M_CST2REF:  genOutputCast2Ref(reg, call, ctx); return;
      case M_CST2OBJ:  genOutputCast2ObjStruct(reg, call, ctx); return;
      case M_ADDR:     genOutputAddr(reg, call, ctx); return;
      case M_CLINDESC: genOutputClssDesc(reg, call, ctx); return;
      case M_MTHDOFF:  genOutputMthdOff(reg, call, ctx); return;
      case M_GETCDOF:  genOutputGetCodeOff(reg, ctx); return;
      case M_I_SCLSZ:  //do the same as M_I_RLCEN
      case M_I_ISCSZ:  //do the same as M_I_RLCEN
      case M_I_RLCEN:  genOutputUnitProp(call.magicType, reg, call, ctx); return;
      case M_BITMEM8:  genOutputBtsMem(false, StdTypes.T_BYTE, call, ctx); return;
      case M_BITMEM16: genOutputBtsMem(false, StdTypes.T_SHRT, call, ctx); return;
      case M_BITMEM32: genOutputBtsMem(false, StdTypes.T_INT, call, ctx); return;
      case M_BITMEM64: genOutputBtsMem(false, StdTypes.T_LONG, call, ctx); return;
      case M_BITIOS8:  genOutputBtsMem(true, StdTypes.T_BYTE, call, ctx); return;
      case M_BITIOS16: genOutputBtsMem(true, StdTypes.T_SHRT, call, ctx); return;
      case M_BITIOS32: genOutputBtsMem(true, StdTypes.T_INT, call, ctx); return;
      case M_BITIOS64: genOutputBtsMem(true, StdTypes.T_LONG, call, ctx); return;
      case M_GETRAMAD: genOutputGetRamAddr(reg, ctx); return;
      case M_GETRAMSZ: genOutputGetRamSize(reg, ctx); return;
      case M_GETRINAD: genOutputGetRamInitAddr(reg, ctx); return;
      case M_USEASTHS: genOutputUseAsThis(call, ctx); return;
      case M_GETCNSTM: genOutputGetConstMemorySize(reg, ctx); return;
      case M_INLOFFST: genOutputInlineOffset(call, ctx); return;
      case M_INLCODAD: genOutputInlineCodeAddress(call, ctx); return;
      case M_STATINIT: genOutputDoStaticInit(call, ctx); return;
      case M_IGNORE:   return; //nothing to code
      case M_STOPCODE: return; //nothing to code
      case M_ASSIGN:   genAssign(call, ctx); return;
    }
  ...
  private boolean resolveDoStaticInit(ExCall call, Unit unitContext, Mthd mthdContext, int resolveFlags, Context ctx) {
    ...
    if (ctx.dynaMem) call.dest=unitContext.initStat; //remember unitContext for unit references
    //insert references to all units with static initialization and check type hierarchy sorted order
    lm=null;
    m=ctx.staticInitMthds;
    while (m!=null) {
      if (ctx.dynaMem) unitContext.getRefUnit(m.owner, true); //just prepare, don't remember referenced unit
      mthdContext.stmtCnt+=m.stmtCnt; //update statistics
      if (ctx.verbose) { //give verbose information if requested
      ...
      while (tm.nextMthd!=null) {
        if (tm.nextMthd.owner.isParent(m.owner, ctx)) { //tm.nextMthd must be before m, but is after => re-order
          ...
          if (lm==null) ctx.staticInitMthds=tmp; //no previous method, insert at start of list
          else lm.nextMthd=tmp; //set as next method of last method right before current method
          m=tmp; //continue with method of parent
          changedOrder=true; //do not step onto next method
        }
        else tm=tm.nextMthd; //checked method is not parent, check next one
      }
      if (!changedOrder) {
        lm=m;
        m=m.nextMthd;
  ...
  private void genOutputInline(int type, ExCall call, Context ctx) {
    ...
    switch (type) {
      case StdTypes.T_BYTE: size=1; break;
      case StdTypes.T_SHRT: size=2; break;
    ...
    if (instr!=null) instr.size=cnt; //fix size of last instruction
  }
  ...
  private void genOutputWMem(boolean isIO, int type, ExCall call, Context ctx) {
    ...
    if (!isIO && ((addrConstType=addrEx.calcConstantType(ctx))!=0 | (valConstType=valEx.calcConstantType(ctx))!=0)) {
      if (addrConstType==StdTypes.T_INT) {
        if (valConstType==StdTypes.T_INT) { //both addr and val are constant, value is 8/16/32 bit
          ctx.arch.genStoreVarConstVal(0, null, addrEx.getConstIntValue(ctx), valEx.getConstIntValue(ctx), type);
          return;
        }
        if (valConstType==StdTypes.T_LONG) { //both addr and val are constant, value is 64 bit
      ...
      if (valConstType==StdTypes.T_INT || valConstType==StdTypes.T_LONG) { //addr is not constant, value is constant
        ...
        if (valConstType==StdTypes.T_INT) ctx.arch.genStoreVarConstVal(addr, null, 0, valEx.getConstIntValue(ctx), type);
        else ctx.arch.genStoreVarConstDoubleOrLongVal(addr, null, 0, valEx.getConstLongValue(ctx), type==StdTypes.T_DBL);
    ...
    valEx.genOutputVal(val, ctx);
    if (isIO) {
      if (pa.nextParam!=null) ioMemLoc=pa.nextParam.expr.getConstIntValue(ctx);
      ctx.arch.genWriteIO(addr, val, type, ioMemLoc);
    }
    else ctx.arch.genAssign(addr, val, type);
    ctx.arch.deallocRestoreReg(val, 0, restoreVal);
    ctx.arch.deallocRestoreReg(addr, 0, restoreAddr);
  }
  ...
  private void genOutputRMem(boolean isIO, int reg, int type, ExCall call, Context ctx) {
    ...
    else ctx.arch.genLoadVarVal(reg, addr, null, 0, type);
  ...
  private void genOutputCast2Ref(int reg, ExCall call, Context ctx) {
    ...
    pa=call.par;
    restore=ctx.arch.prepareFreeReg(0, 0, reg,
  ...
  private void genOutputCast2ObjStruct(int reg, ExCall call, Context ctx) {
    ...
    pa=call.par;
    if ((valType=pa.expr.getRegType(ctx))!=StdTypes.T_INT && relocBytes==4) { //post-check if invalid type is used
  ...
  private void genOutputClssDesc(int reg, ExCall call, Context ctx) {
    ...
    if (ctx.dynaMem) ctx.arch.genLoadUnitContext(reg, exc.importedClass.relOff);
    else ctx.arch.genLoadConstUnitContext(reg, exc.destUnit.outputLocation);
  }
  ...
  private void genOutputUnitProp(int dest, int reg, ExCall call, Context ctx) {
    ...
    switch (dest) {
      case M_I_SCLSZ: val=unit.instScalarTableSize; break;
      case M_I_ISCSZ: val=unit.instIndirScalarTableSize; break;
      case M_I_RLCEN: val=unit.instRelocTableEntries; break;
      default:
  ...
  private void genOutputBtsMem(boolean isIO, int type, ExCall call, Context ctx) {
    ...
    if ((maskConst=pa.expr.calcConstantType(ctx))!=0
        && pa.nextParam.expr.calcConstantType(ctx)==StdTypes.T_INT
        && pa.nextParam.expr.getConstIntValue(ctx)==0) {
      switch (maskConst) {
        case StdTypes.T_INT:
    ...
    else pa.expr.genOutputVal(mask, ctx);
    restore3=ctx.arch.prepareFreeReg(addr, mask, 0, type);
    preVal=ctx.arch.allocReg();
    if (isIO) ctx.arch.genReadIO(preVal, addr, type, 0);
    else ctx.arch.genLoadVarVal(preVal, addr, null, 0, type);
    pa=pa.nextParam;
    restore4=ctx.arch.prepareFreeReg(addr, mask, preVal, type);
    val=ctx.arch.allocReg();
    if (pa.expr.calcConstantType(ctx)==StdTypes.T_INT) { //constant clear/set
      if (pa.expr.getConstIntValue(ctx)==0) { //clear -> not+and
        if (doNot) {
        ...
        else ctx.arch.genBinOp(val, preVal, mask, (Ops.S_ARI<<16)|Ops.A_AND, type);
      }
      else { //set -> or
        ctx.arch.genBinOp(val, preVal, mask, (Ops.S_ARI<<16)|Ops.A_OR, type);
      }
    }
    else { //mini-if to check or/and
    ...
    if (isIO) ctx.arch.genWriteIO(addr, val, type, 0);
    else ctx.arch.genAssign(addr, val, type);
  ...
  private void genOutputInlineOffset(ExCall call, Context ctx) {
    ...
    switch (var.location) {
      ...
      case AccVar.L_CLSSREL:
        if (ctx.dynaMem) objReg=ctx.arch.regClss;
        else {
  ...
  private void genOutputInlineCodeAddress(ExCall call, Context ctx) {
    ...
    if ((pa=call.par)==null) ctx.arch.inlineCodeAddress(true, 0);
    else ctx.arch.inlineCodeAddress(false, pa.expr.getConstIntValue(ctx));
  }
  ...
  private void genOutputDoStaticInit(ExCall call, Context ctx) {
    ...
    m=ctx.staticInitMthds;
    if (ctx.dynaMem) caller=call.dest.owner; //prepared in resolveDoStaticInit to remember unitContext
    while (m!=null) {
      if (ctx.dynaMem) {
