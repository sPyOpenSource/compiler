...
public abstract class JUnit extends Unit {
  ...
  public Unit searchUnitInView(StringList what, boolean isRecursion) {
    ...
    //check ourself
    if (what.next==null && what.str.equals(name)) return this;
    //check inner units
    cur=what;
    if (cur.next!=null && cur.str.equals(name)) cur=cur.next; //full qualified type with ourself in front
    res=innerUnits;
    while (res!=null) {
    ...
    //find in outer units
    if (outerUnit!=null) return outerUnit.searchUnitInView(what, false);
    //find in imports if not recursive search
    if (!isRecursion) {
      //find in unit imports
      if ((res=searchUnitInImports(what, false))!=null) return res;
      //find in package
      if ((res=pack.packDest.searchUnit(what))!=null) return res;
      //find in package imports
      if ((res=searchUnitInImports(what, true))!=null) return res;
    }
    if (extsID!=null && extsID.unitDest!=null) return extsID.unitDest.searchUnitInView(what, true);
    //not found
    return null;
  }
  ...
  public void resetResolveState(boolean newOutput, Context ctx) {
    ...
    if (extsID!=null && extsID.unitDest==ctx.langRoot) extsID=null;
  ...
  public boolean validateModifier(Context ctx) {
    ...
    switch (intfResolveState) {
      case R_NOTSTARTED: break; //normal mode
  ...
  public boolean resolveInterface(Context ctx) {
    ...
    switch (intfResolveState) {
      case R_NOTSTARTED: case R_MODSTARTED: //this should never happen as validateModifier should always be called before resolveInterface
        compErr(ctx, "resolveInterface called before modifier is fixed");
        ctx.out.println();
        intfResolveState=R_ERROR;
        return false;
      case R_MODVALID: break; //normal mode
      case R_STARTED: //this indicates cyclic call of CompileUnit.resolveInterface
        ...
        intfResolveState=R_ERROR;
        return false;
      case R_SUCCESS: return true; //already done, nothing to do
      case R_ERROR: return false; //silently abort, we printed an error-message already
    ...
    if (error //abort if there was an error in import-statements
        ...
        || (outerUnit!=null && !outerUnit.resolveInterface(ctx))) { //check outer if existing
      intfResolveState=R_ERROR;
      return false;
    }
    //check methods
    ...
    while (var!=null) {
      ...
      if (!var.type.resolveType(this, ctx) || !var.checkNameAgainstVrbl(vars, ctx)
          || !var.checkNameAgainstUnits(innerUnits, ctx)
          || !var.enterSize(var.location==Vrbl.L_NOTRDY ? Vrbl.L_UNIT : var.location, ctx)
          || ((var.modifier&Modifier.M_STAT)==0 && var.init!=null && !var.init.isConstZero() && !checkInstVarInit(var, ctx))) {
        ctx.out.print(" in unit ");
        ctx.out.println(name);
        intfResolveState=R_ERROR;
        return false;
  ...
  protected boolean resolveExtsIplsQID(QualID qid, Context ctx) { //called by Clss and TIntf only
    int accessLevel;
    //search for unit
    if ((qid.unitDest=searchUnitInView(qid.name, false))!=null //use normal search function
        ...
        || (qid.unitDest=ctx.root.searchUnit(qid.name))!=null) {//search in root package
      if (qid.unitDest==this) {
        qid.printPos(ctx, "cannot extend/implement itself");
        return false;
      }
      if ((accessLevel=minimumAccessLevel(qid, qid.unitDest.name,
          qid.unitDest, qid.unitDest.modifier, true, ctx))==Modifier.M_ERROR) return false;
