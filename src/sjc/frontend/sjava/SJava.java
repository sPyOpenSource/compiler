...
public class SJava extends Language {
  ...
  protected static boolean checkVarInitType(Vrbl var, boolean explConv, Unit unitContext, Context ctx) {
    ...
    if (var.init!=null && (cmpRes=var.type.compareType(var.init, true, ctx))!=TypeRef.C_EQ && cmpRes!=TypeRef.C_TT) {
      ...
      if (!var.type.isObjType() && !explConv) destType=var.init.getBaseTypeConvertionType(var.type, false);
      else if (cmpRes!=TypeRef.C_NP && var.type.isIntfType()) destType=var.type;
      if (destType==null || (var.init=ExEnc.getConvertedResolvedExpr(var.init, destType, unitContext, ctx))==null) success=false;
    }
    if (success) return true;
    if (var.init!=null) var.init.printPos(ctx, "incompatible or not implicitly convertible types");
    return false;
  }
