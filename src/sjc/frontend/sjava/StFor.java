...
import sjc.compbase.VrblStateList;
...
public class StFor extends StLoop {
  ...
  protected int innerResolve(int flowCode, Unit unitContext, Mthd mthdContext, Context ctx) {
    ...
    jel=init;
    while (jel!=null) {
      if ((jel.resolve(0, unitContext, mthdContext, ctx)&FA_ERROR)!=0) return FA_ERROR; //init-stmts may not change flow
      jel=jel.nextStmt;
    }
    if (cond!=null) {
    ...
    if (((flowCode=loStmt.resolve(flowCode, unitContext, mthdContext, ctx))&FA_ERROR)!=0) {
      if (preState!=null) ctx.recycleVrblStatelist(preState);
      return FA_ERROR;
    }
    if ((flowCode&FA_NEXT_IS_UNREACHABLE)!=0 && (flowAnalysisBuffer&FA_HAS_CONTINUE)==0
        && lupd!=null) lupd.flowWarn(ctx, ERR_UNREACHABLE_CODE);
    ...
    if ((cond==null || (cond.calcConstantType(ctx)==StdTypes.T_INT && cond.getConstIntValue(ctx)==1))
        && (flowAnalysisBuffer&FA_HAS_ENDBLOCK)==0) { //endless loop without break
      flowCode|=FA_NEXT_IS_UNREACHABLE;
    }
    if ((flowAnalysisBuffer&FA_HAS_ENDBLOCK)!=0) flowCode&=~FA_NEXT_IS_UNREACHABLE; //break existing
