...
/**
 * Scanner: scanner for the SJava-language
 * 
 * @author S. Frenz
 * @version 121017 allowed '$' for names
  ...
  version 091209 moved to frontend package and renamed to Scanner
	...
	private boolean getSym() {
      ...
      if (isNum(r.nextChar) || (r.nextChar=='.' && isNum(r.lookAhead))) {
        ...
        return true;
          ...
          if (!checkKeyword(chrBuf, bufLen)) { //checkKeyword will set type and par
          ...
          return true;
  		  case CCT_BASEOP1: //'!', '"', '#', '%', '&', '''
  	        ...
  	        case '"':
  	          ...
  	          while (r.nextChar!='\0' && r.nextChar!='"') {
  	            ...
  	            if (l2aSym.type==S_ERR) return false; //there were an error in readChValue
  	            bufLen++;
  	            if (bufLen>MAX_SYMBOL_LENGTH) {
  	              scannerError("too many characters for buffer");
  	              return false;
  	        ...
  	        case '&':
  	          r.readChar();
  	          if (r.nextChar=='&') { // "&&": logical and
  	            r.readChar();
  	            l2aSym.type=S_LOG;
  	            l2aSym.par=L_AND;
  	            return true;
  	          }
  	          if (r.nextChar=='=') { // "&=": arithmetic and + assign
  	            r.readChar();
  		  ...
  		  case CCT_BASEOP2: //'(', ')', '*', '+', ',', '-', '.', '/'
  		    switch (r.nextChar) {
  	        case '(': r.readChar(); l2aSym.type=S_ENC; l2aSym.par=E_RO; return true;
  	        case ')': r.readChar(); l2aSym.type=S_ENC; l2aSym.par=E_RC; return true;
  	        case '.': r.readChar(); l2aSym.type=S_DEL; l2aSym.par=D_DOT; return true;
  	        case ',': r.readChar(); l2aSym.type=S_DEL; l2aSym.par=D_COM; return true;
  	        case '*':
  	          r.readChar();
  	          if (r.nextChar=='=') { // "*=": multply + assign
  	            r.readChar();
  	            l2aSym.type=S_ASNARI;
  	            l2aSym.par=A_MUL;
  	            return true;
  	        ...
  	        case '+':
  	          r.readChar();
  	          if (r.nextChar=='+') { // "++": prae- or postfix increase
  	            r.readChar();
  	            l2aSym.type=S_PFX;
  	            l2aSym.par=P_INC;
  	            return true;
  	          }
  	          if (r.nextChar=='=') { // "+=": plus + assign
  	            r.readChar();
  	            l2aSym.type=S_ASNARI;
  	            l2aSym.par=A_PLUS;
  	            return true;
  	        ...
  	        case '-':
  	          r.readChar();
  	          if (r.nextChar=='-') { // "--": prae- or postfix decrease
  	            r.readChar();
  	            l2aSym.type=S_PFX;
  	            l2aSym.par=P_DEC;
  	            return true;
  	          }
  	          if (r.nextChar=='=') { // "-=": minus + assign
  	            r.readChar();
  	            l2aSym.type=S_ASNARI;
  	            l2aSym.par=A_MINUS;
  	            return true;
  	        ...
  	        case '/':
  	          r.readChar();
  	          if (r.nextChar=='/') { // "//": comment rest of line
  	            while (r.nextChar!='\0' && r.nextChar!='\r' && r.nextChar!='\n') {
  	              r.readChar();
  	            }
  	            continue;
  	          }
  	          else if (r.nextChar=='*') { // "/*": start of comment
  	            r.readChar();
  	            while (r.nextChar!='\0') {
  	              if (r.nextChar=='*' && r.lookAhead=='/') break; //leave while-loop
  	              r.readChar();
  	            }
  		  ...
  		  case CCT_BASEOP3: //':', ';', '<', '=', '>', '?'
          switch (r.nextChar) {
            case ';': r.readChar(); l2aSym.type=S_DEL; l2aSym.par=D_SEM; return true;
            case ':': r.readChar(); l2aSym.type=S_DEL; l2aSym.par=D_COL; return true;
            case '?': r.readChar(); l2aSym.type=S_ASK; l2aSym.par=RES; return true;
            case '=':
              r.readChar();
              if (r.nextChar=='=') { // "==": equal to
                r.readChar();
                l2aSym.type=S_CMP;
                l2aSym.par=C_EQ;
                return true;
            ...
            case '>':
              r.readChar();
              if (r.nextChar=='=') { // ">=": greater than or equal to
                r.readChar();
                l2aSym.type=S_CMP;
                l2aSym.par=C_GE;
                return true;
              }
              if (r.nextChar=='>') { // ">>" so far 
                r.readChar();
                if (r.nextChar=='>') { // ">>>": shift right without sign
                  r.readChar();
                  if (r.nextChar=='=') { // ">>>=: shift right without sign with assignment
                    r.readChar();
                ...
                if (r.nextChar=='=') { // ">>=: shift right with assignment
                  r.readChar();
                  l2aSym.type=S_ASNBSH;
                  l2aSym.par=B_SHRA;
                  return true;
            ...
            case '<':
              r.readChar();
              if (r.nextChar=='=') { // "<=": lower than or equal to
                r.readChar();
                l2aSym.type=S_CMP;
                l2aSym.par=C_LE;
                return true;
              ...
              if (r.nextChar=='<') { // "<<" so far
                r.readChar();
                if (r.nextChar=='=') { // "<<=": shift left with assignment
                  r.readChar();
                  l2aSym.type=S_ASNBSH;
                  l2aSym.par=B_SHL;
                  return true;
  		...
  		//not done yet
			switch (r.nextChar) {
				case '{': r.readChar(); l2aSym.type=S_ENC; l2aSym.par=E_BO; return true;
				case '}': r.readChar(); l2aSym.type=S_ENC; l2aSym.par=E_BC; return true;
				case '[': r.readChar(); l2aSym.type=S_ENC; l2aSym.par=E_SO; return true;
				case ']': r.readChar(); l2aSym.type=S_ENC; l2aSym.par=E_SC; return true;
				case '^':
					r.readChar();
					if (r.nextChar=='=') { // "^=": xor + assign
						r.readChar();
						l2aSym.type=S_ASNARI;
						l2aSym.par=A_XOR;
						return true;
					...
					l2aSym.par=A_XOR;
					return true;
				case '~': r.readChar(); l2aSym.type=S_ARI; l2aSym.par=A_CPL; return true;
				case '|':
					r.readChar();
					if (r.nextChar=='|') { // "||": logical or
						r.readChar();
						l2aSym.type=S_LOG;
						l2aSym.par=L_OR;
						return true;
					}
					if (r.nextChar=='=') { // "|=": arithmetic or + assign
						r.readChar();
						l2aSym.type=S_ASNARI;
						l2aSym.par=A_OR;
						return true;
...
?      63 | _      95 | (dl) 127
  ...
  private int getCharCat(char c) {
    if ((c>='A' && c<='Z') || (c>='a' && c<='z') || c=='_' || c=='$') return CCT_ALPHAEXT;
    if (c<=' ') return CCT_CTRL; 
    if (c<='/') {
     if (c<='\'') return CCT_BASEOP1;
     return CCT_BASEOP2;
    }
    if (c>=':') return CCT_BASEOP3;
    return CCT_OTHER;
  }
  ...
  private boolean readNumValue(int base) {
    ...
    long ov=0L, buf=0L, lb=(long)base;
    ...
    if (r.nextChar!='.') {
      ...
      switch (r.nextChar) {
        case 'l': case 'L': //type to long
          r.readChar();
          l2aSym.par=T_LONG;
          l2aSym.longBuf=buf;
          return true;
        case 'f': case 'F': //convert to float, handled below
        case 'd': case 'D': //convert to double, handled below
        case 'e': case 'E': case '.': //floating-point, handled below
          break;
        default:
          if (isAlphaExt(r.nextChar)) { //invalid constant
            scannerError("invalid constant value");
            return false;
          }
          //default type is int, check range before conversion
          if ((base==10 && (buf&0xFFFFFFFF80000000l)!=0l && buf!=0x0000000080000000l)
              || (buf&0xFFFFFFFF00000000l)!=0l) {
    ...
    if (r.nextChar=='.') {
      if (base!=10) { //float and double are always base 10, but may be we already read too much of them in another base
        if (buf>=8l || buf<0l) { //used invalid base, which is not supported by SJC
    ...
    if (r.nextChar=='e') {
      r.readChar();
      switch (r.nextChar) {
        case '-': expNeg=true; //no break
        case '+': r.readChar(); //accept both - and +
      }
      nv=getNumericExt(r.nextChar, base);
      while (r.nextChar!='\0' && nv>=0) {
        if ((expTmp=expTmp*10+nv)>330) { //double is -308..308, add precision of double for safety
      ...
      if (expNeg) exp-=expTmp;
      else exp+=expTmp;
    }
    //check trailing character
    switch (r.nextChar) {
      case 'f': case 'F': useFloat=true; //no break
      case 'd': case 'D': r.readChar(); //accept all f, F, d, D
    }
    if (isAlphaExt(r.nextChar)) { //invalid constant
      scannerError("invalid constant value");
      return false;
  ...
  private char readChValue() {
    ...
    switch (r.nextChar) {
      ...
      case 'b':  r.readChar(); return '\b';
      case 'n':  r.readChar(); return '\n';
      case 'r':  r.readChar(); return '\r';
      case 't':  r.readChar(); return '\t';
      case '0':  r.readChar(); return '\0';
      case '\\': r.readChar(); return '\\';
      case '\'': r.readChar(); return '\'';
      case 'u':
        r.readChar();
        iv=getNumericExt(r.nextChar, 16)<<12;    r.readChar();
        iv=iv|getNumericExt(r.nextChar, 16)<<8;  r.readChar();
        iv=iv|getNumericExt(r.nextChar, 16)<<4;  r.readChar();
        iv=iv|getNumericExt(r.nextChar, 16);     r.readChar();
        if (iv==-1) {
