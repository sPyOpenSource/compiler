...
public class ExVal extends Expr {
    ...
    //nothing to resolve for a constant value, but check if there is a preferred type
	  if (preferredType!=null && preferredType.arrDim==0
	      && compareType(preferredType, false, ctx)!=TypeRef.C_EQ) {
	    if (calcConstantType(ctx)==T_INT && baseType!=T_FLT) {
	      switch (preferredType.baseType) {
  	      case T_BYTE:
  	        if ((intValue&0xFFFFFF80)==0) baseType=T_BYTE;
            break;
  	      case T_SHRT:
            if ((intValue&0xFFFF8000)==0) baseType=T_SHRT;
            break;
  	      case T_CHAR:
  	        if ((intValue&0xFFFF0000)==0) baseType=T_CHAR;
  	        break;
  	      case T_INT:
            baseType=T_INT;
  	        break;
	...
	public void genOutputCondJmp(Instruction jumpDest, boolean isTrue, Instruction elseDest, Context ctx) {
	  ...
	  if (intValue==(isTrue?1:0)) ctx.arch.genJmp(jumpDest);
	}
}
