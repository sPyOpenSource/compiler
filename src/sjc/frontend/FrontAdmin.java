...
public class FrontAdmin {
  ...
  public boolean checkLangEnvironment() {
    ...
    error|=(ctx.rteDRNewArrayMd=checkMthd(ctx.rteDynamicRuntime, "newArray", //length, dimension, entrySize, basicType, extType
        ...
        +parSize(StdTypes.T_INT)+parSize(StdTypes.T_PTR)))==null;
    error|=(ctx.rteDRNewMultArrayMd=checkMthd(ctx.rteDynamicRuntime, "newMultArray", //root-array, root-level, current level, length, dimension, entrySize, basicType, extType
        parSize(StdTypes.T_PTR)+parSize(StdTypes.T_INT)+parSize(StdTypes.T_INT)+parSize(StdTypes.T_INT)
        +parSize(StdTypes.T_INT)+parSize(StdTypes.T_INT)+parSize(StdTypes.T_INT)+parSize(StdTypes.T_PTR)))==null;
    ...
    error|=(ctx.rteDRIsArrayMd=checkMthd(ctx.rteDynamicRuntime, "isArray", //object, basicType, extType, dimension, asCast
        parSize(StdTypes.T_PTR)+parSize(StdTypes.T_INT)+parSize(StdTypes.T_PTR)
        +parSize(StdTypes.T_INT)+parSize(StdTypes.T_BOOL)))==null;
    if (ctx.assignCall || ctx.assignHeapCall) error|=(ctx.rteDRAssignMd=checkMthd(ctx.rteDynamicRuntime, "assign", //asInterface, address, intfMap, object
        parSize(StdTypes.T_BOOL)+parSize(StdTypes.T_PTR)+parSize(StdTypes.T_PTR)+parSize(StdTypes.T_PTR)))==null;
    if (ctx.doArrayStoreCheck) error|=(ctx.rteDRCheckArrayStoreMd=checkMthd(ctx.rteDynamicRuntime, "checkArrayStore", //address, newValue
        parSize(StdTypes.T_PTR)+parSize(StdTypes.T_PTR)))==null;
    if (ctx.runtimeBound) error|=(ctx.rteDRBoundExcMd=checkMthd(ctx.rteDynamicRuntime, "boundException", //object, index
        parSize(StdTypes.T_PTR)+parSize(StdTypes.T_INT)))==null;
    if (ctx.runtimeNull) error|=(ctx.rteDRNullExcMd=checkMthd(ctx.rteDynamicRuntime, "nullException", //no parameter
        0))==null;
    if (ctx.profilerUsed) {
      if ((ctx.rteDRProfilerMd=checkMthd(ctx.rteDynamicRuntime, "profiler", //id, mode
          parSize(StdTypes.T_INT)+parSize(StdTypes.T_BYTE)))==null) error=true;
      else ctx.rteDRProfilerMd.marker|=Marks.K_NPRF;
    }
    if (ctx.stackExtremeUsed) {
      if ((ctx.rteDRStackExtremeErrMd=checkMthd(ctx.rteDynamicRuntime, "stackExtremeError", //no parameter
          0))==null) error=true;
      else ctx.rteDRStackExtremeErrMd.marker|=Marks.K_NSPC;
    ...
    if (ctx.arch.binAriCall[StdTypes.T_BYTE]<0) error|=(ctx.rteDABinAriCallMds[StdTypes.T_BYTE]=checkMthd(ctx.rteDynamicAri, "binByte",
        parSize(StdTypes.T_BYTE)+parSize(StdTypes.T_BYTE)+parSize(StdTypes.T_BYTE)))==null;
    if (ctx.arch.binAriCall[StdTypes.T_SHRT]<0) error|=(ctx.rteDABinAriCallMds[StdTypes.T_SHRT]=checkMthd(ctx.rteDynamicAri, "binShort",
        parSize(StdTypes.T_SHRT)+parSize(StdTypes.T_SHRT)+parSize(StdTypes.T_BYTE)))==null;
    if (ctx.arch.binAriCall[StdTypes.T_INT]<0) error|=(ctx.rteDABinAriCallMds[StdTypes.T_INT]=checkMthd(ctx.rteDynamicAri, "binInt",
        parSize(StdTypes.T_INT)+parSize(StdTypes.T_INT)+parSize(StdTypes.T_BYTE)))==null;
    if (ctx.arch.binAriCall[StdTypes.T_LONG]<0) error|=(ctx.rteDABinAriCallMds[StdTypes.T_LONG]=checkMthd(ctx.rteDynamicAri, "binLong",
        parSize(StdTypes.T_LONG)+parSize(StdTypes.T_LONG)+parSize(StdTypes.T_BYTE)))==null;
    if (ctx.arch.binAriCall[StdTypes.T_FLT]<0) error|=(ctx.rteDABinAriCallMds[StdTypes.T_FLT]=checkMthd(ctx.rteDynamicAri, "binFloat",
        parSize(StdTypes.T_FLT)+parSize(StdTypes.T_FLT)+parSize(StdTypes.T_BYTE)))==null;
    if (ctx.arch.binAriCall[StdTypes.T_DBL]<0) error|=(ctx.rteDABinAriCallMds[StdTypes.T_DBL]=checkMthd(ctx.rteDynamicAri, "binDouble",
        parSize(StdTypes.T_BYTE)+parSize(StdTypes.T_BYTE)+parSize(StdTypes.T_BYTE)))==null;
    if (ctx.arch.unaAriCall[StdTypes.T_BYTE]<0) error|=(ctx.rteDAUnaAriCallMds[StdTypes.T_BYTE]=checkMthd(ctx.rteDynamicAri, "unaByte",
        parSize(StdTypes.T_BYTE)+parSize(StdTypes.T_BYTE)+parSize(StdTypes.T_BYTE)))==null;
    if (ctx.arch.unaAriCall[StdTypes.T_SHRT]<0) error|=(ctx.rteDAUnaAriCallMds[StdTypes.T_SHRT]=checkMthd(ctx.rteDynamicAri, "unaShort",
        parSize(StdTypes.T_SHRT)+parSize(StdTypes.T_SHRT)+parSize(StdTypes.T_BYTE)))==null;
    if (ctx.arch.unaAriCall[StdTypes.T_INT]<0) error|=(ctx.rteDAUnaAriCallMds[StdTypes.T_INT]=checkMthd(ctx.rteDynamicAri, "unaInt",
        parSize(StdTypes.T_INT)+parSize(StdTypes.T_INT)+parSize(StdTypes.T_BYTE)))==null;
    if (ctx.arch.unaAriCall[StdTypes.T_LONG]<0) error|=(ctx.rteDAUnaAriCallMds[StdTypes.T_LONG]=checkMthd(ctx.rteDynamicAri, "unaLong",
        parSize(StdTypes.T_LONG)+parSize(StdTypes.T_LONG)+parSize(StdTypes.T_BYTE)))==null;
    if (ctx.arch.unaAriCall[StdTypes.T_FLT]<0) error|=(ctx.rteDAUnaAriCallMds[StdTypes.T_FLT]=checkMthd(ctx.rteDynamicAri, "unaFloat",
        parSize(StdTypes.T_FLT)+parSize(StdTypes.T_FLT)+parSize(StdTypes.T_BYTE)))==null;
    if (ctx.arch.unaAriCall[StdTypes.T_DBL]<0) error|=(ctx.rteDAUnaAriCallMds[StdTypes.T_DBL]=checkMthd(ctx.rteDynamicAri, "unaDouble",
        parSize(StdTypes.T_BYTE)+parSize(StdTypes.T_BYTE)+parSize(StdTypes.T_BYTE)))==null;
    if (ctx.throwUsed) {
      if ((ctx.rteDoThrowMd=checkMthd(ctx.rteDynamicRuntime, "doThrow", //object
          parSize(StdTypes.T_PTR)))==null) error=true;
      else ctx.rteDoThrowMd.marker|=Marks.K_THRW;
    ...
    if (ctx.syncUsed && !ctx.noSyncCalls) {
      if ((ctx.rteDoSyncMd=checkMthd(ctx.rteDynamicRuntime, "doSynchronize", //object, enter
          parSize(StdTypes.T_PTR)+parSize(StdTypes.T_BOOL)))==null) error=true;
    }
    ...
    if (strTmp.tablePos==2) {
      ...
      while (strTmp.next.next.next!=null) strTmp=strTmp.next;
    ...
    if ((ctx.startUnit=checkUnit(kernelPck, strUnit, false))==null
        || (ctx.startMthd=checkMthd(ctx.startUnit, strMthd, -1))==null) {
  ...
  private Unit checkUnit(Pack pack, String what, boolean mark) {
    ...
    if (mark) ret.modifier|=Modifier.MA_ACCSSD;
    return ret;
  }
  ...
  private int checkVrbl(Unit inUnit, String name, boolean printErr) {
    ...
    if ((var=inUnit.searchVariable(name, ctx))==null) {
      if (printErr) errPrint("missing variable ", name, inUnit, ctx);
