...
public class RelationManager {
  ...
  public void addRelation(Unit usedUnit, Mthd usedMthd, AccVar usedVar, Unit usingUnit, Mthd usingMthd) {
    if (usedUnit==usingUnit || usedUnit.outerUnit==usingUnit) return;
  ...
  private void updateUnitList(Unit unit) {
    UnitList temp;
    if (lastUnits==null) lastUnits=new UnitList(unit);
    else {
      temp=lastUnits;
      while (temp!=null) {
        if (temp.unit==unit) break;
        temp=temp.next;
      }
      if (temp==null) {
  ...
  private void addRelation(Unit usedUnit, Mthd usedMthd, AccVar usedVar, 
      Unit usingUnit, Mthd usingMthd, int type) {
    ...
    while (outer!=null) {
      if (outer.mthd==usedMthd && outer.var==usedVar && outer.type==type) {
        ...
        while (inner!=null) {
          if (inner.unit==usingUnit && inner.mthd==usingMthd) return;
          inner=inner.next;
        }
        break;
      }
