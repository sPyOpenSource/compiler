...
public class HexOut extends BinWriter {
  ...
  public boolean write(byte[] what, int offset, int len) {
    ...
    if (what == null) for (i = 0; i < 16; i++) dataBuffer[i] = (byte)0;
    while (len > 0) {
      if (len < 16) now = len;
      if (what != null) for (i = 0; i < now; i++) dataBuffer[i] = what[offset + off + i];
      if (!writeBuffer(now, now, addr, 0)) return false;
      addr += now;
      off += now;
      len -= now;
    }
  ...
  private boolean writeBuffer(int tellLen, int realLen, int addr, int type) {
    ...
    for (i = 0; i < realLen; i++) insertByte((int)dataBuffer[i], 4 + i); //data bytes
    if (realLen == tellLen) { //place checksum only if tellLen==realLen
  ...
  private void insertByte(int value, int bPos) {
    ...
    checkSum += (value &= 0xFF);
    bPos = (bPos << 1) + 1;
    if ((d = value >>> 4) < 10) textBuffer[bPos++] = (byte)(d + 48);
