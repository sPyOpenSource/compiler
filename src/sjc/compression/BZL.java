...
public class BZL extends Compressor {
  ...
  public int compress(byte[] src, int sourceSize, byte[] dst, int maxDestSize) {
    ...
    bleft=sourceSize;
    if (bleft<block_size) block_size=bleft; //only one block => block_size==file_size
    ...
    for (i=0; bleft>0; ++i) {
      if (bleft<block_size) thisblocksize=bleft;
  ...
  private int readArr(byte[] dst, int offset, int max) {
    ...
    if (srcOffset+max>srcSize) max=srcSize-srcOffset;
    i=offset;
    max+=offset;
    while (i<max) dst[i++]=srcFile[srcOffset++];
    return max;
  }
  ...
  private void writeArr(byte[] src, int offset, int len) {
    int i=0;
    while (i<len) dstFile[dstOffset++]=src[offset+i++];
  }
  ...
  private int compressBlock(int size) {
    ...
    *    |err|                   |ffb|
     */
    if (ff_bug) status|=FF_BUG;
    if (err) status|=BLOCKERROR;
    ...
    if (upperb-lowerb>10) {
      for (i=lowerb+4; i<=upperb; ++i) { 
        ...
        for (j=i-4; j>=lowerb; j-=4) { 
          ...
          if ((((int)arr[bboff])&0xFFFF)>(((int)arr[baoff])&0xFFFF)) break;
          off[j+4]=off[j];
        }       
        off[j+4]=t;
      }
  ...
  private void ternarySort(short[] arr, int[] off, int lowerb_t, int upperb_t, int depth_t) {
    ...
    while (sp>0) {
      ...
      if (r<=MAX_INSERTSORT) {
        if (r>=2) shellSort(arr, off, lowerb, upperb, depth);
        --sp;
        lowerb=s_lb[sp];
        upperb=s_ub[sp];
        depth =s_d[sp];
      ...
      if (r>64) { /* median-of-3 median-of-3 */
        r=r>>3;
        lb=lowerb;
      ...
      lb_v=lowerb;
      ub_v=i-1;
      if ((lb-lowerb)<(i-lb)) n_v=lb-lowerb;
      else n_v=i-lb;
      for(; n_v>0; --n_v) {
        t=off[lb_v];
        off[lb_v]=off[ub_v];
  ...
  private int codeBWT(int len, byte[] bufinout, short[] buffer16, int[] pointer, int[] bwtbuf) {
    ...
    for (i=0; i<BWTBUF_SIZE; i++) bwtbuf[i]=0;
    ...
    for (; i<j; i++) buffer16[i]=(short)0xFFFF;
    ...
    for (j=0; j<BWTBUF_SIZE; ++j) {
      if (l>1) {
        if (l<=(MAX_INSERTSORT+2)) shellSort(buffer16, pointer, deb, deb+l-1, 2);
        else ternarySort(buffer16, pointer, deb, deb+l-1, 2);
      }
      deb+=l;
  ...
  private void codeM1FF2(byte[] bufinout, int len) {
    ...
    for (i=0; i<256; ++i) m1ff2ord[i]=i;
    
    for (i=0; i<len; ++i) {
      c=(((int)bufinout[i])&0xFF);
      l=0;
      while (m1ff2ord[l]!=c) l++;
      bufinout[i]=(byte)l;

      if (l>1) {
        for (; l>1; --l) m1ff2ord[l]=m1ff2ord[l-1];
        m1ff2ord[1]=c;
        l=1;
      }
      else if ((l==1) && flag==1) { 
  ...
  private int split(byte[] bufinout, int off, int end, int dest) {
    ...
    for (; off<end; ++off) if ((((int)bufinout[off])&0xFF)>=2) {
  ...
  private void doneEncoding() {
    bits_to_follow++;
    if (low<FIRST_QTR) bitPlusFollow0();
    else bitPlusFollow1();
  }
  ...
  private void startModel1() {
    ...
    for (i=0; i<NO_OF_GROUPS_0; i++) for (j=0; j<NO_OF_SYMBOLS_0; j++) freq_tab_SM0[i][j]=0;
  }
  ...
  private int codeZIP1(int len, byte[] buf, int bufinOff, int scratchBufferOff) {
    ...
    while (i<len) {
      ...
      if (!in_run && (ch|group)==0) {
        ...
        if (ch!=0) in_run= false;
        
        if (in_run) max=8*1024;
        else max=threshold[group];
        if (tab_cum[0]>max) {
          freq_tab[0]=freq_tab[0]>>1;
          freq_tab[1]=freq_tab[1]>>1;
          freq_tab[2]=freq_tab[2]>>1;
  ...
  private void updateModel2(int symbol, int[] t_i, int[] t_f) {
    ...
    if (max>MAX_FREQ_MODEL) max=MAX_FREQ_MODEL;
  ...
  private void updateModel1(int smbl) {
    ...
    k=symbol-1; fs=SM1_f[symbol*2];
    while ((k>0) && (fs==SM1_f[k*2])) k--;
    ...
    SM1_f[k*2]++;
    SM1_f[symbol*2]++;
    while (symbol>k) SM1_f[(--symbol)*2+1]++;
    while (symbol>0) SM1_f[(--symbol)*2+1]+=2;
  }
