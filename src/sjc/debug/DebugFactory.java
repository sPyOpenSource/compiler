...
public class DebugFactory {
  ...
  public static DebugWriter getDebugWriter(String name, String filename, Context ctx) {
    ...
    if (name==null || name.equals("sym")) return new SymInfo(filename, ctx);
    if (name.equals("rel")) return new RelationInfo(filename, ctx);
    if (name.equals("dwrf")) return new Dwarf(filename, ctx);
    if (name.equals("mthd")) return new MthdInfo(filename, ctx);
    if (name.equals("code")) return new CodeInfo(filename, ctx);
    if (name.equals("size")) return new SizeInfo(filename, ctx);
    if (name.equals("java")) return new CodePrinterJava(filename, ctx);
    if (name.equals("gcc")) return new GccInfo(filename, ctx);
    return null;
  }
  ...
  public static DebugLister getDebugLister(String name, String filename, Context ctx) {
    ...
    if (name.equals("x86asm")) return new AsmOutX86(filename, ctx);
    if (name.equals("vcpu2asm")) return new AsmOutVCPU2(filename, ctx);
    return null;
  }
  ...
  public static DebugLister getDefaultAsmDebugLister(String archID, String filename, Context ctx) {
    ...
    if (archID.equals("x86")) return new AsmOutX86(filename, ctx);
    if (archID.equals("vCPU2")) return new AsmOutVCPU2(filename, ctx);
    return null;
  }
}
