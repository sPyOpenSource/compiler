...
public class SizeInfo extends DebugWriter {
  ...
  public void startUnit(String type, Unit unit) {
    ...
    if (unit.pack!=null) unit.pack.printFullQID(finalOut);
    finalOut.print(';');
    unit.printNameWithOuter(finalOut);
    finalOut.print(";$DESC;");
    finalOut.println(unit.clssScalarTableSize+unit.statScalarTableSize
        +(unit.clssRelocTableEntries+unit.statRelocTableEntries)*ctx.arch.relocBytes);
  }
  ...
  public void hasMethod(Mthd mthd, boolean indir) {
    ...
    if (currentUnit.pack!=null) currentUnit.pack.printFullQID(finalOut);
    finalOut.print(';');
