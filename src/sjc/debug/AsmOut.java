...
public abstract class AsmOut extends DebugLister {
  ...
  public void globalRAMInfo(boolean isDecompressor, Object ramInitLoc, int ramSize, int constMemorySize) {
    if (isDecompressor) out.println(";--- Decompressor ---");
  }
  ...
  protected void printRelocs(int pointer, int relocs) {
    int i;
    for (i=relocs; i>0; i--) printReloc(pointer-i*ctx.arch.relocBytes);
  }
  ...
  protected void printSClassDescRelocs(int pointer, int relocs, Unit unit) {
    ...
    for (i=relocs; i>0; i--) {
      ...
      if (var==null) var=searchVar(ctx.rteSClassDesc, AccVar.L_INSTREL, off, true);
      if (var!=null) {
  ...
  protected void printObjectScalars(int pointer, int scalarSize, Unit unit, boolean withNames, int mthdPointerForScalarCalc) {
    ...
    while (off<scalarSize) {
      size=1;
      if (unit!=null) var=searchVar(unit, AccVar.L_INSTSCL, off, true);
      if (var!=null) {
        if (withNames) {
          ctx.printUniqueVarName(out, var, unit);
          out.println(":");
      ...
      if (unit!=null && off==4 && mthdPointerForScalarCalc!=0) { //_r_scalarSize
      ...
      else printScalar(pointer+off, size);
      off+=size;
    }
  }
  ...
  protected void printSClassDescScalars(int pointer, int scalarSize, Unit unit) {
    ...
    while (off<scalarSize) {
      ...
      if (var==null) var=searchVar(ctx.rteSClassDesc, AccVar.L_INSTSCL, off, true);
      if (var!=null) {
        ctx.printUniqueVarName(out, var, unit);
        out.println(":");
        size=var.minSize;
  ...
  protected void printEscapedString(String what, int desiredByteLength) {
    int i;
    if (ctx.byteString) out.print("  db ");
    else out.print("  dw ");
    for (i=0; i<what.length(); i++) {
      ...
      if (ctx.byteString) desiredByteLength--;
      else desiredByteLength-=2;
    }
    out.println();
  ...
  protected void printScalar(int pointer, int size) {
    if (size>ctx.arch.relocBytes) { //too big for single access, split into multiple smaller portions
      while (size>0) {
  ...
  protected Mthd searchMthd(Unit unit, int offset) {
    ...
    while (unit!=null) {
      mthds=unit.mthds;
      while (mthds!=null) {
        if (mthds.relOff==offset && (stat || (mthds.modifier&Modifier.M_STAT)==0)) return mthds;
        mthds=mthds.nextMthd;
      }
      if (unit.extsID!=null) unit=unit.extsID.unitDest;
      else unit=null;
      stat=false;
    }
    return null;
  ...
  protected AccVar searchVar(Unit unit, int reqLocation, int offset, boolean searchParents) {
    ...
    while (unit!=null) {
      vars=unit.vars;
      while (vars!=null) {
        if (vars.location==reqLocation && vars.relOff==offset) return vars;
        vars=vars.nextVrbl;
      }
      if (searchParents && unit.extsID!=null) unit=unit.extsID.unitDest;
      else unit=null;
    }
    return null;
  }
