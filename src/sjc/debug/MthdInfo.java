...
public class MthdInfo extends DebugWriter {
  ...
  public void startUnit(String type, Unit unit) {
    if ((unit.modifier&Modifier.M_INDIR)!=0) unitIsIndir=true;
    else {
      unitIsIndir=false;
      myBuffer.reset();
      if (unit.pack!=null) {
  ...
  public void hasMethod(Mthd mthd, boolean indir) {
    ...
    if (!unitIsIndir && mthd.outputLocation!=null) {
      ...
      if (lastMthd==null) firstMthd=lastMthd=now; //no object so far
      else {
        if (now.addr>lastMthd.addr) { //append (normal case)
          lastMthd.nextMthd=now;
          lastMthd=now;
        }
        else if (lastMthd==firstMthd || now.addr<firstMthd.addr) { //insert at start of list 
          now.nextMthd=firstMthd;
          firstMthd=now;
        }
        else { //search for correct place
          search=firstMthd;
          while (now.addr>search.nextMthd.addr) search=search.nextMthd; //should never result in a null-pointer
