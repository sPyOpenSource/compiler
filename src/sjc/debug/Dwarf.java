...
public class Dwarf extends DebugWriter {
  ...
  public static final int[] STD_FILELINE_OPCODE_LEN = {
    0, //opcode 0: extended opcode escape - ignore in this list
  ...
  private TextBuffer txtBuf;
  private Section[] sections;
  private Section info, abbrev, str, aranges, line;
  private int abbrevCnt, cuHeaderStartInfo, cuLineStart;
  
  //abbreviation numbers
  private int unitAbbrev, baseTypeAbbrev, mthdAbbrev/*, vrblAbbrev*/;
  ...
  private final static byte[] ELF_Ehdr = {
    0x7F, 'E', 'L', 'F', 1, 1, 1,      //magic
  ...
  private class Section { //dumb container class
    public String name;
    ...
    public void writeHeader() {
      ...
      if (type!=SHT_NULL) { //fill only if not null segment
    ...
    public void appendByte(int value) { //append lowest 8 bits of value
      if (byteBuf==null || usedBytes+1>=byteBuf.length) {
        byte[] newBuf=new byte[byteBuf==null ? BUF_INC : byteBuf.length+BUF_INC];
        for (int i=0; i<usedBytes; i++) newBuf[i]=byteBuf[i];
        byteBuf=newBuf;
      }
      byteBuf[usedBytes++]=(byte)value;
    }
    
    public void appendTwoBytes(int v1, int v2) { //append lowest 8 bits of each value
      appendByte(v1);
      appendByte(v2);
    }
    
    public void appendShort(int value) { //append lowest 16 bits of value
      appendByte(value);
      appendByte(value>>>8);
    }
    
    public void appendInt(int value) { //append all 32 bits of value
      appendByte(value);
      appendByte(value>>>8);
      appendByte(value>>>16);
      appendByte(value>>>24);
    ...
    public void appendUnsignedLEB128(int value) { //append int (unsigned) of arbitraty length in LEB128 encoding
      while (true) {
        if ((value&~0x7F)==0) {
          appendByte(value);
          return;
    ...
    public void appendSignedLEB128(int value) { //append int (signed) of arbitraty length in LEB128 encoding
      while (true) {
        int b=value&0x7F;
        value>>=7;
    ...
    public void appendNullString(String value) {
      ...
      for (int i=0; i<len; i++) appendByte((int)value.charAt(i));
  ...
  public void startUnit(String unitType, Unit unit) {
    ...
    while (mthd!=null) {
      if (mthd.codeSize>0) {
        ...
        if (start<lowestPc) lowestPc=start;
        if (end>highestPc) highestPc=end;
      }
      mthd=mthd.nextMthd;
    }
    if (highestPc==0) lowestPc=0;
    ...
    if (fullpath!=null) {
      ...
      if (slWin>slash) slash=slWin;
      if (slash>=0) {
        file=fullpath.substring(slash+1, fullpath.length());
        dir=fullpath.substring(0, slash);
      }
      else file=fullpath;
    }
    if (file==null || file.equals("")) file="[intern]"; //should never happen
    txtBuf.reset();
    ...
    if (highestPc-lowestPc>0) { //write only if there is binary code which needs information
      ...
      for (int i=1; i<STD_FILELINE_OPCODE_LEN.length; i++) line.appendUnsignedLEB128(STD_FILELINE_OPCODE_LEN[i]); //standard opcode lengths
    ...
    else { //remember that there is nothing to end
      cuLineStart=-1;
      info.appendInt(0); //no line info
    }
    //enter base types
  ...
  public void hasMethod(Mthd mthd, boolean indir) {
    if (mthd.codeSize>0) { //only output info for really generated methods
      ...
      if (mthd.lineInCodeOffset!=null) {
        ...
        for (int i=0; i<mthd.lineInCodeOffset.length; i+=2) {
          ...
          if (incLine<LINE_RANGE && special<=255) line.appendByte(special); //encode as special
          else { //encoding explicitly
  ...
  private void addShStrTabEntry(String name) {
    int pos=0;
    if (shstrtab==null) shstrtab=new StringList(name);
    else {
  ...
  private int getShStrTabOffset(String name) {
    ...
    while (search!=null) {
      if (search.str.equals(name)) return offset;
  ...
  private int getDbgStringOffset(String value) {
    ...
    while (search!=null) { //try to recycle string
      if (search.str.equals(value)) return search.tablePos;
      search=search.next;
    }
