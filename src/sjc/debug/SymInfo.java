...
public class SymInfo extends DebugWriter {
  ...
  public void startImageInfo(boolean isDecompressor) {
    if (!isDecompressor) out.println("# --- SJC syminfo--- #");
    else {
      out.println();
      out.println("# --- decompressor --- #");
      out.println();
    }
    if (ctx.verbose) out.print('v');
    if (ctx.leanRTE) out.print('L');
    if (ctx.mem.streamObjects) out.print('l');
    if (ctx.indirScalars) out.print('M');
    if (ctx.dynaMem) out.print('m');
    if (ctx.assignHeapCall) out.print('h');
    if (ctx.assignCall) out.print('c');
    if (!ctx.doArrayStoreCheck) out.print('C');
    if (!ctx.doBoundCheck) out.print('B');
    if (ctx.runtimeBound) out.print('b');
    if (ctx.runtimeNull) out.print('n');
    if (ctx.debugCode) out.print('d');
    if (ctx.byteString) out.print('y');
    if (ctx.noThrowFrames) out.print('Y');
    if (ctx.embedded) out.print('e');
    if (ctx.embConstRAM) out.print('E');
    if (ctx.globalProfiling) out.print('F');
    if (ctx.globalStackExtreme) out.print('x');
    if (ctx.alternateObjNew) out.print('w');
    if (ctx.noInlineMthdObj) out.print('k');
    if (ctx.alignBlockMask!=0) out.print('K');
    out.println();
  }
  ...
  public void hasMethod(Mthd mthd, boolean indir) {
    ...
    if (indir) {
      ...
      out.print(": ");
      mthd.printSig(out);
      if ((mthd.modifier&Modifier.MA_ACCSSD)==0) out.println(" (never used)");
      else out.println();
    }
    else {
      if (mthd.relOff!=0) out.printHexFix(mthd.relOff, 8);
      else out.print("no entry");
      if ((mthd.modifier&Modifier.M_ABSTR)!=0) out.print("  abstract");
      else { 
        out.print("->");
      ...
      out.print(": ");
      mthd.printSig(out);
      if ((mthd.modifier&(Modifier.M_ABSTR|Modifier.M_OVERLD|Modifier.MA_ACCSSD))==0
          && !ovldMthdAccessed(mthd.ovldMthd)) out.print(SKIPCAND);
      else printMinimumAccessLevel(mthd.modifier);
      if (!ctx.dynaMem && ((mthd.modifier&(Modifier.M_STAT|Modifier.M_FIN|Modifier.M_PRIV))!=0
          || (mthd.modifier&Modifier.M_OVERLD)==0)) out.print(" (statCall)");
      else out.print(" (dynaCall)");
      if (mthd.redirect!=null) {
        ...
        if ((ctx.debugCode || (mthd.marker&Marks.K_DEBG)!=0) && (mthd.modifier&Modifier.M_ABSTR)==0) {
          ...
          if (mthd.retType!=null) mthdSig[0]=mthd.retType.getSig();
          else mthdSig[0]='C';
          par=mthd.param;
  ...
  public void hasStatObj(int rela, Object loc, String value, boolean inFlash) {
    ...
    if (loc!=null) out.printHexFix(ctx.mem.getAddrAsInt(loc, 0), 8);
    else out.print("skipped ");
    if (value!=null) {
      if (inFlash) out.print(" in Flash");
      out.print(": ");
      out.print(value);
    }
    out.println();
  ...
  private void printMinimumAccessLevel(int modifier) {
    int maxAccess;
    if ((modifier&Modifier.MA_PUB)!=0) maxAccess=Modifier.M_PUB;
    else if ((modifier&(Modifier.MA_PROT|Modifier.M_OVERLD))!=0) maxAccess=Modifier.M_PROT;
    else if ((modifier&Modifier.MA_PACP)!=0) maxAccess=Modifier.M_PACP;
    else if ((modifier&Modifier.MA_PRIV)!=0) maxAccess=Modifier.M_PRIV;
    else return;
    if ((modifier&(Modifier.M_PUB|Modifier.M_PROT|Modifier.M_PACP|Modifier.M_PRIV))<maxAccess) {
      out.print(" (may be reduced to ");
      switch (maxAccess) {
        case Modifier.M_PROT: out.print("protected)"); break;
        case Modifier.M_PACP: out.print("package private)"); break;
        case Modifier.M_PRIV: out.print("private)"); break;
      }
    }
  }
  ...
  private boolean ovldMthdAccessed(Mthd ovldMthd) {
    while (ovldMthd!=null) {
      if ((ovldMthd.modifier&Modifier.MA_ACCSSD)!=0) return true;
