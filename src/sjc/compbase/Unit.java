...
public abstract class Unit extends Token {
  ...
  public boolean isParent(Unit cmp, Context ctx) {
    ...
    while (u!=null) {
      ...
      if (u.extsID!=null) {
        if ((u=u.extsID.unitDest)==null) { //destination is not valid ==> resolving error
      ...
      while (list!=null) {
        if (isParent(list.qid.unitDest, ctx)) return true;
  ...
  public boolean isOuter(Unit inner) {
    while (inner!=null) {
      if (inner==this) return true;
      inner=inner.outerUnit;
    }
    return false;
  }
  
  public int minimumAccessLevel(Token token, String itemMsg, Unit destUnit, int destLevel,
      boolean allowExtensionAccess, Context ctx) {
    Unit myOutest, destOutest;
    if (ctx.doNotCheckVisibility) return Modifier.MA_PRIV; //do not check visibility in instance conversion block
    if (itemMsg==null) itemMsg="item";
    //check trivial private
    if (this==destUnit || isOuter(destUnit) || (destUnit!=null && destUnit.isOuter(this))) return Modifier.MA_PRIV;
    //check multiple inner classes where private is allowed, too
    if (outerUnit!=null && destUnit!=null && destUnit.outerUnit!=null) {
      myOutest=outerUnit;
      while (myOutest.outerUnit!=null) myOutest=myOutest.outerUnit;
      destOutest=destUnit.outerUnit;
      while (destOutest.outerUnit!=null) destOutest=destOutest.outerUnit;
      if (myOutest==destOutest) return Modifier.MA_PRIV;
    }
    ...
    if (pack.packDest!=destUnit.pack.packDest
        && (!allowExtensionAccess || !destUnit.isParent(this, ctx))) { //check modifier to be public
    ...
    if (pack.packDest!=destUnit.pack.packDest && destUnit.isParent(this, ctx)) { //check modifier to be protected
      if ((destLevel&(Modifier.M_PUB|Modifier.M_PROT))==0) { //check modifier to be public or protected
        token.printPos(ctx, itemMsg);
        ctx.out.print(ERR_NOTVIS);
        return Modifier.M_ERROR;
      }
    ...
    if ((destLevel&(Modifier.M_PUB|Modifier.M_PROT|Modifier.M_PACP))==0) { //check modifier to be public or protected or package private
      token.printPos(ctx, itemMsg);
      ctx.out.print(ERR_NOTVIS);
      return Modifier.M_ERROR;
    }
  ...
  public Vrbl searchVariable(String cmp, Context ctx) {
    ...
    while (me!=null) {
      if (me.name.equals(cmp)) return me;
      me=me.nextVrbl;
    }
    if (extsID!=null) {
      if (extsID.unitDest.hasValidInterface()
          && (me=extsID.unitDest.searchVariable(cmp, ctx))!=null) return me;
    }
    extsList=extsImplIDList;
    while (extsList!=null) {
      if (extsList.qid.unitDest.hasValidInterface()
          && (me=extsList.qid.unitDest.searchVariable(cmp, ctx))!=null) {
        if (res==null) res=me;
        else if (res!=me) {
  ...
  public Mthd searchMethod(Mthd cmp, Context ctx) {
    ...
    while (me!=null) {
      if (me.matches(cmp, ctx)) return me;
      me=me.nextMthd;
    }
    if (extsID==null) return null;
    if (!extsID.unitDest.hasValidInterface()) return null;
  ...
  public UnitList listAllInterfaceParents(UnitList listToExtend, Context ctx) {
    ...
    while (qids!=null) {
      ...
      cmp=listToExtend;
      while (cmp!=null) { //check if unit is already in list
        if (cmp.unit==u) break;
        cmp=cmp.next;
      }
      if (cmp==null) { //unit is not in list so far => add it with all parents
        last=new UnitList(u);
        if ((last.next=listToExtend)!=null) last.relOff=listToExtend.relOff+1;
