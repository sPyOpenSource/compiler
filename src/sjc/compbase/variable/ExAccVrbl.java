...
public abstract class ExAccVrbl extends Expr {
  ...
  public boolean canGenAddr(Unit unitContext, boolean allowSpecialWriteAccess, int resolveFlags, Context ctx) {
    if (isThis || constType>0 || constObject || dest==null
        || (dest.location==AccVar.L_STRUCT && dest.type.typeSpecial!=TypeRef.S_NOSPECIAL)
        || (!allowSpecialWriteAccess && (dest.modifier&Modifier.M_FIN)!=0 && ((resolveFlags&RF_INSIDE_LOOP)!=0
             || (dest.modifier&(Modifier.MF_ISWRITTEN|Modifier.MF_MAYBEWRITTEN))!=0)))
      return false;
  ...
  public void genOutputAddr(int reg, int obj, Context ctx) {
    ...
    if (isThis || constType>0 || constObject
        || (dest.location!=AccVar.L_PARAM && dest.location!=AccVar.L_LOCAL
          && dest.type.typeSpecial!=TypeRef.S_NOSPECIAL
          && (dest.modifier&Modifier.MM_REFTOFLASH)==0)) {
      compErr(ctx, "can not generate output address in ExAccVrbl.genOutputAddr");
      ctx.out.println();
      return;
    }
    switch (dest.location) {
      case AccVar.L_STRUCT: case AccVar.L_STRUCTREF: //struct variable is a very special case
        ...
        return;
      case AccVar.L_CONST: case AccVar.L_CONSTDC: case AccVar.L_CONSTTR:
    ...
    pos=dest.relOff;
    if (ensureClassContext) { //access to variable possibly needs change of class context
      switch (dest.location) {
        case AccVar.L_CLSSSCL:
        case AccVar.L_CLSSREL:
          if (ctx.dynaMem) obj=ctx.arch.regClss;
          break;
        default:
      ...
      switch (dest.location) { //normal access
        case AccVar.L_LOCAL:
        case AccVar.L_PARAM:
          if (useResu) { compErr(ctx, ERR_INVMODE); return; }
          obj=ctx.arch.regBase;
          break;
        case AccVar.L_CLSSSCL:
        case AccVar.L_CLSSREL:
          if (ctx.dynaMem) {
            if (!useResu) obj=ctx.arch.regClss;
          }
          else {
            objBase=!ctx.embedded ? dest.owner.outputLocation : ctx.ramLoc;
            obj=0;
        ...
        case AccVar.L_INSTREL:
          if (!useResu) obj=ctx.arch.regInst;
          break;
        case AccVar.L_INSTIDS:
          if (!useResu) obj=ctx.arch.regInst;
          if (outerLevel>0) obj=genDeOuter(reg, obj, ctx);
          ...
          if (outerLevel>0) cleanUpDeOuter(reg, ctx);
          ctx.arch.deallocRestoreReg(obj, oldObj2, addrRestore2);
          if (ensureClassContext && ctx.dynaMem) ctx.arch.deallocRestoreReg(obj, oldObj1, addrRestore1);
          return;
        default:
    ...
    if (outerLevel>0) cleanUpDeOuter(reg, ctx);
    if (addr2Used) ctx.arch.deallocRestoreReg(obj, oldObj2, addrRestore2);
    if (ensureClassContext && ctx.dynaMem) ctx.arch.deallocRestoreReg(obj, oldObj1, addrRestore1);
  }
  ...
  public void genOutputVal(int reg, int obj, Context ctx) {
    ...
    if (isThis) {
      ...
      else ctx.arch.genDup(reg, ctx.arch.regInst, StdTypes.T_PTR);
      return;
    }
    if (constType>0) {
      switch (constType) {
    ...
    if (constObject && (!ctx.dynaMem || isStructType())) { //in dynaMem-mode outputLocation may only be used for constant structs
    ...
    if (dest.location==AccVar.L_STRUCT || dest.location==AccVar.L_STRUCTREF) { //this is a very special case
      //reg must contain the value of the struct (i.e. base address)
      //so the offset has to be calculated and returned
      if (dest.location!=AccVar.L_STRUCTREF && (dest.type.arrDim!=0 || dest.type.baseType<0)) 
        ctx.arch.genLoadVarAddr(reg, obj, null, dest.relOff);
      else ctx.arch.genLoadVarVal(reg, obj, null, dest.relOff, getRegType(ctx));
      return;
    }
    ...
    if (ensureClassContext) { //access to variable possibly needs change of class context
      ...
      if (ctx.dynaMem) {
        ...
        obj=ctx.arch.allocReg();
        ctx.arch.genLoadUnitContext(obj, importedClass.relOff);
        if ((typeSpecial==S_FLASHREF || typeSpecial==S_FLASHINLARR) && (dest.modifier&Modifier.MM_REFTOFLASH)==0) ctx.arch.nextValInFlash();
      ...
      switch (dest.location) {
        ...
        case AccVar.L_CLSSREL:
          if ((typeSpecial==S_FLASHREF || typeSpecial==S_FLASHINLARR) && (dest.modifier&Modifier.MM_REFTOFLASH)==0) ctx.arch.nextValInFlash();
          if (ctx.dynaMem) ctx.arch.genLoadVarVal(reg, useResu ? obj : ctx.arch.regClss, null, dest.relOff, regType);
          else ctx.arch.genLoadVarVal(reg, 0, !ctx.embedded ? dest.owner.outputLocation : ctx.ramLoc, dest.relOff, regType);
          break;
        case AccVar.L_INSTSCL:
        case AccVar.L_INSTREL:
          if (!useResu) obj=ctx.arch.regInst;
        ...
        case AccVar.L_INLARR:
          ...
          else if (useResu) ctx.arch.genDup(reg, obj, StdTypes.T_PTR);
          else ctx.arch.genDup(reg, ctx.arch.regInst, StdTypes.T_PTR);
          break;
        default:
  ...
  public void genOutputAssignTo(int obj, int newValueReg, Expr newValue, Context ctx) {
    ...
    switch (dest.location) {
      case AccVar.L_STRUCT: case AccVar.L_STRUCTREF: //struct variable is a very special case
        //reg must contain the value of the struct (i.e. base address)
        //so the offset has to be calculated and returned
        if (newValueReg==0 && (newValueCalcConstType=newValue.calcConstantType(ctx))!=0) { //result is never read and constant, so assign directly
        ...
        else { //generate result normally and assign
          if (newValueReg==0) {
          ...
          if (deallocNewValueReg) ctx.arch.deallocRestoreReg(newValueReg, 0, newValueRestore);
        }
        return;
      case AccVar.L_CONST: case AccVar.L_CONSTDC: case AccVar.L_CONSTTR:
        printPos(ctx, "address of constant variable not available");
    ...
    if (outerLevel>0) obj=genDeOuter(0, obj, ctx);
    ...
    if (newValueReg==0 && (newValueCalcConstType=newValue.calcConstantType(ctx))!=0) { //result is never read and constant, so assign directly
      switch (newValueCalcConstType) {
        case StdTypes.T_INT:
          ...
          break;
        case StdTypes.T_LONG:
          ctx.arch.genStoreVarConstDoubleOrLongVal(obj, loc, pos, newValue.getConstLongValue(ctx),
              regType==StdTypes.T_DBL);
          break;
        default:
          compErr(ctx, "invalid constant type in ExAccVrbl.genOutputAssignTo");
      }
    ...
    if (outerLevel>0) cleanUpDeOuter(0, ctx);
    if (addr2Used) ctx.arch.deallocRestoreReg(obj, oldObj2, addrRestore2);
    if (ensureClassContext && ctx.dynaMem) ctx.arch.deallocRestoreReg(obj, oldObj1, addrRestore1);
  }
