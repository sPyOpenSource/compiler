...
public class Vrbl extends AccVar {
  ...
  public boolean enterSize(int loc, Context ctx) {
    ...
    if (!super.enterSize(loc, ctx)) return false;
    ...
    if ((modifier&Modifier.M_STRUCT)!=0) {
      if (type.arrDim>0) {
        ...
        if (type.baseType!=TypeRef.T_QID) minSize=TypeRef.getMinSize(type.baseType);
      }
      ...
      if (type.baseType==TypeRef.T_QID) {
        ...
        if (location==AccVar.L_STRUCTREF) minSize=ctx.arch.relocBytes;
        else {
          if (!type.qid.unitDest.resolveInterface(ctx)) return false; //parent must be ok for inlining
          minSize=type.qid.unitDest.instScalarTableSize; //enter size of one element if array
        }
      }
      //check if location is to be set
      if (loc==L_UNIT) location=L_STRUCT;
    }
    //otherwise check if variable is declared in a unit and location has to be set
    else if (loc==L_UNIT) {
      if ((modifier&(Modifier.M_FIN|Modifier.M_STAT))==(Modifier.M_FIN|Modifier.M_STAT)) location=L_CONSTDC;
      else if (minSize<0) {
        if ((modifier&Modifier.M_STAT)!=0) location=L_CLSSREL;
        else location=L_INSTREL;
      }
      else {
        if ((modifier&Modifier.M_STAT)!=0) location=L_CLSSSCL;
        else if (ctx.indirScalars) location=L_INSTIDS;
        else location=L_INSTSCL;
      }
    }
