...
public abstract class AccVar extends Token {
    ...
    else { //reference, check size
      if ((modifier&(Modifier.MM_FLASH|Modifier.MM_REFTOFLASH))!=0) type.typeSpecial=TypeRef.S_FLASHREF;
      if (type.arrDim>0) minSize=-1; //array
        ...
        if (((mod=type.qid.unitDest.modifier)&Modifier.M_INDIR)!=0) minSize=-2;
        else if ((mod&Modifier.M_STRUCT)==0) minSize=-1;
        else minSize=ctx.arch.relocBytes; //destination is struct and should not be treated as ref
      }
    }
