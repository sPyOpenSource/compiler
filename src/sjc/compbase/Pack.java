...
public class Pack {
  ...
  public QualID getQIDTo() {
	...
	public Pack searchSubPackage(String what) {
	  ...
	  sub=subPacks;
	  while (sub!=null) {
	    if (sub.name.equals(what)) return sub;
	    sub=sub.nextPack;
	  }
    return null;
	}
	...
	public Pack searchSubPackage(StringList what, boolean createIfNew) {
	  ...
	  if (!found) {
	    if (!createIfNew) return null; //not found
	    if (searchUnit(what.str)!=null) return null; //class with this name already exists
	    if (last!=null) {
	  ...
	  if (what.next==null) return sub; //found everything
    ...
    u=units;
    while (u!=null) {
      if (u.name.equals(what)) return u;
      u=u.nextUnit;
    }
    ...
    StringList cur;
	  ...
	  if (what.next==null) return searchUnit(what.str); //no subpackage left, search units
    //subpackages existing
	  if (what.next!=null) while (sub!=null) { //search subpackage
	    if (sub.name.equals(what.str)) return sub.searchUnit(what.next);
	    sub=sub.nextPack;
	  }
    ...
    while (u!=null) {
      ...
      while (res!=null) {
        ...
        else res=res.nextUnit; //try next inner unit
      }
	...
	public boolean addUnit(Unit c) {
	  ...
	  while (sub!=null) {
	    if (sub.name.equals(c.name)) return false; //package with this name already exists
	    sub=sub.nextPack;
	  }
	  if (units==null) {
	    units=c;
	  ...
	  u=units;
	  while (u.nextUnit!=null) { //search entered units except last one
	    if (u.name.equals(c.name)) return false; //name already entered
	    u=u.nextUnit;
	  }
  ...
  public Object searchNamedObject(String name) {
    NamedObject o=objs;
    while (o!=null) {
      if (o.name.equals(name)) return o.item;
      o=o.nextObj;
    }
    return null;
  }
