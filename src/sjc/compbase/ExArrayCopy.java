...
public class ExArrayCopy extends Expr {
  ...
  public boolean resolve(Unit unitContext, Mthd mthdContext, int resolveFlags,
    ...
    if (!array.resolve(unitContext, mthdContext, resolveFlags, preferredType, ctx)) return false;
    getTypeOf(array);
  ...
  public void genOutputVal(int reg, Context ctx) {
    ...
    else ctx.arch.genLoadVarAddr(regArray, 0, array.outputLocation, array.getOutputLocationOffset(ctx));
    ...
    else ctx.arch.genCallConst(ctx.rteArrayDeepCopyMd, ctx.rteArrayDeepCopyMd.parSize);
