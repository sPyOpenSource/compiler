...
public class ExCheckType extends Expr {
  ...
  public boolean prepareObjectConversion(TypeRef srcType, TypeRef destType, Unit unitContext, Context ctx) {
    ...
    if (destType.baseType==TypeRef.T_QID) dstUnit=destType.qid.unitDest;
    if (srcType.baseType<0 && srcType.arrDim==0 && srcType.qid==null) {
      printPos(ctx, "source-type will never match");
      return false;
    }
    if (srcType.isStructType() || srcType.isFlashType()
        || destType.isStructType() || destType.isFlashType()) {
    ...
    if (dstArr) {
      ...
      else checkMode=C_ISINST;
    }
    if (ctx.dynaMem) {
      if (dstUnit!=null) importedClass=unitContext.getRefUnit(dstUnit, true);
  ...
  public static void genIsType(int dst, int src, boolean asCast, TypeRef dstType,
      int checkMode, UnitList runtimeClass, UnitList importedClass, Context ctx) {
    ...
    if (asCast) ctx.arch.genPush(src, StdTypes.T_PTR);
    if (ctx.dynaMem) ctx.arch.genSaveUnitContext();
    ctx.arch.genPush(src, StdTypes.T_PTR);
    if (checkMode==C_ISARRAY) ctx.arch.genPushConstVal(dstType.baseType, StdTypes.T_INT);
    if (dstType.qid!=null) { //
      if (ctx.dynaMem) ctx.arch.genLoadUnitContext(unitReg, importedClass.relOff); 
      else ctx.arch.genLoadConstUnitContext(unitReg, dstType.qid.unitDest.outputLocation);
      ctx.arch.genPush(unitReg, StdTypes.T_PTR);
    }
    else ctx.arch.genPushConstVal(0, StdTypes.T_PTR); //may only occur if checkMode==C_ISARRAY
    if (checkMode==C_ISARRAY) ctx.arch.genPushConstVal(dstType.arrDim, StdTypes.T_INT);
    ctx.arch.genPushConstVal(asCast ? 1 : 0, StdTypes.T_BOOL);
    if (ctx.dynaMem) {
      ctx.arch.genLoadUnitContext(ctx.arch.regClss, runtimeClass.relOff);
      switch (checkMode) {
        case C_ISINST: ctx.arch.genCall(ctx.rteDRIsInstMd.relOff, ctx.arch.regClss, ctx.rteDRIsInstMd.parSize); break;
        case C_ISIMPL: ctx.arch.genCall(ctx.rteDRIsImplMd.relOff, ctx.arch.regClss, ctx.rteDRIsImplMd.parSize); break;
        case C_ISARRAY: ctx.arch.genCall(ctx.rteDRIsArrayMd.relOff, ctx.arch.regClss, ctx.rteDRIsArrayMd.parSize); break;
      }
      ctx.arch.genRestUnitContext();
    }
    else {
      switch (checkMode) {
        case C_ISINST: ctx.arch.genCallConst(ctx.rteDRIsInstMd, ctx.rteDRIsInstMd.parSize); break;
        case C_ISIMPL: ctx.arch.genCallConst(ctx.rteDRIsImplMd, ctx.rteDRIsImplMd.parSize); break;
        case C_ISARRAY: ctx.arch.genCallConst(ctx.rteDRIsArrayMd, ctx.rteDRIsArrayMd.parSize); break;
      }
    }
    if (checkMode==C_ISIMPL) { //interface
      if (asCast) ctx.arch.genMoveIntfMapFromPrimary(dst); //move map from result to secodary reg for use
      else ctx.arch.genMoveFromPrimary(dst, StdTypes.T_PTR); //move map from result to primary reg for check
    }
    else if (!asCast) ctx.arch.genMoveFromPrimary(dst, StdTypes.T_BOOL); //if no cast, result not of interest
    if (asCast) ctx.arch.genPop(dst, StdTypes.T_PTR);
    if (dstType.qid!=null) ctx.arch.deallocRestoreReg(unitReg, src, unitRestore);
