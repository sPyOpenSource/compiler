...
public class ExConstNew extends ExConstInitObj {
  ...
  public boolean fillConstNew(Unit unitContext, Unit dtu, TypeRef targetType, FilledParam arrS,
      int resolveFlags, Context ctx) {
  ...
  public boolean generateObject(Context ctx, boolean doFlash) {
    ...
    if (arrDim==0) {
      if ((outputLocation=ctx.mem.allocate(destTypeUnit.instScalarTableSize,
          destTypeUnit.instIndirScalarTableSize, destTypeUnit.instRelocTableEntries,
          destTypeUnit.outputLocation))==null) {
      ...
      if ((outputLocation=ctx.allocateMultiArray(arrDim, arrSizes,
          objElements ? -1 : TypeRef.getMinSize(baseType),
              baseType, extType))==null) {
