...
public abstract class Expr extends TypeRef {
  ...
  public boolean canGenAddr(Unit unitContext, boolean allowSpecialWriteAccess, int resolveFlags, Context ctx) { //by default, address is not available
    return false;
  }
  public int calcConstantType(Context ctx) { //results 0 for not constant or StdTypes.T_INT or T_LONG (maybe later also T_FLT or T_DBL)
    return 0;
  }
  ...
  public boolean isAddrOnStack() { //only expressions with addr on stack have to overwrite
    return false;
  }
  ...
  public void genOutputPrepareAssignTo(int destReg, int newValueReg, Expr newValue, Context ctx) {
    ...
    else { //if addr or value may be changed, do not delay address calculation
      genOutputAddr(destReg, ctx);
      newValue.genOutputVal(newValueReg, ctx);
    }
  }
  ...
  public int getAssignType(Context ctx) {
    return (baseType<0 || arrDim>0) ?
        (isIntfType() ? StdTypes.T_DPTR : StdTypes.T_PTR)
        : baseType;
  }
  ...
  public void genOutputAssignTo(int newValueReg, Expr newValue, Context ctx) {
    ...
    if (deallocReg) ctx.arch.deallocRestoreReg(newValueReg, 0, regRestore);
  }
