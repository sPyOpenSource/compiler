...
public class ExArrayInit extends ExConstInitObj {
  ...
  public boolean resolve(Unit unitContext, Mthd mthdContext, int resolveFlags,
    ...
    if (forcedType!=null) {
      if (forcedType.qid!=null && forcedType.qid.unitDest==null && !forcedType.resolveType(unitContext, ctx)) return false;
    ...
    if (mthdContext!=unitContext.initStat && mthdContext!=unitContext.initDyna) dependsOn=mthdContext;
    if ((mthdContext.modifier&Modifier.M_STAT)==0) {
      ensureClassContext=true;
      if (ctx.dynaMem) importedClass=unitContext.getRefUnit(unitContext, true);
    }
    ...
    if (preferredType!=null) {
      ...
      if (--preferredElemType.arrDim<0) preferredElemType=null;
    }
    pa=par;
    len=0;
    while (pa!=null) {
      ...
      if (!ex.resolve(unitContext, mthdContext, resolveFlags|RF_CHECKREAD, preferredElemType, ctx)) return false;
      if (!ex.isCompInitConstObject(ctx)) {
        switch (ex.calcConstantType(ctx)) {
          case 0:
            if (ex.baseType==StdTypes.T_NULL) continue; //null-type valid, do not copy type
    ...
    if (preferredType!=null && (preferredType.typeSpecial&TypeRef.S_FLASHREF)!=0) typeSpecial=TypeRef.S_FLASHREF;
  ...
  public boolean generateObject(Context ctx, boolean doFlash) {
    ...
    if ((outputLocation=arr=ctx.mem.allocateArray(len, arrDim,
        objElements ? -1 : TypeRef.getMinSize(baseType), baseType, extType))==null) {
      ctx.out.println("error allocating memory while creating constant array for initialization");
      return false;
    }
    //enter values
    if (objElements) arrOff=-(ctx.rteSArray.instRelocTableEntries+1)*ctx.arch.relocBytes;
    else {
      if (ctx.indirScalars) {
        arr=ctx.mem.getIndirScalarObject(arr);
        arrOff=ctx.rteSArray.instIndirScalarTableSize;
      }
      else arrOff=ctx.rteSArray.instScalarTableSize;
    }
    ...
    for (off=0; off<len; off++) {
      ...
      if (objElements) {
        if (pa.expr.baseType!=StdTypes.T_NULL) {
          ...
          ctx.arch.putRef(arr, arrOff-off*ctx.arch.relocBytes,
              coi.outputLocation, coi.getOutputLocationOffset(ctx));
        }
      }
