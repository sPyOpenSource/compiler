...
import norswap.lang.java.ast.expr.MethodCall;
import norswap.lang.java.ast.expr.NewReference;
import norswap.lang.java.ast.expr.ParenExpression;
import norswap.lang.java.ast.expr.Super;
import norswap.lang.java.ast.expr.SuperCall;
import norswap.lang.java.ast.expr.TernaryExpression;
...
public class TestGrammar extends AutumnTestFixture
{
    ...
    // []
    private static Dimension dim = new Dimension(noAnnotations);

    // primitive type (int, float, etc)
    private static PrimitiveType prim (BasicType type) {
        return new PrimitiveType(noAnnotations, type);
    }

    // an identifier part of a class name
    private static ClassTypePart cpart (String name) {
        return new ClassTypePart(noAnnotations, Identifier.mk(name), noTypeArgs);
    }
    ...
    private static ClassType sclass (String name, List<TType> typeArgs) {
        return new ClassType(list(
            new ClassTypePart(noAnnotations, Identifier.mk(name), typeArgs)));
    }
    ...
    // class name "T"
    private static ClassType T = new ClassType(list(cpart("T")));
    ...
    @Test public void annotations()
    {
        ...
        Expression hval = TernaryExpression.mk(
            Literal.mk(true),
            new DotIden(Identifier.mk("x"), Identifier.mk("y")),
            new ArrayAccess(
        ...
        successExpect("@Single(" + hairy + ")",
            new SingleElementAnnotation(idList("Single"), hval));
        successExpect("@Single(@Marker)",
            new SingleElementAnnotation(idList("Single"), marker));

        successExpect("@java.util.Single(@java.util.Marker)",
            new SingleElementAnnotation(
        ...
        successExpect("@Single({@Marker, " + hairy + "})",
            new SingleElementAnnotation(
                idList("Single"),
                new AnnotationElementList(list(marker, hval))));

        successExpect("@Single({})",
            new SingleElementAnnotation(idList("Single"), new AnnotationElementList(list())));
        ...
        successExpect("@Single({x,})",
            new SingleElementAnnotation(
                idList("Single"),
                new AnnotationElementList(list(Identifier.mk("x")))));

        successExpect("@Single(x)",
            new SingleElementAnnotation(idList("Single"), Identifier.mk("x")));
        ...
        successExpect("@Pairs(x = @Marker, y = " + hairy + ", z = {@Marker, " + hairy + "}, u = x)",
            NormalAnnotation.mk(list(Identifier.mk("Pairs")), list(
                ...
                new Pair<>(Identifier.mk("z"), new AnnotationElementList(list(marker, hval))),
    ...
    @Test public void types()
    {
        ...
        successExpect("char",      new PrimitiveType(list(), _char));
        successExpect("int",       new PrimitiveType(list(), _int));
        successExpect("double",    new PrimitiveType(list(), _double));
        successExpect("void",      new PrimitiveType(list(), _void));

        successExpect("java.util.String",
            new ClassType(list(cpart("java"), cpart("util"), cpart("String"))));
        successExpect("List<?>",
            sclass("List", list(new Wildcard(noAnnotations, null))));
        successExpect("List<T>",
            sclass("List", list(T)));
        successExpect("List<? super T>",
            sclass("List", list(new Wildcard(noAnnotations, new SuperBound(T)))));
        successExpect("List<? extends T>",
            sclass("List", list(new Wildcard(noAnnotations, ExtendsBound.mk(T)))));
        ...
        successExpect("char[]",
            new ArrayType(prim(_char), list(dim)));
        successExpect("int[][][]",
            new ArrayType(prim(_int), list(dim, dim, dim)));
        successExpect("T[]",
            new ArrayType(T, list(dim)));
        successExpect("List<T>[][]",
            new ArrayType(sclass("List", list(T)), list(dim, dim)));
        ...
        successExpect("List<? extends List<? super T>>",
            sclass("List", list(new Wildcard(noAnnotations, ExtendsBound.mk(sclass("List",
                list(new Wildcard(noAnnotations, new SuperBound(T)))))))));

        successExpect("@Marker int",
            new PrimitiveType(list(marker), _int));

        successExpect("@Marker java.@test.Mbrker util . @Mcrker String",
            new ClassType(list(
                new ClassTypePart(list(marker), Identifier.mk("java"), noTypeArgs),
                new ClassTypePart(
        ...
        successExpect("List<@Marker ?>",
            sclass("List", list(new Wildcard(list(marker), null))));

        successExpect("List<? extends @Marker T>",
            sclass("List", list(new Wildcard(noAnnotations, ExtendsBound.mk(new ClassType(list(
                    new ClassTypePart(list(marker), Identifier.mk("T"), noTypeArgs))))))));
        ...
        successExpect("@Marker int @Mbrker []",
            new ArrayType(
                new PrimitiveType(list(marker), _int),
                list(new Dimension(list(MarkerAnnotation.mk(list(Identifier.mk("Mbrker"))))))));
    }
    ...
    @Test public void primaryExpressions()
    {
        ...
        successExpect("(1)",
            new ParenExpression(Literal.mk(1)));
        successExpect("this",
            This.mk());
        successExpect("super",
            new Super());
        ...
        successExpect("int.class",
            new ClassExpression(prim(_int)));
        successExpect("List.class",
            new ClassExpression(sclass("List", noTypeArgs)));
        successExpect("java.util.List.class",
            new ClassExpression(new ClassType(list(cpart("java"), cpart("util"), cpart("List")))));

        successExpect("new int[42]",
            new ArrayConstructorCall(prim(_int),
                list(new DimExpression(noAnnotations, Literal.mk(42))),
                noDims, null));
        successExpect("new int[42][]",
            new ArrayConstructorCall(prim(_int),
                list(new DimExpression(noAnnotations, Literal.mk(42))),
        ...
        successExpect("new int[1][2][][]",
            new ArrayConstructorCall(prim(_int),
                list(
                    new DimExpression(noAnnotations, Literal.mk(1)),
                    new DimExpression(noAnnotations, Literal.mk(2))),
                list(dim, dim), null));
        successExpect("new int[] { 1, 2, 3 }",
            new ArrayConstructorCall(prim(_int), noDimExprs, list(dim),
                new ArrayInitializer(list(Literal.mk(1), Literal.mk(2), Literal.mk(3)))));
        successExpect("new int[] { 1, 2, }",
            new ArrayConstructorCall(prim(_int), noDimExprs, list(dim),
                new ArrayInitializer(list(Literal.mk(1), Literal.mk(2)))));
        successExpect("new int[] { , }",
            new ArrayConstructorCall(prim(_int), noDimExprs, list(dim),
                new ArrayInitializer(noArgs)));
        successExpect("new int[][] { {1, 2}, {3, 4} }",
            new ArrayConstructorCall(prim(_int), noDimExprs, list(dim, dim),
                new ArrayInitializer(list(
                    new ArrayInitializer(list(Literal.mk(1), Literal.mk(2))),
                    new ArrayInitializer(list(Literal.mk(3), Literal.mk(4)))))));
        successExpect("new List<T>[1]",
            new ArrayConstructorCall(
                sclass("List", list(T)),
                list(new DimExpression(noAnnotations, Literal.mk(1))),
                noDims, null));

        successExpect("Foo::bar",
            new TypeMethodReference(sclass("Foo"), noTypeArgs, Identifier.mk("bar")));
        successExpect("Foo::new",
            NewReference.mk(sclass("Foo"), noTypeArgs));
        successExpect("Foo::<T>bar",
            new TypeMethodReference(sclass("Foo"), list(T), Identifier.mk("bar")));
        successExpect("Foo::<T, V>new",
            NewReference.mk(sclass("Foo"), list(T, sclass("V"))));
        successExpect("List.Foo::<T>bar",
            new TypeMethodReference(
                new ClassType(list(cpart("List"), cpart("Foo"))), list(T), Identifier.mk("bar")));
